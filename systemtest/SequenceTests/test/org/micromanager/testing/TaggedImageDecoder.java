...
public class TaggedImageDecoder {
  public static InfoPacket decode(TaggedImage image) throws IOException {
    byte[] bytes = (byte[]) image.pix;
    return TestImageDecoder.decode(bytes);
  }

  public static void dumpJSON(TaggedImage image, java.io.OutputStream out) throws IOException {
    byte[] bytes = (byte[]) image.pix;
