 ...
 *
 * <p>No effort is made for nice API design here, as this is solely for testing. The wire format
 * (MessagePack schema) may change in backward-incompatible ways as tests evlove.
 *
 * <p>Note that order of fields in static nested classes is important, since MessagePack
 * deserializes these objects from an array, not a map.
 */
@org.junit.Ignore
public class TestImageDecoder {
  ...
  public static class InfoPacket {
    ...
    public boolean hasBeenSetSincePreviousPacket(String device, String settingName) {
      for (SettingEvent event : history) {
        if (event.key.device.equals(device) && event.key.key.equals(settingName)) {
          return true;
        }
      }
      return false;
  ...
  public static InfoPacket decode(byte[] image) throws IOException {
    ...
    try {
      ...
      Assert.fail(
          "SeqTester image is all-zero; make sure image size is "
              + "sufficient to fit MessagePack data");
      throw e; // Placate compiler
    }
  }

  public static void dumpJSON(byte[] image, java.io.OutputStream out) throws IOException {
    MessagePack msgpack = new MessagePack();
