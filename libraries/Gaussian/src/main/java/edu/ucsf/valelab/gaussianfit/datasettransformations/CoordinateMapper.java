...
/**
 * Provides facilities for mapping one coordinate system into another Currently implements LWM and
 * affine transforms implementation of Local Weighted Mean algorithm,as first described by Ardeshir
 * Goshtasby. (1988). Image registration by local approximation methods, and used in Matlab by
 * cp2tform
 *
 * @author Arthur
 */
public class CoordinateMapper {
  ...
  /**
   * Weightfunction as defined in Ardeshir Goshtasby. (1988). Used for LWM only
   *
  ...
  /**
   * Generates exponents for given order, see Ardeshir Goshtasby (1988). Only used in LWM
   *
   * @param order
  ...
  public static class EnhancedKDTree extends KdTree.SqrEuclid {
    ...
    public List<Point2D.Double> nearestNeighbor(
        Point2D.Double testPoint, int size, boolean ordered) {
      @SuppressWarnings("unchecked")
      List<Entry<Integer>> neighbors =
          super.nearestNeighbor(new double[] {testPoint.x, testPoint.y}, size, ordered);
    ...
    /**
     * Variant of nearestNeighbor that returns up to size points closest to testPoint at a maximum
     ...
     */
    public List<Point2D.Double> nearestNeighbor(
        Point2D.Double testPoint, int size, double maxDistance) {
  ...
  /**
   * Selects a subset of points in the input PointMap, which is a: HashMap&lt;Point2D.Double,
   * Point2D.Double&gt;
   *
  ...
  public static class ControlPoint {
    ...
    /**
     * Generates a Controlpoint for the given srcPoint. First finds n (order!) control points around
     * scrPoint Then fits a polynomial to minimize the error between points in the two channels
     *
     * @param kdTree - kdTree used to find the nearest neighbor in the input channel
     * @param srcPoint - point in the input channel used to generate a control point.
     * @param order - desired order of the polynomial
     * @param pointMap - input map with channel 1 points as keys and channel 2 points as values
     */
    public ControlPoint(
        EnhancedKDTree kdTree, Point2D.Double srcPoint, int order, PointMap pointMap) {
      point = srcPoint;
  ...
  public static PolynomialCoefficients fitPolynomial(
      ExponentPairs exponentPairs, Map<Point2D.Double, Point2D.Double> pointPairs) {
  ...
  public static double evaluatePolynomial(
      double x, double y, double[] coeffs, ExponentPairs exponentPairs) {
    double result = 0;
  ...
  /**
   * For each point in the input pointMap, calculates a control-point, which contains polynomial
   * coefficients in x and y, as well as a normalization factor the polynomial coefficients are
   * calculated by fitting a polynomial to minimize the error in the nearest neighbor pairs
   *
   * @param kdTree -
   * @param order
   * @param pointMap
   * @return
   */
  public static ControlPoints createControlPoints(
      EnhancedKDTree kdTree, int order, PointMap pointMap) {
   ...
   * @return
   */
  public static Point2D.Double computeTransformation(
      EnhancedKDTree kdTree,
        ...
        sumWeightedPolyX +=
            weight
                * evaluatePolynomial(
                    testPoint.x,
                    testPoint.y,
                    controlPoint.polynomialCoefficients.polyX,
                    exponentPairs);
        sumWeightedPolyY +=
            weight
                * evaluatePolynomial(
                    testPoint.x,
                    testPoint.y,
                    controlPoint.polynomialCoefficients.polyY,
                    exponentPairs);
      }
    }
   ...
   * planar coordinate system. At least three point pairs are needed.
   *
  ...
  /**
   * Rigid body transform (rotation and translation only) Creates an AffineTransform object that
  ...
  /**
   * Generates an affine transform using only control points that are close by The code finds the
   * closest by maxNrControlPoints and then removes any points that are more than maxDinstance away
   * from the testPoint It uses the resulting set of control points to generate an affine transform
   *
  ...
  public Point2D.Double transform(Point2D.Double srcTestPoint) {
    ...
    if (method_ == PIECEWISEAFFINE) {
      try {
        AffineTransform piecewiseAf =
            generateLocalAffineTransform(
                srcTestPoint, pieceWiseAffineMaxControlPoints_, pieceWiseAffineMaxDistance_);
        if (piecewiseAf != null) {
  ...
  /**
   * Feeds control points into this class Performs initial calculations for lwm and affine
   * transforms
   *
