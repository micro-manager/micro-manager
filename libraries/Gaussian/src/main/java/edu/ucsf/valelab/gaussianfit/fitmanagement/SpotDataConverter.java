...
public class SpotDataConverter {
  ...
  public static SpotData convert(
      SpotData spot, GaussianFit.Data fitResult, GaussianInfo info, ZCalibrator zc) {
    ...
    if (fitResult.getParms().length >= 4) {
      ...
      double xMax =
          (fitResult.getParms()[GaussianFit.XC] - info.getHalfBoxSize() + spot.getX())
              * info.getPixelSize();
      double yMax =
          (fitResult.getParms()[GaussianFit.YC] - info.getHalfBoxSize() + spot.getY())
              * info.getPixelSize();
      // express background in photons after base level correction
      double bInElectrons = cPCF * (fitResult.getParms()[GaussianFit.BGR] - info.getBaseLevel());
      ...
      double N = cPCF * fitResult.getParms()[GaussianFit.INT] * (2 * Math.PI * gs * gs);
      ...
      double bgrAperture = cPCF * (fitResult.getApertureBackground() - info.getBaseLevel());
      // Add the read-noise of the camera (expressed in electrons)
      ...
      double varX =
          sasqr / N
              + (8 * Math.PI * s * s * s * s * bgr * bgr)
                  / (info.getPixelSize() * info.getPixelSize() * N * N);
  ...
  public static Double calcIntegral(double N, double a, double sasqr, double bgr) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      sum += step * (Math.log(t) / (1 + (N * a * a * t) / (2 * Math.PI * sasqr * bgr * bgr)));
    }
    return sum;
  }
}
