...
public class SBSPlate {
  ...
  private double[] A1Position = {
    0, 0
  }; // if stage can't be zeroed, then use this an an offset, in microns.
  ...
  /**
   * checks the number of positions of a given well. It will check the position list for the current
   * well and if that exits, return that.
   *
   * <p>Otherwise it will return the number of positions of the global list.
   *
   * @param well number within the plate, or within region of the plate if first and last wells are
    ...
    if (wellArray[currentWell[0]][currentWell[1]] != null) {
      if (index < wellArray[currentWell[0]][currentWell[1]].wellPositionList.size()) {
        ...
        if (index < globalPositionList.size()) {
          ...
          System.err.println(
              "getNextWellPosition: Global Well index out of bounds " + globalPositionList.size());
      }
    } else {
      if (index < globalPositionList.size()) {
        position[0] += globalPositionList.get(index)[0];
   ...
   *     first global position of that well, which is usually the center of that well, if not
   *     defined.
   */
  ...
  public int[] getPlateCoordinates(int well) {
    ...
    // reverse well position because we reverse direction on odd rows in reference to starting
    // column, which may not be the first column
