      ...
      try {
        ...
        if (c == 'I') {
          ...
          for (int x = 0; x < data_size; x++) {
            if (file_endian == 'B') data[x] = in.readShort();
            else data[x] = swap(in.readShort());
          }
        }
      } finally {
        in.close();
  ...
  private void writeHeader(RandomAccessFile out) {
    ...
    try {
      if (out != null) {
        ...
        for (int x = 0; x < 33; x++) out.writeByte(32);
        // write history
        ...
        for (int x = 0; x < 15; x++) {
          if (history[x] != null) {
            ...
            if (history[x].length() < 64) {
              for (int y = history[x].length(); y < 64; y++) out.writeByte(32); // fill with spaces
            }
          } else {
