...
public class LUT extends IndexColorModel implements Cloneable {
   ...
   public byte[] getBytes() {
      ...
      for (int i = 0; i < 256; i++) {
         bytes[i] = (byte) getRed(i);
      }
      for (int i = 0; i < 256; i++) {
         bytes[256 + i] = (byte) getGreen(i);
      }
      for (int i = 0; i < 256; i++) {
         bytes[512 + i] = (byte) getBlue(i);
      }
      return bytes;
   }
   ...
   public LUT createInvertedLut() {
      ...
      getGreens(greens);
      getBlues(blues);
      final byte[] reds2 = new byte[mapSize];
      final byte[] greens2 = new byte[mapSize];
      final byte[] blues2 = new byte[mapSize];
