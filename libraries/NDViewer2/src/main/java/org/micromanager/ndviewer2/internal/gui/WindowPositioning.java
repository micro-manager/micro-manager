...
public final class WindowPositioning {
    ...
    * @param preferences      Java Preferences to store location memory
    */
   public static void setUpLocationMemory(Window window, Preferences preferences) {
      GeometrySaver.createAndRestoreLocation(window, preferences);
   }
   ...
   private static class GeometrySaver implements ComponentListener {
      ...
      private void saveGeometry() {
         ...
         switch (mode_) {
            case MEMORIZE_BOUNDS:
               preferences_.putInt(ProfileKey.WINDOW_LOCATION_X.name(),
                     bounds.getLocation().x);
               preferences_.putInt(ProfileKey.WINDOW_LOCATION_Y.name(),
                     bounds.getLocation().y);
               preferences_.putInt(ProfileKey.WINDOW_BOUNDS_W.name(), bounds.width);
               preferences_.putInt(ProfileKey.WINDOW_BOUNDS_H.name(), bounds.height);
               break;
      ...
      private void restoreGeometry() {
         int x = preferences_.getInt(ProfileKey.WINDOW_LOCATION_X.name(),
               window_.getLocation().x);
         int y = preferences_.getInt(ProfileKey.WINDOW_LOCATION_Y.name(),
               window_.getLocation().y);
         switch (mode_) {
            case MEMORIZE_BOUNDS:
               window_.setBounds(new Rectangle(x, y,
                     preferences_.getInt(ProfileKey.WINDOW_BOUNDS_W.name(),
                           window_.getBounds().width),
                     preferences_.getInt(ProfileKey.WINDOW_BOUNDS_H.name(),
                           window_.getBounds().height)));
               break;
            case MEMORIZE_LOCATION:
               window_.setLocation(x, y);
               break;
