...
public interface ProjectionDevice {
  ...
  /**
   * returns the external shutter device. Will be null if no external shutter device was previously
   * set, or if it was not a valid MM device
   *
  ...
  /**
   * Switches all pixels of an SLM device to the "on"/"Active" position. This function has no effect
   * on a Galvo device
   */
