...
public class MappingStorage {
  ...
  static void saveMapping(
    ...
    settings.putPropertyMap(getCalibrationKey(core, dev), mapToPropertyMap(mapping));
  }
   ...
   *     <p>Polygon PropertyMaps are structured as: - Integer - nrEntries - x-i - x position of
   *     point #i - y-i - y position of point #i
   */
   ...
   * @return Mapping
   */
  public static Mapping loadMapping(
      final CMMCore core, final ProjectionDevice dev, final PropertyMap settings) {
  ...
  static Mapping mapFromPropertyMap(final CMMCore core, final PropertyMap pMap) {
    ...
    for (int i = 0; i < nrEntries; i++) {
      if (pMap.containsPropertyMap(MAP_POLYGON_MAP + i)) {
        ...
        for (int j = 0; j < nrPolygonCorners; j++) {
          p.addPoint(polygonMap.getInteger("x-" + j, 0), polygonMap.getInteger("y-" + j, 0));
        }
    ...
    mb.setROI(cameraROI)
        .setApproximateTransform(approximateAF)
        .setBinning(cameraBinning)
        .setMap(mapping);
