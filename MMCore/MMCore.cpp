...
Configuration CMMCore::getSystemState()
{
   ...
   for (vector<string>::const_iterator i = devices.begin(), end = devices.end(); i != end; ++i)
   {
      ...
      MMThreadGuard guard(deviceManager_.getModuleLock(pDev));
...
void CMMCore::assignDefaultRole(boost::shared_ptr<DeviceInstance> pDevice)
{
   ...
   MMThreadGuard guard(deviceManager_.getModuleLock(pDevice));
...
void CMMCore::initializeAllDevices() throw (CMMError)
{
   ...
   for (size_t i=0; i<devices.size(); i++)
   {
      boost::shared_ptr<DeviceInstance> pDevice;
      try {
         pDevice = deviceManager_.GetDevice(devices[i].c_str());
      }
      catch (CMMError& err) {
...
bool CMMCore::deviceTypeBusy(MM::DeviceType devType) throw (CMMError)
{
   ...
   for (size_t i=0; i<devices.size(); i++)
   {
      try {
         boost::shared_ptr<DeviceInstance> pDevice =
            deviceManager_.GetDevice(devices[i].c_str());
         MMThreadGuard guard(deviceManager_.getModuleLock(pDevice));
         if (pDevice->Busy())
            return true;
      }
...
void CMMCore::setPosition(const char* label, double position) throw (CMMError)
{
   ...
   MMThreadGuard guard(deviceManager_.getModuleLock(pStage));
...
{
   boost::shared_ptr<XYStageInstance> pXYStage = GetDeviceWithCheckedLabelAndType<XYStageInstance>(label);

   LOG_DEBUG(coreLogger_) << "Will start absolute move of " << label <<
      " to position (" << std::fixed << std::setprecision(3) << x << ", " <<
      y << ") um";

   MMThreadGuard guard(deviceManager_.getModuleLock(pXYStage));
...
bool CMMCore::isExposureSequenceable(const char* cameraLabel) throw (CMMError)
{
   ...
   MMThreadGuard guard(deviceManager_.getModuleLock(pCamera));
...
void CMMCore::snapImage() throw (CMMError)
{
   ...
   if (camera)
   {
      ...
      MMThreadGuard guard(deviceManager_.getModuleLock(camera));
...
void CMMCore::setShutterOpen(const char* shutterLabel, bool state) throw (CMMError)
{
   ...
   if (pShutter)
   {
      MMThreadGuard guard(deviceManager_.getModuleLock(pShutter));
...
void CMMCore::startSequenceAcquisition(const char* label, long numImages, double intervalMs, bool stopOnOverflow) throw (CMMError)
{
   ...
   MMThreadGuard guard(deviceManager_.getModuleLock(pCam));
   if(pCam->IsCapturing())
...
void CMMCore::setState(const char* deviceLabel, long state) throw (CMMError)
{
   ...
   MMThreadGuard guard(deviceManager_.getModuleLock(pStateDev));
...
void CMMCore::setSLMImage(const char* deviceLabel, unsigned char* pixels) throw (CMMError)
{
   ...
   MMThreadGuard guard(deviceManager_.getModuleLock(pSLM));
...
void CMMCore::pointGalvoAndFire(const char* deviceLabel, double x, double y, double pulseTime_us) throw (CMMError)
{
   ...
   MMThreadGuard guard(deviceManager_.getModuleLock(pGalvo));
...
void CMMCore::saveSystemConfiguration(const char* fileName) throw (CMMError)
{
   ...
   for (size_t i=0; i<config.size(); i++)
   {
      ...
      boost::shared_ptr<DeviceInstance> pDevice = deviceManager_.GetDevice(s.getDeviceLabel().c_str());
      if (pDevice)
      {
         MMThreadGuard guard(deviceManager_.getModuleLock(pDevice));
   ...
   for (size_t i=0; i<deviceLabels.size(); i++)
   {
      ...
      MMThreadGuard guard(deviceManager_.getModuleLock(pSD));
...
double CMMCore::getLastFocusScore()
{
   ...
   if (autofocus)
   {
      MMThreadGuard guard(deviceManager_.getModuleLock(autofocus));
      double score;
...
{
   CheckDeviceLabel(label);
   return deviceManager_.GetDevice(label);
}
...
void CMMCore::applyConfiguration(const Configuration& config) throw (CMMError)
{
   ...
   for (size_t i=0; i<config.size(); i++)
   {
      ...
      {
         // normal processing
         boost::shared_ptr<DeviceInstance> pDevice = deviceManager_.GetDevice(setting.getDeviceLabel().c_str());
         MMThreadGuard guard(deviceManager_.getModuleLock(pDevice));
         try
         {
...
int CMMCore::applyProperties(vector<PropertySetting>& props, string& lastError)
{
  // int succeeded = 0;
   ...
   {
      // normal processing
      boost::shared_ptr<DeviceInstance> pDevice = deviceManager_.GetDevice(props[i].getDeviceLabel().c_str());
      MMThreadGuard guard(deviceManager_.getModuleLock(pDevice));
      try
      {
...
string CMMCore::getDeviceErrorText(int deviceCode, boost::shared_ptr<DeviceInstance> device)
{
   ...
   MMThreadGuard guard(deviceManager_.getModuleLock(device));
...
MM::DeviceDetectionStatus CMMCore::detectDevice(char* deviceName)
{
   ...
   try
   {
      ...
      boost::shared_ptr<DeviceInstance> pDevice  = deviceManager_.GetDevice(deviceName);

      // XXX BUG: pDevice is never NULL (deviceManager_.GetDevice() would have
...
std::vector<std::string> CMMCore::getInstalledDevices(const char* hubDeviceLabel)
{
   ...
   MMThreadGuard guard(deviceManager_.getModuleLock(pHub));
