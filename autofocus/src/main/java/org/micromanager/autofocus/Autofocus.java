...
public class Autofocus extends AutofocusBase implements AutofocusPlugin, SciJavaPlugin {
  ...
  public void run(String arg) {
    ...
    try {
      ...
      // ########System setup##########
      if (!CHANNEL.equals(NOCHANNEL)) core_.setConfig(channelGroup_, CHANNEL);
      core_.waitForSystem();
      ...
      // ===IJ.log("BEST_DIST_FIRST"+String.valueOf(bestDist)+"
  ...
  public PropertyItem[] getProperties() {
    ...
    try {
      ...
      for (int i = 0; i < channels.size(); i++) {
        ...
        if (p.value.equals(channels.get(i))) found = true;
      }
      p.allowed = allowedChannels;
      if (!found) p.value = allowedChannels[0];
      setProperty(p);
    } catch (Exception e1) {
      // TODO Auto-generated catch block
      e1.printStackTrace();
