...
public class HardwareFocusExtender extends AutofocusBase implements AutofocusPlugin, SciJavaPlugin {
  ...
  public PropertyItem[] getProperties() {
    ...
    String[] zDrives = null;
    try {
      autofocusDevices = core.getLoadedDevicesOfType(DeviceType.AutoFocusDevice).toArray();
  ...
  public final void applySettings() {
    try {
      ...
      stepSize_ = NumberUtils.displayStringToDouble(getPropertyValue(STEP_SIZE));
      lowerLimit_ = NumberUtils.displayStringToDouble(getPropertyValue(LOWER_LIMIT));
      upperLimit_ = NumberUtils.displayStringToDouble(getPropertyValue(UPPER_LIMIT));
    } catch (Exception e) {
      ReportingUtils.logError(e);
    }
  }
  ...
  public void setContext(Studio app) {
    gui_ = app;
    gui_.events()
        .registerForEvents(this); // We subscribe to the AutofocusPluginShouldInitialize event.
  }
  ...
  public double fullFocus() throws Exception {
    ...
    try {
      ...
      gui_.logs()
          .showError("HardwareFocusExtender: Hardware focus device and/or ZDrive were not set");
      return 0.0;
    }
    double pos = 0.0;
    try {
    ...
    for (int i = 0; i < lowerLimit_ / stepSize_ && !success; i++) {
      ...
      gui_.logs()
          .logDebugMessage(
              String.format("HardwareFocusExtender: Checking hardware focus at %.2f.", z));
      success = testFocus();
    }
  ...
  private boolean testFocus() {
    CMMCore core = gui_.getCMMCore();
    try {
      // specific for Nikon TI 1 PFS. If the TI reports that it is out of search range there is not
      // point trying fullFocus.
