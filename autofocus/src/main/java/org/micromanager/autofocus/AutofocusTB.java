...
public class AutofocusTB extends AutofocusBase implements AutofocusPlugin, SciJavaPlugin {
  ...
  public void run(String arg) {
    ...
    if (core_ == null) {
      IJ.error(
          "Unable to get Micro-Manager Core API handle.\n"
    ...
    try {
      ...
      // ===IJ.write("BEST_DIST_FIRST"+String.valueOf(bestDist)+"
      // BEST_SH_FIRST"+String.valueOf(bestSh));
      ...
      IJ.log(
          "BEST_DIST_SECOND= "
              + String.valueOf(bestDist)
              + " BEST_SH_SECOND= "
              + String.valueOf(bestSh));
  ...
  private double sharpNessp(ImageProcessor impro) {
    ...
    for (int k = 1; k < width - 1; k++) {
      for (int l = 1; l < height - 1; l++) {
        ...
        // sharpNess = sharpNess + Math.pow((-2 * medPix[k - 1][l - 1] - medPix[k][l - 1] - medPix[k
        // - 1][l] + medPix[k + 1][l] + medPix[k][l + 1] + 2 * medPix[k + 1][l + 1]), 2);
  ...
  public PropertyItem[] getProperties() {
    ...
    try {
      ...
      for (int i = 0; i < channels.size(); i++) {
        ...
        if (p1.value.equals(channels.get(i))) found1 = true;
        if (p2.value.equals(channels.get(i))) found2 = true;
      }
      p1.allowed = allowedChannels;
      if (!found1) p1.value = allowedChannels[0];
      setProperty(p1);
      p2.allowed = allowedChannels;
      if (!found2) p2.value = allowedChannels[0];
      setProperty(p2);
    } catch (Exception e1) {
      // TODO Auto-generated catch block
      e1.printStackTrace();
