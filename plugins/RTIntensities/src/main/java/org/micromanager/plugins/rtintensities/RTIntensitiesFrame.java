...
public class RTIntensitiesFrame extends JFrame {
        ...
        e -> {
          ...
          int result =
              JOptionPane.showConfirmDialog(
                  this, settingsPanel, "Plot settings", JOptionPane.OK_CANCEL_OPTION);
  ...
  private void setupPlot() {
    ...
    for (int i = 0; i < ROIs_; i++) {
      if (!roi_[i].isCursor()) {
        for (int p = 0; p < plots_; p++) {
          data_[i * plots_ + p] =
              new XYSeries("" + (i + 1) + ((plots_ > 1) ? ("c" + (p + 1)) : ""));
    ...
    if (backgroundeq_ >= 0) {
      for (int p = 0; p < plots_; p++) {
        data_[backgroundeq_ * plots_ + p] =
            new XYSeries("bg" + ((plots_ > 1) ? ("c" + (p + 1)) : ""));
    ...
    graphFrame_ =
        plotData(
            plotmode + " of " + dataProvider_.getName(),
            dataset_,
            "Time(ms)",
            "Value",
            plots_,
   ...
   */
  public static ChartFrame plotData(
      String title,
