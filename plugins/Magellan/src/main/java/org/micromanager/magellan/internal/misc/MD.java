...
public class MD {
  ...
  public static String getPixelType(JSONObject map) {
    try {
      if (map != null) return map.getString(PIX_TYPE);
    } catch (JSONException e) {
      try {
  ...
  public static int getNumberOfComponents(JSONObject map) {
    ...
    if (pixelType.contentEquals(PIX_TYPE_GRAY8)) return 1;
    else if (pixelType.contentEquals(PIX_TYPE_GRAY16)) return 1;
    //      else if (pixelType.contentEquals("GRAY32"))
    //         return 1;
  ...
  public static int getDepth(JSONObject tags) {
    ...
    if (pixelType.contains(PIX_TYPE_GRAY8)) return 1;
    else if (pixelType.contains(PIX_TYPE_GRAY16)) return 2;
