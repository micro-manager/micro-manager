...
public class ExploreAcquisition extends Acquisition implements MagellanAcquisition {
  ...
  private ConcurrentHashMap<Integer, LinkedBlockingQueue<ExploreTileWaitingToAcquire>>
      queuedTileEvents_ =
          new ConcurrentHashMap<Integer, LinkedBlockingQueue<ExploreTileWaitingToAcquire>>();
   ...
   * @return
   */
  public LinkedBlockingQueue<ExploreTileWaitingToAcquire> getTilesWaitingToAcquireAtSlice(
      int sliceIndex) {
  ...
  public void acquireTileAtCurrentLocation() {
    ...
    int posIndex =
        ((MagellanDataManager) dataSink_).getFullResPositionIndexFromStageCoords(xPos, yPos);
    //      controls.setZLimitSliderValues(sliceIndex);

    submitEvents(
        new int[] {(int) ((MagellanDataManager) dataSink_).getXYPosition(posIndex).getGridRow()},
        new int[] {(int) ((MagellanDataManager) dataSink_).getXYPosition(posIndex).getGridCol()},
        sliceIndex,
        sliceIndex);
  }
            ...
            () -> {
              ...
              for (int r = row1; r <= row2; r++) {
                for (int c = col1; c <= col2; c++) {
                  ...
                  int i =
                      ((relativeCol % 2 == 0) ? relativeRow : (numRows - relativeRow - 1))
                          + numRows * relativeCol;
  ...
  private void submitEvents(
      int[] newPositionRows, int[] newPositionCols, int minZIndex, int maxZIndex) {
    int[] posIndices =
        ((MagellanDataManager) dataSink_).getPositionIndices(newPositionRows, newPositionCols);
    ...
    Function<AcquisitionEvent, AcquisitionEvent> removeTileToAcquireFn =
        (AcquisitionEvent e) -> {
          queuedTileEvents_
              .get(e.getZIndex())
              .remove(
                  new ExploreTileWaitingToAcquire(
                      e.getGridRow(), e.getGridCol(), e.getZIndex(), e.getChannelConfig()));
          return e;
        };

    ArrayList<Function<AcquisitionEvent, Iterator<AcquisitionEvent>>> list =
        new ArrayList<Function<AcquisitionEvent, Iterator<AcquisitionEvent>>>();
    Function<AcquisitionEvent, Iterator<AcquisitionEvent>> fn =
        (AcquisitionEvent t) -> {
    ...
    submitEventIterator(
        new AcquisitionEventIterator(null, list, removeTileToAcquireFn),
    ...
    return (AcquisitionEvent event) -> {
      try {
        ...
        if (!queuedTileEvents_.containsKey(event.getZIndex())) {
          queuedTileEvents_.put(
              event.getZIndex(), new LinkedBlockingQueue<ExploreTileWaitingToAcquire>());
        }

        ExploreTileWaitingToAcquire tile =
            new ExploreTileWaitingToAcquire(
                event.getGridRow(),
                event.getGridCol(),
                event.getZIndex(),
  ...
  public class ExploreTileWaitingToAcquire {
    ...
    public boolean equals(Object other) {
      ...
      if (((ExploreTileWaitingToAcquire) other).col == col
          && ((ExploreTileWaitingToAcquire) other).row == row
