...
public class MagellanGUIAcquisition extends Acquisition implements MagellanAcquisition {
  ...
  /**
   * Acquisition with fixed XY positions (although they can potentially all be translated between
   * time points Supports time points Z stacks that can change at positions between time points
   *
   * <p>Acquisition engine manages a thread that reads events, fixed area acquisition has another
   * thread that generates events
   *
   * @param settings
  ...
  private Iterator<AcquisitionEvent> buildAcqEventGenerator() {
    ...
    if (((MagellanGUIAcquisitionSettings) settings_).timeEnabled_) {
      acqFunctions.add(
          AcqEventModules.timelapse(
              ((MagellanGUIAcquisitionSettings) settings_).numTimePoints_,
              (int)
                  (((MagellanGUIAcquisitionSettings) settings_).timePointInterval_
                      * (((MagellanGUIAcquisitionSettings) settings_).timeIntervalUnit_ == 1
                          ? 1000
                          : (((MagellanGUIAcquisitionSettings) settings_).timeIntervalUnit_ == 2
                              ? 60000
                              : 1)))));
    }
    if (positions_ != null) {
  ...
  public double getTimeInterval_ms() {
    return ((MagellanGUIAcquisitionSettings) settings_).timePointInterval_
        * (((MagellanGUIAcquisitionSettings) settings_).timeIntervalUnit_ == 1
            ? 1000
            : (((MagellanGUIAcquisitionSettings) settings_).timeIntervalUnit_ == 2 ? 60000 : 1));
  }
    ...
    return (AcquisitionEvent event) -> {
      return new Iterator<AcquisitionEvent>() {
        ...
        public AcquisitionEvent next() {
          ...
          // Do plus equals here in case z positions have been modified by another function (e.g.
          // channel specific focal offsets)

          sliceEvent.setZ(
              sliceIndex_,
              (sliceEvent.getZPosition() == null ? 0 : sliceEvent.getZPosition()) + zPos);
          sliceIndex_++;
          return sliceEvent;
        }
      };
  ...
  private Function<AcquisitionEvent, Iterator<AcquisitionEvent>> surfaceGuided2D() {
    return (AcquisitionEvent event) -> {
      ...
      if (((MagellanGUIAcquisitionSettings) settings_)
          .collectionPlane_
          .getCurentInterpolation()
          .isInterpDefined(event.getXPosition(), event.getYPosition())) {
        zPos =
            ((MagellanGUIAcquisitionSettings) settings_)
                .collectionPlane_
                .getCurentInterpolation()
                .getInterpolatedValue(event.getXPosition(), event.getYPosition());
      } else {
        zPos =
            ((MagellanGUIAcquisitionSettings) settings_)
                .collectionPlane_.getExtrapolatedValue(event.getXPosition(), event.getYPosition());
      }
      event.setZ(0, zPos);
      // Make z index all 0 for the purposes of the display even though they may be in very
      // differnet locations
      return Stream.of(event).iterator();
    };
  }

  public static boolean isImagingVolumeUndefinedAtPosition(
      int spaceMode, MagellanGUIAcquisitionSettings settings, Point2D.Double[] displayPosCorners) {
  ...
  /**
   * This function and the one below determine which slices will be collected for a given position
   *
   * @param zPos
   * @return
   */
  public static boolean isZAboveImagingVolume(
      int spaceMode,
    ...
    if (spaceMode == MagellanGUIAcquisitionSettings.SURFACE_FIXED_DISTANCE_Z_STACK) {
      ...
      return settings.fixedSurface_.isPositionCompletelyAboveSurface(
          positionCorners,
          settings.fixedSurface_,
          zPos + settings.distanceAboveFixedSurface_,
          extrapolate);
    } else if (spaceMode == MagellanGUIAcquisitionSettings.VOLUME_BETWEEN_SURFACES_Z_STACK) {
      return settings.topSurface_.isPositionCompletelyAboveSurface(
          positionCorners, settings.topSurface_, zPos + settings.distanceAboveTopSurface_, false);
  ...
  public static boolean isZBelowImagingVolume(
      int spaceMode,
      MagellanGUIAcquisitionSettings settings,
      Point2D.Double[] positionCorners,
      double zPos,
  ...
  //
  public static double getZTopCoordinate(
      int spaceMode,
      MagellanGUIAcquisitionSettings settings,
      boolean zStageHasLimits,
      double zStageLowerLimit,
      double zStageUpperLimit,
      String zStage) {
    ...
    if (spaceMode == MagellanGUIAcquisitionSettings.VOLUME_BETWEEN_SURFACES_Z_STACK
      ...
      if (dir > 0) {
        ...
        Log.log(
            "Couldn't get focus direction of Z drive. Configre using \"Devices--Hardware Configuration Wizard\"");
    ...
    if (spaceMode == MagellanGUIAcquisitionSettings.SURFACE_FIXED_DISTANCE_Z_STACK) {
      ...
      if (towardsSampleIsPositive) {
        ...
        double top =
            interpPoints.get(interpPoints.size() - 1).z + settings.distanceAboveFixedSurface_;
      ...
      if (towardsSampleIsPositive) {
        ...
        List<Point3d> interpPoints = settings.topSurface_.getPoints();
        double top =
            interpPoints.get(interpPoints.size() - 1).z + settings.distanceAboveTopSurface_;
        return zStageHasLimits ? Math.max(zStageLowerLimit, top) : top;
      }
    } else if (spaceMode == MagellanGUIAcquisitionSettings.CUBOID_Z_STACK) {
  ...
  // TODO: this could be generalized into a method to get metadata specific to any acwuisiton
  // surface type
  ...
  private void createXYPositions() {
    try {
      if (((MagellanGUIAcquisitionSettings) settings_).xyFootprint_
          == null) { // Use current stage position
