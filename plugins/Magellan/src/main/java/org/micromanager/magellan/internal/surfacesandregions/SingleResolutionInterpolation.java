...
public class SingleResolutionInterpolation {
  ...
  public SingleResolutionInterpolation(
      int pixPerPoint,
      boolean[][] defined,
      float[][] interp,
      float[][] normals,
      ...
      double boundYMin,
      double boundYMax,
      Region<Euclidean2D> ch,
      Vector2D[] convexHullVertices) {
  ...
  /**
   * Get the angle of the surface normal at this point to vertical If extrpolated value, return 0
   *
   * @param x
   * @param y
   * @return
  ...
  public float getNormalAngleToVertical(double x, double y) {
    ...
    int xIndex =
        (int) Math.round(((x - boundXMin_) / (boundXMax_ - boundXMin_)) * (numInterpPointsX - 1));
    int yIndex =
        (int) Math.round(((y - boundYMin_) / (boundYMax_ - boundYMin_)) * (numInterpPointsY - 1));
  ...
  public boolean isInterpDefined(double x, double y) {
    ...
    if (xIndex >= 0
        && yIndex >= 0
        && xIndex < interpolation_[0].length
        && yIndex < interpolation_.length) {
