...
/**
 * Subclass that implements a particular interpolation method This one creates a plane based on the
 * 3 closest points in XY space using the delaunay triangulation and calculates specific z values in
 * that plane
 */
public class SurfaceInterpolatorSimple extends SurfaceInterpolator {
      ...
      for (int yInd = 0; yInd < interpVals.length; yInd++) {
        for (int xInd = 0; xInd < interpVals[0].length; xInd++) {
          ...
          if (inHull) {
            ...
            float angle =
                (float)
                    (Vector3D.angle(plane.getNormal(), new Vector3D(0, 0, 1)) / Math.PI * 180.0);
  ...
  public float getExtrapolatedValue(double x, double y) {
    // If there are only three points, assume that user wants to extrapolate to do a tilted plane
    // acquistion
    // otherwise, do a nearest neightbor interpolation to avoid the extrapolation of unintended
    // crazy z values
    ...
    if (points.size() == 3) {
      ...
      // intersetion of vertical line at these x+y values with plane gives point in plane
      Vector3D pointInPlane =
          plane.intersection(new Line(new Vector3D(x, y, 0), new Vector3D(x, y, 1), TOLERANCE));
