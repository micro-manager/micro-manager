...
public class MagellanAcquisitionsManager {
  ...
  private CopyOnWriteArrayList<MagellanGUIAcquisitionSettings> acqSettingsList_ =
      new CopyOnWriteArrayList<MagellanGUIAcquisitionSettings>();
  ...
  public MagellanAcquisitionsManager(GUI gui) {
    ...
    acqManageExecuterService_ =
        Executors.newSingleThreadScheduledExecutor(
            (Runnable r) -> new Thread(r, "Acquisition manager thread"));
  }
  ...
  public void abort() {
    int result =
        JOptionPane.showConfirmDialog(
            null,
            "Abort current acquisition and cancel future ones?",
            "Finish acquisitions?",
  ...
  public ExploreAcquisition createExploreAcquisition(
      double zStep, double overlap, String dir, String name, String cGroup) {
    if (!AffineTransformUtils.isAffineTransformDefined()) {
      ReportingUtils.showError(
          "XY Stage and Camera are not calibrated to each other."
    ...
    if (exploreAcq_ != null && !exploreAcq_.areEventsFinished()) {
      int result =
          JOptionPane.showConfirmDialog(
              ...
              "Finish Current Explore Acquisition",
              JOptionPane.OK_CANCEL_OPTION);
      if (result == JOptionPane.OK_OPTION) {
        exploreAcq_.abort();
      } else {
    ...
    exploreAcq_ =
        new ExploreAcquisition(
            settings, new MagellanDataManager(settings.dir_, settings.name_, true));
    return exploreAcq_;
  }
  ...
  public MagellanGUIAcquisition createAcquisition(int index) {
    ...
    return new MagellanGUIAcquisition(
        settings, new MagellanDataManager(settings.dir_, settings.name_, true));
  }
  ...
  public void runAllAcquisitions() {
    for (MagellanGUIAcquisitionSettings settings : acqSettingsList_) {
      try {
        ...
        JOptionPane.showMessageDialog(
            gui_, "Problem with acquisition settings: \n" + ex.getMessage());
        return;
      }
    }
    ...
    acqManageExecuterService_.submit(
        () -> {
          // TODO: once an API, disable adding and deleting of acquisitions here or just copy the
          // list
  ...
  private void validateSettings(MagellanGUIAcquisitionSettings settings) throws Exception {
    ...
    if ((settings.spaceMode_ == MagellanGUIAcquisitionSettings.REGION_2D
      ...
      throw new Exception(
          "Error: XY device for surface/grid does match XY device in MM core in " + settings.name_);
    }
    if (settings.spaceMode_ == MagellanGUIAcquisitionSettings.SURFACE_FIXED_DISTANCE_Z_STACK
        && !settings.fixedSurface_.getXYDevice().equals(Magellan.getCore().getXYStageDevice())) {
      Log.log(
          "Error: XY device for surface does match XY device in MM core in " + settings.name_,
          true);
      throw new Exception();
    }
    if (settings.spaceMode_ == MagellanGUIAcquisitionSettings.VOLUME_BETWEEN_SURFACES_Z_STACK
        && (!settings.topSurface_.getXYDevice().equals(Magellan.getCore().getXYStageDevice())
            || !settings
                .bottomSurface_
                .getXYDevice()
                .equals(Magellan.getCore().getXYStageDevice()))) {
      throw new Exception(
          "Error: XY device for surface does match XY device in MM core in " + settings.name_);
    }
    ...
    if (settings.spaceMode_ == MagellanGUIAcquisitionSettings.SURFACE_FIXED_DISTANCE_Z_STACK
      ...
      throw new Exception(
          "Error: Z device for surface does match Z device in MM core in " + settings.name_);
    }
    if (settings.spaceMode_ == MagellanGUIAcquisitionSettings.VOLUME_BETWEEN_SURFACES_Z_STACK
