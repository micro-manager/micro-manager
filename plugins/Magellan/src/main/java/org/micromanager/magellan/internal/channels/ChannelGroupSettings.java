...
/**
 * Class to encapsulate a bunch of ChannelsSettings. Should be owned by a specific acquisition
 * settings object
 *
 * @author Henry
 */
public class ChannelGroupSettings {
  ...
  private static final Color[] DEFAULT_COLORS = {
    new Color(160, 32, 240), Color.blue, Color.green, Color.yellow, Color.red, Color.pink
  };
  ...
  public void updateChannelGroup(String channelGroup) {
    ...
    if (numCamChannels <= 1) {
      for (String config : getChannelConfigs(channelGroup)) {
        channels_.add(
            new SingleChannelSetting(
                channelGroup,
                channelGroup == null || channelGroup.equals("") ? null : config,
                1,
                0));
      }
    }
  ...
  public Color getPreferredChannelColor(String channelName) {
    String prefix =
        MagellanGUIAcquisitionSettings.PREF_PREFIX
            + "CHANNELGROUP"
            + group_
            + "CHANNELNAME"
            + channelName;
    return new Color(
        GlobalSettings.getInstance()
            .getIntInPrefs(
                prefix + PREF_COLOR,
                DEFAULT_COLORS[new Random().nextInt(DEFAULT_COLORS.length)].getRGB()));
  }
  ...
  private void setValuesFromPrefs(SingleChannelSetting setting) {
    ...
    try {
      setting.exposure_ =
          GlobalSettings.getInstance()
              .getDoubleInPrefs(prefix + PREF_EXPOSURE, Magellan.getCore().getExposure());
    } catch (Exception ex) {
      Log.log(ex);
    }
  ...
  public void storeValuesInPrefs() {
    for (SingleChannelSetting setting : channels_) {
      ...
      GlobalSettings.getInstance().storeIntInPrefs(prefix + PREF_COLOR, setting.color_.getRGB());
