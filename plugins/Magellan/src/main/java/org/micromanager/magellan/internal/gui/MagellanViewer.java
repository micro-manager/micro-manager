...
/**
 * Extends the ND viewer with the information that the image is made up of a bunch of tiles with Row
 * and column indices
 *
 * @author henrypinkard
 */
public class MagellanViewer implements ViewerInterface {
  ...
  public MagellanViewer(
      MagellanDataManager cache, ViewerAcquisitionInterface acq, JSONObject summmaryMD) {
    viewer_ =
        new NDViewer(
            cache,
            acq,
            summmaryMD,
            MagellanMD.getPixelSizeUm(summmaryMD),
  ...
  private void moveViewToVisibleArea() {
    ...
    Set<Point> tiles =
        manager_.getTileIndicesWithDataAt(viewer_.getAxisPosition(AcqEngMetadata.Z_AXIS));
    if (tiles.size() == 0) {
      return;
    }
    ...
    for (Point p : tiles) {
      ...
      double tl =
          ((tileX1 - fovX2) * (tileX1 - fovX2)
              + (tileY1 - fovY2) * (tileY1 - fovY2)); // top left tile, botom right fov
      double tr =
          ((tileX2 - fovX1) * (tileX2 - fovX1)
              + (tileY1 - fovY2) * (tileY1 - fovY2)); // top right tile, bottom left fov
      double bl =
          ((tileX1 - fovX2) * (tileX1 - fovX2)
              + (tileY2 - fovY1) * (tileY2 - fovY1)); // bottom left tile, top right fov
      double br =
          ((tileX1 - fovX1) * (tileX1 - fovX1)
              + (tileY2 - fovY1) * (tileY2 - fovY1)); // bottom right tile, top left fov
    ...
    DoubleStream dists =
        newPos.stream()
            .mapToDouble(
                value ->
                    Math.pow(value.x - finalCurrentX, 2) + Math.pow(value.y - finalCurrentY, 2));
  ...
  /**
   * return the pixel location in coordinates at appropriate res level of the top left pixel for the
   * given row/column
   *
   * @param row
   * @param col
