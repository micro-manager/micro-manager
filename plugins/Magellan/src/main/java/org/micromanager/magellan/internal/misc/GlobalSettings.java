...
public class GlobalSettings {
  ...
  public GlobalSettings() {
    ...
    try {
      for (int i = 0; i < 6; i++) {
        chOffsets_[i] =
            prefs_.getInteger(
                CHANNEL_OFFSET_PREFIX + Magellan.getCore().getCurrentPixelSizeConfig() + i, 0);
      }
    } catch (Exception ex) {
  ...
  public String getStoredSavingDirectory() {
    return prefs_.getString(
        SAVING_DIR, FileSystemView.getFileSystemView().getHomeDirectory().getAbsolutePath());
  }
