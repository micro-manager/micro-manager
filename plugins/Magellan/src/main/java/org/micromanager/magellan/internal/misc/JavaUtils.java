...
public class JavaUtils {
  ...
  /**
   * Add directories and JARs to the classpath, and return the classes found in the process.
   *
   * <p>This method does two things that are really completely separate tasks.
   *
   * <p>First, it adds the given directory and all JARs directly in directory to the search path of
   * the system class loader. If recursionLevel is greater than 0, it does the same with
   * subdirectories, up to that level of nesting.
   *
   * <p>Second, it finds all classes in the directories and JARs that were added to the class path
   * in the first step, and returns them. Classes are found anywhere within the JARs, or as .class
   * files directly within directories.
   *
   * <p>There is no support for .class files contained in a hierarchy of directories representing
   * the package names (they will be loadable because the directory is in the search path, but they
   * will not be included in the returned list unless they are within the recursionLevel).
   *
    ...
    for (File file : files) {
      ...
      if (file.isDirectory() && recursionLevel > 0) {
        ...
        try {
          ...
          Log.log("Failed to load class: " + className + " (expected in " + fileName + ")");
        }
  ...
  /**
   * Call a private method using reflection. Use looks like invokeRestrictedMethod(Object obj, Class
   * theClass, String methodName, Object param1, Class paramType1, Object param2, Class paramType2,
   * ...)
   */
  public static Object invokeRestrictedMethod(
      Object obj, Class theClass, String methodName, Object... paramsAndTypes)
      throws NoSuchMethodException, IllegalAccessException, IllegalArgumentException,
          InvocationTargetException {
    Object[] params;
    Class[] paramTypes;
   ...
   */
  public static Object invokeRestrictedMethod(
      Object obj, Class<?> theClass, String methodName, Object[] params, Class[] paramTypes)
      throws NoSuchMethodException, IllegalAccessException, IllegalArgumentException,
          InvocationTargetException {
   ...
   * Pass a null first argument for static fields.
   */
  public static void setRestrictedFieldValue(
      Object obj, Class theClass, String fieldName, Object value) throws NoSuchFieldException {
    Field field = theClass.getDeclaredField(fieldName);
    field.setAccessible(true);
    try {
      field.set(obj, value);
  ...
  /**
   * Test whether preference can be written to disk from:
  ...
  /**
   * Find out how much unused memory (in bytes) is still available for the JVM to use. On a MacBook
   * Pro this call takes 0.5 usec.
   */
   ...
   *
   * <p>Copies elements in original array to a new array, from index start(inclusive) to
   ...
   * @param end the end index, exclusive, may bigger than length of the array
   * @return the new copied array
   * @throws ArrayIndexOutOfBoundsException if start is smaller than 0 or bigger than
   *     original.length
   * @throws IllegalArgumentException if start is bigger than end
