...
public class MagellanMouseListener implements CanvasMouseListenerInterface {
  ...
  private void mouseReleasedActions(MouseEvent e) {
    if (exploreMode_ && SwingUtilities.isLeftMouseButton(e)) {
      ...
      if (exploreStartTile_ != null) {
        ...
        exploreStartTile_ =
            viewer_.getTileIndicesFromDisplayedPixel(
      ...
      viewer_.redrawOverlay();
        ...
        && manager_.getCurrentEditableSurfaceOrGrid() != null
        && manager_.getCurrentEditableSurfaceOrGrid() instanceof SurfaceInterpolator
        && manager_.isCurrentlyEditableSurfaceGridVisible()) {
      SurfaceInterpolator currentSurface =
          (SurfaceInterpolator) manager_.getCurrentEditableSurfaceOrGrid();
      if (SwingUtilities.isRightMouseButton(e) && !mouseDragging_) {
        ...
        if (e.isShiftDown()) {
          ...
          currentSurface.deletePointsWithinZRange(
          ...
          Point2D.Double stagePos =
              manager_.stageCoordsFromPixelCoords(e.getPoint().x, e.getPoint().y);
          // calculate tolerance
          Point2D.Double toleranceStagePos =
          ...
          double stageDistanceTolerance =
              Math.sqrt(
                  (toleranceStagePos.x - stagePos.x) * (toleranceStagePos.x - stagePos.x)
                      + (toleranceStagePos.y - stagePos.y) * (toleranceStagePos.y - stagePos.y));
          currentSurface.deleteClosestPoint(
              stagePos.x,
              stagePos.y,
              stageDistanceTolerance,
              Math.min(z - manager_.getZStep() / 2, z + manager_.getZStep() / 2),
              Math.max(z - manager_.getZStep() / 2, z + manager_.getZStep() / 2));
        }
      } else if (SwingUtilities.isLeftMouseButton(e)) {
  ...
  private void mouseDraggedActions(MouseEvent e) {
    ...
    if (SwingUtilities.isRightMouseButton(e)) {
      // pan
      viewer_.pan(
          mouseDragStartPointRight_.x - currentPoint.x,
