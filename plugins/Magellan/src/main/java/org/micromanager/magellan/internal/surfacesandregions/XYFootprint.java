...
public abstract class XYFootprint {
  ...
  protected Region<Euclidean2D> getStagePositionRegion(Point2D.Double[] dispPositionCorners) {
    ...
    double centerX =
        (dispPositionCorners[0].x
                + dispPositionCorners[1].x
                + dispPositionCorners[2].x
                + dispPositionCorners[3].x)
            / 4;
    double centerY =
        (dispPositionCorners[0].y
                + dispPositionCorners[1].y
                + dispPositionCorners[2].y
                + dispPositionCorners[3].y)
            / 4;

    return square.checkPoint(new Vector2D(centerX, centerY)) == Region.Location.OUTSIDE
        ? regionFactory_.getComplement(square)
        : square;
  }
}
