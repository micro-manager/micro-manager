...
public class DuplicatorPluginFrame extends JDialog {
  ...
  public DuplicatorPluginFrame(Studio studio, DisplayWindow window) {
      ...
      for (final String axis : axes) {
        if (ourProvider_.getNextIndex(axis) > 1) {
          ...
          SpinnerNumberModel model =
              new SpinnerNumberModel(1, 1, (int) ourProvider_.getNextIndex(axis), 1);
          mins.put(axis, 0);
          ...
          model =
              new SpinnerNumberModel(
                  (int) ourProvider_.getNextIndex(axis),
                  1,
                  (int) ourProvider_.getNextIndex(axis),
                  1);
    ...
    OKButton.addActionListener(
        new ActionListener() {
          ...
          public void actionPerformed(ActionEvent ae) {
            cpFrame.dispose();
            DuplicatorExecutor de =
                new DuplicatorExecutor(studio_, ourWindow_, nameField.getText(), mins, maxes);
            final ProgressBar pb = new ProgressBar(ourWindow_.getWindow(), "Duplicating..", 0, 100);
            de.addPropertyChangeListener(
    ...
    super.setLocation(xCenter - super.getWidth() / 2, yCenter - super.getHeight());
