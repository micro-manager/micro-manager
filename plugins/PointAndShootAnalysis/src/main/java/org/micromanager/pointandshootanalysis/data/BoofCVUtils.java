...
public class BoofCVUtils {
   ...
   /**
    * compress a histogram.  For instance, given a histogram of 1024 values
    * and factor 4, a similarly shaped histogram of 256 values will be returned
   ...
   /**
    * The MaxEntropy threshold method gives quite different values when using
    * the "full" histogram of U16 images (for instance, a histogram with a length
    * of 4000 gray scale values), rather than a histogram "compressed" to 
    * 256 gray scale values.  The latter somehow often works better.  This 
    * function calculates the threshold based on a user-defines range
    * of the histogram.  Note this can also speed up the calculations quite a bit.
    * 
    * @param img
