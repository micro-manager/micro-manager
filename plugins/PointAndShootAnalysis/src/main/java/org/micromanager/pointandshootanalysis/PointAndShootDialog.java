...
public class PointAndShootDialog extends JDialog {
  ...
  PointAndShootDialog(Studio studio) {
    studio_ = studio;
    profileSettings_ = studio_.profile().getSettings(PointAndShootDialog.class);
    wasDisposed_ = false;
    ...
    locationsField.setText(
        profileSettings_.getString(
            Terms.LOCATIONSFILENAME, profileSettings_.getString(Terms.LOCATIONSFILENAME, "")));
    ...
    locationsFieldButton.addActionListener(
        (ActionEvent evt) -> {
          File f =
    ...
    beforeSpinner.addChangeListener(
        (ChangeEvent e) -> {
    ...
    */
    ...
    maxDistanceSpinner.addChangeListener(
        (ChangeEvent e) -> {
    ...
    fixBleachBox.addChangeListener(
        (ChangeEvent e) -> {
    ...
    final SpinnerNumberModel fixFramesModel =
        new SpinnerNumberModel(nrFramesToFixBleach, 1, 100, 1);
    final JSpinner fixBleachSpinner = new JSpinner(fixFramesModel);
    fixBleachSpinner.setMinimumSize(spinnerSize);
    fixBleachSpinner.addChangeListener(
        (ChangeEvent e) -> {
          profileSettings_.putInteger(
              Terms.NRFRAMESTOFIXBLEACH, (Integer) fixBleachSpinner.getValue());
        });
        ...
        (ActionEvent evt) -> {
          new Thread(
                  org.micromanager.internal.utils.GUIUtils.makeURLRunnable(
                      "https://micro-manager.org/wiki/Point_and_Shoot_Analysis"))
              .start();
        });
    ...
    okButton.addActionListener(
        (ActionEvent evt) -> {
          ...
          Thread analysisThread =
              new Thread(new PointAndShootAnalyzer(studio, profileSettings_.toPropertyMap(), this));
