...
public class PointAndShootAnalyzer implements Runnable {
  ...
  static final double DIST_UNCERTAINTY = 5.0; // in pixels

  private final Studio studio_;
  private final PointAndShootDialog psd_;
  ...
  /*
    ...
    May need to be changed by the user
  */
  private final int findMinFramesAfter_ = 10;
  private final int roiSize_ = 64; // keep at factor of 2 to enable FFT
  private final int halfROISize_ = roiSize_ / 2;
  private final int roiWidth_ = roiSize_; // may need to changed by user
  private final int roiHeight_ = roiSize_; // may need to changed by user
  ...
  public void run() {
    ...
    final boolean continueBleachSpotTracking =
        settings_.getBoolean(Terms.FIXBLEACHINPARTICLE, true);
    final int nrFramesToMeasureBleachToParticleVector =
        settings_.getInteger(Terms.NRFRAMESTOFIXBLEACH, 20);
    ...
    for (Map.Entry<String, Point> entry : coordinates_.entrySet()) {
      datedCoordinates.put(dateStringToInstant(entry.getKey()), entry.getValue());
    }
    ...
    try {
      ...
      for (int frame = 0; frame < dataProvider.getNextIndex(Coords.T); frame++) {
        bCVStack.setBand(
            frame,
            BoofCVImageConverter.mmToBoofCV(dataProvider.getImage(cbb.t(frame).build()), false));
      }
      ...
      while (pasDataIt.hasNext()) {
        ...
        */
        ...
        String id = "" + pasEntry.framePasClicked() + ": " + pasActual.x + ", " + pasActual.y;
        pasDataIt.set(pasEntry.copyBuilder().id(id).pasActual(pasActual).pasFrames(null).build());
      }
      ...
      while (pasDataIt.hasNext()) {
        ...
        ParticleData firstParticle =
            ParticleData.centralParticle(
                dataProvider, cb, pasEntry.framePasClicked() + 1, bleachPoint, halfROISize_);
        if (firstParticle == null) {
          continue;
        }
        Point2D_I32 currentPoint = firstParticle.getCentroid().copy();
        for (int frame = pasEntry.framePasClicked() + 1; frame >= 0; frame--) {
          ParticleData nextParticle =
              ParticleData.centralParticle(dataProvider, cb, frame, currentPoint, halfROISize_);
        ...
        GrayU16 preBleach =
            (GrayU16)
                BoofCVImageConverter.subImage(
                    dataProvider, cb, pasEntry.framePasClicked() + 1, currentPoint, halfROISize_);
        if (preBleach == null) {
          continue;
        }
        ...
        for (int frame = pasEntry.framePasClicked() + 2;
            frame < dataProvider.getNextIndex(Coords.T);
            frame++) {
          ParticleData nextParticle =
              ParticleData.centralParticle(dataProvider, cb, frame, currentPoint, halfROISize_);
          if (nextParticle == null
        ...
        final int getCalculateVectorFrame =
            pasEntry.framePasClicked() + 5 + nrFramesToMeasureBleachToParticleVector;
        ...
        for (int frame = pasEntry.framePasClicked() + 2;
          ...
          if (bleachSpotsMissed < 5 || continueBleachSpotTracking) {
            ...
            ImageGray current =
                BoofCVImageConverter.subImage(dataProvider, cb, frame, currentPoint, halfROISize_);
            if (current != null) {
              Point2D_I32 offset =
              ...
              if (offsetVector != null) {
                ...
                Point2D_I32 bp =
                    new Point2D_I32(centroid.x - offsetVector.x, centroid.y - offsetVector.y);
                particle =
                    ParticleData.addBleachSpotToParticle(
                        particle, (GrayU16) current, offset, bp, bleachSpotRadius);
              } else {
                particle =
                    ParticleData.addBleachSpotToParticle(
                        fPreBleach,
          ...
          if (continueBleachSpotTracking && frame == getCalculateVectorFrame) {
            List<Point2D_I32> vectors = new ArrayList<>(nrFramesToMeasureBleachToParticleVector);
            ...
            for (int vFrame = startFrame; vFrame < endFrame; vFrame++) {
              ...
              if (pd != null) {
                ...
                if (c != null && b != null) {
                  Point2D_I32 vector = new Point2D_I32(c.x - b.x, c.y - b.y);
                  vectors.add(vector);
                }
              }
            }
        ...
        tracks.add(track);
        pasDataIt.set(pasEntry.copyBuilder().particleDataTrack(track).build());
            ...
            for (int j = 0; j < tracks.size(); j++) {
              if (j != i) {
                ...
                if (otherFirstParticle != null
                    && firstParticle.getCentroid().distance(otherFirstParticle.getCentroid())
                        < identityDistance) {
                ...
                .forEach(
                    (entry) -> {
                      List<ParticleData> particlesInFrame =
                          tracksIndexedByFrame.get(entry.getKey());
      ...
      try {
        ...
        GrayU16 img0 = (GrayU16) BoofCVImageConverter.mmToBoofCV(dataProvider.getImage(ct0), false);
      ...
      if (controlTracks.size() > 0) {
        ...
        for (PASData d : cleanedPASData) {
          d.normalizeBleachSpotIntensities(findMinFramesBefore, cameraOffset, controlAvgIntensity);
          d.normalizeParticleIncludingBleachIntensities(
              findMinFramesBefore, cameraOffset, controlAvgIntensity);
        }
        ...
        List<XYSeries> bleachPlotData =
            plottableNormalizedBleachSpotIntensities(cleanedPASData, frameTimeStamps);
        ...
        DataExporter bleachExporter =
            new DataExporter(studio_, cleanedPASData, frameTimeStamps, DataExporter.Type.BLEACH);
        pu.plotData(
            "Bleach Intensity Profile",
            plots,
            "Time (ms)",
        ...
        List<XYSeries> particlePlotData =
            plottableNormalizedParticleIntensities(cleanedPASData, frameTimeStamps);
        ...
        DataExporter particleABExporter =
            new DataExporter(
                studio_, cleanedPASData, frameTimeStamps, DataExporter.Type.PARTICLE_AND_BLEACH);
        pu2.plotData(
   ...
   *
   * @param timeStampString String should be formatted as: 2018-10-22 17:05:12.241 yyyy-mm-dd
   *     hh:mm:ss.ms Part after the "." can be 3 digits (ms) or 6 digits (microseconds)
   ...
   * @param img
   * @param ip
   * @return public static Point findMinPixel(ImageProcessor ip) { Point p = new Point(0, 0); Float
   *     val = ip.getPixelValue(0, 0); for (int x = 0; x < ip.getWidth(); x++) { for (int y = 0; y <
   *     ip.getHeight(); y++) { if (ip.getPixelValue(x, y) < val) { p.x = x; p.y = y; val =
   *     ip.getPixelValue(x, y); } } } return p; }
   */
  ...
  private class pointAndShootParser implements Consumer<String> {
    ...
    public void accept(String t) {
      ...
      if (parts.length == 3) {
        coordinates_.put(
            parts[0], new Point(Integer.parseInt(parts[1]), Integer.parseInt(parts[2])));
      }
    }
  }

  private void plotControlParticleIntensities(
      final Map<Integer, Double> controlAvgIntensity, final Map<Integer, Instant> frameTimeStamps) {
    ...
    for (Map.Entry<Integer, Double> frameControlData : controlAvgIntensity.entrySet()) {
      data.add(
          (frameTimeStamps.get(frameControlData.getKey()).toEpochMilli()
                  - frameTimeStamps.get(0).toEpochMilli())
              / 1000.0,
      ...
      if (d.particleDataTrack() != null && d.id() != null) {
        ...
        for (int frame = 0; frame < d.particleDataTrack().size(); frame++) {
          if (d.particleDataTrack().get(frame) != null) {
            Double normalizedBleachMaskAvg =
                d.particleDataTrack().get(frame).getNormalizedBleachMaskAvg();
            ...
            if (normalizedMaskIncludingBleachAvg != null
                && normalizedMaskIncludingBleachAvg != Double.NaN) {
              particleCounter++;
            }
          }
  ...
  private List<XYSeries> plottableNormalizedBleachSpotIntensities(
      final List<PASData> pasData, final Map<Integer, Instant> frameTimeStamps) {
    List<XYSeries> plotData = new ArrayList<>();
    for (PASData d : pasData) {
      if (d.particleDataTrack() != null && d.id() != null) {
        ...
        for (int frame = 0; frame < d.particleDataTrack().size(); frame++) {
          if (d.particleDataTrack().get(frame) != null) {
            ...
            if (normalizedBleachMaskAvg != null && normalizedBleachMaskAvg != Double.NaN) {
              data.add(
                  frameTimeStamps.get(frame).toEpochMilli()
  ...
  private List<XYSeries> plottableNormalizedParticleIntensities(
      final List<PASData> pasData, final Map<Integer, Instant> frameTimeStamps) {
    List<XYSeries> plotData = new ArrayList<>();
    for (PASData d : pasData) {
      if (d.particleDataTrack() != null) {
        XYSeries data =
            new XYSeries(
                "" + d.framePasClicked() + ": " + d.pasActual().x + ", " + d.pasActual().y,
                false,
                false);
        for (int frame = 0; frame < d.particleDataTrack().size(); frame++) {
          if (d.particleDataTrack().get(frame) != null) {
  ...
  */
  ...
  private Rectangle2D_I32 boundingBoxSize(
      DataProvider dp, Coords.Builder cb, int frame, Point2D_I32 p) throws IOException {
    ...
    ImageGray sub =
        (ImageGray)
            ig.subimage(
                (int) p.getX() - halfROISize_,
                (int) p.getY() - halfROISize_,
                (int) p.getX() + halfROISize_,
                (int) p.getY() + halfROISize_);
    int threshold =
        GThresholdImageOps.computeOtsu2(
            sub, 0, (int) sub.getImageType().getDataType().getMaxValue());
    ...
    Map<Point2D_I32, List<Point2D_I32>> centroidedClusters = new HashMap<>();
    clusters.forEach(
        (cluster) -> {
