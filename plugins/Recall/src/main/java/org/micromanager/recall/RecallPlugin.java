 ...
 *
 * <p>This plugin will copy all images in the Micro-Manager circular buffer into a viewer window.
 * The counter of the circular buffer is set to 0 (i.e. images in the circular buffer will be
 * destroyed.
 *
 * <p>This is useful when you see something interesting in live mode and want to save this data.
 *
 * <p>Nico Stuurman, 2009(?)
 *
 * <p>Updated June 2013 to use ImageProcssor Queue, so that images from multiple cameras are
 * displayed correctly and so that images are all processed by the default image processing queue.
 *
    ...
    if (numCameraChannels == 1) {
      ...
      for (int i = 0; i < remaining; i++) {
        try {
          ...
          Image convertedTaggedImage = studio_.data().convertTaggedImage(tImg);
          if (convertedTaggedImage != null) {
            store_.putImage(normalizeTags(convertedTaggedImage, tImg, frameCounters[channelIndex]));
          }
