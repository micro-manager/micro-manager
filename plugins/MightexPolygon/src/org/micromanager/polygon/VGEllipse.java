 ...
 * @author Wayne
 */
public class VGEllipse implements VectorGraphElement {
    ...
    {
        if( vg instanceof VGEllipse ){
            VGEllipse e = ( VGEllipse ) vg;
            return e.color_ == color_ && e.pt1_ == pt1_ && e.pt2_ == pt2_;
        }
        else
            return false;
    }
    ...
    public void draw( Graphics g, Rectangle2D.Float r, boolean selected )
    {
        ...
        g2.setColor( color_ );
        g2.fill( new Ellipse2D.Float( left, top, right - left, bottom - top ) );
    ...
    public boolean isSelected( Point pt, Rectangle2D.Float r )
    {
        ...
        float x2 = r.x + pt2_.x * r.width;
        float y2 = r.y + pt2_.y * r.height;
        Ellipse2D.Float rc = new Ellipse2D.Float();
        rc.x = Math.min( x1, x2 );
        rc.y = Math.min( y1, y2 );
    ...
    public void moveKeyPoint( int kpidx, Point2D.Float pt )
    {
        switch( kpidx ){
            ...
            case 7:
                ...
                break;
            default:
                throw new IllegalArgumentException( "Key point index " + kpidx + " invalid" );
        }
    }
