...
/**
 * The main frame class of the plugin , since the plugin is simple didn't use model-view-controller
 * convention
 *
...
public class AsiGamepadFrame extends JFrame {
  ...
  public AsiGamepadFrame(Studio gui) {
    this.setLayout(new MigLayout("", "[grow,fill]rel[grow,fill]", "[]8[]"));
  ...
  private Vector<String> getColumnNames(JTable table) {
    ...
    for (int i = 0; i < table.getColumnCount(); i++) columnNames.add(table.getColumnName(i));
    return columnNames;
  }
