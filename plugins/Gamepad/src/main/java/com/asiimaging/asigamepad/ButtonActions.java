...
public class ButtonActions {
    ...
    * @param arg string with the beanshell scripts absolute path. can use null
    *            if enum to be executed isn't Run_Beanshell_script
    */
   public void executeAction(ActionItems ai, String arg) {

      MMStudio mmStudio = MMStudio.getInstance();
      SnapLiveManager snapLiveManager = mmStudio.getSnapLiveManager();
      ...
      switch (ai) {
         ...
         case Snap:
            ...
            break;
         case Toggle_Live:
            snapLiveManager.setLiveModeOn(!snapLiveManager.isLiveModeOn());
            break;
         case Toggle_Shutter:
            try {
         ...
         case Autostretch_histograms:
            ...
            if (dv != null) {
               dv.setDisplaySettings(
                     dv.getDisplaySettings().copyBuilder().autostretch(true).build());
               dv.setDisplaySettings(
                     dv.getDisplaySettings().copyBuilder().autostretch(false).build());
            }
            break;
