  ...
  public CustomSettingsFrame(Studio studio, SiteGenerator parent) {
    ...
    super.add(
        new JLabel(
            "<html><body>If you believe your settings would be useful to other labs, "
                ...
                + "micro-manager-general@lists.sourceforge.net</body></html>"),
        "span, wrap");
    ...
    save.addActionListener(
        (ActionEvent e) -> {
          File target =
              FileDialogs.save(
                  CustomSettingsFrame.this,
                  "Select a file to save the settings to",
                  SBSPlate.PLATE_FILE);
          if (target != null) {
            saveToFile(target);
          }
  ...
  private void saveToFile(File target) {
    try {
      try (FileWriter writer = new FileWriter(target)) {
        writer.write(
            SBSPlate.serialize(
                Integer.parseInt(rows_.getText()),
