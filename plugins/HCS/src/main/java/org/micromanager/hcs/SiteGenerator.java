...
public class SiteGenerator extends JFrame implements ParentPlateGUI {
      ...
      if (result == JOptionPane.YES_OPTION) {
        try {
          MenuPlugin pc =
              app_.plugins()
                  .getMenuPlugins()
                  .get("org.micromanager.pixelcalibrator.PixelCalibratorPlugin");
          pc.onPluginSelected();
        } catch (Exception ex) {
  ...
  public SiteGenerator(Studio app) {
    ...
    JPanel contentsPanel = new JPanel(new MigLayout("fill, flowx, insets 0, gap 0"));
    ...
    selectWells_ =
        new JToggleButton(
            "Select", IconLoader.getIcon("/org/micromanager/icons/mouse_cursor_on.png"));
    moveStage_ =
        new JToggleButton("Move", IconLoader.getIcon("/org/micromanager/icons/move_hand.png"));

    selectWells_.setToolTipText("Click and drag to select wells.");
    selectWells_.addActionListener(
        (ActionEvent arg0) -> {
    ...
    plateIDCombo_.addActionListener(
        (final ActionEvent e) -> {
    ...
    FocusListener regeneratePlateOnLossOfFocus =
        new FocusListener() {
          @Override
    ...
    JPanel gridPanel = new JPanel(new MigLayout("flowx, gap 0, insets 0", "0[]0[]0", "0[]0[]0"));
    ...
    spacingMode_ = new JComboBox(new String[] {EQUAL_SPACING, DIFFERENT_SPACING, VIEW_SPACING});
    sidebar.add(spacingMode_, "growx");
    spacingMode_.setSelectedIndex(0);
    spacingMode_.addActionListener(
        (ActionEvent e) -> {
          String mode = (String) spacingMode_.getSelectedItem();
          switch (mode) {
            case EQUAL_SPACING:
    ...
    final JButton refreshButton =
        new JButton("Refresh", IconLoader.getIcon("/org/micromanager/icons/arrow_refresh.png"));
    ...
    final JButton calibrateXyButton =
        new JButton("Calibrate XY...", IconLoader.getIcon("/org/micromanager/icons/cog.png"));
    calibrateXyButton.addActionListener(
        (final ActionEvent e) -> {
          calibrateXY();
        });
    sidebar.add(calibrateXyButton, "growx");

    final JButton setPositionListButton =
        new JButton("Build MM List", IconLoader.getIcon("/org/micromanager/icons/table.png"));
    setPositionListButton.addActionListener(
        (final ActionEvent e) -> {
          if (!isCalibratedXY_) {
    ...
    JButton btnMarkPt =
        new JButton("Mark Point", IconLoader.getIcon("/org/micromanager/icons/plus.png"));
    btnMarkPt.addActionListener(
        (ActionEvent arg0) -> {
          markOnePoint();
        });
    sidebar.add(btnMarkPt, "growx");

    JButton btnSetThreePt =
        new JButton(
            "Set 3-Point List", IconLoader.getIcon("/org/micromanager/icons/asterisk_orange.png"));
    btnSetThreePt.addActionListener(
        (ActionEvent e) -> {
          setThreePoint();
        });
    ...
    chckbxThreePt_.addActionListener(
        (ActionEvent e) -> {
          platePanel_.repaint();
  ...
  protected void saveSettings() {
    app_.profile()
        .setString(SiteGenerator.class, PLATE_FORMAT_ID, (String) plateIDCombo_.getSelectedItem());
    app_.profile()
        .setString(SiteGenerator.class, SITE_SPACING_X, spacingFieldX_.getText().replace(',', '.'));
    app_.profile()
        .setString(SiteGenerator.class, SITE_SPACING_Y, spacingFieldY_.getText().replace(',', '.'));
    app_.profile().setString(SiteGenerator.class, SITE_OVERLAP, overlapField_.getText());
    app_.profile().setString(SiteGenerator.class, SITE_ROWS, rowsField_.getText());
    app_.profile().setString(SiteGenerator.class, SITE_COLS, columnsField_.getText());
    Double[] offset;
    if (isCalibratedXY_) {
  ...
  protected final void loadSettings() {
    plateIDCombo_.setSelectedItem(
        app_.getUserProfile()
            .getString(SiteGenerator.class, PLATE_FORMAT_ID, SBSPlate.SBS_96_WELL));
    rowsField_.setText(app_.profile().getString(SiteGenerator.class, SITE_ROWS, "1"));
    columnsField_.setText(app_.profile().getString(SiteGenerator.class, SITE_COLS, "1"));
    spacingFieldX_.setText(app_.profile().getString(SiteGenerator.class, SITE_SPACING_X, "200"));
    spacingFieldY_.setText(app_.profile().getString(SiteGenerator.class, SITE_SPACING_Y, "200"));
    overlapField_.setText(app_.profile().getString(SiteGenerator.class, SITE_OVERLAP, "10"));
    Double[] offset =
        app_.profile()
            .getDoubleArray(
                SiteGenerator.class, SITE_OFFSET, new Double[] {Double.NaN, Double.NaN});
    isCalibratedXY_ =
    ...
    for (WellPositionList wpl1 : wpl) {
      ...
      for (int j = 0; j < pl.getNumberOfPositions(); j++) {
        ...
        for (int k = 0; k < msp.size(); k++) {
          ...
          if (sp.is2DStagePosition()) {
            sp.set2DPosition(
                ...
                sp.get2DPositionX() + offset_.getX(),
        ...
        if (useThreePtAF()) {
          ...
          StagePosition sp =
              StagePosition.create1D(
                  focusPlane_.getZStage(), focusPlane_.getZPos(msp.getX(), msp.getY()));
          msp.add(sp);
        }
  ...
  /**
   * Mark current position as one point in the 3-pt set Ensures that one XY stage and one Z stage
   * are selected in the PositionList Also ensures that the same stages as in the previous points
   * are selected
   */
  private void markOnePoint() {
    ...
    if (nrPositions > 3) {
      app_.logs()
          .showMessage("PositionList contains > 3 points.  Try to clear the Stage Position List");
      return;
    }
    if (nrPositions == 3) {
    ...
    if (nrPositions > 1) {
      ...
      for (int i = 0; i < nrPositions; i++) {
        ...
        if (!mspTest.get(0).getStageDeviceLabel().equals(stage0)
          ...
          app_.logs()
              .showMessage("Make sure that the same stages are checked for all 3 positions", this);
        }
      }
    }
  }
  ...
  private PositionList generateSites() {
    ...
    boolean isTypewriterMode = ((String) visitOrder_.getSelectedItem()).equals(TYPEWRITER_ORDER);
  ...
  private void displayStatus() {
    ...
    String statusTxt =
        "Cursor: X="
            + TextUtils.FMT2.format(cursorOffsetPos.x)
            + "um, Y="
  ...
  public void updateStagePositions(
      double x, double y, double z, String wellLabel, String siteLabel) {
    xyStagePos_.x = x;
    xyStagePos_.y = y;
    zStagePos_ = z;
