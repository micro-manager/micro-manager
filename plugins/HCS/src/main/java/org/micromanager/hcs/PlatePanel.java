...
public class PlatePanel extends JPanel {
  ...
  private class WellBox {
    ...
    public void draw(Graphics2D g) {
      ...
      if (selected) g.setPaint(c.darker());
      else g.setPaint(c);
      ...
      if (circular) g.drawOval(wellRect.x, wellRect.y, wellRect.width, wellRect.height);
      else g.draw(wellRect);
      ...
      for (int j = 0; j < sites_.getNumberOfPositions(); j++) {
        siteRect.x =
            (int)
                (sites_.getPosition(j).getX() * params_.xFactor
                    + params_.xTopLeft
                    - siteOffsetX
                    + 0.5);
        siteRect.y =
            (int)
                (sites_.getPosition(j).getY() * params_.yFactor
                    + params_.yTopLeft
                    - siteOffsetY
                    + 0.5);
        g.draw(siteRect);
  ...
  public PlatePanel(SBSPlate plate, PositionList pl, ParentPlateGUI gui, Studio app) {
    gui_ = gui;
    app_ = app;
    plate_ = plate;
    mode_ = Tool.SELECT;
    ...
    if (pl == null) wells_ = plate_.generatePositions(gui_.getXYStageName());
    else wells_ = plate_.generatePositions(gui_.getXYStageName(), pl);
  ...
  protected void onMouseClicked(MouseEvent e) throws HCSException {
    ...
    if (mode_ == Tool.MOVE) {
      ...
      if (!plate_.isPointWithin(pt.x, pt.y)) return;
      ...
      try {
        ...
        app_.getCMMCore()
            .waitForDeviceType(
                DeviceType
                    .XYStageDevice); // wait for the stage to stop moving before updating the gui.
  ...
  protected void onMouseReleased(MouseEvent e) {
    ...
    drawSelRect(previous_);
    Rectangle selRect =
        new Rectangle(anchor_.x, anchor_.y, e.getX() - anchor_.x, e.getY() - anchor_.y);
  ...
  private Point2D.Double scalePixelToDevice(int x, int y) {
    // Thge point returned from this method will still need to have the offset applied to it in
    // order to map to the device coordinate system.
  ...
  public void setTool(Tool t) {
    mode_ = t;
    if (mode_ == Tool.MOVE) setCursor(Cursor.getPredefinedCursor(Cursor.CROSSHAIR_CURSOR));
  ...
  private void rescale() {
    ...
    if (lockAspect_) {
      ...
      else drawingParams_.xFactor = drawingParams_.yFactor;
    }
  ...
  private void drawWells(Graphics2D g) {
    ...
    for (int i = 0; i < wells_.length; i++) {
      ...
      wb.wellBoundingRect.setBounds(
          (int) (activeRect_.getX() + wells_[i].getColumn() * wellX + drawingParams_.xOffset + 0.5),
      ...
      wb.wellRect.setBounds(
          (int)
              (activeRect_.getX()
                  + wells_[i].getColumn() * wellX
                  + drawingParams_.xOffset
                  + wellOffsetX
                  + 0.5),
          (int)
              (activeRect_.getY()
                  + wells_[i].getRow() * wellY
                  + drawingParams_.yOffset
                  + wellOffsetY
                  + 0.5),
          (int) wellInsideX,
          (int) wellInsideY);
  ...
  private void drawStagePointer(Graphics2D g) {
    ...
    if (xyStagePos_ == null) xyStagePos_ = new Point2D.Double(0.0, 0.0);
  ...
  public WellPositionList[] getSelectedWellPositions() {
    ...
    for (int i = 0; i < wells_.length; i++) {
      if (wellBoxes_[i].selected) wal.add(wells_[i]);
    }
