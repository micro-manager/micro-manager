...
public class TIRFControlModel {
    ...
    @Expose private int numImages;
    @Expose private int numFastCircles;
    @Expose private int startupScriptDelay;
    @Expose private boolean useStartupScript;
    @Expose private boolean useShutdownScript;
    @Expose private String startupScriptPath;
    @Expose private String shutdownScriptPath;
    @Expose private String datastoreSavePath;
    @Expose private String datastoreSaveFileName;
    @Expose private Datastore.SaveMode datastoreSaveMode;
    ...
    public void calibrateFastCircles(final int nImages, final float startSize, final float radiusIncrement) {
        final SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() throws Exception {
                ...
                while (time < nImages) {
                    ...
                    scanner.setFastCirclesState(Scanner.Values.FAST_CIRCLES_STATE.ON);
                    ...
                    scanner.setFastCirclesState(Scanner.Values.FAST_CIRCLES_STATE.OFF);
                }
        ...
        final SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() throws Exception {
                ...
                SwingUtilities.invokeLater(() -> {
                    ...
                    frame.getTabPanel().getScannerTab().getSpinnerScannerH().setValue(scanner.getPositionH());
                    frame.getTabPanel().getScannerTab().getSpinnerScannerI().setValue(scanner.getPositionI());
                });
                ...
                while (core.getRemainingImageCount() > 0 || camera.isSequenceRunning()) {
                    if (core.getRemainingImageCount() > 0) {
                        ...
                        image = studio.data().convertTaggedImage(taggedImage, builder.time(time).build(), null);
                        datastore.putImage(image);
                        time++;
