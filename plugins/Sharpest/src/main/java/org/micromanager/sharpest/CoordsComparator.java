...
public class CoordsComparator implements Comparator<String> {
   HashMap<String, Integer> cAxesNumeric = new HashMap<>();
   ...
   public int compare(String s1, String s2) {
      ...
      if (cAxesNumeric.containsKey(s1) && cAxesNumeric.containsKey(s2)) {
         int val1 = cAxesNumeric.get(s1);
         int val2 = cAxesNumeric.get(s2);
         return val1 > val2 ? 1 : -1;
      } else if (cAxesNumeric.containsKey(s1)) {
         return -1;
      } else if (cAxesNumeric.containsKey(s2)) {
         return 1;
      } 
