...
/**
 * LaseThread implements the laser controller. It decreases power if density is above a certain
 * maximum threshold and increases power if density is below a minimum threshold.
 *
 * @author Thomas Pengo
 */
public class LaseThread implements Runnable, DensityMonitor {
  ...
  /**
   * Creates a new controller by tying it to a MicroManager instance and specifying the
   * configuration for the device in LaserConfig.
   *
  ...
  /**
   * Adds a new laser power monitor, which will get notified each time the laser power changes.
   *
   * @param m
   */
  ...
  public void run() {
    ...
    while (!stopping) {
      ...
      if (running) {
        ...
        if (curDensity < densityLower) incPower();

        // If above upperThreshold
        if (curDensity > densityUpper) decPower();
      }
      try {
  ...
  private void incPower() {
    setPower(laserPower + lc.minStep);
  }
  ...
  private void setPower(double power) {
    if (power <= lc.maxValue && power >= lc.minValue) {
      try {
        ...
        core.logMessage(
            "Autolase was not able to set the laser power of "
                + lc.deviceName
                + "/"
                + lc.propertyName
                + " to "
