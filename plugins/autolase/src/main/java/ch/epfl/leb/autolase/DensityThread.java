...
/**
 * This class estimates the density of activations by sampling a Camera at regular intervals
 * (default 20ms). The density at a particular point relates to the maximum time a certain pixel is
 * "on", or above a certain threshold. The density is calculated as a moving average (default 1s).
 *
 * <p>The code only works for 2 bytes per pixel cameras for now.
 *
 * @author Thomas Pengo
 */
public class DensityThread implements Runnable {
  ...
  List<DensityMonitor> monitors = Collections.synchronizedList(new ArrayList<DensityMonitor>());
  ...
  public void addDensityMapMonitor(DensityMapMonitor m) {
    if (!mapMonitors.contains(m)) mapMonitors.add(m);
  }
    ...
    while (!stopping) {
      ...
      if (running && camera.isAcquiring())
        ...
        try {
          ...
          for (int i = 0; i < curMask.length; i++) curMask[i] = image[i] > threshold;
          ...
          if (accumulator == null) {
            ...
            for (int i = 0; i < accumulator.length; i++)
              if (curMask[i]) accumulator[i] = timeInterval;
          } else {
            // A_i = (I_i > t) (1 + A_i-1)
            for (int i = 0; i < accumulator.length; i++)
              if (!curMask[i]) {
          ...
          double curd = 0;
          for (int i = 0; i < image.length; i++) if (accumulator[i] > curd) curd = accumulator[i];

          // Moving average estimate
          if (density_fifo.size() == fifoNumElems) density_fifo.remove();
          density_fifo.offer(curd);

          double mean_density = 0;
          for (Double d : density_fifo) mean_density += d;
          mean_density /= density_fifo.size();

          currentDensity = mean_density;

          for (DensityMonitor m : monitors) m.densityChanged(currentDensity);
