...
public enum AutoLase {
  ...
  public void setLaserToMinPower() {
    if (laseThread != null) laseThread.setLaserToMinPower();
  }

  /** Set the laser power to the initial power. */
  public void setLaserToStartPower() {
    if (laseThread != null) laseThread.setLaserToStartPower();
  }
  ...
  void log(String msg) {
    if (verbose) Logger.getLogger(LOG_NAME).log(Level.INFO, msg);
  }
  ...
  /**
   * Always call this method before doing anything with the plugin. This means that it actually
   * needs to be called by MM or by another plugin to work.
   *
   * @param app
   */
  public void setup(Studio app) {
    ...
    // Prepare laser
    if (laseThread == null) laseThread = new LaseThread(core, config);

    // Prepare density monitor
    if (densityThread == null) densityThread = new DensityThread(new MMCamera(core));
  }
  ...
  public void setMinGoodDensity(float m) {
    ...
    if (dmm != null) dmm.setMinGoodDensity(m);
    else log("Density monitor not initialized. No action taken.");

    if (laseThread != null) laseThread.setDensityLower(m);
  }
  ...
  public void setMaxGoodDensity(float m) {
    ...
    if (dmm != null) dmm.setMaxGoodDensity(m);
    else log("Density monitor not initialized. No action taken.");

    if (laseThread != null) laseThread.setDensityUpper(m);
  }
  ...
  public void setThreshold(int threshold) {
    ...
    if (densityThread != null) {
      ...
      dmm.clearMap();
    } else log("Density thread not initialized. No action taken.");
  }
  ...
  public void pauseDensityMonitor() {
    if (densityThread == null || dt1 == null) return;
  ...
  void showDensityMapMonitor(boolean enabled) {
    ...
    if (dmm == null) dmm = new DensityMap();

    dmm.setVisible(enabled);
    dmm.setRunning(enabled);

    if (densityThread != null) densityThread.addDensityMapMonitor(dmm);
