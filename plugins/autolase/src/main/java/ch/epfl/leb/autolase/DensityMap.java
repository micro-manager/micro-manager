...
/**
 * Gives a visual feedback for the current density levels. Green indicates density above 0 and below
 * minimum. White indicates above minimum and below maximum. Red indicates above maximum.
 *
 * @author Thomas Pengo
 */
public class DensityMap implements DensityMapMonitor, Runnable {
  ...
  public void clearMap() {
    density = null;
    if (theImage != null) theImage.setHideOverlay(true);
  ...
  /**
   * This is called when the density map is changed. It updates the image overlay with the
  ...
  public void densityMapChanged(int width, int height, float[] density) {
    ...
    for (int i = 0; i < density.length; i++) {
      if (density[i] > 0 && density[i] < minGoodDensity) overlay[i] = 2;
      else if (density[i] < maxGoodDensity) overlay[i] = 1;
      else overlay[i] = 3;
    }

    densityOverlay =
        new Overlay(new ImageRoi(0, 0, new ByteProcessor(width, height, overlay, buildLUT())));
