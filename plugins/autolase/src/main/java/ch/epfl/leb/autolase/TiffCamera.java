...
public class TiffCamera implements Camera {
  ...
  public short[] getNewImage() throws Exception {
    if (win.getSlice() < stack.getSize()) win.setSlice(win.getSlice() + 1);
    else win.setSlice(1);
  ...
  public int getBytesPerPixel() {
    if (stack.getPixels(win.getSlice()) instanceof short[]) return 2;
    if (stack.getPixels(win.getSlice()) instanceof char[]) return 1;
    else return 4;
  }
