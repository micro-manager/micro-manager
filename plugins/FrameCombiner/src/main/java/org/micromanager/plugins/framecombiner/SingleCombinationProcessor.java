...
public class SingleCombinationProcessor {
  ...
  public SingleCombinationProcessor(
      Coords coords,
      Studio studio,
      String processorAlgo,
      String processorDimension,
      int numerOfImagesToProcess,
  ...
  public void logMe() {
    log_.logMessage(
        "Z : "
            + Integer.toString(coords_.getZ())
            + " | Channel : "
            + Integer.toString(coords_.getChannel())
            + " | Stage Position : "
            + Integer.toString(coords_.getStagePosition()));
  }
  ...
  void addImage(Image image, ProcessorContext context) {
    ...
    if (currentBufferIndex_ == (numerOfImagesToProcess_ - 1)) {
      ...
      if (userData != null) {
        ...
        userData =
            userData.copy().putInt("FrameProcessed-StackNumber", numerOfImagesToProcess_).build();
  ...
  public void processBufferImages() throws Exception {
    ...
    if (processorAlgo_.equals(FrameCombinerPlugin.PROCESSOR_ALGO_MEAN)) {
      ...
      throw new Exception(
          "FrameCombiner : Algorithm called "
              + processorAlgo_
  ...
  public void meanProcessImages(boolean onlySum) {
    ...
    processedImage_ =
        studio_
            .data()
            .createImage(
                resultPixels, width, height, bytesPerPixel, numComponents, coords, metadata);
  }
