...
public class FlipperProcessor implements Processor {
  ...
  public FlipperProcessor(Studio studio, String camera, int rotation, boolean isMirrored) {
    studio_ = studio;
    camera_ = camera;
    if (rotation != R0 && rotation != R90 && rotation != R180 && rotation != R270) {
      // Invalid rotation.
  ...
  public void processImage(Image image, ProcessorContext context) {
    ...
    context.outputImage(transformImage(studio_, image, isMirrored_, rotation_));
  }

  /**
   * Executes image transformation First mirror the image if requested, than rotate as requested
   *
   * @param studio
   ...
   */
  public static Image transformImage(Studio studio, Image image, boolean isMirrored, int rotation) {
