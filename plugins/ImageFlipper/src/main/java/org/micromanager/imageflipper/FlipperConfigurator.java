...
public class FlipperConfigurator extends JFrame implements ProcessorConfigurator {
  ...
  private static final List<Integer> R_INTS =
      Arrays.asList(
          new Integer[] {
            FlipperProcessor.R0, FlipperProcessor.R90, FlipperProcessor.R180, FlipperProcessor.R270
          });
  private static final String EXAMPLE_ICON_PATH = "/org/micromanager/icons/R.png";
  ...
  public FlipperConfigurator(Studio studio, PropertyMap settings) {
    ...
    selectedCamera_ =
        settings.getString(
            "camera", defaults_.getString(DEFAULT_CAMERA, studio_.core().getCameraDevice()));
    Boolean shouldMirror =
        settings.getBoolean("shouldMirror", defaults_.getBoolean(DEFAULT_MIRRORED, false));
    Integer rotation =
        settings.getInteger(
            "rotation", defaults_.getInteger(DEFAULT_ROTATION, FlipperProcessor.R0));
   ...
   *
   * <p>Shows all available cameras and sets the currently selected camera as the selected item in
   * the drop down box
   */
  ...
  private void initComponents() {
    ...
    mirrorCheckBox_.addActionListener(
    ...
    cameraComboBox_.addActionListener(
        new java.awt.event.ActionListener() {
          @Override
          public void actionPerformed(java.awt.event.ActionEvent evt) {
    ...
    rotateComboBox_.addActionListener(
        new java.awt.event.ActionListener() {
          @Override
          public void actionPerformed(java.awt.event.ActionEvent evt) {
  ...
  private void mirrorCheckBox_ActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_mirrorCheckBox_ActionPerformed
    processExample();
    String camera = (String) cameraComboBox_.getSelectedItem();
    if (camera != null) {
      defaults_.putBoolean(DEFAULT_MIRRORED + "-" + camera, mirrorCheckBox_.isSelected());
    }
    studio_.data().notifyPipelineChanged();
  }

  private void rotateComboBox_ActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_rotateComboBox_ActionPerformed
    processExample();
    String camera = (String) cameraComboBox_.getSelectedItem();
    if (camera != null && rotateComboBox_.getSelectedItem() != null) {
      defaults_.putString(
          DEFAULT_ROTATION + "-" + camera, (String) rotateComboBox_.getSelectedItem());
    }
    studio_.data().notifyPipelineChanged();
  }

  private void cameraComboBox_ActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_cameraComboBox_ActionPerformed
    String camera = (String) cameraComboBox_.getSelectedItem();
    if (camera != null) {
      mirrorCheckBox_.setSelected(defaults_.getBoolean(DEFAULT_MIRRORED + "-" + camera, false));
      rotateComboBox_.setSelectedItem(defaults_.getString(DEFAULT_ROTATION + "-" + camera, R0));
  ...
  /**
   * Indicates users choice for rotation: 0 - 0 degrees 1 - 90 degrees 2 - 180 degrees 3 - 270
   * degrees degrees are anti-clockwise
   *
  ...
  private void processExample() {
    ByteProcessor proc = new ByteProcessor(IconLoader.loadFromResource(EXAMPLE_ICON_PATH));
    ...
    Image testImage =
        studio_
            ...
            .ij()
            .createImage(
                proc, Coordinates.builder().build(), studio_.data().metadataBuilder().build());
    testImage = FlipperProcessor.transformImage(studio_, testImage, getMirror(), getRotate());
