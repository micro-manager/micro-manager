  ...
  public ZProjectorPluginFrame(Studio studio, DisplayWindow window) {
    ...
    if (ourProvider_ instanceof Datastore) {
      if (!((Datastore) ourProvider_).isFrozen()) {
        studio_.logs().showMessage("Can not Project ongoing acquisitions", window.getWindow());
        return;
      }
    }
    ...
    methodBox.addActionListener(
        (ActionEvent e) -> {
          settings_.putString(
              ZProjectorPlugin.PROJECTION_METHOD, (String) methodBox.getSelectedItem());
        });
