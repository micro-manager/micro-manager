...
public class ZProjectorPluginExecutor {
    ...
    class ZProjectTask extends SwingWorker<Void, Void> {
      ...
      public Void doInBackground() throws Exception { // TODO use Exceptions
        ...
        if (save) {
          File result =
              FileDialogs.openDir(
                  null, "Please choose a directory to save " + newName, FileDialogs.MM_DATA_SET);
          if (result != null) {
            try {
              newStore =
                  studio_
                      .data()
                      .createMultipageTIFFDatastore(
                          result.getAbsolutePath() + File.separator + newName,
                          true,
        ...
        metadata = metadata.copyBuilder().intendedDimensions(newSizeCoordsBuilder.build()).build();
        ...
        try {
          ...
          if (!save) {
            ...
            copyDisplay.setDisplaySettings(window_.getDisplaySettings().copyBuilder().build());
            ...
            progressBar_ =
                new ProgressBar(window_.getWindow(), "Projection Progress", 1, nrProjections_);
            progressBar_.setVisible(true);
          }

          findAllProjections(
              newStore, axes, cb, projectionAxis, firstFrame, lastFrame, projectionMethod);
  ...
  /**
   * Recursively figures out which projections need to be performed It does so by taking the first
   * remaining axes, cycle through all positions in that axes, and recursively calling this function
   * (omitting that axis). When no more axes are remining, the actual projection will be exected.
   *
   ...
   */
  private void findAllProjections(
      Datastore newStore,
      List<String> remainingAxes,
      Coords.CoordsBuilder cbp,
    ...
    for (int i = 0; i < oldStore_.getNextIndex(currentAxis); i++) {
      ...
      if (rcAxes.isEmpty()) {
        ...
        findAllProjections(newStore, rcAxes, cbp, projectionAxis, min, max, projectionMethod);
      }
    }
  }
  ...
  private void executeProjection(
    ...
    if (tmpImg == null) {
      studio_
          .alerts()
          .postAlert("Projection problem", this.getClass(), "No images found while projecting");
      return;
    }
    ImageStack stack = new ImageStack(tmpImg.getWidth(), tmpImg.getHeight());
    ...
    if (stack.getSize() > 0 && imgMetadata != null) {
      ...
      Image outImg =
          studio_
              ...
              .getImageJConverter()
              .createImage(
                  projection.getProcessor(),
                  cbp.index(projectionAxis, 0).build(),
