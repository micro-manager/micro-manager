 ...
 * and presses a key, the corresponding image will move to the correct slice and draw a symbol
 * indicating where the object was found
 *
 * <p>Works only in conjunction with appropriate column names Up and down keys also work as expected
 */
public class ResultsListener implements KeyListener, MouseListener {
  ...
  /** Move display to the position in which the object was found and draw a symbol there */
  private void update() {
    if (siPlus_ == null) {
      return;
    }
  ...
  /**
   * Creates the symbols. Symbol size will be a portion of the image size (currently 5%)
   *
   ...
   */
  void drawCross(ImagePlus imp, Point2D.Double p, GeneralPath path, double space) {
