...
class RegionList {
  ...
  public void removeRegion(int idx) {
    if (idx >= 0 && idx < regions_.size()) regions_.remove(idx);
  }

  public Region getRegion(int idx) {
    if (idx < 0 || idx >= regions_.size()) return null;
  ...
  public boolean isFileNameUnique(String directory, String filename) {
    ...
    for (int i = 0; i < regions_.size(); i++) {
      ...
      if (directory.equals(r.directory) && filename.equals(r.filename)) return false;
    }
    return true;
  }
