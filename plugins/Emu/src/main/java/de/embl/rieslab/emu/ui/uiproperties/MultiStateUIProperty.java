...
/**
 * A UIProperty with multiple allowed states, whose values are unknown at compilation time. Upon
 * instantiation the number of states is set, while the user can change the values of each state in
...
public class MultiStateUIProperty extends UIProperty {
   ...
   */
  public MultiStateUIProperty(
      ConfigurablePanel owner, String label, String description, PropertyFlag flag, int size) {
  ...
  public MultiStateUIProperty(ConfigurablePanel owner, String label, String description, int size) {
    ...
    if (size <= 0) {
      throw new IllegalArgumentException(
          "The number of state of a MultiStateUIProperty cannot be negative or zero.");
    }
  ...
  /**
   * Sets values for the states of the UI property. If the array of values is too long, then the
   * supernumerary values are ignored. If the array is too short, then the corresponding states are
   * modified while the other ones are left unchanged.
   *
  ...
  /**
   * Returns the index of the state corresponding to the value val. If the value is not amongst the
   * states, returns 0.
   *
  ...
  /**
   * Returns the name of the state corresponding to value. The first state will be returned if the
   * value does not correspond to any state.
   *
   * @param value Value
  ...
  /**
   * Sets the value of the MMProperty to {@code val} if {@code val} equals either one of the state
   * values, state names or state indices (in that order). To avoid collisions, you can use
   * setPropertyValueByState or setPropertyValueByStateIndex.
   */
  @Override
