...
/**
 * Class instantiating the {@link MMPropertiesRegistry} and {@link MMPresetGroupRegistry}. In
 * addition, it registers the configuration groups as MMProperties and add them to the
...
public class MMRegistry {
  ...
  private MMPresetGroupRegistry
      mmPresetGroupsRegistry_; // holds the configuration groups from Micro-manager
  private Studio studio_;
  private Logger logger_;

  /**
   * Constructor. Instantiate the {@link MMPropertiesRegistry} and {@link MMPresetGroupRegistry} and
   * register the configuration groups as MMProperties.
   *
  ...
  private void registerMMConfAsDevice() {
    ...
    while (it.hasNext()) {
      ...
      dev.registerProperty(
          new PresetGroupAsMMProperty(
              studio_.app(), studio_.core(), logger_, group, values, affectedmmprops));
    }
