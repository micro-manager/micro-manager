...
/**
 * Class holding a HashMap of the {@link MMPresetGroup}s and their name as displayed in the current
 * Micro-manager session.
 *
...
public class MMPresetGroupRegistry {
  ...
  /**
   * The constructor receives the current Micro-manager core instance and extracts all the preset
   * groups, building a HashMap of the MMPresetGroup indexed by the group name.
   *
      ...
      for (int i = 0; i < groups.size(); i++) {
        ...
        try {
          ...
          for (int j = 0; j < conf.size(); j++) {
            affectedmmprops.add(
                mmproperties
                    .getProperties()
                    .get(
                        conf.getSetting(j).getDeviceLabel()
                            + "-"
          ...
          groups_.put(
              groups.get(i),
              new MMPresetGroup(
                  groups.get(i), core_.getAvailableConfigs(groups.get(i)), affectedmmprops));
  ...
  /**
   * Returns a HashMap mapping the preset group names (keys) and an array of string representing the
   * names of the different presets within each group.
   *
