...
/**
 * Class representing the main JFrame of a {@link de.embl.rieslab.emu.plugin.UIPlugin}. Subclasses
 * must implement the {@link #initComponents()} method, in which the {@link ConfigurablePanel}s must
 * be instantiated and added the same way a JPanel is added to a JFrame.
 *
 * <p>The ConfigurableMainFrame aggregates the UIParameters and the UIProperties, as well as linking
 * together the InternalProperties. If two UIProperties have the same name, then the last added
 * UIproperty will replace the first ones. The order is the order of discovery while going through
 * the components of the JFrame.
 *
 * <p>For UIParameters, on the other hand, two UIParameters are allowed to have the same hash
 * ({ConfigurablePanel name}-{UIParameter name}) only if they have the same type. Should such case
 * arise, all UIParameters but the first one to appear (in order of registration of the
 * ConfigurablePanel that owns it) are replaced in their owner ConfigurablePanel by the first
 * UIParameter. There, UIParameters with same hash and type are made replaced by a single reference
 * and are shared by all the corresponding ConfigurationPanel. Note that if two UIParameters have
 * same name but different types, the second one to appear is ignored altogether.
 *
...
public abstract class ConfigurableMainFrame extends JFrame implements ConfigurableFrame {
  ...
  /**
   * Constructor, it sets up the JMenu, calls {@link #initComponents()} from the subclass, then
   * links InternaProperties and gather UIPropertiers and UIParameters. The plugin settings will
  ...
  public ConfigurableMainFrame(
      String title, SystemController controller, TreeMap<String, String> pluginSettings) {
    ...
    this.setLocation(
        g.getCenterPoint().x - this.getWidth() / 2, g.getCenterPoint().y - this.getHeight() / 2);
    // this.setVisible(true);

    // retrieves all properties, parameters and link internal properties
    panels_ =
        listConfigurablePanels(
            this.getContentPane().getComponents(), new ArrayList<ConfigurablePanel>());
  ...
  private ArrayList<ConfigurablePanel> listConfigurablePanels(
      Component[] c, ArrayList<ConfigurablePanel> list) {
    if (list == null) {
  ...
  private void setUpMenu() {
    ...
    JMenuItem wiz =
        new JMenuItem(
            new AbstractAction("Modify configuration") {
              private static final long serialVersionUID = 1L;
    ...
    JMenuItem manageconfig =
        new JMenuItem(
            new AbstractAction("Manage configurations") {
              private static final long serialVersionUID = 1L;
  ...
  private void retrieveUIPropertiesAndParameters() {
    ...
    while (it.hasNext()) { // loops over the PropertyPanel contained in the MainFrame
      ...
      // and substituted to the second UIParameter in its owner PropertyPanel: "same name" = "same
      // parameter"
      // Because UIParameters do not update their ConfigurablePanel owner by themselves, then by
      // doing so
    ...
    while (panelsIt.hasNext()) { // iterate over panels
      ...
      while (propsit.hasNext()) { // iterate over one panel's internal props
        ...
        if (allinternalprops.containsKey(internalprop)) { // if the internal property already exists
          // add to a temporary HashMap, and will take care of them later to avoid a concurrent
          // modifications of panelinternalprops
  ...
  /**
   * Updates all properties and parameters from each known ConfigurablePanel by calling {@link
   * ConfigurablePanel#updateAllProperties()} and {@link ConfigurablePanel#updateAllParameters()} on
   * each panel.
   */
  ...
  /**
   * Updates all properties from each known ConfiguablePanel. The method calls {@link
   * ConfigurablePanel#updateAllProperties()}.
   */
  ...
  /**
   * Shuts down all ConfigurablePanels by calling {@link ConfigurablePanel#shutDown()} on each
   * panel.
   */
  ...
  public void updateMenu() {
    ...
    for (int i = 0; i < plugins.length; i++) {
      ...
      JMenuItem item =
          new JMenuItem(
              new AbstractAction(plugins[index]) {
    ...
    for (int i = 0; i < confs.length; i++) {
      ...
      JMenuItem item =
          new JMenuItem(
              new AbstractAction(confs[index]) {
  ...
  /**
   * Sets-up the frame, in this method the subclasses should instantiate the ConfigurablePanels and
   * add them to the ConfigurableMainFrame.
   */
