...
/**
 * UIProperty that only accepts two states: ON or OFF. The value of these states are not known at
 * compilation time and can be changed in the configuration wizard.
 *
...
public class TwoStateUIProperty extends UIProperty {
   ...
   * @return True if the value was correctly set, false otherwise (for instance if the value is not
   *     allowed)
   */
  ...
  /**
   * Sets the value of the UIproperty to {@code newValue} if {@code newValue} is either equal to the
   * ON state or to the OFF state, or their respective value. The method also accepts 1/0 or
   * true/false as state labels.
   */
  @Override
  ...
  /**
   * Sets the value of the assigned MMProperty to the On/Off state if stateIndex equals to 1/0
   * respectively.
   *
  ...
  public boolean isOnState(String value) {
    if (value != null && isAssigned()) {
      if (getMMProperty().getType()
          == MMProperty.MMPropertyType
              .FLOAT) { // if float, then "0" decimals might be added and need to be compared.
