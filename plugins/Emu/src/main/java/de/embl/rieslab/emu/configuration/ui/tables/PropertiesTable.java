...
public class PropertiesTable extends JPanel {
   ...
   private final MMPropertiesRegistry mmproperties_;
   private final String[] uipropkeys_;
   ...
   public PropertiesTable(Map<String, UIProperty> uipropertySet, MMPropertiesRegistry mmproperties,
      ...
      for (int i = 0; i < uipropkeys_.length; i++) {
         ...
         if (uipropertySet.get(uipropkeys_[i]) instanceof TwoStateUIProperty) {
            ...
            model.addRow(new Object[] {uipropkeys_[i] + TwoStateUIProperty.getOffStateLabel(), "",
                  ConfigurationWizardUI.KEY_ENTERVALUE});
            ...
            model.addRow(new Object[] {uipropkeys_[i] + RescaledUIProperty.getSlopeLabel(), "",
                  String.valueOf(slope)});
            model.addRow(new Object[] {uipropkeys_[i] + RescaledUIProperty.getOffsetLabel(), "",
    ...
    */
   public PropertiesTable(Map<String, UIProperty> uipropertySet, MMPropertiesRegistry mmproperties,
                          Map<String, String> propertymapping, HelpWindow help) {
         ...
         if (propertymapping.containsKey(uipropkeys_[i]) && mmproperties.isProperty(mmprop)) {
            ...
            if (uipropertySet.get(uipropkeys_[i]) instanceof TwoStateUIProperty) { // if two state
               ...
               model.addRow(new Object[] {uipropkeys_[i] + TwoStateUIProperty.getOnStateLabel(), "",
                     uion});
               model.addRow(
                     new Object[] {uipropkeys_[i] + TwoStateUIProperty.getOffStateLabel(), "",
                           uioff});
            } else if (uipropertySet.get(
                  uipropkeys_[i]) instanceof SingleStateUIProperty) { // if single value property
               // gets the value of the state and adds the corresponding row
               uisingle =
                     propertymapping.get(uipropkeys_[i] + SingleStateUIProperty.getStateLabel());
               if (uisingle == null) {
                  uion = ConfigurationWizardUI.KEY_ENTERVALUE;
               }
               model.addRow(
                     new Object[] {uipropkeys_[i] + SingleStateUIProperty.getStateLabel(), "",
                           uisingle});
            } else if (uipropertySet.get(
               ...
               if (uitemp == null) { // get the default one
                  uitemp = String.valueOf(
                        ((RescaledUIProperty) uipropertySet.get(uipropkeys_[i])).getSlope());
               }
               model.addRow(new Object[] {uipropkeys_[i] + RescaledUIProperty.getSlopeLabel(), "",
                     uitemp});
      ...
      for (int i = 0; i < nrow; i++) {
         ...
         if (!isStateValue((String) model.getValueAt(i, 0)) &&
            ...
            if (mmproperties_.getDevice((String) model.getValueAt(i, 1))
               ...
               String hash = mmproperties_.getDevice((String) model.getValueAt(i, 1))
                     .getHashFromLabel(propertyname);
               map.put((String) model.getValueAt(i, 0), hash); // put with the property hash
            } else {
               map.put((String) model.getValueAt(i, 0),
   ...
   public boolean isStateValue(String s) {
      return s.contains(SingleStateUIProperty.getStateLabel())
