...
public class PropertiesTable extends JPanel {
  ...
  public PropertiesTable(
      Map<String, UIProperty> uipropertySet, MMPropertiesRegistry mmproperties, HelpWindow help) {
    ...
    for (int i = 0; i < uipropkeys_.length; i++) {
      ...
      model.addRow(
          new Object[] {
            uipropkeys_[i], GlobalConfiguration.KEY_UNALLOCATED, GlobalConfiguration.KEY_UNALLOCATED
          });
      ...
      if (uipropertySet.get(uipropkeys_[i]) instanceof TwoStateUIProperty) {
        ...
        model.addRow(
            new Object[] {
              uipropkeys_[i] + TwoStateUIProperty.getOnStateLabel(),
              "",
              ConfigurationWizardUI.KEY_ENTERVALUE
            });
        model.addRow(
            new Object[] {
              uipropkeys_[i] + TwoStateUIProperty.getOffStateLabel(),
              "",
              ConfigurationWizardUI.KEY_ENTERVALUE
            });
        ...
        model.addRow(
            new Object[] {
              uipropkeys_[i] + SingleStateUIProperty.getStateLabel(),
              "",
              ConfigurationWizardUI.KEY_ENTERVALUE
            });
        ...
        for (int j = 0; j < numpos; j++) {
          model.addRow(
              new Object[] {
                uipropkeys_[i] + MultiStateUIProperty.getConfigurationStateLabel(j),
                "",
                ConfigurationWizardUI.KEY_ENTERVALUE
              });
        }
        ...
        model.addRow(
            new Object[] {
              uipropkeys_[i] + RescaledUIProperty.getSlopeLabel(), "", String.valueOf(slope)
            });
        model.addRow(
            new Object[] {
              uipropkeys_[i] + RescaledUIProperty.getOffsetLabel(), "", String.valueOf(offset)
            });
      }
    }
   ...
   */
  public PropertiesTable(
      Map<String, UIProperty> uipropertySet,
    ...
    for (int i = 0; i < uipropkeys_.length; i++) {
      ...
      // if the configuration indeed contains the property and is mapped in the configuration with
      // an existing MM property
      if (propertymapping.containsKey(uipropkeys_[i]) && mmproperties.isProperty(mmprop)) {
        ...
        model.addRow(
            new Object[] {
              uipropkeys_[i],
              mmproperties.getProperty(mmprop).getDeviceLabel(),
              mmproperties.getProperty(mmprop).getMMPropertyLabel()
            });

        // if the property is an instance of SingleState, TwoState or MultiState property, then
        // looks for the assigned values
        if (uipropertySet.get(uipropkeys_[i]) instanceof TwoStateUIProperty) { // if two state
          ...
          model.addRow(
              new Object[] {uipropkeys_[i] + TwoStateUIProperty.getOnStateLabel(), "", uion});
          model.addRow(
              new Object[] {uipropkeys_[i] + TwoStateUIProperty.getOffStateLabel(), "", uioff});
        } else if (uipropertySet.get(uipropkeys_[i])
            instanceof SingleStateUIProperty) { // if single value property
          ...
          model.addRow(
              new Object[] {uipropkeys_[i] + SingleStateUIProperty.getStateLabel(), "", uisingle});
        } else if (uipropertySet.get(uipropkeys_[i])
            instanceof MultiStateUIProperty) { // if multiple values property
          // gets the number of positions and extracts all position values. If null then sets to
          // default. Finally creates the corresponding row.
          int numpos =
          ...
          for (int j = 0; j < numpos; j++) {
            ...
            model.addRow(
                new Object[] {
                  uipropkeys_[i] + MultiStateUIProperty.getConfigurationStateLabel(j), "", uitemp
                });
          }
        } else if (uipropertySet.get(uipropkeys_[i]) instanceof RescaledUIProperty) {
          // slope
          ...
          if (uitemp == null) { // get the default one
            uitemp =
                String.valueOf(((RescaledUIProperty) uipropertySet.get(uipropkeys_[i])).getSlope());
          }
          model.addRow(
              new Object[] {uipropkeys_[i] + RescaledUIProperty.getSlopeLabel(), "", uitemp});
          ...
          if (uitemp == null) { // get the default one
            uitemp =
                String.valueOf(
                    ((RescaledUIProperty) uipropertySet.get(uipropkeys_[i])).getOffset());
          }
          model.addRow(
              new Object[] {uipropkeys_[i] + RescaledUIProperty.getOffsetLabel(), "", uitemp});
        }
      } else {
        ...
        // if the property is an instance of SingleState, TwoState or MultiState property, creates
        // rows for the states value
        if (uipropertySet.get(uipropkeys_[i]) instanceof TwoStateUIProperty) {
          model.addRow(
              new Object[] {
                uipropkeys_[i] + TwoStateUIProperty.getOnStateLabel(),
        ...
        new JTable(model) {
          ...
          public TableCellEditor getCellEditor(int row, int column) {
            ...
            if (column == 2 && isStateValue(s)) {
              // if in the last column and corresponds to a field value, returns a JTextfield cell
              // editor
  ...
  // Creates a JComboBox containing all the Micro-manager device properties corresponding to the
  // device.
  private JComboBox<String> getDevicePropertiesComboBox(String device, String uipropName) {
    ...
    if (!device.equals(GlobalConfiguration.KEY_UNALLOCATED)) {
      ...
      if (uipropertySet_.containsKey(uipropName)
  ...
  // Updates the helper window text with the description of the UI property represented by the
  ...
  private void updateHelper(int row) {
    ...
    if (s.matches(MultiStateUIProperty.getGenericStateName())) {
      ...
      help_.update(
          s,
          "Set the device property value corresponding to the state number "
              + diff
  ...
  public HashMap<String, String> getSettings() {
    ...
    for (int i = 0; i < nrow; i++) {
      // if device column is not empty (from state property, e.g. single state value or multistates)
      // and is not unallocated
      if (!isStateValue((String) model.getValueAt(i, 0))
          && !((String) model.getValueAt(i, 1)).equals(GlobalConfiguration.KEY_UNALLOCATED)) {
        String propertyname = (String) model.getValueAt(i, 2);
        if (mmproperties_
            .getDevice((String) model.getValueAt(i, 1))
            .hasLabelProperty(propertyname)) {
          String hash =
              mmproperties_
                  .getDevice((String) model.getValueAt(i, 1))
                  .getHashFromLabel(propertyname);
          map.put((String) model.getValueAt(i, 0), hash); // put with the property hash
        } else {
          map.put(
              (String) model.getValueAt(i, 0),
              GlobalConfiguration.KEY_UNALLOCATED); // set to unallocated
        }
      } else {
        map.put(
            (String) model.getValueAt(i, 0),
            (String) model.getValueAt(i, 2)); // keep the unallocated as is
      }
    }
  ...
  /**
   * Tests if the string has been generated by a SingelStateUIProperty, a TwoStateUIProperty, a
   * MultiStateUIProperty or a RescaledUIProperty.
   *
