...
public class IBeamSmartPanel extends ConfigurablePanel {
  ...
  private void setupPanel() {
    ...
    TitledBorder border2 =
        BorderFactory.createTitledBorder(
            ...
            TitledBorder.DEFAULT_POSITION,
            null,
            ColorRepository.getColor(ColorRepository.strblack));
  ...
  protected void initializeProperties() {
    ...
    addUIProperty(
        new UIProperty(this, getPropertyName(LASER_PERCFINEA), "Fine a percentage.", new NoFlag()));
    addUIProperty(
        new UIProperty(this, getPropertyName(LASER_PERCFINEB), "Fine b percentage.", new NoFlag()));
    addUIProperty(
        new UIProperty(this, getPropertyName(LASER_MAXPOWER), "Maximum power (mW).", new NoFlag()));

    addUIProperty(
        new TwoStateUIProperty(
            this, getPropertyName(LASER_OPERATION), "On/off operation property.", new NoFlag()));
    addUIProperty(
        new TwoStateUIProperty(
            this, getPropertyName(LASER_ENABLEFINE), "Enable/disable fine.", new NoFlag()));
    addUIProperty(
        new TwoStateUIProperty(
            this,
            getPropertyName(LASER_EXTERNALTRIGGER),
            "Enable/disable digital trigger.",
            new NoFlag()));
  }
    ...
    if (getPropertyName(LASER_POWER).equals(name)) {
      ...
      try {
        togglebuttonLaserOnOff_.setSelected(
            ((TwoStateUIProperty) getUIProperty(getPropertyName(LASER_OPERATION)))
                .isOnState(newvalue));
      ...
      try {
        togglebuttonExternalTrigger_.setSelected(
            ((TwoStateUIProperty) getUIProperty(getPropertyName(LASER_EXTERNALTRIGGER)))
                .isOnState(newvalue));
      } catch (UnknownUIPropertyException e) {
        e.printStackTrace();
      }
      ...
      try {
        togglesliderenableFine_.setSelected(
            ((TwoStateUIProperty) getUIProperty(getPropertyName(LASER_ENABLEFINE)))
                .isOnState(newvalue));
      } catch (UnknownUIPropertyException e) {
        e.printStackTrace();
      }
  ...
  protected void addComponentListeners() {
    ...
    SwingUIListeners.addActionListenerOnIntegerValue(
        this, getPropertyName(LASER_POWER), textfieldUserPower_, sliderPower_);

    // slider power
    SwingUIListeners.addActionListenerOnIntegerValue(
        this, getPropertyName(LASER_POWER), sliderPower_, textfieldUserPower_);

    // slider fine a
    SwingUIListeners.addActionListenerOnIntegerValue(
        this, getPropertyName(LASER_PERCFINEA), sliderFinea_, fineaperc_, "", " %");

    // slider fine b
    SwingUIListeners.addActionListenerOnIntegerValue(
        this, getPropertyName(LASER_PERCFINEA), sliderFineb_, finebperc_, "", " %");

    // laser on/off
    try {
      SwingUIListeners.addActionListenerToTwoState(
          this, getPropertyName(LASER_OPERATION), togglebuttonLaserOnOff_);
    ...
    try {
      SwingUIListeners.addActionListenerToTwoState(
          this, getPropertyName(LASER_EXTERNALTRIGGER), togglebuttonExternalTrigger_);
    } catch (IncorrectUIPropertyTypeException e1) {
      e1.printStackTrace();
    }
    ...
    try {
      SwingUIListeners.addActionListenerToTwoState(
          this, getPropertyName(LASER_ENABLEFINE), togglesliderenableFine_);
    } catch (IncorrectUIPropertyTypeException e1) {
      e1.printStackTrace();
    }
  }
