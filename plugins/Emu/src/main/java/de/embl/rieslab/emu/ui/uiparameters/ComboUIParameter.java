...
/**
 * UIParameter represented by a String value that can only take a finite number of values. The
 * possible values are passed to the constructor and only values contained in the array will be
 * accepted, otherwise the first value will be chosen.
 *
...
public class ComboUIParameter extends UIParameter<String> {
  ...
  /**
   * Constructor, a String array of allowed values must be passed as well as the index of the
   * default value in the array. If the index is not valid, then the first entry of the array is
   * used as default.
   *
   ...
   */
  public ComboUIParameter(
      ConfigurablePanel owner, String label, String description, String[] allowedValues, int ind) {
    ...
    if (ind >= 0 && ind < comboValues_.length) {
      ...
      throw new IllegalArgumentException(
          "["
              + ind
              + "] is not a valid index, it should obey: 0 <= ind < "
              + comboValues_.length
              + ".");
    }
  }
