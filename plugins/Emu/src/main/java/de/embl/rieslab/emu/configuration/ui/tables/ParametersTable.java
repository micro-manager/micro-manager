...
public class ParametersTable extends JPanel {
  ...
  /**
   * Constructor called when no configuration exists. All the parameter values are set to default
   * values.
   *
  ...
  public ParametersTable(
      Map<String, UIParameter> uiparameterSet,
    ...
    for (int i = 0; i < uiparamkeys_.length; i++) {
      if (uiparameterSet_.get(uiparamkeys_[i])
          instanceof BoolUIParameter) { // if bool, just passes the value
        model.addRow(
            new Object[] {uiparamkeys_[i], uiparameterSet_.get(uiparamkeys_[i]).getValue()});
      } else { // else, converts the value to a String
        model.addRow(
            new Object[] {uiparamkeys_[i], uiparameterSet_.get(uiparamkeys_[i]).getStringValue()});
      }
    }
   ...
   * @param paramValues Map of the UIParameter names (keys) and their value (values) from the
   *     configuration.
   * @param uipropertySet Map of the UIProperties, indexed by their name.
   * @param help Help window
   */
    ...
    for (int i = 0; i < uiparamkeys_.length; i++) {
      if (paramValues.containsKey(
        ...
        if (uiparameterSet_.get(uiparamkeys_[i]) instanceof BoolUIParameter) {
          model.addRow(
              new Object[] {
                uiparamkeys_[i], EmuUtils.convertStringToBool(paramValues.get(uiparamkeys_[i]))
              });
        } else {
        ...
        if (uiparameterSet_.get(uiparamkeys_[i]) instanceof BoolUIParameter) {
          ...
          model.addRow(
              new Object[] {
                uiparamkeys_[i], uiparameterSet_.get(uiparamkeys_[i]).getStringValue()
              });
        }
      }
    }
