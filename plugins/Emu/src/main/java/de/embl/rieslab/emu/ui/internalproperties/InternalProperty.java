...
/**
 * An InternalProperty is aimed at passing information between {@link
 * de.embl.rieslab.emu.ui.ConfigurablePanel}s using an atomic variable of type T. In the
 * ConfigurablePanel, the value can then be retrieved as a wrapper for one of the corresponding
 * primitive type. An InternalProperty is instantiated with a {@code name}. InternalProperties with
 * the same name will be shared between the different ConfigurablePanels that created them, as long
 * as they are of the same InternalPropertyType.
 *
 * <p>Upon modification of its value, through a call to {@link #setInternalPropertyValue(Object,
 * ConfigurablePanel)}, the value of the atomic variable is updated and a call to all other
 * listeners (ConfigurablePanel) except the source of the call notifies them of the change.
 *
...
public abstract class InternalProperty<T, V> {
  ...
  /**
   * Constructor, initializes the atomic value with a default one and adds the {@code owner} to the
   * lsit of listeners.
   *
  ...
  /**
   * Sets the value of the InternalProperty to {@code val} and notifies all listeners but the {@code
   * source}.
   *
   * @param val New value
  ...
  /**
   * Registers {@code listener}. If {@code listener} has an InternalProperty with same name and
   * type, then the InternalProperty is replaced by this one in the ConfigurablePanel. If no such
   * InternalProperty exists, nothing happens.
   *
  ...
  /**
   * Initialize the default value of the internal atomic value member to the value of the primitive
   * wrapper type {@code defaultval}.
   *
