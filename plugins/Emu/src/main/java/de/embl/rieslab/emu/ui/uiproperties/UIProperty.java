...
/**
 * A UIProperty acts as a link between a JComponent and a Micro-Manager device property ( {@link
 * de.embl.rieslab.emu.micromanager.mmproperties.MMProperty MMProperty}). It should be instantiated
 * in the initializeProperties() method of a ConfigurablePanel and added with the addUIProperty().
 * The UIProperty will appear in the configuration wizard of EMU. The user can then assign a
 * Micro-Manager device property to the UIProperty. While several UIProperties can be allocated to a
 * single MMProperty, each UIPropoerty can only be assigned to a single MMproperty.
 *
 * <p>A user interaction with a JComponent should trigger a corresponding UIProperty. The
 * ConfigurablePanel calls {@link #setPropertyValue(String)}. In turns, this will change the value
 * of the MMProperty. All other UIProperty allocated to the MMProperty will be updated. Finally,
 * updated UIProperty will call the owner ConfigurablePanel (triggerPropertyHasChanged()) to change
 * the relevant JComponents.
 *
 * <p>UIProperties can be labeled with a PropertyFlag to allow categorizing them. This mechanism is
 * not used within EMU, but can be exploited in the plugins.
 *
 * <p>
 *
...
public class UIProperty {
  ...
  /**
   * Returns the UIProperty's description, this description is used in the Help window of the
  ...
  /**
   * Assigns the UIProperty to a MMProperty. A UIProperty can only be assigned once and to a single
   * MMProperty. This method will throw an AlreadyAssignedUIPropertyException if there is an attempt
   * to assign it to a second MMProperty.
   *
   * @param prop MMproperty to assign the UIProperty to.
   * @return True if the property was allocated successfully, false otherwise.
   * @throws AlreadyAssignedUIPropertyException Exception thrown when the UIProperty has already
   *     been assigned
   * @throws IncompatibleMMProperty exception thrown when the MMProperty is not compatible with the
   *     UIProperty.
   */
   ...
   *
   * @return Value of the MMProperty or an empty String if the UIProperty has not been assigned to a
   *     MMProperty
   */
  ...
  /**
   * Sets the value of the assigned MMProperty to {@code newState}. For a pure UIProperty, this
   * method is equivalent to setPropertyValue (subclasses might have a different implementation).
   *
   ...
   *
   * @return True if the MMProperty is read-only, false otherwise. If the UIProperty is not
   ...
   *
   * @return Allowed values or null if the MMProperty does not have any or if the UIProperty is not
   *     assigned.
   */
   ...
   *
   * @return Limits or null if the MMProperty does not have any or if the UIProperty is not
   *     assigned.
   */
   ...
   * @return True if the value is allowed, false otherwise. If the UIProperty is not assigned,
   *     returns true
   */
