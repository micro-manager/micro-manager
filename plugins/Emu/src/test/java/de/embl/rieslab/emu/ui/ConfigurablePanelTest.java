...
public class ConfigurablePanelTest {
  ...
  public void testAddUIParameter() throws UnknownUIParameterException {
    ...
    for (int i = 0; i < params.length; i++) {
      ...
      assertEquals(
          UIParameter.getHash(cp, params[i]),
  ...
  public void testSubsituteUIParameter()
    ...
    ConfigurableTestPanel cp1 =
        new ConfigurableTestPanel(panel) {
          private static final long serialVersionUID = 1L;
  ...
  public void testAddInternalProperty() throws UnknownInternalPropertyException {
    ...
    assertEquals(
        InternalProperty.InternalPropertyType.INTEGER, cp.getInternalPropertyType(intprop[0]));
    assertEquals(
        InternalProperty.InternalPropertyType.BOOLEAN, cp.getInternalPropertyType(intprop[1]));
    assertEquals(
        InternalProperty.InternalPropertyType.DOUBLE, cp.getInternalPropertyType(intprop[2]));
  }
  ...
  public void testSetDoubleInternalProperty()
    ...
    assertEquals(
        defval, (double) cp.getInternalProperty(intprop).getInternalPropertyValue(), 1E-20);
  ...
  public void testSubsituteInternalPropertyOfDifferentType()
    ...
    assertEquals(
        InternalProperty.InternalPropertyType.INTEGER, cp1.getInternalPropertyType(intprop));
    assertEquals(
        InternalProperty.InternalPropertyType.DOUBLE, cp2.getInternalPropertyType(intprop));
  ...
  public void testSubsituteInternalPropertyOfDifferentLabel()
    ...
    assertEquals(
        InternalProperty.InternalPropertyType.INTEGER, cp1.getInternalPropertyType(intprop));
    assertEquals(
        InternalProperty.InternalPropertyType.INTEGER, cp2.getInternalPropertyType(intprop2));
  ...
  public void testComponentTriggerSafeGuard()
    ...
    // triggerPropertyHasChanged(String, String) and
    // setUIPropertyValue(String, String)), multiple call to the same MMProperty or an infinite loop
    // (when multiple UIProperties
    // are involved) can occur when ConfigurablePanel.propertyhasChanged(String, String) modifies
    // the state of a JComponent,
    // which in turn triggers an action listener and hereby a call to
    // ConfigurablePanel.setUIProperty(String, String).
    // The component trigger mechanism is private in the ConfigurablePanel and prevents updating a
    // UIProperty/MMproperty during
    ...
    final ConfigurableTestPanel cp1 =
        new ConfigurableTestPanel("My") {
          private static final long serialVersionUID = 1L;

          @Override
          protected void addComponentListeners() {
            tgl1.addItemListener(
                new ItemListener() {
    ...
    final ConfigurableTestPanel cp2 =
        new ConfigurableTestPanel("My") {
          private static final long serialVersionUID = 1L;

          @Override
          protected void addComponentListeners() {
            tgl2.addItemListener(
                new ItemListener() {
                  public void itemStateChanged(ItemEvent ev) {
                    if (ev.getStateChange() == ItemEvent.SELECTED) {
                      setUIPropertyValue(uipropLabel, vals[1]);
    ...
    // set tgl1 selected, this should trigger setUIproperty(String, String) and change mmprop to
    // val[0]
    tgl1.setSelected(true);
