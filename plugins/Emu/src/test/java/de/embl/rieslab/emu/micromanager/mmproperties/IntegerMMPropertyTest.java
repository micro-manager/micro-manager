...
public class IntegerMMPropertyTest {
  ...
  public void testArrayFromString() {
    ...
    for (int i = 0; i < s.length; i++) assertEquals(Integer.valueOf(s[i]), ints[i]);
  }
  ...
  public void testAllowedValues() {
    ...
    final IntegerMMProperty prop =
        new IntegerMMProperty(null, new Logger(), "", "", vals) {
          @Override
          public Integer getValue() { // avoids NullPointerException
            return 0;
          }
  ...
  public void testLimitedValues() {
    ...
    final IntegerMMProperty prop =
        new IntegerMMProperty(
            null, new Logger(), "", "", min, max) { // inverted min and max on purpose
          @Override
          public Integer getValue() { // avoids NullPointerException
            return 0;
          }
