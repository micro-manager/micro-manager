...
public class MultiStateUIPropertyTest {
   ...
   public void testSettingStateValueNames()
      ...
      String[] namesB = cp.property.getStatesName();
      assertEquals(names.length, namesB.length);

      for (int i = 0; i < names.length; i++) {
         assertEquals(names[i], namesB[i]);
   ...
   public void testSettingTooFewStateValueNames()
      ...
      for (int i = names.length; i < cp.property.getNumberOfStates(); i++) {
         assertEquals(MultiStateUIProperty.getConfigurationStateLabel(i), namesB[i]);
      }
   }
   ...
   public void testSettingStateValues()
      ...
      String[] statesB = cp.property.getStateValues();
      assertEquals(vals.length, statesB.length);

      for (int i = 0; i < vals.length; i++) {
         assertEquals(vals[i], statesB[i]);
      }
   }
