/**
 * @author ernieyu https://github.com/ernieyu/Swing-range-slider Modified by @nicost to allow moving
 *     the complete range
 * @license MIT
 */
...
class RangeSliderUI extends BasicSliderUI {
  ...
  /** Indicator to show that the whole section is being dragged */
  ...
  /**
   * Listener to handle mouse movements in the slider track. Modified to handle dragging of the
   * middle range by Nico Stuurman, Dec. 2015
   */
    ...
    public void mousePressed(MouseEvent e) {
      ...
      Rectangle middleRect =
          new Rectangle(
              thumbRect.x,
              thumbRect.y,
              upperThumbRect.x - thumbRect.x + upperThumbRect.width,
              thumbRect.height);
      ...
      if (upperThumbSelected || slider.getMinimum() == slider.getValue()) {
        if (upperThumbRect.contains(currentMouseX, currentMouseY)) {
          ...
          lowerPressed = true;
        } else if (middleRect.contains(currentMouseX, currentMouseY)) {
          middlePressed = true;
        }
      } else {
    ...
    private void moveMiddle(Point newPos) {
      synchronized (slider) {
        switch (slider.getOrientation()) {
          case JSlider.VERTICAL:
            {
              int distance = upperThumbRect.y - thumbRect.y;
              if (newPos.y > offset) {
                int oldUTY = upperThumbRect.y;
                moveUpperThumb(newPos, offset - upperThumbRect.y);
                if (upperThumbRect.x > oldUTY) {
                  moveLowerThumb(new Point(upperThumbRect.y - distance, newPos.x), 0);
                }
              } else {
          ...
          case JSlider.HORIZONTAL:
            {
              ...
              // We ensure that the range between top and bottom stays constant

              int distance = upperThumbRect.x - thumbRect.x;
              if (newPos.x > offset) {
                int oldUTX = upperThumbRect.x;
                moveUpperThumb(newPos, offset - upperThumbRect.x);
                if (upperThumbRect.x > oldUTX) {
                  moveLowerThumb(new Point(upperThumbRect.x - distance, newPos.y), 0);
                }
              } else {
