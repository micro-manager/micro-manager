...
public class Controller {
  ...
  public synchronized Dimension getTileDimensions() {
    return new Dimension(
        ((slideexplorerROI_.width - trimx) / 2) * 2, ((slideexplorerROI_.height - trimy) / 2) * 2);
  }
  ...
  public synchronized Dimension getCurrentRoiDimensions() {
    ...
    double roiScalingFactor =
        currentPixelSize / MathFunctions.getScalingFactor(mapToStageTransform_);

    int width = (int) (currentRoi.width * roiScalingFactor);
    int height = (int) (currentRoi.height * roiScalingFactor);
    return new Dimension((width / 2) * 2, (height / 2) * 2);
  }
  ...
  private ImageProcessor trimImage(
      ImageProcessor img, Point mapPosition, Point measuredMapPosition) {
    ...
    Rectangle croppingRoi =
        new Rectangle(trimx / 2 + dx, trimy / 2 + dy, tileDimensions.width, tileDimensions.height);
  ...
  public synchronized void specifyMapRelativeToStage(
      Point2D.Double mapOriginOnStage, double angle, double pixelSize) {
