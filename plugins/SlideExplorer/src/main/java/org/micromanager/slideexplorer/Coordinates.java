...
public class Coordinates {
  ...
  public Point getNearestTileFromMapPosition(Point mapPosition) {
    Point centerPosition =
        new Point(
            mapPosition.x + tileDimensions_.width / 2, mapPosition.y + tileDimensions_.height / 2);
  ...
  public Dimension getRoiDimensionsOffScreen() {
    return new Dimension(
        roiDimensions_.width / scaleOffScreenToMap_, roiDimensions_.height / scaleOffScreenToMap_);
  }

  public Point getOffScreenCenter() {
    return new Point(
        viewDimensionsOffScreen_.width * 3 / 2, viewDimensionsOffScreen_.height * 3 / 2);
  }
  ...
  public Rectangle offScreenToRoiRect(Point offScreen) {
    ...
    int y =
        offScreen.y + (tileDimensions_.height - roiDimensions_.height) / scaleOffScreenToMap_ / 2;
    ;
    System.out.println(w + "x" + h);
    return new Rectangle(x, y, w, h);
  }

  // The following methods may be repeatedly called as user pans, zooms, and resizes the
  // ImageWindow.
  ...
  public void zoomIn(Point zoomTargetOffScreen) {
    panTo(new Point(viewDimensionsOffScreen_.width / 4, viewDimensionsOffScreen_.height / 4));
    setZoomLevel(zoomLevel_ + 1);
  }

  public void zoomOut(Point zoomTargetOffScreen) {
    panTo(new Point(-viewDimensionsOffScreen_.width / 2, -viewDimensionsOffScreen_.height / 2));
    setZoomLevel(zoomLevel_ - 1);
  }
  ...
  public ArrayList<Point3D> getMultiTilesOffScreen() {
    return getMultiTilesInRegion(
        new Rectangle(0, 0, viewDimensionsOffScreen_.width, viewDimensionsOffScreen_.height));
  }
  ...
  public Roi roiOffScreenToMap(Roi roi) {
    ...
    Point ulCorner =
        offScreenToMap(
            new Point(
                rect.x - viewDimensionsOffScreen_.width / 2,
