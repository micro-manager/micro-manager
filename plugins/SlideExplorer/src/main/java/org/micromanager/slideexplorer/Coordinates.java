 ...
 * mappings.
 *
 * The "Map" is an integer coordinate system in units of single pixels.
 * The tiled camera images' pixels correspond directly to the Map's coordinates.
 * The Tile coordinate system is in integer pairs.
 * The OffScreen coordinate system is (0,0) in the center of the display.
 * The MultiTile coordinate system is similar to Tiles, but the numbers are lower because
 * the grid is coarser.
 *
 * Map <--> Tile
 * Map <--> OffScreen
...
public class Coordinates {
   ...
   public Point multiTileToULTile(Point3D multiTileIndex) {
      return new Point(multiTileIndex.i << -multiTileIndex.k,
            multiTileIndex.j << -multiTileIndex.k);
   }
   ...
   public Dimension getRoiDimensionsOffScreen() {
      return new Dimension(roiDimensions_.width / scaleOffScreenToMap_,
            roiDimensions_.height / scaleOffScreenToMap_);
   }

   public Point getOffScreenCenter() {
      return new Point(viewDimensionsOffScreen_.width * 3 / 2,
            viewDimensionsOffScreen_.height * 3 / 2);
   }

   public Point offScreenClickToMap(Point offScreenPixel) {
      Point mapPos = offScreenToMap(offScreenPixel);
      Point newMapPos =
            new Point(mapPos.x - tileDimensions_.width / 2, mapPos.y - tileDimensions_.height / 2);
      return newMapPos;
   }
   ...
   public Rectangle offScreenToRoiRect(Point offScreen) {
      ...
      int x =
            offScreen.x + (tileDimensions_.width - roiDimensions_.width) / scaleOffScreenToMap_ / 2;
      int y = offScreen.y
            + (tileDimensions_.height - roiDimensions_.height) / scaleOffScreenToMap_ / 2;
      ;
      System.out.println(w + "x" + h);
      return new Rectangle(x, y, w, h);
   }

   // The following methods may be repeatedly called as user pans, zooms,
   // and resizes the ImageWindow.
   ...
   public ArrayList<Point> getTilesOnScreen() {
      ...
      Point lrOffScreen =
            new Point(2 * viewDimensionsOnScreen_.width, 2 * viewDimensionsOnScreen_.height);
   ...
   public Roi roiOffScreenToMap(Roi roi) {
      ...
      Point ulCorner = offScreenToMap(new Point(rect.x - viewDimensionsOffScreen_.width / 2,
            rect.y - viewDimensionsOffScreen_.height / 2));
      rect.x = ulCorner.x;
      rect.y = ulCorner.y;
      ...
      try {
         shape =
               (Shape) JavaUtils.invokeRestrictedMethod(shapeRoi, shapeRoi.getClass(), "getShape");
      } catch (Exception ex) {
