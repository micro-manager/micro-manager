...
public class Window extends ImageWindow {
  ...
  Window(ImagePlus imgp, ImageCanvas cvs, Display display) {
    ...
    this.addComponentListener(
        new ComponentAdapter() {
          public void componentResized(ComponentEvent e) {
            if (!fullscreen_) unmaximizedBounds_ = new Rectangle(Window.this.getBounds());
          }
  ...
  /**
   * Overrides super.windClosing(WindowEvent e) to make sure that if user hits Ctrl-W, the
   * slideexplorer stops and cleans the cache.
   */
  ...
  /**
   * Overrides super.close() to make sure that if user hits Ctrl-W, the slideexplorer stops and
   * cleans the cache.
   */
