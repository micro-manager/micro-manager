...
public class Hub {
    ...
    public Hub(ScriptInterface app) {
        ...
        if (pixelSize_ == 0) {
            app_.showMessage("No pixel size is defined for the current magnification.\nPlease calibrate before running the Surveyor.");
            return;
        }
        ...
        controller_.specifyMapRelativeToStage(angle_, pixelSize_);
    ...
    void acquireMosaics() {
        ...
        Controller mosaicController = new Controller(core_);
        mosaicController.specifyMapRelativeToStage(angle_, core_.getPixelSizeUm());
        MMAcquisitionEngineMTMosaic eng = new MMAcquisitionEngineMTMosaic(mosaicController, coords_, app_);
                ...
                try {
                    if (core_.hasProperty("PFS-Offset", "Position"))
                        if (pixelConfig.equals(surveyPixelSizeConfig_))
                            core_.setProperty("PFS-Offset", "Position", "2.832");
                        else
                            core_.setProperty("PFS-Offset", "Position", "12.526");
                    } catch (Exception ex) {
                        ReportingUtils.logError(ex);
                    }
