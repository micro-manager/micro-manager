...
public class ShowWorker {
  ...
  public static void execute(
    ...
    targetStore =
        AssembleDataAlgo.assemble(studio, form, targetStore, dp1, dp2, xOffset, yOffset, 0, test);
    ...
    if (targetStore != null) {
      ...
      for (int i = 0; i < targetStore.getNextIndex(Coords.C); i++) {
        // this is scary stuff
        dpb.colorModeComposite()
            .channel(
                i,
                dispSettings
                    .getChannelSettings(0)
                    .copyBuilder()
                    .color(ColorPalettes.getFromColorblindFriendlyPalette(i))
                    .build());
      }
