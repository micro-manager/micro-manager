...
public class AssembleDataForm extends JDialog {
  ...
  public AssembleDataForm(Studio studio) {
    studio_ = studio;
    profileSettings_ = studio_.profile().getSettings(AssembleDataForm.class);
    ...
    locationsField.setText(
        profileSettings_.getString(DIRNAME, profileSettings_.getString(DIRNAME, "")));
          ...
          File f =
              FileDialogs.openDir(
                  ...
                  new FileDialogs.FileType(
                      "MMProjector", "Locations File", locationsField.getText(), true, ""));
          if (f != null) {
        ...
        (ActionEvent e) -> {
          new Thread(
                  org.micromanager.internal.utils.GUIUtils.makeURLRunnable(
                      "https://micro-manager.org/wiki/AssembleData"))
              .start();
        });
  ...
  private void assembleDataSets(boolean test) {
    ...
    int yOffset = profileSettings_.getInteger(YOFFSET, DEFAULTY);
    ShowWorker.run(
        studio_, this, dv1.getDataProvider(), dv2.getDataProvider(), xOffset, yOffset, test);
  }
  ...
  private void setupDataViewerBox(final JComboBox<String> box, final String key) {
    ...
    box.setModel(new DefaultComboBoxModel<>(dataViewers));
