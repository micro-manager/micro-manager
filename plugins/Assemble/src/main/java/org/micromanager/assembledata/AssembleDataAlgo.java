...
public class AssembleDataAlgo {
  ...
  public static Datastore assemble(
      Studio studio,
    ...
    try {
      ...
      double xMinUm =
          singlePositionImg.getMetadata().getXPositionUm()
              - (0.5 * singlePositionImg.getWidth() * singlePositionPixelSize);
      double yMinUm =
          singlePositionImg.getMetadata().getYPositionUm()
      ...
      */
      ...
      for (int t = 0; t < spdTLength; t++) {
        for (int c = 0; c < spdCLength; c++) {
          ...
          if (img != null) {
            newMetadataB = img.getMetadata().copyBuilderWithNewUUID().pixelSizeUm(basePixelSize);
            /*
            ...
            GImageMiscOps.copy(0, 0, 0, 0, img.getWidth(), img.getHeight(), oldImgBoof, newImgBoof);
            Coords coords = cb.p(targetPosition).c(c).t(t).build();
            System.out.println(coords.toString());
            newMetadataB.positionName("Site-" + targetPosition);
            Image newImage =
                BoofCVImageConverter.boofCVToMM(
                    newImgBoof, cb.p(targetPosition).c(c).t(t).build(), newMetadataB.build());
      ...
      for (int t = 0; t < mpdTLength; t++) {
        for (int c = 0; c < mpdCLenghth; c++) {
          ...
          if (newMetadataB != null) {
            ...
            Image newImage =
                BoofCVImageConverter.boofCVToMM(tmpImgBoof, coords, newMetadataB.build());
            output.putImage(newImage);
  ...
  public static Homography2D_F64 affineToHomography(Affine2D_F64 aff) {
    return new Homography2D_F64(aff.a11, aff.a12, 0, aff.a21, aff.a22, 0, aff.tx, aff.ty, 1);
  }
}
