...
public class SplitViewFrame extends JFrame implements ProcessorConfigurator {
  ...
  private static final String[] SPLIT_OPTIONS = new String[] {"Two", "Three", "Four", "Five"};
  ...
  private void initComponents() {
    ...
    splitSelector.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            // Because the first option at index 0 is 2 splits, etc.
            updateSettings(orientation_, splitSelector.getSelectedIndex() + 2);
          }
        });
  ...
  private void updateSettings(String orientation, int numSplits) {
    ...
    studio_.profile().getSettings(SplitViewFrame.class).putString(ORIENTATION, orientation);
    studio_.profile().getSettings(SplitViewFrame.class).putInteger(NUM_SPLITS, numSplits_);
