...
/**
 * Micro-Manager plugin that can split the acquired image top-down or left-right and arrange the
 * split images along the channel axis.
 *
...
public class PtcToolsFrame extends JFrame {
  ...
  private void initComponents() {
    ...
    minExpTF_ = new JTextField(settings_.getString(PtcToolsTerms.MINIMUMEXPOSURE, "0.0"));
    add(minExpTF_, "w 60, wrap");
    add(new JLabel(PtcToolsTerms.MAXIMUMEXPOSURE), "");
    maxExpTF_ = new JTextField(settings_.getString(PtcToolsTerms.MAXIMUMEXPOSURE, "0.0"));
    add(maxExpTF_, "w 60, wrap");
    add(new JLabel(PtcToolsTerms.NREXPOSURES), "");
    nrExpSp_ = new JSpinner();
    nrExpSp_.setModel(
        new SpinnerNumberModel(settings_.getInteger(PtcToolsTerms.NREXPOSURES, 30), 1, null, 1));
    add(nrExpSp_, "w 60, wrap");
    add(new JLabel(PtcToolsTerms.NRFRAMES), "");
    nrFramesSp_ = new JSpinner();
    nrFramesSp_.setModel(
        new SpinnerNumberModel(settings_.getInteger(PtcToolsTerms.NRFRAMES, 100), 1, null, 1));
        ...
        (ActionEvent e) -> {
          new Thread(
                  org.micromanager.internal.utils.GUIUtils.makeURLRunnable(
                      "https://micro-manager.org/wiki/Photon_Transfer_Curve_Assistant"))
              .start();
        });
