...
public class RatioImagingFrame extends JFrame implements ProcessorConfigurator {
  ...
  public RatioImagingFrame(PropertyMap configuratorSettings, Studio studio) {
    ...
    background1Button_ = createBackgroundButton(background1TextField_, settings_, BACKGROUND1);
    background2Button_ = createBackgroundButton(background2TextField_, settings_, BACKGROUND2);

    final JTextField bc1TextField = new JTextField(5);
    bc1TextField.setText(settings_.getString(BACKGROUND1CONSTANT, "0"));
    bc1TextField
        .getDocument()
        .addDocumentListener(new TextFieldUpdater(bc1TextField, BACKGROUND1CONSTANT, settings_));
    ...
    bc2TextField
        .getDocument()
        .addDocumentListener(new TextFieldUpdater(bc2TextField, BACKGROUND2CONSTANT, settings_));
    ...
    factorTextField.setText(
        settings_.getString(FACTOR, NumberUtils.intToDisplayString((maxValue))));
    factorTextField
        .getDocument()
        .addDocumentListener(new TextFieldUpdater(factorTextField, FACTOR, settings_));
    ...
    super.add(
        new JLabel("(Ch1 - (background + constant)) / (Ch2 - (background + constant) *"),
  ...
  private void copySettings(MutablePropertyMapView settings, PropertyMap configuratorSettings) {
    ...
    settings.putString(
        BACKGROUND1CONSTANT, configuratorSettings.getString(BACKGROUND1CONSTANT, ""));
    settings.putString(
        BACKGROUND2CONSTANT, configuratorSettings.getString(BACKGROUND2CONSTANT, ""));
    settings.putString(FACTOR, configuratorSettings.getString(FACTOR, ""));
  }

  private JComboBox createChannelCombo(MutablePropertyMapView settings, String prefKey) {
  ...
  private JTextField createBackgroundTextField(MutablePropertyMapView settings, String prefKey) {
    ...
    backgroundTextField.addFocusListener(
        new FocusListener() {
          @Override
  ...
  private String processBackgroundImage(
      String bFile, MutablePropertyMapView settings, String prefKey) {
    settings.putString(prefKey, bFile);
    return bFile;
  }

  private JButton createBackgroundButton(
      final JTextField backgroundField, MutablePropertyMapView settings, String prefKey) {
            ...
            File f =
                FileDialogs.openFile(
                    ...
                    new FileDialogs.FileType(
                        "MMAcq", "Dark image", backgroundField.getText(), true, IMAGESUFFIXES));
            if (f != null) {
