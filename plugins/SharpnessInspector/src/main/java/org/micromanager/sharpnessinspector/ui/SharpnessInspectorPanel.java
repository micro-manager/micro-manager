...
public class SharpnessInspectorPanel extends JPanel {
  ...
  private final JFreeChart zChart =
  ...
  private final JFreeChart tChart =
      ChartFactory.createXYLineChart(
          null, // title
          "Time", // xlabel
          "Sharpness (A.U.)", // ylabel
  ...
  private final JComboBox<SharpnessInspectorController.PlotMode> plotModeBox =
      new JComboBox<>(new DefaultComboBoxModel<>(SharpnessInspectorController.PlotMode.values()));
  ...
  private final XYSeries zDataSeries =
      ((XYSeriesCollection) zChart.getXYPlot().getDataset()).getSeries(SERIES_NAME);
  private final XYSeries tDataSeries =
      ((XYSeriesCollection) tChart.getXYPlot().getDataset()).getSeries(SERIES_NAME);
  ...
  public SharpnessInspectorPanel() {
    ...
    this.zChart
        .getXYPlot()
        .setDomainCrosshairVisible(true); // A crosshair overlay to display the current z position.
    this.zChart.getXYPlot().setDomainCrosshairPaint(new Color(0, 0, 0)); // black crosshair
    this.zChart
        .getXYPlot()
        .setRangeCrosshairVisible(true); // A crosshair overlay to display the current sharpness.
    this.zChart.getXYPlot().setRangeCrosshairPaint(new Color(0, 0, 0)); // black crosshair
    this.tChart
        .getXYPlot()
        .getDomainAxis()
        .setTickLabelsVisible(false); // hide the values for `time`
    this.chartPanel.addOverlay(
        noRoiOverlay); // An overlay that tells the user that there needs to be a roi selected.
    ...
    ((NumberAxis) zChart.getXYPlot().getRangeAxis())
        .setAutoRangeIncludesZero(false); // Don't always include 0 in the vertical autoranging.
    tChart.setBackgroundPaint(trans);
    tChart.getXYPlot().setBackgroundPaint(trans);
    ((NumberAxis) tChart.getXYPlot().getRangeAxis())
        .setAutoRangeIncludesZero(false); // Don't always include 0 in the vertical autoranging.

    this.plotModeBox.addItemListener(
        (evt) -> {
          SharpnessInspectorController.PlotMode mode =
              (SharpnessInspectorController.PlotMode) this.plotModeBox.getSelectedItem();
          this.setPlotMode(mode);
        });

    evaluationMode.addActionListener(
        (evt) -> {
          this.pcs.firePropertyChange(
              "evalMethod", null, (ImgSharpnessAnalysis.Method) evaluationMode.getSelectedItem());
        });
        ...
        (evt) -> {
          JOptionPane.showMessageDialog(
              ...
              "<html><body><p style='width: 200px;'>"
                  + SharpnessInspectorPlugin.README
  ...
  public void setValue(double z, double time, double sharpness) {
    ...
    this.zChart
        ...
        .setRangeCrosshairValue(
            sharpness); // Set the vertical crosshair to the current sharpness value.
    this.setZPos(z);
  ...
  public void setRoiSelected(boolean hasRoi) {
    // If there is no roi selected use this method to make an overlay visible that tells this to the
    // user.
    if (!hasRoi) {
      ...
      this.startButton.addActionListener(
          (evt) -> {
            SharpnessInspectorController.RequestScanEvent event =
                new SharpnessInspectorController.RequestScanEvent(
                    this,
