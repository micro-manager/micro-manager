...
public class SharpnessInspectorController extends AbstractInspectorPanelController {
  ...
  private SharpnessInspectorController(Studio studio) {
    ...
    panel_.addPropertyChangeListener(
        "evalMethod",
        (evt) -> {
          eval_.setMethod((ImgSharpnessAnalysis.Method) evt.getNewValue());
        });

    panel_.addScanRequestedListener(
        (evt) -> {
          SwingWorker worker =
              new SwingWorker() {
                @Override
  ...
  public void onNewImage(DataProviderHasNewImageEvent evt) {
    /// This is fired because we register for the dataprovider events. Happens each time a new image
  ...
  @Subscribe
  public void onZPosChanged(
      StagePositionChangedEvent evt) { // TODO Many z stages don't fire this. use polling instead
  ...
  private void beginScan(double intervalUm, double rangeUm) {
    ...
    if (roi == null || !roi.isArea()) {
      r =
          new Rectangle( // use full image fov
