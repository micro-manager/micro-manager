...
public class TrackerControl extends JFrame {
    ...
    addWindowListener(
        new WindowAdapter() {
          ...
          public void windowOpened(WindowEvent e) {
            ...
            diskRadioButton_.setSelected(
                up.getBoolean(
                    TrackerControl.this.getClass(), DISK_RECORDING, diskRadioButton_.isSelected()));
  ...
  public void track() {
    ...
    if (diskRadioButton_.isSelected()) {
      try {
        store_ = app_.data().createMultipageTIFFDatastore(rootField_.getText(), true, false);
    ...
    TrackerUtils.plotData(
        "Cell Track: " + acqName_, xySeries_, "X (micron)", "Y (micron)", 100, 100);
    timer_.start();
  }
    ...
    try {
      ...
      if (acqName_ != null) {
        ...
        if (tagged.pix instanceof byte[]) {
          ...
          for (int i = 0; i < size; i++) pixelsCur_[i] = pixels[i];
        }
  ...
  private void processOneFrame(TaggedImage tagged, boolean moveStage) {
    ...
    if (mirrorX_) shiftXUm = -shiftXUm;
    if (mirrorY_) shiftYUm = -shiftYUm;
    if (rotate_) {
      double tmp = shiftXUm;
      shiftXUm = shiftYUm;
      shiftYUm = tmp;
    ...
    speedLabel_.setText(
        "n="
            ...
            + " s, d="
            + TextUtils.FMT2.format(d)
            + " um, l="
            + TextUtils.FMT2.format(distUm_)
            + " um, v="
            + TextUtils.FMT2.format(v)
            + " um/s");
    try {
      tagged.tags.put(D, d);
      tagged.tags.put(V, v);
  ...
  private void initialize() {
    ...
    try {
      mirrorX_ =
          app_.core().getProperty(camera, MMCoreJ.getG_Keyword_Transpose_MirrorX()).equals("1");
      mirrorY_ =
          app_.core().getProperty(camera, MMCoreJ.getG_Keyword_Transpose_MirrorY()).equals("1");
      rotate_ =
    ...
    topLeftButton_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(final ActionEvent e) {
    ...
    bottomRightButton_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(final ActionEvent e) {
            double[] x = new double[1];
