...
public class ChannelCorrectorPanel extends JPanel {
  ...
  public ChannelCorrectorPanel(
      Studio studio,
      DataViewer dataViewer,
      int ch2nr,
      ExecutorService executor,
      boolean useAllPositions) {
    studio_ = studio;
  ...
  public AffineTransform calculateTransform() throws IOException {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          null,
          "Inexplicably failed to invert the affine transform",
          "Annoying Error!",
          JOptionPane.ERROR_MESSAGE);
    }
  ...
  private ArrayList<Point2D.Double> detectPoints(Image img, GaussianFit gf) {
    ...
    Polygon maxima =
        FindLocalMaxima.FindMax(siPlus, distance_, threshold_, FindLocalMaxima.FilterType.NONE);
    ...
    for (int j = 0; j < sC.length; j++) {
      ...
      if (sC[j][0] > halfSize_
          && sC[j][0] < siPlus.getWidth() - halfSize_
          && sC[j][1] > halfSize_
          && sC[j][1] < siPlus.getHeight() - halfSize_) {
        ImageProcessor sp = SpotData.getSpotProcessor(siProc, halfSize_, sC[j][0], sC[j][1]);
        if (sp == null) {
          continue;
        }
