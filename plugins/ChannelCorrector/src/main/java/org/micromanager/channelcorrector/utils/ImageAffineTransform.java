...
public class ImageAffineTransform {
  ...
  public void apply(boolean allPositions) throws IOException, ImageAffineTransformException {
    ...
    for (int c = 1; c <= maxChan; c++) {
      ...
      Image out = transformImage(dp.getImage(builder.c(c).build()), affineTransformOp);
    ...
    outStore.setSummaryMetadata(
        dp.getSummaryMetadata().copyBuilder().intendedDimensions(intendedDimensions).build());
      ...
      for (int t = 0; t < dp.getNextIndex(Coords.T); t++) {
        for (int z = 0; z < dp.getNextIndex(Coords.Z); z++) {
          ...
          for (int c = 1; c < dp.getNextIndex(Coords.C); c++) {
            ...
            outStore.putImage(
                transformImage(inImage, affineTransformOps_.get(c - 1), minWidth, minHeight));
          }
        }
      }
    }
  ...
  public Image crop(Image inImg, int x, int y, int width, int height)
      throws ImageAffineTransformException {
    ImageProcessor ip = null;
    if (inImg.getBytesPerPixel() == 1) {
      ip = new ByteProcessor(inImg.getWidth(), inImg.getHeight(), (byte[]) inImg.getRawPixels());
    } else if (inImg.getBytesPerPixel() == 2) {
      ip = new ShortProcessor(inImg.getWidth(), inImg.getHeight());
    ...
    if (ip != null) {
      ...
      return studio_
          ...
          .createImage(
              ...
              copyIp.getWidth(),
              copyIp.getHeight(),
              inImg.getBytesPerPixel(),
              inImg.getNumComponents(),
  ...
  /**
   * Given an input image and affine transform, will apply the affine transform to the image and
   * returned the transformed image (which may be of a different size from the input image For now
   * only works on 16 bit (short) images
   *
  ...
  public Image transformImage(Image inImg, AffineTransformOp aOp)
    ...
    if (inImg.getBytesPerPixel() != 2) {
      throw new ImageAffineTransformException(
          "ImageAffineTransform only works with 2 bytes per pixel");
    }
    ...
    return studio_
        .data()
        .ij()
        .createImage(p.getProcessor(), inImg.getCoords(), inImg.getMetadata());
  }

  /**
   * Uses the provided AffineTransform to transfrom the input image, then crops it to given width
   * and height
   *
  ...
  public Image transformImage(Image inImg, AffineTransformOp aOp, int width, int height)
    ...
    ShortProcessor testProc = (ShortProcessor) studio_.data().ij().createProcessor(inImg);
    BufferedImage bi16 = testProc.get16BitBufferedImage();
    BufferedImage aOpResult =
        new BufferedImage(bi16.getWidth(), bi16.getHeight(), BufferedImage.TYPE_USHORT_GRAY);
