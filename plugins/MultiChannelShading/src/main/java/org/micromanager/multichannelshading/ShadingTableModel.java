...
public class ShadingTableModel extends AbstractTableModel {
  ...
  public final String[] COLUMN_NAMES = new String[] {"Preset", "Image File", ""};
  private String channelGroup_;
  private List<String> presetList_;
  private List<String> fileList_;
  private final ImageCollection imageCollection_;

  public ShadingTableModel(Studio gui, ImageCollection imageCollection) {
    gui_ = gui;
  ...
  public void setChannelGroup(String newGroup) {
    try {
      ...
      if (channelGroup_ != null) {
        String[] channels = presetList_.toArray(new String[presetList_.size()]);
        String[] files = fileList_.toArray(new String[fileList_.size()]);
        gui_.profile()
            .getSettings(this.getClass())
            .putStringList(channelGroup_ + "-channels", channels);
        gui_.profile().getSettings(this.getClass()).putStringList(channelGroup_ + "-files", files);
      }
  ...
  /**
   * Removes selected rows from the tablemodel calls fireTableDataChanged to update the UI
   *
