...
/**
 * Utility class for Shading plugin that holds the background and flatfield images. The images are
 * stored as ImagePlusInfo objects. Images should be provided as full frame, unbinned images. When
 * binned images, and/or ROIs of (binned) images are requested, these are provided on the fly, and a
 * copy is cached for later use.
 *
 * @author nico
 */
public class ImageCollection {
  ...
  public ImagePlusInfo getBackground(int binning, Rectangle roi) throws ShadingException {
  ...
  /**
   * Stores flatfield image in the internal data store If a background image is present, it will be
   * subtracted first Then, the average pixel value will be calculated A normalized image will be
   * calculated by dividing the mean through the pixel value. Therefore, use these flatfield images
   * by multiplying background corrected images, rather than by dividing. This behavior improves
   * performance.
   *
  ...
  public void addFlatField(String preset, String file) throws ShadingException {
    ...
    try {
      ...
      if (bg != null) {
        ...
        dp = ImageUtils.subtractImageProcessors(ip.getProcessor(), bg.getProcessor());
      } else {
        dp = ip.getProcessor();
      }
      long total = 0;
      ...
      gui_.logs().logError("Shading plugin, addFlatField in ImageCollection: " + ex.getMessage());
    }
  ...
  private String makeKey(int binning, Rectangle roi) {
    ...
    String key = "" + binning;
    if (roi != null) {
      key = binning + "-" + roi.x + "-" + roi.y + "-" + roi.width + "-" + roi.height;
    }
    return key;
  }
  ...
  private ImagePlusInfo makeDerivedImage(ImagePlusInfo ipi, int binning, Rectangle roi)
    ...
    if (ipi.getBinning() != 1) {
      throw new ShadingException(
          "This is not an unbinned image.  " + "Can not derive binned images from this one");
    }
    ...
    ImagePlusInfo newIp =
        new ImagePlusInfo(new ImagePlus("", resultProcessor.crop()), binning, roi);
