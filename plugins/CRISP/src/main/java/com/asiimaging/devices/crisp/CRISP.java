...
public class CRISP {
   ...
   private String getFocusCurveData(final int n) {
      ...
      try {
         ...
         studio.logs().showError(
               "could not get the data from " + PropName.MS2000.FOCUS_CURVE_DATA_PREFIX + n);
      }
      return result;
   }
   ...
   public void setRefreshPropertyValues(final boolean state) {
      try {
         core.setProperty(deviceName, PropName.REFRESH_PROP_VALUES,
               state ? PropValue.YES : PropValue.NO);
      } catch (Exception e) {
   ...
   public void setOnlySendSerialCommandOnChange(final boolean state) {
      try {
         ...
         studio.logs()
               .logError("TigerCommHub could not set OnlySendSerialCommandOnChange to " + state);
      }
   }
