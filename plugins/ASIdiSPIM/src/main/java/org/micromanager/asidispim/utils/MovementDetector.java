...
public class MovementDetector {
  ...
  /**
   * This class determines the position of the object in a stack and returns the difference in XYZ
   * position of that object with the previous time point (or a zeroed 3D point if this is the first
   * timepoint).
   *
   ...
   */
  public MovementDetector(final Prefs prefs, final int ch, final int pos, final Rotation rotation) {
    prefs_ = prefs;
    ch_ = ch;
    pos_ = pos;
    rotation_ = rotation;
  ...
  /**
   * Determines sample position (using indicated method) and returns the difference with the
   * position in the previous frame (or the 0,0,0 vector for the first time point). May remember the
   * position of the object so that it can be used next. It is therefore important to call this
   * function consecutively for each time point. However, this will result in the stages moving back
   * and forth between 2 positions. To avoid this, the last stage position is corrected by the
   * movement (that was presumably executed by the stages
   *
   * @param method
   * @return Movement (in microns) since the last time-point (or a zeroed vector if the movement is
