...
public class CameraModes {
  ...
  /**
   * Enum to store the acquisition mode along with associated preference code. Wanted to store
   * integer code instead of string so that the mode descriptions can be easily changed.
   */
  ...
  /**
   * Does camera support pseudo overlap/synchronous mode? Both PCO and Photometrics 95B do
   *
   * @param devKey
   * @return
   */
  private static boolean cameraSupportsPseudoOverlap(Devices.Libraries camLib) {
    return (camLib == Devices.Libraries.PCOCAM || camLib == Devices.Libraries.PVCAM);
  }
  ...
  private static boolean cameraInvalid(Devices.Libraries camLib) {
    return (camLib == Devices.Libraries.UNKNOWN || camLib == Devices.Libraries.NODEVICE);
  }

  /**
   * get list of all valid camera triggering modes for specified camera library (can't do with
   * Device key because needs to be static for reference elsewhere)
   *
   * @param camLib
   * @return
   */
  ...
  private class CameraModeComboBoxListener implements ActionListener, DevicesListenerInterface {
    ...
    /**
     * Resets the items in the combo box based on devices available. Besides being called on
     * original combo box creation, it is called whenever something in the devices tab is changed
     */
      ...
      if (origItem != null) {
        ...
        if (jcb_.getSelectedItem() != null) {
          ...
          MyDialogUtils.showError(
              "For preference "
                  + Properties.Keys.PLUGIN_CAMERA_MODE.toString()
                  + " the previous selection \""
                  + getKeyFromPrefCode(origCode)
                  + "\" is not valid.  Changing to default.");
          prefs_.putInt(
              MyStrings.PanelNames.SETTINGS.toString(),
              Properties.Keys.PLUGIN_CAMERA_MODE,
              ((Keys) jcb_.getSelectedItem()).getPrefCode());
    ...
    /**
     * Returns whatever acquisition modes are available based on devices and installed firmware. Can
     * be expanded in the future Currently returns all modes supported by either camera and then we
     * check on acquisition to make sure the selected camera supports the selected mode.
     *
     * @return
     */
    private List<Keys> getValidModeKeys() {
      List<Keys> keyList =
          CameraModes.getValidModeKeys(devices_.getMMDeviceLibrary(Devices.Keys.CAMERAA));
      List<Keys> keyListB =
          CameraModes.getValidModeKeys(devices_.getMMDeviceLibrary(Devices.Keys.CAMERAB));
      for (Keys k : keyListB) {
