...
/**
 * Ported from Micro-Manager source code Base class for frame windows. Saves and restores window
 * size and position.
 */
public class SPIMFrame extends JFrame {
   ...
   * screen is removed (e.g. had 2 monitors and go to 1). TODO: this code is duplicated between here
   * and MMDialog.
   *
  ...
  private void ensureSafeWindowPosition(int x, int y) {
    ...
    int prefX = profile.getSettings(this.getClass()).getInteger(prefPrefix_ + WINDOW_X, 0);
    int prefY = profile.getSettings(this.getClass()).getInteger(prefPrefix_ + WINDOW_Y, 0);
    if (getGraphicsConfigurationContaining(prefX, prefY) == null) {
      // only reach this code if the pref coordinates are off screen
      profile.getSettings(this.getClass()).putInteger(prefPrefix_ + WINDOW_X, x);
      profile.getSettings(this.getClass()).putInteger(prefPrefix_ + WINDOW_Y, y);
    }
  }
  ...
  public void loadPosition(int x, int y, int width, int height) {
    ...
    UserProfile profile = gui_.profile();
    setBounds(
        profile.getSettings(this.getClass()).getInteger(prefPrefix_ + WINDOW_X, x),
        profile.getSettings(this.getClass()).getInteger(prefPrefix_ + WINDOW_Y, y),
        profile.getSettings(this.getClass()).getInteger(prefPrefix_ + WINDOW_WIDTH, width),
        profile.getSettings(this.getClass()).getInteger(prefPrefix_ + WINDOW_HEIGHT, height));
    offsetIfNecessary();
  }
  ...
  /**
   * Scan the program for other MMFrames that have the same type as this MMFrame, and make certain
   * we don't precisely overlap any of them.
   */
  private void offsetIfNecessary() {
    ...
    do {
      ...
      for (Frame frame : Frame.getFrames()) {
        if (frame != this && frame.getClass() == getClass() && frame.getLocation().equals(newLoc)) {
          foundOverlap = true;
          newLoc.x += 22;
          newLoc.y += 22;
        }
  ...
  /**
   * Load window position and size from profile if possible. If not possible then sets them from
   * arguments Attaches a listener to the window that will save the position when the window closing
   * event is received
   *
  ...
  /**
   * Load window position and size from profile if possible. If not possible then sets it from
   * arguments Attaches a listener to the window that will save the position when the window closing
   * event is received
   *
   * @param x - X position of this dialog if preference value invalid
  ...
  public void savePosition() {
    ...
    UserProfile profile = gui_.profile();
    profile.getSettings(this.getClass()).putInteger(prefPrefix_ + WINDOW_X, r.x);
    profile.getSettings(this.getClass()).putInteger(prefPrefix_ + WINDOW_Y, r.y);
    profile.getSettings(this.getClass()).putInteger(prefPrefix_ + WINDOW_WIDTH, r.width);
    profile.getSettings(this.getClass()).putInteger(prefPrefix_ + WINDOW_HEIGHT, r.height);
  }
