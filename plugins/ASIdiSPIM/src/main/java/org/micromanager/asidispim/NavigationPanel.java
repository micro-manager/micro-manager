...
public class NavigationPanel extends ListeningJPanel {
  ...
  public NavigationPanel(
    ...
    super(
        MyStrings.PanelNames.NAVIGATION.toString(),
        new MigLayout("", "[center]8[center]", "[]8[]8[]"));
    devices_ = devices;
    props_ = props;
    joystick_ = joystick;
    ...
    JPanel loadPanel = new JPanel(new MigLayout("", "[center]", "[]4[]4[]10[]4[]4[]"));
    ...
    headUpGo.setIcon(new ImageIcon(getClass().getResource("/org/micromanager/icons/arrow_up.png")));
    headUpGo.setText("");
    headUpGo.setToolTipText("Move SPIM head to set height for sample loading");
    headUpGo.addActionListener(
        new ActionListener() {
          @Override
    ...
    JPanel navPanel =
        new JPanel(
            new MigLayout(
                ...
                "[right]8["
                    + positionWidth
                    + "px!,left]8[center]8[center]2[center]2[center]8[center]8[center]8[center]8[center]",
                "[]4[]"));
    navPanel.setBorder(BorderFactory.createLineBorder(ASIdiSPIM.BORDERCOLOR));

    navPanel.add(
        new JLabel(devices_.getDeviceDisplayVerbose(Devices.Keys.XYSTAGE, Directions.X) + ":"));
    ...
    navPanel.add(
        new JLabel(devices_.getDeviceDisplayVerbose(Devices.Keys.XYSTAGE, Directions.Y) + ":"));
    ...
    navPanel.add(
        makeIncrementButton(Devices.Keys.LOWERZDRIVE, Directions.NONE, deltaZField, "-", -1));
    ...
    navPanel.add(
        makeIncrementButton(Devices.Keys.UPPERZDRIVE, Directions.NONE, deltaFField, "-", -1));
    ...
    if (!ASIdiSPIM.OSPIM) {
      ...
      navPanel.add(
          makeIncrementButton(Devices.Keys.UPPERHDRIVE, Directions.NONE, deltaGField, "-", -1));
    ...
    navPanel.add(
        new JLabel(devices_.getDeviceDisplayVerbose(Devices.Keys.GALVOA, Directions.X) + ":"));
    ...
    navPanel.add(
        new JLabel(devices_.getDeviceDisplayVerbose(Devices.Keys.GALVOA, Directions.Y) + ":"));
    ...
    if (!ASIdiSPIM.OSPIM) {
      navPanel.add(
          new JLabel(devices_.getDeviceDisplayVerbose(Devices.Keys.GALVOB, Directions.X) + ":"));
      ...
      navPanel.add(
          new JLabel(devices_.getDeviceDisplayVerbose(Devices.Keys.GALVOB, Directions.Y) + ":"));
    ...
    joystickPanel_ =
        new JoystickSubPanel(joystick_, devices_, panelName_, Devices.Sides.NONE, prefs_);
    ...
    cameraPanel_ =
        new CameraSubPanel(gui_, cameras_, devices_, panelName_, Devices.Sides.NONE, prefs_);
  ...
  private JButton makeMoveToOriginButton(Devices.Keys key, Joystick.Directions dir) {
    ...
    jb.setToolTipText(
        "Similar to pressing \"HOME\" button on joystick case, but only for this axis");
  ...
  /**
   * Creates a button which set the origin to the current position in specified axis. Somewhat
   * inefficient implementation because actionPerformed() handles all the cases every call instead
   * of having the constructor sort through cases and attaching variants of the actionPerformed()
   * listener
   *
   * @param key
   * @param dir
  ...
  private JButton makeIncrementButton(
    ...
    class incrementButtonActionListener implements ActionListener {
      ...
      public void actionPerformed(ActionEvent e) {
        try {
          positions_.setPositionRelative(key_, dir_, ((Double) field_.getValue()) * scaleFactor_);
        } catch (Exception ex) {
          MyDialogUtils.showError(ex);
        }
      }
  ...
  /**
   * Gets called when this tab gets focus. Sets the physical UI in the Tiger controller to what was
   * selected in this pane
   */
  @Override
  ...
  public void raiseSPIMHead() {
    positions_.setPosition(
        Devices.Keys.UPPERZDRIVE, ((Number) headUpPosition_.getValue()).doubleValue());
  }

  public void lowerSPIMHead() {
    positions_.setPosition(
        Devices.Keys.UPPERZDRIVE, ((Number) headDownPosition_.getValue()).doubleValue());
  }
  ...
  public void haltAllMotion() {
    try {
      final Set<Devices.Keys> TigerDevices =
          EnumSet.of(Devices.Keys.UPPERZDRIVE, Devices.Keys.LOWERZDRIVE, Devices.Keys.XYSTAGE);
