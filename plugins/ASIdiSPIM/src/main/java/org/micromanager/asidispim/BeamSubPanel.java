...
public final class BeamSubPanel extends ListeningJPanel {
  ...
  public BeamSubPanel(
      ...
      Properties props) {
    super(
        MyStrings.PanelNames.BEAM_SUBPANEL.toString() + instanceLabel,
        new MigLayout("", "[right]8[left]", "[]4[]"));
    ...
    beamABox_ =
        pu.makeCheckBox(
            ...
            Properties.Keys.BEAM_ENABLED,
            instanceLabel_,
            Prefs.Keys.SHEET_BEAM_ENABLED);
    sheetABox_ =
        pu.makeCheckBox(
            "Sheet",
            Properties.Values.SAM_DISABLED,
            Properties.Values.SAM_ENABLED,
            Devices.getSideSpecificKey(Devices.Keys.GALVOA, side_),
            Properties.Keys.SA_MODE_X,
            instanceLabel_,
            Prefs.Keys.SHEET_SCAN_ENABLED);
    ...
    updateOnTab_.setSelected(
        prefs_.getBoolean(instanceLabel_, Prefs.Keys.ENABLE_BEAM_SETTINGS, true));
    super.add(updateOnTab_, "center, span 3");
  } // constructor

  /**
   * Listens to changes in state of the beam-enabled checkbox Enables/disables the sheet checkbox
   * accordingly When beam is re-enabled and sheet checkbox is checked, re-enables sheet This is
   * implemented as an actionlistener, so that it gets called after the itemlisteners have been
   * called. This also means that it is not called after programmatic changes to the Beam-enabled
   * checkbox
   */
  ...
  /**
   * Called when enclosing panel gets focus (need to call in gotSelected() function of enclosing
   * frame). Includes on plugin open if that frame was open the last time the plugin was run.
   */
  @Override
  public void gotSelected() {
    // have to do this the "hard way" because itemStateChanged only responds to _changes_ in value
    //  and changing the beam setting (to refresh it) erases the state of the sheet setting in
    // firmware
    ...
    if (updateOnTab_.isSelected()
        && props_
            .getPropValueString(deviceA, Properties.Keys.BEAM_ENABLED)
            .equals(Properties.Values.YES.toString())
        && !beamABox_.isSelected()) {
      syncPropertyAndCheckBox(
          beamABox_,
          deviceA,
          Properties.Keys.BEAM_ENABLED,
          Properties.Values.YES,
    ...
    if (updateOnTab_.isSelected()
        && props_
            .getPropValueString(deviceB, Properties.Keys.BEAM_ENABLED)
            .equals(Properties.Values.YES.toString())
  ...
  /**
   * Internal function for updating the controller properties according to checkbox settings or vice
   * versa.
   *
   * @param box
   * @param devKey
  ...
  private void syncPropertyAndCheckBox(
    ...
    if (devices_.getMMDevice(devKey) == null) {
      ...
      if (checkBoxWins) {
        boolean boxVal =
            (box.isSelected() && box.isEnabled()); // disabled box <=> sheet box with beam disabled
  ...
  /** @return true if beam A is on (navigation panel) or if imaging beam is on (setup panel) */
  public boolean isBeamAOn() {
    return beamABox_.isSelected();
  }

  /** @return true if beam B is on (navigation panel) or if epi beam is on (setup panel) */
  public boolean isBeamBOn() {
    return beamBBox_.isSelected();
  }

  /** @return true if sheet A is on (navigation panel) or if imaging sheet is on (setup panel) */
  public boolean isSheetAOn() {
    return sheetABox_.isSelected();
  }

  /** @return true if sheet B is on (navigation panel) or if epi sheet is on (setup panel) */
  public boolean isSheetBOn() {
    return sheetBBox_.isSelected();
  }

  /**
   * sets the imaging beam (or path A from navigation panel) to the desired state
   *
