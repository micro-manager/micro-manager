...
public class AcquisitionPanel extends ListeningJPanel implements DevicesListenerInterface {
  ...
  private final AtomicBoolean cancelAcquisition_ =
      new AtomicBoolean(false); // true if we should stop acquisition
  private final AtomicBoolean acquisitionRequested_ =
      new AtomicBoolean(false); // true if acquisition has been requested to start or is underway
  private final AtomicBoolean acquisitionRunning_ =
      new AtomicBoolean(false); // true if the acquisition is actually underway
  ...
  public AcquisitionPanel(
    ...
    ChangeListener recalculateTimingDisplayCL =
        new ChangeListener() {
          @Override
    ...
    // slice period, volume duration, and time lapse duration
    ActionListener recalculateTimingDisplayAL =
        new ActionListener() {
          @Override
    ...
    String[] str12 = {"1", "2"};
    numSides_ =
        pu.makeDropDownBox(str12, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_NUM_SIDES, str12[1]);
    ...
    if (ASIdiSPIM.OSPIM) {
      props_.setPropValue(
          Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_FIRST_SIDE, Devices.Sides.A.toString());
    }
    firstSide_ =
        pu.makeDropDownBox(
            ab, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_FIRST_SIDE, Devices.Sides.A.toString());
    ...
    // used to read/write directly to galvo/micro-mirror firmware, but want different stage scan
    // behavior
    delaySide_ =
        pu.makeSpinnerFloat(
            0, 10000, 0.25, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_DELAY_BEFORE_SIDE, 50);
    ...
    numSlices_ =
        pu.makeSpinnerInteger(1, 65000, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_NUM_SLICES, 20);
    ...
    stepSize_ =
        pu.makeSpinnerFloat(
            0, 100, 0.1, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_SLICE_STEP_SIZE, 1.0);
    pu.addListenerLast(
        stepSize_,
    ...
    JSpinner lsScanReset =
        pu.makeSpinnerFloat(
            1,
            100,
            0.25, // practical lower limit of 1ms
            Devices.Keys.PLUGIN,
    ...
    JSpinner lsScanSettle =
        pu.makeSpinnerFloat(
            0.25, 100, 0.25, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_LS_SCAN_SETTLE, 1);
    ...
    JSpinner lsShutterWidth =
        pu.makeSpinnerFloat(
            0.1, 100, 1, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_LS_SHUTTER_WIDTH, 5);
    ...
    desiredSlicePeriod_ =
        pu.makeSpinnerFloat(
            1, 1000, 0.25, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_DESIRED_SLICE_PERIOD, 30);

    minSlicePeriodCB_ =
        pu.makeCheckBox(
            "Minimize slice period",
            Properties.Keys.PREFS_MINIMIZE_SLICE_PERIOD,
            panelName_,
            false);
    minSlicePeriodCB_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    desiredLightExposure_ =
        pu.makeSpinnerFloat(
            1.0, 1000, 0.25, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_DESIRED_EXPOSURE, 8.5);
    desiredLightExposure_.addChangeListener(
        PanelUtils.coerceToQuarterIntegers(desiredLightExposure_));
    desiredLightExposure_.addChangeListener(recalculateTimingDisplayCL);
    normalPanel_.add(desiredLightExposure_, "wrap");
    ...
    slicePanelContainer_.add(
        getSPIMCameraMode() == CameraModes.Keys.LIGHT_SHEET ? lightSheetPanel_ : normalPanel_,
        "growx");
    ...
    advancedSliceTimingCB_ =
        pu.makeCheckBox(
            "Use advanced timing settings",
            Properties.Keys.PREFS_ADVANCED_SLICE_TIMING,
            panelName_,
            false);
    slicePanel_.add(advancedSliceTimingCB_, "left, span 2, wrap");
    ...
    lineScanDuration_ =
        pu.makeSpinnerFloat(
            ...
            0.25,
            new Devices.Keys[] {Devices.Keys.GALVOA, Devices.Keys.GALVOB},
            Properties.Keys.SPIM_DURATION_SCAN,
            10);
    ...
    durationCamera_ =
        pu.makeSpinnerFloat(
            ...
            0.25,
            new Devices.Keys[] {Devices.Keys.GALVOA, Devices.Keys.GALVOB},
            Properties.Keys.SPIM_DURATION_CAMERA,
            0);
    ...
    ItemListener sliceTimingDisableGUIInputs =
        new ItemListener() {
          @Override
    ...
    ActionListener showAdvancedTimingFrame =
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            boolean enabled = advancedSliceTimingCB_.isSelected();
    ...
    numTimepoints_ =
        pu.makeSpinnerInteger(
            1, 100000, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_NUM_ACQUISITIONS, 1);
    numTimepoints_.addChangeListener(recalculateTimeLapseDisplay);
    numTimepoints_.addChangeListener(
        new ChangeListener() {
          @Override
    ...
    acquisitionInterval_ =
        pu.makeSpinnerFloat(
            0.1, 32000, 0.1, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_ACQUISITION_INTERVAL, 60);
    ...
    useTimepointsCB_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    separateTimePointsCB_ =
        pu.makeCheckBox(
            "Separate viewer / file for each time point",
            Properties.Keys.PREFS_SEPARATE_VIEWERS_FOR_TIMEPOINTS,
            panelName_,
            false);

    saveCB_ =
        pu.makeCheckBox(
            "Save while acquiring", Properties.Keys.PREFS_SAVE_WHILE_ACQUIRING, panelName_, false);

    // make sure that when separate viewer is enabled then saving gets enabled too
    separateTimePointsCB_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    rootField_.addPropertyChangeListener(
        new PropertyChangeListener() {
          // will respond to commitEdit() as well as GUI edit on commit
          @Override
          public void propertyChange(PropertyChangeEvent evt) {
            prefs_.putString(
                panelName_, Properties.Keys.PLUGIN_DIRECTORY_ROOT, rootField_.getText());
          }
        });
    ...
    browseRootButton.addActionListener(
        new ActionListener() {
          @Override
    ...
    prefixField_.addPropertyChangeListener(
        new PropertyChangeListener() {
          @Override
          public void propertyChange(PropertyChangeEvent evt) {
            prefs_.putString(
                panelName_, Properties.Keys.PLUGIN_NAME_PREFIX, prefixField_.getText());
          }
        });
    ...
    durationPanel_.setPreferredSize(
        new Dimension(125, 0)); // fix width so it doesn't constantly change depending on text
    ...
    buttonTestAcq_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    buttonStart_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    usePositionsCB_ =
        pu.makeCheckBox(
            "Multiple positions (XY)", Properties.Keys.PREFS_USE_MULTIPOSITION, panelName_, false);
    ...
    componentBorder =
        new ComponentTitledBorder(
            usePositionsCB_, positionPanel, BorderFactory.createLineBorder(ASIdiSPIM.BORDERCOLOR));
    ...
    editPositionListButton.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    positionDelay_ =
        pu.makeSpinnerFloat(
            0.0, 10000.0, 100.0, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_POSITION_DELAY, 0.0);
    positionPanel.add(positionDelay_, "wrap");

    // enable/disable panel elements depending on checkbox state
    usePositionsCB_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    navigationJoysticksCB_.setSelected(
        prefs_.getBoolean(panelName_, Properties.Keys.PLUGIN_USE_NAVIGATION_JOYSTICKS, false));
    navigationJoysticksCB_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            updateJoysticks();
            prefs_.putBoolean(
                panelName_,
                Properties.Keys.PLUGIN_USE_NAVIGATION_JOYSTICKS,
    ...
    useAutofocusCB_.setSelected(
        prefs_.getBoolean(panelName_, Properties.Keys.PLUGIN_ACQUSITION_USE_AUTOFOCUS, false));
    useAutofocusCB_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            prefs_.putBoolean(
    ...
    useMovementCorrectionCB_.setSelected(
        prefs_.getBoolean(
            panelName_, Properties.Keys.PLUGIN_ACQUSITION_USE_MOVEMENT_CORRECTION, false));
    useMovementCorrectionCB_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            prefs_.putBoolean(
  ...
  private void updateCalibrationOffset(final Sides side, final AutofocusUtils.FocusResult score) {
    if (score.getFocusSuccess()) {
      double offsetDelta = score.getOffsetDelta();
      double maxDelta =
          props_.getPropValueFloat(
              Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_AUTOFOCUS_MAXOFFSETCHANGE);
  ...
  /**
   * Sets the acquisition name prefix programmatically. Added so that name prefix can be changed
   * from a script.
   *
   * @param acqName
   */
  ...
  private void updateStartButton() {
    ...
    ImageIcon cancelIcon =
        new ImageIcon(getClass().getResource("/org/micromanager/icons/cancel.png"));
    ImageIcon arrowIcon =
        new ImageIcon(getClass().getResource("/org/micromanager/icons/arrow_right.png"));
    buttonStart_.setIcon(started ? cancelIcon : arrowIcon);
    buttonTestAcq_.setEnabled(!started);
  }

  /**
   * @return CameraModes.Keys value from Camea panel (edge, overlap, pseudo-overlap, light sheet)
   */
  private CameraModes.Keys getSPIMCameraMode() {
    ...
    try {
      ...
      val =
          CameraModes.getKeyFromPrefCode(
              prefs_.getInt(
  ...
  /**
   * convenience method to avoid having to regenerate acquisition settings public for API use
   *
  ...
  public AcquisitionSettings getCurrentAcquisitionSettings() {
    ...
    acqSettings.isStageScanning =
        (acqSettings.spimMode == AcquisitionModes.Keys.STAGE_SCAN
    ...
    acqSettings.acquireBothCamerasSimultaneously =
        prefs_.getBoolean(
            MyStrings.PanelNames.SETTINGS.toString(),
            Properties.Keys.PLUGIN_ACQUIRE_BOTH_CAMERAS_SIMULT,
            false);
    ...
    acqSettings.hardwareTimepoints =
        false; //  when running acquisition we check this and set to true if needed
    acqSettings.separateTimepoints = getSavingSeparateFile();
    return acqSettings;
  }

  /**
   * gets the correct value for the slice timing's sliceDuration field based on other values of
   * slice timing
   *
   * @param s
   * @return
  ...
  private float getSliceDuration(final SliceTiming s) {
    ...
    return Math.max(
        Math.max(
            s.scanDelay + (s.scanPeriod * s.scanNum), // scan time
  ...
  /**
   * gets the slice timing from advanced settings (normally these advanced settings are read-only
   * and we populate them ourselves depending on the user's requests and our algorithm below)
   *
   * @return
   */
  ...
  private SliceTiming getTimingFromPeriodAndLightExposure(boolean showWarnings) {
    ...
    // 3. camera reset time
    // 4. start scan 0.25ms before camera global exposure and shifted up in time to account for
    // delay introduced by Bessel filter
    // 5. turn on laser as soon as camera global exposure, leave laser on for desired light exposure
    // time
    // 7. end camera exposure in final 0.25ms, post-filter scan waveform also ends now

    final float scanLaserBufferTime =
        MyNumberUtils.roundToQuarterMs(0.25f); // below assumed to be multiple of 0.25ms
    ...
    SliceTiming s = new SliceTiming();
    final float cameraResetTime =
        computeCameraResetTime(); // recalculate for safety, 0 for light sheet
    final float cameraReadoutTime =
        computeCameraReadoutTime(); // recalculate for safety, 0 for overlap
    ...
    // we will wait cameraReadout_max before triggering camera, then wait another cameraReset_max
    // for global exposure
    ...
    // group delay for 5th-order bessel filter ~0.39/freq from theory and ~0.4/freq from IC
    // datasheet
    final float scanFilterFreq =
        Math.max(
    ...
    // scanDelayFilter won't be negative unless scanFilterFreq is more than 3kHz which shouldn't
    // happen
    ...
    s.laserDelay =
        globalExposureDelay_max; // turn on laser as soon as camera's global exposure is reached
    ...
    // special adjustment for Photometrics cameras that possibly has extra clear time which is
    // counted in reset time
    //    but not in the camera exposure time
    final float actualCameraResetTime =
        (camLibrary == Devices.Libraries.PVCAM
                && props_
                    .getPropValueString(camKey, Properties.Keys.PVCAM_CHIPNAME)
    ...
    final float cameraExposure =
        MyNumberUtils.ceilToQuarterMs(actualCameraResetTime) + laserDuration;
    ...
    switch (acqSettings.cameraMode) {
      ...
      case PSEUDO_OVERLAP: // PCO or Photometrics, enforce 0.25ms between end exposure and start of
        ...
        if (camLibrary == Devices.Libraries.PCOCAM) {
          s.cameraExposure =
              getSliceDuration(s) - s.cameraDelay; // s.cameraDelay should be 0.25ms for PCO
        ...
        if (s.cameraDelay < 0.24f) {
          MyDialogUtils.showError(
              "Camera delay should be at least 0.25ms for pseudo-overlap mode.");
        }
        break;
      case LIGHT_SHEET:
        ...
        // 4. scan for total of exposure time plus readout time (total time some row is exposing)
        // plus settle time plus extra 0.25ms to prevent artifacts
        ...
        final float totalExposure_max =
            MyNumberUtils.ceilToQuarterMs(
                cameraReadoutTime + s.cameraExposure + 0.05f); // 50-300us extra cushion time
        final float scanSettle =
            props_.getPropValueFloat(Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_LS_SCAN_SETTLE);
        final float scanReset =
            props_.getPropValueFloat(Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_LS_SCAN_RESET);
    ...
    if (!acqSettings.minimizeSlicePeriod) {
      float globalDelay =
          acqSettings.desiredSlicePeriod
              - getSliceDuration(s); // both should be in 0.25ms increments // TODO fix
    ...
    //      // Add 0.25ms to globalDelay if it is 0 and we are on overlap mode and scan has been
    // shifted forward
    //      // basically the last 0.25ms of scan time that would have determined the slice period
    // isn't
    //      //   there any more because the scan time is moved up  => add in the 0.25ms at the start
    // of the slice
    //      // in edge or level trigger mode the camera trig falling edge marks the end of the slice
    // period
    //      // not sure if PCO pseudo-overlap needs this, probably not because adding 0.25ms
    ...
    float globalDelay =
        MyNumberUtils.ceilToQuarterMs((s.cameraExposure + cameraReadoutTime) - getSliceDuration(s));
    if (globalDelay > 0) {
      s.scanDelay += globalDelay;
      s.cameraDelay += globalDelay;
      s.laserDelay += globalDelay;
  ...
  /**
   * Re-calculate the controller's timing settings for "easy timing" mode. Changes panel variable
   * sliceTiming_. The controller's properties will be set as needed
   *
   * @param showWarnings will show warning if the user-specified slice period too short or if
   *     cameras aren't assigned
   */
  ...
  private void updateActualSlicePeriodLabel() {
    ...
    actualSlicePeriodLabel_.setText(
        NumberUtils.doubleToDisplayString(sliceTiming_.sliceDuration) + " ms");
  }

  /**
   * calculate the total ramp time for stage scan in units of milliseconds (includes both
   * acceleration and settling time given by "delay before side" setting)
   *
   * @param acqSettings
   * @return
   */
  private double getStageRampDuration(AcquisitionSettings acqSettings) {
    final double accelerationX =
        controller_.computeScanAcceleration(controller_.computeScanSpeed(acqSettings))
            + 1; // extra 1 for rounding up that often happens in controller
    ReportingUtils.logDebugMessage(
        "stage ramp duration is "
            + (acqSettings.delayBeforeSide + accelerationX)
            + " milliseconds");
  ...
  private double getStageRetraceDuration(AcquisitionSettings acqSettings) {
    ...
    ReportingUtils.logDebugMessage(
        "stage retrace duration is " + retraceDuration + " milliseconds");
    return retraceDuration;
  }

  /**
   * Compute the volume duration in ms based on controller's timing settings. Includes time for
   * multiple channels. However, does not include for multiple positions.
   *
  ...
  /**
   * Compute the timepoint duration in ms. Only difference from computeActualVolumeDuration() is
   * that it also takes into account the multiple positions, if any.
   *
   * @return duration in ms
   */
  ...
  private void updateActualVolumeDurationLabel() {
    ...
    if (duration > 1000) {
      actualVolumeDurationLabel_.setText(
          NumberUtils.doubleToDisplayString(duration / 1000d) + " s"); // round to ms
    } else {
      actualVolumeDurationLabel_.setText(
  ...
  private double computeActualTimeLapseDuration() {
    double duration =
        (getNumTimepoints() - 1) * getTimepointInterval() + computeTimepointDuration() / 1000;
    return duration;
  }
  ...
  /**
   * Computes the per-row readout time of the SPIM cameras set on Devices panel. Handles single-side
   * operation. Needed for computing camera exposure in light sheet mode
   *
   * @return
   */
  private double getRowReadoutTime() {
    if (getNumSides() > 1) {
      return Math.max(
          cameras_.getRowReadoutTime(Devices.Keys.CAMERAA),
  ...
  /**
   * Computes the reset time of the SPIM cameras set on Devices panel. Handles single-side
   * operation. Needed for computing (semi-)optimized slice timing in "easy timing" mode.
   *
   * @return
   */
  private float computeCameraResetTime() {
    ...
    if (getNumSides() > 1) {
      return Math.max(
          cameras_.computeCameraResetTime(Devices.Keys.CAMERAA, camMode),
  ...
  /**
   * Computes the readout time of the SPIM cameras set on Devices panel. Handles single-side
   * operation. Needed for computing (semi-)optimized slice timing in "easy timing" mode.
   *
   * @return
   */
  private float computeCameraReadoutTime() {
    ...
    if (getNumSides() > 1) {
      return Math.max(
          cameras_.computeCameraReadoutTime(Devices.Keys.CAMERAA, camMode),
  ...
  /**
   * Makes sure that cameras are assigned to the desired sides and display error message if not
   * (e.g. if single-sided with side B first, then only checks camera for side B)
   *
  ...
  private boolean checkCamerasAssigned(boolean showWarnings) {
    ...
    if (firstCamera == null) {
      if (showWarnings) {
        MyDialogUtils.showError(
            "Please select a valid camera for the first side (Imaging Path "
                + (firstSideA ? "A" : "B")
    ...
    if (getNumSides() > 1 && secondCamera == null) {
      if (showWarnings) {
        MyDialogUtils.showError(
            "Please select a valid camera for the second side (Imaging Path "
                + (firstSideA ? "B" : "A")
                + ") on the Devices Panel.");
      }
      return false;
  ...
  /**
   * runs a test acquisition with the following features: - not saved to disk - window can be closed
   * without prompting to save - timepoints disabled - autofocus disabled
   *
   * @param side Devices.Sides.NONE to run as specified in acquisition tab, Devices.Side.A or B to
   *     run only that side
   */
  public void runTestAcquisition(Devices.Sides side) {
    ReportingUtils.logDebugMessage(
        "User requested start of test diSPIM acquisition with side "
            + side.toString()
            + " selected.");
    try {
      ...
      if (spimMode == AcquisitionModes.Keys.STAGE_SCAN
        ...
        if (prefs_.getBoolean(
            MyStrings.PanelNames.DATAANALYSIS.toString(),
            Properties.Keys.PLUGIN_DESKEW_AUTO_TEST,
            false)) {
  ...
  /**
   * Implementation of acquisition that orchestrates image acquisition itself rather than using the
   * acquisition engine.
   *
   * <p>This methods is public so that the ScriptInterface can call it Please do not access this
   * yourself directly, instead use the API, e.g. import org.micromanager.asidispim.api.*;
   * ASIdiSPIMInterface diSPIM = new ASIdiSPIMImplementation(); diSPIM.runAcquisition();
   */
   ...
   * buffer, starting the cameras, grabbing the images and putting them into the acquisition, etc.
   *
  ...
  private boolean runAcquisitionPrivate(boolean testAcq, Devices.Sides testAcqSide)
    ...
    if (!sliceTiming_.valid) {
      MyDialogUtils.showError(
          "Error in calculating the slice timing; is the camera mode set correctly?");
      return false;
    }
    ...
    if (testAcq) {
      ...
      // work around limitation of not being able to use PLogic per-volume switching with single
      // side
      // => do per-volume switching instead (only difference should be extra time to switch)
      if (acqSettingsOrig.useChannels
          && acqSettingsOrig.channelMode == MultichannelModes.Keys.VOLUME_HW
    ...
    if (acqSettingsOrig.useMultiPositions) {
      if (acqSettingsOrig.hardwareTimepoints
        ...
        MyDialogUtils.showError(
            "Timepoint interval may not be sufficient "
    ...
    if (camActiveA) {
      ...
      if (!CameraModes.getValidModeKeys(devices_.getMMDeviceLibrary(Devices.Keys.CAMERAA))
          .contains(getSPIMCameraMode())) {
        MyDialogUtils.showError(
            "Camera trigger mode set to "
                + getSPIMCameraMode().toString()
    ...
    boolean usingDemoCam =
        (devices_.getMMDeviceLibrary(Devices.Keys.CAMERAA).equals(Devices.Libraries.DEMOCAM)
                && camActiveA)
            || (devices_.getMMDeviceLibrary(Devices.Keys.CAMERAB).equals(Devices.Libraries.DEMOCAM)
                && camActiveB);

    // set up channels
    int nrChannelsSoftware =
        acqSettings.numChannels; // how many times we trigger the controller per stack
    ...
    if (acqSettings.useChannels) {
      ...
      switch (acqSettings.channelMode) {
        ...
        case VOLUME_HW:
        case SLICE_HW:
          if (acqSettings.numChannels
              == 1) { // only 1 channel selected so don't have to really use hardware switching
            multiChannelPanel_.initializeChannelCycle();
        ...
        default:
          MyDialogUtils.showError(
              "Unsupported multichannel mode \"" + acqSettings.channelMode.toString() + "\"");
          return false;
      }
    }
    ...
    if (acqSettings.hardwareTimepoints) {
      ...
      if (acqSettings.cameraMode == CameraModes.Keys.OVERLAP) {
        ...
        // This holds for all multi-channel modes, just the order in which the extra trigger comes
        // varies
        // Very last trigger won't ever return a frame so subtract 1.
        nrSlicesSoftware =
            ((acqSettings.numSlices + 1) * acqSettings.numChannels * acqSettings.numTimepoints);
        if (twoSided && acqBothCameras) {
          nrSlicesSoftware *= 2;
        }
        nrSlicesSoftware -= 1;
    ...
    String dataPathName =
        gui_.data().getUniqueSaveDirectory(rootDir + File.separator + prefixField_.getText());

    if (acqSettings.separateTimepoints) {
      // because separate timepoints closes windows when done, force the user to save data to disk
      // to avoid confusion
      if (!save) {
        MyDialogUtils.showError(
            "For separate timepoints, \"Save while acquiring\" must be enabled.");
        return false;
      }
      // for separate timepoints, make sure the directory is empty to make sure naming pattern is
      // "clean"
    ...
    if (acqSettings.hardwareTimepoints) {
      if (acqSettings.useChannels && acqSettings.channelMode == MultichannelModes.Keys.VOLUME_HW) {
        ...
        // TODO: this seems a severe limitation, maybe this could be changed in the future via
        // firmware change
        MyDialogUtils.showError(
            "Cannot use hardware time points (small time point interval)"
      ...
      if (spimMode == AcquisitionModes.Keys.NO_SCAN) {
        MyDialogUtils.showError(
            "Cannot do hardware timepoints when no scan mode is used."
    ...
    if (acqSettings.useChannels
        && acqSettings.channelMode == MultichannelModes.Keys.VOLUME_HW
        && acqSettings.numSides < 2) {
      MyDialogUtils.showError(
          "Cannot do PLogic channel switching of volume when only one"
    ...
    if (!acqSettings.useMultiPositions && acqSettings.numTimepoints > 1) {
      if (timepointIntervalMs < volumeDuration) {
        MyDialogUtils.showError(
            "Time point interval shorter than" + " the time to collect a single volume.\n");
        return false;
      }
    }
    ...
    if (acqSettings.useAutofocus) {
      autofocusAtT0 =
          prefs_.getBoolean(
              ...
              Properties.Keys.PLUGIN_AUTOFOCUS_ACQBEFORESTART,
              false);
      autofocusEachNFrames =
          props_.getPropValueInteger(
              Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_AUTOFOCUS_EACHNIMAGES);
      autofocusChannel =
          props_.getPropValueString(Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_AUTOFOCUS_CHANNEL);
      // double-check that selected channel is valid if we are doing multi-channel
      if (acqSettings.useChannels) {
        String channelGroup =
            props_.getPropValueString(
                Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_MULTICHANNEL_GROUP);
    ...
    if (acqSettings.useMovementCorrection) {
      correctMovementEachNFrames =
          props_.getPropValueInteger(
              Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_AUTOFOCUS_CORRECTMOVEMENT_EACHNIMAGES);
      correctMovementChannel =
          props_.getPropValueString(
    ...
    if (acqSettings.isStageScanning) {
      try {
        ...
        origXSpeed =
            props_.getPropValueFloat(Devices.Keys.XYSTAGE, Properties.Keys.STAGESCAN_MOTOR_SPEED);
        origXAccel =
            props_.getPropValueFloat(Devices.Keys.XYSTAGE, Properties.Keys.STAGESCAN_MOTOR_ACCEL);
      } catch (Exception ex) {
        MyDialogUtils.showError(
      ...
      // if X speed is less than 0.2 mm/s then it probably wasn't restored to correct speed some
      // other time
      // we offer to set it to a more normal speed in that case, until the user declines and we stop
      // asking
      if (origXSpeed < 0.2 && resetXaxisSpeed_) {
        ...
        // once the user selects "no" then resetXaxisSpeed_ will be false and stay false until
        // plugin is launched again
        if (resetXaxisSpeed_) {
    ...
    for (int acqNum = 0; !cancelAcquisition_.get() && acqNum < nrRepeats_; acqNum++) {
      ...
      if (acqSettings.isStageScanning) {
        ...
        //   by getting the volume duration and subtracting the acquisition duration and then
        // dividing by two
        extraStageScanTimeout =
            (long)
                    Math.ceil(
                        computeActualVolumeDuration(acqSettings)
                            - (acqSettings.numSlices
                                * acqSettings.numChannels
                                * acqSettings.sliceTiming.sliceDuration))
                / 2;
      }
      ...
      try {
        if (save) {
          store =
              gui_.data()
                  .createMultipageTIFFDatastore(dataPathName, false, usePositionsCB_.isSelected());
        } else {
        ...
        ReportingUtils.logMessage(
            "diSPIM plugin starting acquisition "
                + acqName
                + " with following settings: "
                + acqSettingsJSON);

        final int numMMChannels =
            acqSettings.numSides * acqSettings.numChannels * (acqBothCameras ? 2 : 1);
        ...
        String name =
            prefs_.getString(panelName_, Properties.Keys.PLUGIN_NAME_PREFIX, "diSPIM Data");
        if (acqNum > 0) {
          name += "_" + acqNum;
        }
        SummaryMetadata.Builder smb = gui_.data().summaryMetadataBuilder();
        smb =
            smb.channelNames(channelNames_)
        ...
        pmb.putDouble("LaserExposure_ms", (double) PanelUtils.getSpinnerFloatValue(durationLaser_));
        pmb.putString("VolumeDuration", actualVolumeDurationLabel_.getText());
        pmb.putString("SPIMmode", spimMode.toString());
        // Multi-page TIFF saving code wants this one:
        // TODO: support other types than GRAY16  (NS: Why?? Cameras are all 16-bits, so not much
        ...
        for (int trigNum = 0; trigNum < nrFrames; trigNum++) {
          ...
          if (acqSettings.useAutofocus) {
            ...
            if ((autofocusAtT0 && timePoint == 0)
              ...
              if (sideActiveA) {
                AutofocusUtils.FocusResult score =
                    autofocus_.runFocus(this, Devices.Sides.A, false, sliceTiming_, false);
                updateCalibrationOffset(Devices.Sides.A, score);
              }
              if (sideActiveB) {
                AutofocusUtils.FocusResult score =
                    autofocus_.runFocus(this, Devices.Sides.B, false, sliceTiming_, false);
          ...
          for (int positionNum = 0; positionNum < nrPositions; positionNum++) {
            if (acqSettings.useMultiPositions) {
              ...
              if (acqSettings.isStageScanning) {
                scanXSpeed =
                    props_.getPropValueFloat(
                        Devices.Keys.XYSTAGE, Properties.Keys.STAGESCAN_MOTOR_SPEED);
                props_.setPropValue(
                    Devices.Keys.XYSTAGE, Properties.Keys.STAGESCAN_MOTOR_SPEED, origXSpeed);
                scanXAccel =
                    props_.getPropValueFloat(
                        Devices.Keys.XYSTAGE, Properties.Keys.STAGESCAN_MOTOR_ACCEL);
                props_.setPropValue(
                    Devices.Keys.XYSTAGE, Properties.Keys.STAGESCAN_MOTOR_ACCEL, origXAccel);
              }
              ...
              if (acqSettings.isStageScanning) {
                props_.setPropValue(
                    Devices.Keys.XYSTAGE, Properties.Keys.STAGESCAN_MOTOR_SPEED, scanXSpeed);
                props_.setPropValue(
                    Devices.Keys.XYSTAGE, Properties.Keys.STAGESCAN_MOTOR_ACCEL, scanXAccel);
              }
              ...
              if (acqSettings.useMultiPositions) {
                StagePosition pos =
                    positionList
                        ...
                        .get(devices_.getMMDevice(Devices.Keys.XYSTAGE));
            ...
            for (int channelNum = 0; channelNum < nrChannelsSoftware; channelNum++) {
              try {
                ...
                boolean success =
                    controller_.triggerControllerStartAcquisition(spimMode, firstSideA);
                if (!success) {
                ...
                ReportingUtils.logDebugMessage(
                    "Starting time point "
                        ...
                        + " of "
                        + nrFrames
                        + " with (software) channel number "
                        + channelNum);

                // Wait for first image to create ImageWindow, so that we can be sure about image
                // size
                // Do not actually grab first image here, just make sure it is there
                long start = System.currentTimeMillis();
                long now = start;
                final long timeout =
                    Math.max(3000, Math.round(10 * sliceDuration + 2 * acqSettings.delayBeforeSide))
                        + extraStageScanTimeout
                        + extraMultiXYTimeout;
                ...
                if (now - start >= timeout) {
                  ...
                  if (acqSettings.isStageScanning) {
                    msg +=
                        "Make sure jumpers are correct on XY card and also micro-micromirror card.";
                  } else {
                ...
                if (acqSettings
                  ...
                  if (acqSettings.spimMode == AcquisitionModes.Keys.STAGE_SCAN_UNIDIRECTIONAL) {
                    timeout2 +=
                        (long)
                            Math.ceil(
                                getStageRetraceDuration(
                                    acqSettings)); // in unidirectional case also need to rewind
                  }
                }
                ...
                try {
                  while ((core_.getRemainingImageCount() > 0
                    ...
                    if (core_.getRemainingImageCount() > 0) { // we have an image to grab
                      ...
                      //    and adjacent pairs will be same color (e.g. 0 and 1 will be from first
                      // color, 2 and 3 from second, etc.) String camera = (String)
                      // timg.tags.get("Camera");
                      ...
                      if (acqSettings.hardwareTimepoints
                        ...
                        if (checkForSkips) {
                          // one extra image per MM channel, this includes case of only 1 color
                          // (either multi-channel disabled or else only 1 channel selected)
                          // if we are interleaving by slice then next nrChannel images will be from
                          // extra slice position
                      ...
                      if (now - last >= timeout2) {
                        ReportingUtils.logError(
                            "Camera did not send all expected images within"
            ...
            if (acqSettings.useMovementCorrection
              ...
              if (movementDetectors[positionNum] == null) {
                ...
                movementDetectors[positionNum] =
                    new MovementDetector(prefs_, cmChannelNumber, positionNum, rotation);
              }

              Vector3D movement =
                  movementDetectors[positionNum].detectMovement(Method.PhaseCorrelation);
              ...
              if (!movement.equals(zeroPoint)) {
                String msg =
                    "ASIdiSPIM motion corrector moving stages: X: "
                        + movement.getX()
                        + ", Y: "
                        + movement.getY()
                        + ", Z: "
                ...
                if (acqSettings.useMultiPositions) {
                  ...
                  core_.setRelativeXYPosition(
                      devices_.getMMDevice(Devices.Keys.XYSTAGE), movement.getX(), movement.getY());
                  core_.setRelativePosition(
                      devices_.getMMDevice(Devices.Keys.UPPERZDRIVE), movement.getZ());
                }
              }
            }
          }
        ...
        try {
          ...
          ReportingUtils.logMessage(
              "diSPIM plugin acquisition "
                  + " took: "
                  + (System.currentTimeMillis() - acqButtonStart)
                  + "ms");
          ...
          if (lastAcquisitionPath_ != null
              && prefs_.getBoolean(
                  MyStrings.PanelNames.SETTINGS.toString(),
                  Properties.Keys.PLUGIN_WRITE_ACQ_SETTINGS_FILE,
                  false)) {
            String path = "";
            try {
              ...
              MyDialogUtils.showError(
                  ex, "Could not save acquisition settings to file as requested to path " + path);
            }
          }
    ...
    controller_.cleanUpControllerAfterAcquisition(
        acqSettings.numSides, acqSettings.firstSideIsA, true);
    ...
    if (acqSettings.isStageScanning) {
      try {
        ...
        props_.setPropValue(
            Devices.Keys.XYSTAGE, Properties.Keys.STAGESCAN_STATE, Properties.Values.SPIM_IDLE);
        props_.setPropValue(
            Devices.Keys.XYSTAGE, Properties.Keys.STAGESCAN_MOTOR_SPEED, origXSpeed);
        props_.setPropValue(
            Devices.Keys.XYSTAGE, Properties.Keys.STAGESCAN_MOTOR_ACCEL, origXAccel);
    ...
    if (testAcq
        && prefs_.getBoolean(
            MyStrings.PanelNames.SETTINGS.toString(), Properties.Keys.PLUGIN_TESTACQ_SAVE, false)) {
      String path = "";
      try {
        path =
            prefs_.getString(
                MyStrings.PanelNames.SETTINGS.toString(), Properties.Keys.PLUGIN_TESTACQ_PATH, "");
        throw new ASIdiSPIMException("Not sure what is going on here");
        // IJ.saveAs(gui_.getAcquisition(acqName).getAcquisitionWindow().getImagePlus(), "raw",
        // path);
  ...
  public void saveSettings() {
    ...
    props_.setPropValue(
        Devices.Keys.PIEZOA, Properties.Keys.SAVE_CARD_SETTINGS, Properties.Values.DO_SSZ, true);
    props_.setPropValue(
        Devices.Keys.PIEZOB, Properties.Keys.SAVE_CARD_SETTINGS, Properties.Values.DO_SSZ, true);
    props_.setPropValue(
        Devices.Keys.GALVOA, Properties.Keys.SAVE_CARD_SETTINGS, Properties.Values.DO_SSZ, true);
    props_.setPropValue(
        Devices.Keys.GALVOB, Properties.Keys.SAVE_CARD_SETTINGS, Properties.Values.DO_SSZ, true);
    props_.setPropValue(
        Devices.Keys.PLOGIC, Properties.Keys.SAVE_CARD_SETTINGS, Properties.Values.DO_SSZ, true);
  }
  ...
  public void cameraModeChange() {
    ...
    slicePanelContainer_.add(
        (key == CameraModes.Keys.LIGHT_SHEET) ? lightSheetPanel_ : normalPanel_, "growx");
  ...
  private void setRootDirectory(JTextField rootField) {
    File result =
        FileDialogs.openDir(
            null, "Please choose a directory root for image data", FileDialogs.MM_DATA_SET);
    if (result != null) {
  ...
  /**
   * The basic method for adding images to an existing data set. If the acquisition was not
   * previously initialized, it will attempt to initialize it from the available image data. This
   * version uses a blocking queue and is much faster than the one currently implemented in the
   * ScriptInterface Eventually, this function should be replaced by the ScriptInterface version of
   * the same.
   *
   ...
   */
  private void addImageToAcquisition(
      Datastore store,
      int frame,
      int channel,
      int slice,
      ...
      throws JSONException, DatastoreFrozenException, DatastoreRewriteException, Exception {
    ...
    ud =
        ud.copyBuilder()
            .putDouble("Z-Step-um", (double) PanelUtils.getSpinnerFloatValue(stepSize_))
            .build();
  ...
  /** *************** API ****************** */
  /**
   * @return true if an acquisition is currently underway (e.g. all checks passed, controller set
   *     up, MM acquisition object created, etc.)
   */
  ...
  /**
   * @return true if an acquisition has been requested by user. Will also return true if acquisition
   *     is running.
   */
  ...
  /**
   * Stops the acquisition by setting an Atomic boolean indicating that we should halt. Does nothing
   * if an acquisition isn't running.
   */
  ...
  /**
   * @return pathname on filesystem to last completed acquisition (even if it was stopped
   *     pre-maturely). Null if not saved to disk.
   */
