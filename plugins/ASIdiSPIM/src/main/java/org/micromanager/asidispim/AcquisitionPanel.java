...
public class AcquisitionPanel extends ListeningJPanel implements DevicesListenerInterface {
    ...
    /**
     * Sets the acquisition name prefix programmatically. Added so that name
     * prefix can be changed from a script.
     *
     * @param acqName
     */
    ...
    /**
     * @return CameraModes.Keys value from Camea panel (edge, overlap,
    ...
    /**
     * gets the correct value for the slice timing's sliceDuration field based
     * on other values of slice timing
     *
     * @param s
     * @return
     */
    ...
    /**
     * gets the slice timing from advanced settings (normally these advanced
     * settings are read-only and we populate them ourselves depending on the
     * user's requests and our algorithm below)
     *
     * @return
     */
    ...
    /**
     * @param showWarnings true to warn user about needing to change slice
     * period
     * @return
     */
...
//      }
     ...
     * Changes panel variable sliceTiming_. The controller's properties will be
     * set as needed
     *
     * @param showWarnings will show warning if the user-specified slice period
     * too short or if cameras aren't assigned
     */
    ...
    /**
     * calculate the total ramp time for stage scan in units of milliseconds
     * (includes both acceleration and settling time given by "delay before
     * side" setting)
     *
     * @param acqSettings
     * @return
    ...
    /**
     * calculate the retrace time in stage scan raster mode in units of
     * milliseconds
     *
     * @param acqSettings
     * @return
     ...
     * Includes time for multiple channels. However, does not include for
     * multiple positions.
     *
     * @param acqSettings Struct that contains all settings relevant for the
     * acquisition
    ...
    /**
     * Compute the timepoint duration in ms. Only difference from
     * computeActualVolumeDuration() is that it also takes into account the
     * multiple positions, if any.
     *
     * @return duration in ms
     */
    ...
    /**
     * Computes the per-row readout time of the SPIM cameras set on Devices
     * panel. Handles single-side operation. Needed for computing camera
     * exposure in light sheet mode
     *
     * @return
     */
    ...
    /**
     * Computes the reset time of the SPIM cameras set on Devices panel. Handles
     * single-side operation. Needed for computing (semi-)optimized slice timing
     * in "easy timing" mode.
     *
     * @return
     */
    private float computeCameraResetTime() {
        ...
        if (getNumSides() > 1) {
            return Math.max(cameras_.computeCameraResetTime(Devices.Keys.CAMERAA,
                    camMode),
     ...
     * Handles single-side operation. Needed for computing (semi-)optimized
     * slice timing in "easy timing" mode.
     *
     * @return
     */
    ...
    /**
     * Makes sure that cameras are assigned to the desired sides and display
     * error message if not (e.g. if single-sided with side B first, then only
     * checks camera for side B)
     *
    ...
    /**
     * runs a test acquisition with the following features: - not saved to disk
     * - window can be closed without prompting to save - timepoints disabled -
     * autofocus disabled
     *
    ...
    /**
     * Implementation of acquisition that orchestrates image acquisition itself
     * rather than using the acquisition engine.
     *
     * This methods is public so that the ScriptInterface can call it Please do
     * not access this yourself directly, instead use the API, e.g. import
     * org.micromanager.asidispim.api.*; ASIdiSPIMInterface diSPIM = new
     * ASIdiSPIMImplementation(); diSPIM.runAcquisition();
     */
    ...
    /**
     * Actually runs the acquisition; does the dirty work of setting up the
     * controller, the circular buffer, starting the cameras, grabbing the
     * images and putting them into the acquisition, etc.
     *
     * @param testAcq true if running test acquisition only (see
     * runTestAcquisition() javadoc)
     * @param testAcqSide only applies to test acquisition, passthrough from
     * runTestAcquisition()
    ...
    private boolean runAcquisitionPrivate(boolean testAcq, Devices.Sides testAcqSide)
        ...
        if (acqSettings.hardwareTimepoints) {
            ...
            if (spimMode == AcquisitionModes.Keys.NO_SCAN) {
                MyDialogUtils.showError("Cannot do hardware timepoints when no scan mode is used."
                                        ...
                                        if (core_.getRemainingImageCount() > 0) {  // we have an image to grab
                                            ...
                                            switch (acqSettings.channelMode) {
                                                ...
                                                case VOLUME:
                                                    ...
                                                    break;
                                                case VOLUME_HW:
                                                    channelIndex_tmp = cameraFrNumber[cameraIndex]
     ...
     * it from the available image data. This version uses a blocking queue and
     * is much faster than the one currently implemented in the ScriptInterface
     * Eventually, this function should be replaced by the ScriptInterface
     * version of the same.
     *
     ...
     */
    /**
     * @return true if an acquisition is currently underway (e.g. all checks
     * passed, controller set up, MM acquisition object created, etc.)
     */
    ...
    /**
     * @return true if an acquisition has been requested by user. Will also
    ...
    /**
     * Stops the acquisition by setting an Atomic boolean indicating that we
     * should halt. Does nothing if an acquisition isn't running.
     */
    ...
    /**
     * @return pathname on filesystem to last completed acquisition (even if it
     * was stopped pre-maturely). Null if not saved to disk.
     */
