...
public class PanelUtils {
  ...
  /**
   * makes JSlider for double values where the values are multiplied by a scale factor before
   * internal representation as integer (as JSlider requires)
   *
   * @param min
   * @param max
   * @param scalefactor
  ...
  public JSlider makeSlider(
      double min, double max, final int scalefactor, Devices.Keys devKey, Properties.Keys propKey) {

    class sliderListener
        implements ChangeListener, UpdateFromPropertyListenerInterface, DevicesListenerInterface {
      ...
      public sliderListener(
          JSlider js, int scalefactor, Devices.Keys devKey, Properties.Keys propKey) {
        js_ = js;
      ...
      public void stateChanged(ChangeEvent ce) {
        if (!((JSlider) ce.getSource()).getValueIsAdjusting()) { // only change when user releases
          props_.setPropValue(
              devKey_, propKey_, (float) js_.getValue() / (float) scalefactor_, true);
        }
      }
    ...
    JSlider js =
        new JSlider(
            intmin, intmax, intmin); // initialize with min value, will set to current value shortly
    ChangeListener l = new sliderListener(js, scalefactor, devKey, propKey);
    ((UpdateFromPropertyListenerInterface) l)
        .updateFromProperty(); // set to value of property at present
  ...
  /**
   * Creates spinner for integers in the GUI. Implements UpdateFromPropertyListenerInterface,
   * causing updates in the model that were generated by changes in the device to be propagated back
   * to the UI.
   *
   * @param min - minimum value for the spinner
   * @param max - maximum value for the spinner
   * @param devKeys - array of device keys, use inline constructor "new Devices.Keys[]{list of
   *     devices}"
   * @param propKey - property key for this spinner
   * @param defaultVal - initial value
   * @return the created JSpinner
   */
  public JSpinner makeSpinnerInteger(
      int min, int max, Devices.Keys[] devKeys, Properties.Keys propKey, int defaultVal) {

    class SpinnerListenerInt
        implements ChangeListener, UpdateFromPropertyListenerInterface, DevicesListenerInterface {
      private final JSpinner sp_;
      private final Devices.Keys[] devKeys_;
      private final Properties.Keys propKey_;

      public SpinnerListenerInt(JSpinner sp, Devices.Keys[] devKeys, Properties.Keys propKey) {
        sp_ = sp;
        devKeys_ = devKeys;
        propKey_ = propKey;
      }
   ...
   */
  public JSpinner makeSpinnerInteger(
      int min, int max, Devices.Keys devKey, Properties.Keys propKey, int defaultVal) {
  ...
  /**
   * Creates spinner for floats in the GUI. Implements UpdateFromPropertyListenerInterface, causing
   * updates in the model that were generated by changes in the device to be propagated back to the
   * UI.
   *
   * @param min - minimum value for the spinner
   * @param max - maximum value for the spinner
   ...
   * @return the created JSpinner
   */
  public JSpinner makeSpinnerFloat(
      double min,
      double max,
      double step,
      Devices.Keys[] devKeys,
    ...
    class SpinnerListenerFloat
        implements ChangeListener, UpdateFromPropertyListenerInterface, DevicesListenerInterface {
      private final JSpinner sp_;
      private final Devices.Keys[] devKeys_;
      private final Properties.Keys propKey_;

      public SpinnerListenerFloat(JSpinner sp, Devices.Keys[] devKeys, Properties.Keys propKey) {
        sp_ = sp;
        devKeys_ = devKeys;
        propKey_ = propKey;
      }
      ...
      public void stateChanged(ChangeEvent ce) {
        ...
        for (Devices.Keys devKey : devKeys_) {
          ...
          if (!MyNumberUtils.floatsEqual(
              props_.getPropValueFloat(devKey, propKey_), spinnerValue)) {
            props_.setPropValue(devKey, propKey_, spinnerValue, true);
            // ignore error for sake of missing device assignment
          }
        }
  ...
  /**
   * Constructs a DropDown box selecting between multiple strings. Sets selection based on property
   * value and attaches a Listener.
   *
   ...
   */
  public JComboBox makeDropDownBox(
      String[] vals, Devices.Keys[] devKeys, Properties.Keys propKey, String defaultVal) {
    /**
     * Listener for the string-based dropdown boxes Updates the model in the params class with any
     * GUI changes
     */
    class StringBoxListener
        implements ActionListener, UpdateFromPropertyListenerInterface, DevicesListenerInterface {
   ...
   */
  public JComboBox makeDropDownBox(
      String[] vals, Devices.Keys devKey, Properties.Keys propKey, String defaultVal) {
    return makeDropDownBox(vals, new Devices.Keys[] {devKey}, propKey, defaultVal);
  }

  /**
   * Constructs JCheckBox with boolean preference store. The boolean value is read using
   * prefs_.getBoolean(), NOT props_.get... Stored in the prefNode indicated, usually the panel
   * name.
   *
   * @param label the GUI label
   * @param propKey (needed for read/write to preferences now that preferences is based on
   *     properties)
   * @param prefNode
   * @param defaultValue
  ...
  public JCheckBox makeCheckBox(
      String label, Properties.Keys propKey, String prefNode, boolean defaultValue) {
    ...
    class checkBoxListener implements ItemListener {
      ...
      public checkBoxListener(JCheckBox jc, Properties.Keys propKey, String prefNode) {
        jc_ = jc;
        propKey_ = propKey;
        prefNode_ = prefNode;
      }
    ...
    final JCheckBox jc = new JCheckBox(label, prefs_.getBoolean(prefNode, propKey, defaultValue));
    jc.addItemListener(new checkBoxListener(jc, propKey, prefNode));
    return jc;
  }

  /**
   * Constructs JCheckBox tied to "usual" property and also stored in preferences. Uses a separate
   * "prefKey" so that the same property name with different devices can be distinguished. Specify
   * property values for checked/unchecked state.
   *
   * @param label the GUI label
  ...
  public JCheckBox makeCheckBox(
      String label,
      Properties.Values offValue,
      Properties.Values onValue,
      Devices.Keys devKey,
      Properties.Keys propKey,
      String prefNode,
      Prefs.Keys prefKey) {
    ...
    class checkBoxListener
        implements ItemListener, UpdateFromPropertyListenerInterface, DevicesListenerInterface {
      ...
      // only called when the user selects/deselects from GUI or the value _changes_
      // programmatically
      @Override
      public void itemStateChanged(ItemEvent e) {
    ...
    JCheckBox jc = new JCheckBox(label, prefs_.getBoolean(prefNode, prefKey, false));
    checkBoxListener l =
        new checkBoxListener(jc, offValue, onValue, devKey, propKey, prefNode, prefKey);
   ...
   *
   * @param prefNode - String identifying preference node where this variable be store such that its
   *     value can be retrieved on restart
   * @param prefKey - String used to identify this preference
   * @param defaultValue - initial (default) value. Will be overwritten by value in Preferences
   * @param numColumns - width of the GUI element
   * @return - JFormattedTextField element
   */
  public JFormattedTextField makeFloatEntryField(
      String prefNode, String prefKey, double defaultValue, int numColumns) {
   ...
   * @return - JFormattedTextField element
   */
  public JFormattedTextField makeIntEntryField(
      String prefNode, String prefKey, int defaultValue, int numColumns) {
   ...
   * @return
   */
  public JFormattedTextField makeSetPositionField(
      Devices.Keys key, Joystick.Directions dir, Positions positions) {
    ...
    class setPositionListener2 implements ActionListener {
      ...
      setPositionListener2(
          Devices.Keys key, Joystick.Directions dir, Positions positions, JFormattedTextField tf) {
        key_ = key;
        dir_ = dir;
        positions_ = positions;
        tf_ = tf;
  ...
  /**
   * takes a JComboBox and adds a listener that is guaranteed to be called after the other
   * listeners. Modifies the JComboBox!!
   *
  ...
  /**
   * takes a JSlider and adds a listener that is guaranteed to be called after the other listeners.
  ...
  /**
   * creates change listener for float-based spinner that will coerce the value to quarter integers
   * (e.g. 1.00, 1.25, 1.50, 1.75, 2.00, etc.)
   *
   * @param sp
   * @return
   */
   ...
   *
   * @param title
   * @return public static TitledBorder makeTitledBorder(String title) { TitledBorder myBorder =
   *     BorderFactory.createTitledBorder( BorderFactory.createLineBorder(ASIdiSPIM.borderColor), "
   *     " + title + " "); myBorder.setTitleJustification(TitledBorder.CENTER); return myBorder; }
   */
  ...
  public static Border makeTitledBorder(String title, JComponent comp) {
    ...
    return new ComponentTitledBorder(label, comp, BorderFactory.createEtchedBorder());
  }
