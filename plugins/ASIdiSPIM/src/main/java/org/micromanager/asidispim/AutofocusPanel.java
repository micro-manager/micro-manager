...
public class AutofocusPanel extends ListeningJPanel {
  ...
  public AutofocusPanel(
    ...
    super(
        ...
        new MigLayout("", "[center]8[center]8[center]", "[]16[]16[]"));
    gui_ = gui;
    prefs_ = prefs;
    props_ = props;
    devices_ = devices;
    ...
    // start options panel
    final JPanel optionsPanel = new JPanel(new MigLayout("", "[right]16[left]", "[]8[]"));

    // show images checkbox
    final JCheckBox showImagesCheckBox =
        pu.makeCheckBox(
            "Show images", Properties.Keys.PLUGIN_AUTOFOCUS_SHOWIMAGES, panelName_, true);
    optionsPanel.add(showImagesCheckBox);

    optionsPanel.setBorder(PanelUtils.makeTitledBorder("Autofocus Options", optionsPanel));

    // show plot checkbox
    final JCheckBox showPlotCheckBox =
        pu.makeCheckBox("Show plot", Properties.Keys.PLUGIN_AUTOFOCUS_SHOWPLOT, panelName_, true);
    optionsPanel.add(showPlotCheckBox, "wrap");

    // spinner with number of images:
    optionsPanel.add(new JLabel("Number of images:"));
    final JSpinner nrImagesSpinner =
        pu.makeSpinnerInteger(
            1, 1000, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_AUTOFOCUS_NRIMAGES, 21);
    optionsPanel.add(nrImagesSpinner, "wrap");

    // spinner with stepsize:
    optionsPanel.add(new JLabel("Step size [\u00B5m]:"));
    final JSpinner stepSizeSpinner =
        pu.makeSpinnerFloat(
            0.001, 100., 1., Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_AUTOFOCUS_STEPSIZE, 0.5);
    ...
    final JComboBox scanModeCB =
        pu.makeDropDownBox(
            scanOptions,
            Devices.Keys.PLUGIN,
            Properties.Keys.AUTOFOCUS_ACQUSITION_MODE,
            scanOptions[0]);
    ...
    scoringAlgorithmCB.setSelectedItem(
        Fitter.getAlgorithmFromPrefCode(
            ...
            .toString());
    scoringAlgorithmCB.addActionListener(
        new ActionListener() {
          @Override
            ...
            try {
              ...
              if (afDevice == null) {
                throw new ASIdiSPIMException(
                    "Please define autofocus method in the Autofocus panel");
              }

              // select the appropriate algorithm
              afDevice.setPropertyValue(
                  "Maximize",
                  Fitter.getAlgorithmFromPrefCode(
                          prefs_.getInt(
                              MyStrings.PanelNames.AUTOFOCUS.toString(),
              ...
              MyDialogUtils.showError(
                  "Failed to select the Oughtafocus plugin.  Is it present in the mmautofocus directory?");
            }
          }
        });
    ...
    final JSpinner minimumR2Spinner =
        pu.makeSpinnerFloat(
            0.0, 1.0, 0.01, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_AUTOFOCUS_MINIMUMR2, 0.75);
    optionsPanel.add(minimumR2Spinner);

    // end options subpanel

    // start acquisition options panel
    final JPanel acqOptionsPanel =
        new JPanel(new MigLayout("", "[right]8[center]8[left]", "[]8[]"));
    acqOptionsPanel.setBorder(
        PanelUtils.makeTitledBorder("Autofocus Options during Acquisition", acqOptionsPanel));

    // whether or not to run autofocus at the start of the acquisition
    final JCheckBox beforeStartCheckBox =
        pu.makeCheckBox(
            "Autofocus before starting acquisition",
    ...
    eachNTimePointsSpinner_ =
        pu.makeSpinnerInteger(
            1, 1000, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_AUTOFOCUS_EACHNIMAGES, 10);
    ...
    String channelGroup_ =
        props_.getPropValueString(Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_MULTICHANNEL_GROUP);
    StrVector channels = gui.core().getAvailableConfigs(channelGroup_);
    final JComboBox channelSelect =
        pu.makeDropDownBox(
            channels.toArray(), Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_AUTOFOCUS_CHANNEL, "");
    ...
    final JSpinner maxOffsetChangeSpinner =
        pu.makeSpinnerFloat(
            0, 10, 1, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_AUTOFOCUS_MAXOFFSETCHANGE, 2);
    acqOptionsPanel.add(maxOffsetChangeSpinner);
    acqOptionsPanel.add(new JLabel("\u00B5m (\u00B1)"), "left, wrap");

    final JPanel setupOptionsPanel =
        new JPanel(new MigLayout("", "[right]8[center]8[left]", "[]8[]"));
    setupOptionsPanel.setBorder(
        PanelUtils.makeTitledBorder("Options During Setup", setupOptionsPanel));

    autoUpdateOffset_ =
        pu.makeCheckBox(
            "Automatically update offset if focus found",
    ...
    final JComponent[] autoUpdateComponents = {
      maxOffsetChangeSetupLabel1_, maxOffsetChangeSetupSpinner_, maxOffsetChangeSetupLabel2_
    };
    ...
    autoUpdateOffset_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            PanelUtils.componentsSetEnabled(autoUpdateComponents, autoUpdateOffset_.isSelected());
    ...
    final JPanel movementCorrectionsPanel =
        new JPanel(new MigLayout("", "[right]8[center]8[left]", "[]8[]"));
    movementCorrectionsPanel.setBorder(PanelUtils.makeTitledBorder("Movement correction options"));

    // autofocus every nth image
    movementCorrectionsPanel.add(new JLabel("Correct every "));
    correctMEachNTimePointsSpinner_ =
        pu.makeSpinnerInteger(
            1,
            1000,
            Devices.Keys.PLUGIN,
    ...
    // TODO: need to update combobox when the channel group changes
    final JComboBox channelSelectCM =
        pu.makeDropDownBox(
            channels.toArray(),
            Devices.Keys.PLUGIN,
    ...
    final JSpinner maxMovementChangeSpinner =
        pu.makeSpinnerFloat(
            1,
            100,
            1,
            Devices.Keys.PLUGIN,
            Properties.Keys.PLUGIN_AUTOFOCUS_CORRECTMOVEMENT_MAXCHANGE,
            25);
    ...
    final JSpinner minMovementChangeSpinner =
        pu.makeSpinnerFloat(
            0,
            10,
            0.5,
            Devices.Keys.PLUGIN,
            Properties.Keys.PLUGIN_AUTOFOCUS_CORRECTMOVEMENT_MINCHANGE,
            1);
