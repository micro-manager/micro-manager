 ...
 *
 * <p>To avoid depending on the internals restrict yourself to the ASIdiSPIMInterface; always cast
 * the instance the class to ASIdiSPIMInterface, e.g.:
 *
 * <p>import org.micromanager.asidispim.api.ASIdiSPIMInterface; import
 * org.micromanager.asidispim.api.ASIdiSPIMImplementation;
 *
 * <p>ASIdiSPIMInterface diSPIM = new ASIdiSPIMImplementation(); diSPIM.runAcquisition(); // or
 * other API methods in ASIdiSPIMInterface
 */
...
public interface ASIdiSPIMInterface extends Remote {
  ...
  /**
   * Requests an acquisition using the current settings, i.e., the settings as visible in the
   * acquisition panel. The definition of current settings may change in the future. Throws
   * exception if an acquisition is currently running or has been requested. Does not block.
   *
   ...
   * Requests an acquisition using the current settings, i.e., the settings as visible in the
   * acquisition panel. The definition of current settings may change in the future. Throws
   * exception if an acquisition is currently running or has been requested. Blocks until complete.
   *
  ...
  /**
   * Moves the stages to the requested position, then requests an acquisition using the current
   * settings, i.e., the settings as visible in the acquisition panel. The definition of current
   * settings may change in the future. Throws exception if an acquisition is currently running or
   * has been requested. Blocks until complete.
   *
  ...
  /**
   * Stops an acquisition, if one is currently running. Also cancels a requested acquisition that is
   * not yet running.
   *
  ...
  /**
   * @return true if acquisition is currently running. "Downtime" during a multi-timepoint
   *     acquisition is considered running. There is a delay between requesting an acquisition and
   *     actually starting to run it due to time to perform sanity checks, initialize the controller
   *     and cameras, etc.
  ...
  /**
   * @return true if acquisition has been requested or is currently running. There is a delay
   *     between requesting an acquisition and actually starting to run it due to time to perform
   *     sanity checks, initialize the controller and cameras, etc. It is possible for the
   *     acquisition to be stopped before being started during this interval when it has been
   *     requested but not actually started.
   ...
   *     pre-maturely). Will return an empty string if no acquisition has run, or null if the last
   *     acquisition was not saved to disk.
   */
  public String getLastAcquisitionPath() throws ASIdiSPIMException, RemoteException;

  /**
   * @return the internal Micro-Manager name for the last acquisition. Will start with the prefix
   *     specified previously, though it may have appended characters for uniqueness.
   */
  public String getLastAcquisitionName() throws ASIdiSPIMException, RemoteException;

  /**
   * Closes the window associated with the last acquisition. Equivalent to
  ...
  /**
   * Closes the acquisition window corresponding to the specified acquisition name. Note that the
   * acquisition name may be different from the prefix and also from the final field of the
   * filesystem's pathname if it was saved to disk.
   *
   * @param acquisitionName
   */
  public void closeAcquisitionWindow(String acquisitionName)
      throws ASIdiSPIMException, RemoteException;

  /**
   * @return directory is a string comprising the pathname on filesystem where the acquisition data
   *     is being saved
   */
  ...
  /**
   * @return the name of the acquisition, or more precisely the "prefix" which Micro-Manager uses
   *     (it may append numbers to avoid duplicate names)
   */
  public String getSavingNamePrefix() throws ASIdiSPIMException, RemoteException;

  /**
   * Changes the name of the next acquisition. This is the "prefix" in Micro-Manager's terminology.
   * Numbers may be appended to avoid duplicate filenames.
   *
   * @param acqPrefix
   */
  ...
  /** @return true if each timepoint gets its own file, false if all timepoints are in one file */
  public boolean getSavingSeparateFile() throws ASIdiSPIMException, RemoteException;

  /**
   * @param separate is a boolean, true means checkbox is checked so that each timepoint gets its
   *     own file of images.
   */
  ...
  /**
   * @param save, true means checkbox is checked so that data is saved to disk as it is generated
   */
  ...
  /**
   * @param mode determines whether the piezo/slice are moved synchronously, only one is moved,
   *     stage scanning is used, etc.
   */
  ...
  /** @param enabled, true means checkbox is checked so that multiple time points are collected */
  public void setTimepointsEnabled(boolean enabled) throws ASIdiSPIMException, RemoteException;

  /**
   * @return number of time points set. Will return GUI selection even if time points have been
   *     disabled.
   */
  public int getTimepointsNumber() throws ASIdiSPIMException, RemoteException;

  /**
   * @param numTimepoints sets the number of time points. Works even if time point have been
   *     disabled, though in that case the number is meaningless. If numTimepoints is not between 1
   *     and 100000 then ASIdiSPIMException is thrown.
   */
  ...
  /**
   * @param intervalTimepoints sets the interval between time points in seconds. If
   *     intervalTimepoints is not between 0.1 and 32000 then ASIdiSPIMException is thrown.
   */
  ...
  /**
   * @return delay additional time in milliseconds after move completes before imaging begins (e.g.
   *     to let system settle mechanically)
   */
  public double getMultiplePositionsDelay() throws ASIdiSPIMException, RemoteException;

  /**
   * @param delayMs sets the additional delay in milliseconds after each move completes before
   *     imaging begins (e.g. to let system settle mechanically). If delayMs is not between 0 and
   *     10000 then ASIdiSPIMException is thrown.
   */
  public void setMultiplePositionsDelay(double delayMs) throws ASIdiSPIMException, RemoteException;

  /**
   * Convenience method to get the stage position list (which belongs to the main MM application).
   * Its API is at https://valelab.ucsf.edu/~MM/doc/mmstudio/org/micromanager/api/PositionList.html.
   * You can do operations such as adding and removing positions, moving to a specified position in
   * the list, etc.
   */
   ...
   * @see ASIdiSPIMInterface#getChannelEnabled(String) to see if an individual channel is enabled or
   *     not
   */
  public boolean getChannelsEnabled() throws ASIdiSPIMException, RemoteException;

  /**
   * @param enabled, true means checkbox is checked so that channel selections are used (including
   *     multiple channels per acquisition)
   */
  public void setChannelsEnabled(boolean enabled) throws ASIdiSPIMException, RemoteException;

  /**
   * @return list of all valid channel groups (depends on main MM configuration). Somewhat different
   *     from the core's method getAllowedPropertyValues() of the special property "ChannelGroup" of
   *     device "Core".
   */
  public String[] getAvailableChannelGroups() throws ASIdiSPIMException, RemoteException;

  /** @return selected channel group from which presets will be selected for channel table */
  public String getChannelGroup() throws ASIdiSPIMException, RemoteException;

  /**
   * @param channelGroup, string to set the current channel group. If it is not a valid channel,
   *     i.e. not in the list returned by getAvailableChannelGroups(), then an ASIdiSPIMException
   *     will be thrown.
   */
  ...
  /**
   * @param channel
   * @return true if the specified channel is present in the table and selected to be used If it is
   *     not a valid channel, i.e. not in the list returned by getAvailableChannels(), then an
   *     ASIdiSPIMException will be thrown. If the channels operation has been disabled (i.e. if
   *     getChannelsEnabled()==false) then this will always return false. If the channel is valid
   *     but not present in the table then it will return false.
  ...
  /**
   * Use to enable/disable channels. If the specified channel is not yet present in the table then
   * it will be added if it is a valid channel. If it is not a valid channel, i.e. not in the list
   * returned by getAvailableChannels(), then an ASIdiSPIMException will be thrown.
   *
   * @param channel
   * @param enabled
   */
  ...
  /**
   * @return the channel change mode, VOLUME for (software) volume-by-volume, VOLUME_HW for
   *     PLogic-based volume-by-volume, and SLICE_HW for PLogic-based slice-by-slice
  ...
  /**
   * @return either Devices.Sides.A or Devices.Sides.B depending on which side is selected to go
   *     first
   */
  public Devices.Sides getVolumeFirstSide() throws ASIdiSPIMException, RemoteException;

  /**
   * Sets the first side to be acquired (or only side if only 1 sided acquisition is done).
   *
   * @param side Should be Devices.Sides.A or Devices.Sides.B. If anything else is passed an
   *     ASIdiSPIMException is thrown.
   */
  ...
  /**
   * @param delayMs the delay in milliseconds before each side. Usually used to allow for mechanical
   *     settling, at least 50 ms. If delayMs is not between 0 and 10000 then ASIdiSPIMException is
   *     thrown.
   * @throws ASIdiSPIMException
   * @throws RemoteException
   */
  ...
  /**
   * @return number of slices of acquired data per volume. Note that in overlap/sychronous camera
   *     mode more triggers are sent.
   */
  public int getVolumeSlicesPerVolume() throws ASIdiSPIMException, RemoteException;

  /**
   * @param slices number of slices to be acquired per side If slices is not between 0 and 65000
   *     then ASIdiSPIMException is thrown.
   */
  ...
  /**
   * @param stepSizeUm step size in microns between successive slices. If stepSizeUm is not between
   *     0 and 100 then ASIdiSPIMException is thrown.
   */
  ...
  /**
   * @return requested duration of each period in milliseconds. Note this only applies when
   *     "minimize slice period" is unchecked/set to false.
   */
  public double getVolumeSlicePeriod() throws ASIdiSPIMException, RemoteException;

  /**
   * @param periodMs requested duration of each period, only used if the "Minimize slice period" is
   *     unchecked. If periodMs is not between 1 and 1000 then ASIdiSPIMException is thrown.
   */
  public void setVolumeSlicePeriod(double periodMs) throws ASIdiSPIMException, RemoteException;

  /**
   * @return exposure time in milliseconds that the laser will be on to expose the sample. The galvo
   *     sweep time is usually 0.5 ms longer.
   */
  public double getVolumeSampleExposure() throws ASIdiSPIMException, RemoteException;

  /**
   * @param exposureMs is the exposure time of the sample to the laser in milliseconds (related but
   *     not the same as the camera's exposure time). Will be rounded to the nearest multiple of
   *     0.25 ms. If exposureMs is not between 1.0 and 1000 then ASIdiSPIMException is thrown.
   */
  ...
  /**
   * @param enable true to enable autofocus during acquisition. Parameters for that are set on the
   *     autofocus tab or using this API.
   */
  ...
  /**
   * @param side Devices.Sides.A or Devices.Sides.B
   * @param center the imaging piezo's center position on specified side If center position is
   *     outside the range of the piezo then ASIdiSPIMException is thrown.
   */
  public void setSideImagingCenter(Devices.Sides side, double center)
      throws ASIdiSPIMException, RemoteException;
   ...
   * @deprecated out of laziness, can add if needed
   */
  public void setSideSlicePosition(Devices.Sides side, double position)
      throws ASIdiSPIMException, RemoteException;
   ...
   * @deprecated out of laziness, can add if needed
   */
  public double getSideImagingPiezoPosition(Devices.Sides side)
      throws ASIdiSPIMException, RemoteException;
   ...
   * @deprecated out of laziness, can add if needed
   */
  public void setSideImagingPiezoPosition(Devices.Sides side, double position)
      throws ASIdiSPIMException, RemoteException;
   ...
   * @deprecated out of laziness, can add if needed
   */
  public double getSideIlluminationPiezoPosition(Devices.Sides side)
      throws ASIdiSPIMException, RemoteException;
   ...
   * @deprecated out of laziness, can add if needed
   */
  public void setSideIlluminationPiezoPosition(Devices.Sides side, double position)
      throws ASIdiSPIMException, RemoteException;
   ...
   * @deprecated out of laziness, can add if needed
   */
  public void setSideIlluminationPiezoHome(Devices.Sides side)
      throws ASIdiSPIMException, RemoteException;
   ...
   * @deprecated out of laziness, can add if needed
   */
  public void setSideSheetWidth(Devices.Sides side, double width)
      throws ASIdiSPIMException, RemoteException;
   ...
   * @deprecated out of laziness, can add if needed
   */
  public void setSideSheetOffset(Devices.Sides side, double offset)
      throws ASIdiSPIMException, RemoteException;
   ...
   * @deprecated out of laziness, can add if needed
   */
  public double getSideCalibrationSlope(Devices.Sides side)
      throws ASIdiSPIMException, RemoteException;
   ...
   * @deprecated out of laziness, can add if needed
   */
  public void setSideCalibrationSlope(Devices.Sides side, double slope)
      throws ASIdiSPIMException, RemoteException;
   ...
   */
  public double getSideCalibrationOffset(Devices.Sides side)
      throws ASIdiSPIMException, RemoteException;
   ...
   */
  public void setSideCalibrationOffset(Devices.Sides side, double offset)
      throws ASIdiSPIMException, RemoteException;

  /**
   * Updates the offset with the current positions of the slice and imaging piezo, just like
   * clicking the GUI button does
   *
   * @param side Devices.Sides.A or Devices.Sides.B
   */
  public void updateSideCalibrationOffset(Devices.Sides side)
      throws ASIdiSPIMException, RemoteException;

  /**
   * Runs the autofocus just like a GUI button press. If the autofocus is successful then slice or
   * piezo will be adjusted to the best-focus position but the offset isn't updated automatically.
   *
  ...
  /**
   * @return mode used by autofocus, either FIX_PIEZO for fixing the piezo and sweeping the slice or
   *     FIX_SLICE for fixing the slice and sweeping the piezo
   * @deprecated out of laziness, can add if needed
   */
  public org.micromanager.asidispim.AutofocusPanel.Modes getAutofocusMode()
      throws ASIdiSPIMException, RemoteException;

  /**
   * @param mode either FIX_PIEZO or FIX_SLICE depending on whether piezo is fixed and slice is
   *     swept or vice versa.
   * @deprecated out of laziness, can add if needed
   */
  public void setAutofocusMode(org.micromanager.asidispim.AutofocusPanel.Modes mode)
      throws ASIdiSPIMException, RemoteException;

  /**
   * @return true if autofocus will be performed before starting the acquisition. Only applies if
   *     "autofocus during acquisition" has been enabled
   * @deprecated out of laziness, can add if needed
   */
  public boolean getAutofocusBeforeAcquisition() throws ASIdiSPIMException, RemoteException;

  /**
   * @param enable true will run autofocus before the acquisition begins. Only applies if "autofocus
   *     during acquisition" has been enabled
   * @deprecated out of laziness, can add if needed
   */
  ...
  public void setAutofocusTimepointInterval(int numTimepoints)
      throws ASIdiSPIMException, RemoteException;
  ...
  /**
   * Convenience method to set the position of the XY stage. Blocks until move complete.
   *
   * @param x in microns
   * @param y in microns
   */
  ...
  /** Raises the SPIM head to the position on the Navigation panel indicated by "load sample". */
  ...
  /** Lowers the SPIM head to the position on the Navigation panel indicated by "start hunting". */
  ...
  /**
   * Stops all motion by sending a halt signal to the Tiger controller, like the button on the
   * navigation panel
   */
  public void haltAllMotion() throws ASIdiSPIMException, RemoteException;

  /**
   * @return object with all acquisition settings. Intended for informational purposes only. Contact
   *     the developers if you need to use a modified copy of this object to change the acquisition
   *     settings. The fields of the AcquisitionSettings object may change in the future.
   */
  public org.micromanager.asidispim.data.AcquisitionSettings getAcquisitionSettings()
      throws ASIdiSPIMException, RemoteException;
