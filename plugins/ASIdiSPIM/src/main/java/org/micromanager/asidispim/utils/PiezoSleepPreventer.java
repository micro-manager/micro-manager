...
public class PiezoSleepPreventer {
  ...
  /**
   * Utility class to periodically "move" piezos to prevent them from sleeping. (move is of distance
   * 0 so nothing happens in practice)
   *
   * <p>The timer is intended to be started when live mode is enabled and stopped when live mode is
   * disabled.
   *
   * @param gui - MM Studio
   * @param devices
   * @param props
   */
  public PiezoSleepPreventer(Studio gui, Devices devices, Properties props) {
  ...
  /**
   * Start the timer on own thread via java.util.Timer.scheduleAtFixedRate(). Call stop() to stop.
   */
  public void start() {
