...
public class StagePositionUpdater {
   ...
   *
   * <p>The timer will be constructed when the start function is called. Panels to be informed of
   * updated stage positions should be added using the addPanel function.
   *
   * @param positions
   * @param props
   */
        ...
        (int)
            (1000
                * props_.getPropValueFloat(
                    Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_POSITION_REFRESH_INTERVAL));
    if (interval == 0) {
      interval = 1000;
    }
    return interval;
  ...
  /**
   * Add panel to list of ListeningJPanels that get notified whenever we have refreshed positions.
   *
   * @param panel
   */
  ...
  /**
   * Start the updater at whatever interval is. Uses its own thread via
   * java.util.Timer.scheduleAtFixedRate(). Call stop() to stop.
   */
  public void start() {
  ...
  /**
   * Call this with true to temporarily turn off updates. Be sure to call it again with false. Keeps
   * an internal counter so that the calling code does not need to know whether other code also
   * called the pauseUpdate function. If an update cycle is underway when called with true the
   * update cycle will finish.
   *
   * @param pause true disables updates temporarily, and increases the pause counter. false
   *     decreases the pause counter. When the counter returns to 0, unpauses the updater
   */
