...
// TODO calibration for sheet width/offset (automatic based on image analysis?) and then optimize
// based on ROI
// TODO recalculate slice timing automatically changing assigned camera
// TODO add status bar to bottom of window (would include acquisition status, could show other
// messages too)
...
// TODO camera control tab: set ROI, set separate acquisition/alignment exposure times and sweep
// rate, etc.
// TODO method to move sample diagonally (move X and F at same time) for sample finding
// TODO factor out common code for JComboBoxes like MulticolorModes, CameraModes, AcquisitionModes,
// etc.
...
// TODO separate properties into true hardware-related properties and plugin settings (maybe store
// plugin settings directly in prefs?)
...
// TODO save/load plugin settings from file instead of from registry (nice to also include
// controller settings)
// TODO improve efficiency of camera code by pre-calculating key factors and updating when needed
...
// TODO execute autofocus during acquisition before the desired time point is reached instead of
// waiting until a timepoint should be collected
...
// TODO Make it easier to adjust stack center (or start and end) including autodetect start/end
// based on content
// TODO allow different stack center, number of slices, offset, or other settings for each XY
// position
...
public class ASIdiSPIMFrame extends SPIMFrame {
  ...
  private final AtomicBoolean hardwareInUse_ =
      new AtomicBoolean(false); // true if acquisition or autofocus running
  ...
  public ASIdiSPIMFrame(Studio gui) throws ASIdiSPIMException {
    ...
    glassPane.setLayout(
        new MigLayout("", "[" + super.getWidth() + "]", "[" + super.getHeight() + "]"));
  ...
  /**
   * Do not get into the internals of this plugin without relying on ASIdiSPIM.api
   *
   * @return
   */
  ...
  /**
   * For use of acquisition panel code (getting joystick settings) Do not get into the internals of
   * this plugin without relying on ASIdiSPIM.api
   *
  ...
  /**
   * For use of data analysis panel code (running deskew) Do not get into the internals of this
   * plugin without relying on ASIdiSPIM.api
   *
  ...
  /**
   * For use of acquisition panel code (getting camera settings) Do not get into the internals of
   * this plugin without relying on ASIdiSPIM.api
   *
  ...
  /**
   * For use by the acquisition panel code (to update offset setting) Do not get into the internals
   * of this plugin without relying on ASIdiSPIM.api
   *
  ...
  public SetupPanel getSetupPanel(Devices.Sides side) {
    if (side.equals(Devices.Sides.A)) return setupPanelA_;
    else if (side.equals(Devices.Sides.B)) return setupPanelB_;
