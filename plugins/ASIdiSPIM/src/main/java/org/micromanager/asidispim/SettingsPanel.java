...
public class SettingsPanel extends ListeningJPanel {
  ...
  public SettingsPanel(
    ...
    super(
        ...
        new MigLayout("", "[right]16[center]16[center]", "[]16[]"));
    ...
    final JPanel guiPanel = new JPanel(new MigLayout("", "[right]16[center]", "[]8[]"));
    guiPanel.setBorder(PanelUtils.makeTitledBorder("GUI", guiPanel));

    final JCheckBox activeTimerCheckBox =
        pu.makeCheckBox(
            "Update axis positions continually",
            Properties.Keys.PREFS_ENABLE_POSITION_UPDATES,
            panelName_,
            true);
    ActionListener ae =
        new ActionListener() {
    ...
    final JSpinner positionRefreshInterval =
        pu.makeSpinnerFloat(
            0.5,
            1000,
            0.5,
    ...
    final JCheckBox ignoreScannerMissing =
        pu.makeCheckBox(
            "Ignore missing scanner (for debug)",
    ...
    final JPanel scannerPanel = new JPanel(new MigLayout("", "[right]16[center]", "[]8[]"));
    ...
    liveScanMs_ =
        pu.makeSpinnerInteger(
            1, 10000, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_CAMERA_LIVE_SCAN, 10);
    liveScanMs_.addChangeListener(
        new ChangeListener() {
          @Override
    ...
    props_.setPropValue(
        new Devices.Keys[] {Devices.Keys.GALVOA, Devices.Keys.GALVOB},
        Properties.Keys.SA_PERIOD_X,
        (Integer) liveScanMs_.getValue(),
        true);
    scannerPanel.add(liveScanMs_, "wrap");

    // end scanner panel
    // start acquisition panel
    final JPanel acquisitionPanel = new JPanel(new MigLayout("", "[left]", "[]8[]"));
    acquisitionPanel.setBorder(PanelUtils.makeTitledBorder("Acquisition"));

    final JCheckBox acqSettingsWrite =
        pu.makeCheckBox(
            "Write file with acquisition settings",
            Properties.Keys.PLUGIN_WRITE_ACQ_SETTINGS_FILE,
            panelName_,
            false);
    acquisitionPanel.add(acqSettingsWrite, "wrap");

    final JCheckBox acqBothCamerasSimult =
        pu.makeCheckBox(
            "Acquire from both cameras simultaneously",
    ...
    final JPanel testAcqPanel = new JPanel(new MigLayout("", "[right]16[center]", "[]8[]"));
    testAcqPanel.setBorder(PanelUtils.makeTitledBorder("Test Acquisition", testAcqPanel));

    final JCheckBox testAcqSave =
        pu.makeCheckBox(
            "Save test acquisition as raw data",
            Properties.Keys.PLUGIN_TESTACQ_SAVE,
            panelName_,
            false);
    ...
    rawPath_.setText(prefs_.getString(panelName_, Properties.Keys.PLUGIN_TESTACQ_PATH, ""));
    rawPath_.addPropertyChangeListener(
        new PropertyChangeListener() {
          // will respond to commitEdit() as well as GUI edit on commit
          @Override
          public void propertyChange(PropertyChangeEvent evt) {
            prefs_.putString(panelName_, Properties.Keys.PLUGIN_TESTACQ_PATH, rawPath_.getText());
          }
        });
    ...
    browseFileButton.addActionListener(
        new ActionListener() {
          ...
          public void actionPerformed(final ActionEvent e) {
            File result =
                FileDialogs.openFile(null, "Please choose a file for raw image data", null);
            // Studio.MM_DATA_SET);
            if (result != null) {
    ...
    final JPanel stageScanPanel = new JPanel(new MigLayout("", "[right]16[center]", "[]8[]"));

    stageScanPanel.setBorder(PanelUtils.makeTitledBorder("Stage scanning", stageScanPanel));
    ...
    if (devices_.isTigerDevice(Devices.Keys.XYSTAGE)
      ...
      final JSpinner stageAccelFactor =
          pu.makeSpinnerFloat(
              0.1, 10, 1, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_STAGESCAN_ACCEL_FACTOR, 1);
      stageScanPanel.add(stageAccelFactor, "wrap");
      if (props_.hasProperty(
          Devices.Keys.XYSTAGE,
          Properties.Keys.STAGESCAN_OVERSHOOT_DIST)) { // present in 3.17 and above
        stageScanPanel.add(new JLabel("Scan overshoot distance [" + "\u00B5" + "m]:"));
        final JSpinner scanOvershootDistance =
            pu.makeSpinnerInteger(
                0, 1000, Devices.Keys.XYSTAGE, Properties.Keys.STAGESCAN_OVERSHOOT_DIST, 0);
        stageScanPanel.add(scanOvershootDistance, "wrap");
      }
    } else {
      stageScanPanel.add(
          new JLabel("Stage scanning not supported by your firmware."), "left, wrap");
      stageScanPanel.add(
          new JLabel("See http://dispim.org for further information."), "left, wrap");
    }
    ...
    final JPanel imageJPanel = new JPanel(new MigLayout("", "[right]16[center]", "[]8[]"));
    ...
    //      useToolset.setToolTipText("places icons in ImageJ toolbar for quick access of
    // commonly-used image manipulation tasks");
