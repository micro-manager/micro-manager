...
/**
 * Class that holds information about the selected devices. This should always be the only source of
 * device information. The GUI should update this class and use its information.
 *
 * <p>This could be implemented more elegantly using templates
 *
 * @author nico
 * @author Jon
 */
public class Devices {
  ...
  // TODO would like to be able to use static methods on devices (e.g. to see whether a camera
  // supports a particular triggering mode)
  ...
  /**
   * A, B, or N for not associated with either side specifically. Now sides are labeled as "paths"
   * in GUI to avoid confusion, but code still has "sides" nomenclature
   */
  ...
  /**
   * Enum containing Device keys, representing role of each device (each key is mapped to a
   * particular Micro-manager device according to the ComboBoxes in DevicesPanel).
   */
  ...
  public static final Set<Devices.Keys> STAGES1D =
      EnumSet.of(
          Devices.Keys.LOWERZDRIVE,
          Devices.Keys.UPPERZDRIVE,
          Devices.Keys.UPPERHDRIVE,
          Devices.Keys.PIEZOA,
          Devices.Keys.PIEZOB);
  public static final Set<Devices.Keys> PIEZOS =
      EnumSet.of(Devices.Keys.PIEZOA, Devices.Keys.PIEZOB);
  public static final Set<Devices.Keys> STAGES2D =
      EnumSet.of(Devices.Keys.XYSTAGE, Devices.Keys.GALVOA, Devices.Keys.GALVOB);
  public static final Set<Devices.Keys> GALVOS =
      EnumSet.of(Devices.Keys.GALVOA, Devices.Keys.GALVOB);
  public static final Set<Devices.Keys> CAMERAS =
      EnumSet.of(
          Devices.Keys.CAMERAA,
          Devices.Keys.CAMERAB,
          Devices.Keys.MULTICAMERA,
          Devices.Keys.CAMERALOWER);
  public static final Set<Devices.Keys> SPIM_CAMERAS =
      ASIdiSPIM.OSPIM
          ? EnumSet.of(Devices.Keys.CAMERAA)
  ...
  public static class DeviceData {
     ...
     */
    public DeviceData(Keys key, String displayName, Sides side, boolean saveInPref) {
      this.key = key;
      this.mmDevice = "";
  ...
  /**
   * Takes a key and side specifier and returns the corresponding side-specific key or else NONE if
   * it cannot be found or doesn't exist. For example, PIEZOA with side B will return PIEZOB
   *
   * @param genericKey
   * @param side
   * @return
   */
  public static Devices.Keys getSideSpecificKey(Devices.Keys genericKey, Devices.Sides side) {
    switch (genericKey) {
      case PIEZOA:
      case PIEZOB:
        switch (side) {
  ...
  /**
   * Takes a side specifier and returns the opposite side (A-&gt;B and B-&gt;A). NONE gets mapped to
   * NONE.
   *
   * @param side
   * @return
  ...
  /**
   * Associates a Micro-manager device name with the Device key. Changes deviceInfo_.
   *
      ...
      if (d.deviceLibrary == Devices.Libraries.ASITIGER) {
        try {
          ...
          if (core_.hasProperty(mmDevice, "AxisLetterX")
            ...
            d.axisLetter =
                core_.getProperty(mmDevice, "AxisLetterX")
  ...
  /**
   * Looks up the Micro-Manager device names currently set for an array of Device keys.
   *
  ...
  /**
   * returns device description with role for the particular side (used for joystick labels) camera
   * labels use a different mechanicsm, ideally would merge the two
   *
   * @param key
   * @param dir
   * @param side
   * @return
   */
  public String getDeviceDisplayWithRole(
      Devices.Keys key, Joystick.Directions dir, Devices.Sides side) {
    String ret;
    DeviceData d = deviceInfo_.get(key);
    switch (key) {
      case GALVOA:
  ...
  /**
   * Returns display string with axis letter included. For 2D device both axes are included.
   *
   * @param key
   * @return
   */
  ...
  /**
   * Returns display string with axis letter included, but only specified axis for 2D device.
   *
   * @param key
   * @param dir
   * @return
   */
  public String getDeviceDisplayWithAxisLetter1D(Devices.Keys key, Joystick.Directions dir) {
    String ret = getDeviceDisplay(key);
    ...
    switch (dir) {
      ...
      case X:
        ret =
            ret + ", " + Joystick.Directions.X.toString() + " axis (" + axisLetter.charAt(0) + ")";
        break;
      case Y:
        ret =
            ret + ", " + Joystick.Directions.Y.toString() + " axis (" + axisLetter.charAt(1) + ")";
        break;
      case NONE:
      default:
        return getDeviceDisplayWithAxisLetter(key);
  ...
  public void saveSettings() {
    for (Devices.Keys key : deviceInfo_.keySet()) {
      ...
      // TODO make it so empty devices don't get saved, instead add "None" to the list which is
      // saved
  ...
  /**
   * Reads mapping between Device keys and Micro-manager device names from preferences. Assumes
   * loadedDevices_ contains all available devices, and sets any no-longer-existing device to empty
   * string. Changes deviceInfo_.
   *
  ...
  /**
   * Gets the initial/default DeviceData structure for the specified device key. Should include all
   * possible devices.
   *
   * @param key
   * @return
  ...
  final DeviceData getDefaultDeviceData(Devices.Keys key) {
    ...
    switch (key) {
      ...
      case CAMERALOWER:
        return new DeviceData(key, "Lower Camera", Sides.NONE, true);
      case MULTICAMERA:
        return new DeviceData(key, "Multi Camera", Sides.NONE, true);
      case PIEZOA:
  ...
  public Devices(Studio gui, Prefs prefs) throws ASIdiSPIMException {
    ...
    deviceInfo_ =
        Collections.synchronizedMap(new EnumMap<Devices.Keys, DeviceData>(Devices.Keys.class));
  ...
  /**
   * Enable or disable listeners. When switching from disabled to enabled it will perform
   * callListeners()
   *
   * @param enabled
   */
