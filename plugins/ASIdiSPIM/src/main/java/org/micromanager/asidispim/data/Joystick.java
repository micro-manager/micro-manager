...
// TODO implement fast axis reverse checkbox (may need device adapter change and possibly even
...
public class Joystick {
  ...
  private static final Map<Joystick.Keys, String> VALUES = new EnumMap<Keys, String>(Keys.class);
  ...
  /**
   * associative class to store information about axis for joystick combo boxes. Contains string
   * shown in combo box, key of corresponding device, and which direction is to be selected in the
   * case of 1D control of 2D device
   */
  public static class JSAxisData {
     ...
     * @param direction enum for which axis of multiaxis device, or NONE if single axis or not
     *     applicable
     */
    ...
    public int hashCode() {
      return (this.displayString.hashCode()
          ...
          + this.direction.toString().hashCode());
    }
  ...
  /**
   * used to generate selection list for joypod wheel combo list order: none, devices in
   * JOYSTICK1DSET, then individual axes of devices in JOYSTICK2DSET
   *
   * @param side A, B, or none
   * @return array with representative strings of 1 axis stages plus individual axes of 2-axis
   *     stages/galvos
   */
  public JSAxisData[] getWheelJSAxisData(Devices.Sides side) {
    ...
    list.add(
        new JSAxisData(
            devices_.getDeviceDisplayVerbose(Devices.Keys.NONE),
            Devices.Keys.NONE,
            Directions.NONE)); // adds "None" to top of list
  ...
  /**
   * Disables all joysticks by going through all devices and disabling joysticks on them. Loops over
   * all devices that could be associated with joysticks and sets appropriate property.
   */
    ...
    try {
      for (Devices.Keys dev : Devices.STAGES1D) {
        if (devices_.isTigerDevice(dev)) {
          props_.setPropValue(
              dev, Properties.Keys.JOYSTICK_INPUT, VALUES.get(Joystick.Keys.NONE), true);
        }
      }
      if (devices_.isTigerDevice(Devices.Keys.XYSTAGE)) {
        props_.setPropValue(
            Devices.Keys.XYSTAGE, Properties.Keys.JOYSTICK_ENABLED, Properties.Values.NO, true);
      }
      for (Devices.Keys dev : Devices.GALVOS) {
        // must be Tiger devices
        props_.setPropValue(
            dev, Properties.Keys.JOYSTICK_INPUT_X, VALUES.get(Joystick.Keys.NONE), true);
        props_.setPropValue(
            dev, Properties.Keys.JOYSTICK_INPUT_Y, VALUES.get(Joystick.Keys.NONE), true);
      }
    } catch (Exception ex) {
  ...
  /**
   * If any device is attached to the specified joystick then it clears that association. Loops over
   * all devices that could be associated with joysticks and sets appropriate property.
   *
   * @param jkey
   */
  public void unsetAllDevicesFromJoystick(Joystick.Keys jkey) {
    try {
      for (Devices.Keys dev : Devices.STAGES1D) {
        if (devices_.isTigerDevice(dev)) {
          if (props_
              .getPropValueString(dev, Properties.Keys.JOYSTICK_INPUT)
              .equals(VALUES.get(jkey))) {
            props_.setPropValue(
                dev, Properties.Keys.JOYSTICK_INPUT, VALUES.get(Joystick.Keys.NONE));
          }
        }
      }
      ...
      for (Devices.Keys dev : Devices.GALVOS) {
        // must be Tiger devices
        if (props_
            .getPropValueString(dev, Properties.Keys.JOYSTICK_INPUT_X)
            .equals(VALUES.get(jkey))) {
          props_.setPropValue(
              dev, Properties.Keys.JOYSTICK_INPUT_X, VALUES.get(Joystick.Keys.NONE));
        }
        if (props_
            .getPropValueString(dev, Properties.Keys.JOYSTICK_INPUT_Y)
            .equals(VALUES.get(jkey))) {
          props_.setPropValue(
              dev, Properties.Keys.JOYSTICK_INPUT_Y, VALUES.get(Joystick.Keys.NONE));
        }
      }
    } catch (Exception ex) {
  ...
  /**
   * De-selects the joystick input according to the specified device and JSAxisData. If this code is
   * modified then probably also need to modify setJoystick too.
   *
   * @param jkey
   * @param JSAxisData
   */
    ...
    try {
      if (jkey == Joystick.Keys.JOYSTICK) { // setting the joystick, not a wheel
        ...
        switch (JSAxisData.direction) {
          case X:
            prop = Properties.Keys.JOYSTICK_INPUT_X;
            break;
          case Y:
            prop = Properties.Keys.JOYSTICK_INPUT_Y;
            break;
          case NONE:
            prop = Properties.Keys.JOYSTICK_INPUT;
            break;
          default:
            prop = Properties.Keys.JOYSTICK_INPUT;
            break;
      ...
      MyDialogUtils.showError(
          "Problem unsetting joysticks for " + jkey.toString() + " in device " + dev.toString());
    }
  }

  /**
   * Selects the joystick input according to the specified device and JSAxisData. If this code is
   * modified then probably also need to modify unsetJoystick too.
   *
   * @param jkey
   * @param JSAxisData
   */
    ...
    try {
      if (jkey == Joystick.Keys.JOYSTICK) { // setting the joystick, not a wheel
        if (dev == Devices.Keys.XYSTAGE) {
          ...
          props_.setPropValue(
              dev, Properties.Keys.JOYSTICK_INPUT_X, VALUES.get(Joystick.Keys.JOYSTICK_X));
          props_.setPropValue(
              dev, Properties.Keys.JOYSTICK_INPUT_Y, VALUES.get(Joystick.Keys.JOYSTICK_Y));
        }
      } else { // must be either right or left wheel
        Properties.Keys prop;
        switch (JSAxisData.direction) {
      ...
      MyDialogUtils.showError(
          "Problem setting joysticks for " + jkey.toString() + " in device " + dev.toString());
    }
  }
}
