...
public final class CameraSubPanel extends ListeningJPanel {
  ...
  public CameraSubPanel(
      ...
      Prefs prefs) {
    super(
        MyStrings.PanelNames.CAMERA_SUBPANEL.toString() + instanceLabel,
        new MigLayout("", "[right]8[center]", "[]8[]"));
    ...
    JPanel camButtonPanel =
        new JPanel(new MigLayout("", "0[center]4[center]4[center]4[center]0", "0[]0"));
    ...
    toggleButtonLive_.addActionListener(
        new ActionListener() {
          @Override
  ...
  public final void setLiveButtonAppearance(boolean enable) {
    ...
    ImageIcon goIcon =
        new ImageIcon(getClass().getResource("/org/micromanager/icons/camera_go.png"));
  ...
  private JComboBox makeCameraSelectionBox(int maxBoxWidth) {
    ...
    String selectedItem =
        prefs_.getString(instanceLabel_, Prefs.Keys.CAMERA, Devices.Keys.CAMERAPREVIOUS.toString());
  ...
  private class CameraSelectionBoxListener implements ActionListener, DevicesListenerInterface {
     ...
     * every time (easy but inefficient)
     *
     * @param ae
     */
    @Override
     ...
     * changed. Based on similar code for joystick combo box, which is more complex
     */
