...
public class MultiChannelSubPanel extends ListeningJPanel {
  ...
  class DisplayDisabledTableCellRenderer extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(
      ...
      return super.getTableCellRendererComponent(
          table, value, selected, focused, rowIndex, columnIndex);
    }
  }
  ;
   ...
   * @param prefs
   */
  public MultiChannelSubPanel(Studio gui, Devices devices, Properties props, Prefs prefs) {
    super(
        MyStrings.PanelNames.CHANNELS_SUBPANEL.toString(),
        new MigLayout("", "[right]10[left]", "[]8[]"));
    gui_ = gui;
    ...
    useChannelsCB_ =
        pu.makeCheckBox("Channels", Properties.Keys.PREFS_USE_MULTICHANNEL, panelName_, false);
    ...
    ComponentTitledBorder componentBorder =
        new ComponentTitledBorder(
            useChannelsCB_, this, BorderFactory.createLineBorder(ASIdiSPIM.BORDERCOLOR));
    ...
    channelGroup_ =
        pu.makeDropDownBox(
            groups, Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_MULTICHANNEL_GROUP, "");

    updateGroupsCombo();

    channelTableModel_ =
        new ChannelTableModel(
            gui_, prefs_, panelName_, (String) channelGroup_.getSelectedItem(), this);
    channelTable = new JTable(channelTableModel_);

    channelGroup_.addItemListener(
        new ItemListener() {
          @Override
    ...
    tablePanel.setLayout(new MigLayout("flowy, ins 0", "", ""));
    channelTable.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
    TableColumn column_useChannel =
        channelTable.getColumnModel().getColumn(ChannelTableModel.COLUMNINDEX_USECHANNEL);
    TableColumn column_config =
        channelTable.getColumnModel().getColumn(ChannelTableModel.COLUMNINDEX_CONFIG);
    ...
    MultichannelModes channelModes =
        new MultichannelModes(
            devices_,
            props_,
            Devices.Keys.PLUGIN,
    ...
    useChannelsCB_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
  ...
  /**
   * gets all valid groups from Core-ChannelGroup that have more than 1 preset ("config"). Different
   * from the MDA method of getting valid groups.
   *
  ...
  /** @return array of channels that are currently set be "used". Ordered same as in the GUI. */
  public ChannelSpec[] getUsedChannels() {
    return channelTableModel_.getUsedChannels();
  }

  /** @return MultichannelModes.Keys.NONE if channels are disabled, or actual selection otherwise */
  ...
  // could in theory do this from preferences but so much easier to do using multiChanelPanel_
  // object
  ...
  /**
   * @return 1 if channels are not enabled, otherwise the number of channels from the list that are
   *     enabled
   */
  // could in theory do this from preferences but so much easier to do using multiChanelPanel_
  // object
  ...
  public void selectChannel(String channel) {
    try {
      ...
      MyDialogUtils.showError(
          e,
          "Couldn't select preset "
              + channel
              + "of channel group "
              + channelGroup_.getSelectedItem().toString());
    }
  }

  /**
   * Takes care of actually selecting next channel in table. Called by acquisition code. Blocks
   * until devices ready.
   */
  ...
  /**
   * Gets the current configuration/preset from the selected channel group, even if that preset
   * isn't represented in the channel table. Thus we can go back to the original preset after
   * changing it via selectNextChannel using setConfig().
   *
   * @return
  ...
  /**
   * Adds an entry to the table for the specified channel if it doesn't already exist. Marks that
   * channel enabled/disabled as asked
   *
   * @param channel
   * @param enabled
   */
