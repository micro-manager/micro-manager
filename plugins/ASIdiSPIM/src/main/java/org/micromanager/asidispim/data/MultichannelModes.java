...
/**
 * Class that holds utilities related to multicolor sequence order (e.g. change color slice by
 * slice, etc.).
 *
 * @author Jon
 */
public class MultichannelModes {
  ...
  public MultichannelModes(
    ...
    if (!props_.hasProperty(devKey, propKey)) {
      props_.setPropValue(devKey, propKey, defaultKey.getPrefCode());
    }
  }
  ...
  private class ComboBoxListener implements ActionListener, DevicesListenerInterface {
    ...
    /**
     * Returns whatever modes are available. Can be expanded in the future
     *
     * @return
     */
    private List<Keys> getValidModeKeys() {
      ...
      if (devices_.isValidMMDevice(Devices.Keys.PLOGIC)
          && props_.getPropValueFloat(Devices.Keys.PLOGIC, Properties.Keys.FIRMWARE_VERSION)
              > 3.069) {
