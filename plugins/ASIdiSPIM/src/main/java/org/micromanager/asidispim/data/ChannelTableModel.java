 ...
 * org.micromanager.utils.ChannelSpec.java. Handles saving preferences to registry assuming
 * column/row don't change.
 *
 * @author Jon
 * @author Nico
 */
...
public class ChannelTableModel extends AbstractTableModel {
  ...
  public final void setChannelGroup(String channelGroup) {
    channels_.clear();
    int nrChannels = prefs_.getInt(prefNode_ + "_" + channelGroup, Prefs.Keys.NRCHANNELROWS, 1);
    ...
    if (!hasChannel(channelGroup)) nrChannels = 1;
  ...
  private boolean hasChannel(String channelGroup) {
    ...
    String channel = prefs_.getString(prefKey, Prefs.Keys.CHANNEL_CONFIG, "");
  ...
  public final void addChannel(String channelGroup) {
    ...
    addNewChannel(
        new ChannelSpec(
            ...
            channelGroup,
            channel));
    prefs_.putInt(prefNode_ + "_" + channelGroup, Prefs.Keys.NRCHANNELROWS, channels_.size());
  }
  ...
  public final void addNewChannel(ChannelSpec channel) {
    ...
    prefs_.putInt(prefNode_ + "_" + channel.group_, Prefs.Keys.NRCHANNELROWS, channels_.size());
    channels_.add(channel);
  }

  /**
   * Returns array of channels that are currently set be "used". Returns them in order that they are
   * in the table, omitting unused ones.
   *
   * @return
   */
