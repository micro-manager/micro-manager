...
public class ControllerUtils {
    ...
    * This method assumes that prepareControllerForAquisition() has been called
    * already to initialize scanDistance_.
    *
   ...
   */
   public boolean prepareControllerForAquisition(final AcquisitionSettings settings) {
      ...
      if (settings.isStageScanning) {
         ...
         if (isInterleaved) {// assure in acquisition code that we can't have single-sided interleaved // can't have 1 side interleaved 
         }
   ...
   /**
    * Compute appropriate motor speed in mm/s for the given stage scanning
    * settings
    *
    * @param settings
    * @return
    ...
    * Set to be 10ms + 0-100ms depending on relative speed to max, all scaled by
    * factor specified on the settings panel
    *
    * @param motorSpeed
    * @return
    */
