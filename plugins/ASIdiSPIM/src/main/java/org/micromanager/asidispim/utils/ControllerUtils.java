...
public class ControllerUtils {
  ...
  /**
   * Stage scan needs to be setup at each XY position, so call this method. This method assumes that
   * prepareControllerForAquisition() has been called already to initialize scanDistance_.
   *
  ...
  public boolean prepareStageScanForAcquisition(double x, double y) {
    ...
    props_.setPropValue(
        xyDevice, Properties.Keys.STAGESCAN_FAST_START, (float) ((x - scanDistance_ / 2) / 1000d));
    props_.setPropValue(
        xyDevice, Properties.Keys.STAGESCAN_FAST_STOP, (float) ((x + scanDistance_ / 2) / 1000d));
  ...
  /**
   * Sets all the controller's properties according to volume settings and otherwise gets controller
   * all ready for acquisition (except for final trigger).
   *
   * @param settings
   * @return false if there was some error that should abort acquisition
   */
  public boolean prepareControllerForAquisition(final AcquisitionSettings settings) {
    ...
    props_.setPropValue(
        Devices.Keys.GALVOA, Properties.Keys.BEAM_ENABLED, Properties.Values.NO, true);
    props_.setPropValue(
        Devices.Keys.GALVOB, Properties.Keys.BEAM_ENABLED, Properties.Values.NO, true);
    props_.setPropValue(
        Devices.Keys.GALVOA, Properties.Keys.SA_MODE_X, Properties.Values.SAM_DISABLED, true);
    props_.setPropValue(
        Devices.Keys.GALVOB, Properties.Keys.SA_MODE_X, Properties.Values.SAM_DISABLED, true);
    ...
    if ((settings.numSides > 1) || settings.firstSideIsA) {
      boolean success = prepareControllerForAquisition_Side(Devices.Sides.A, settings);
      if (!success) {
        return false;
      }
    }
    ...
    if ((settings.numSides > 1) || !settings.firstSideIsA) {
      boolean success = prepareControllerForAquisition_Side(Devices.Sides.B, settings);
      if (!success) {
        return false;
      }
    }
    ...
    if (settings.isStageScanning) {
      ...
      // because the XY stage is 45 degrees from the objectives have to move it sqrt(2) * slice step
      // size
      // for now use the current X position as the start of acquisition and always start in positive
      // X direction
      // for now always do serpentine scan with 2 passes at the same Y location, one pass each
      ...
      //    scan mode = serpentine for 2-sided non-interleaved, raster otherwise (need to revisit
      // for 2D stage scanning)
      ...
      if (requestedMotorSpeed
          > props_.getPropValueFloat(xyDevice, Properties.Keys.STAGESCAN_MAX_MOTOR_SPEED)) {
        MyDialogUtils.showError(
            "Required stage speed is too fast, please reduce step size or increase sample exposure.");
        return false;
      }
      props_.setPropValue(
          xyDevice, Properties.Keys.STAGESCAN_MOTOR_SPEED, (float) requestedMotorSpeed);

      // ask for the actual speed and calculate the actual step size
      final double actualMotorSpeed =
          props_.getPropValueFloat(xyDevice, Properties.Keys.STAGESCAN_MOTOR_SPEED);
      final double actualStepSizeUm =
          settings.stepSizeUm * (actualMotorSpeed / requestedMotorSpeed);
      ...
      props_.setPropValue(
          ...
          (float) computeScanAcceleration(actualMotorSpeed));
      ...
      props_.setPropValue(
          xyDevice,
          Properties.Keys.STAGESCAN_PATTERN,
          ((settings.spimMode == AcquisitionModes.Keys.STAGE_SCAN) && (settings.numSides == 2)
              ? Properties.Values.SERPENTINE
              : Properties.Values.RASTER));
      props_.setPropValue(
          xyDevice, Properties.Keys.STAGESCAN_SETTLING_TIME, settings.delayBeforeSide);
    ...
    props_.setPropValue(
        Devices.Keys.PLOGIC,
        Properties.Keys.PLOGIC_PRESET,
        Properties.Values.PLOGIC_PRESET_3,
        true);
  ...
  /**
   * Compute appropriate acceleration time in ms for the specified motor speed. Set to be 10ms +
   * 0-100ms depending on relative speed to max, all scaled by factor specified on the settings
   * panel
   *
   * @param motorSpeed
   * @return
  ...
  public double computeScanAcceleration(double motorSpeed) {
    final double maxMotorSpeed =
        props_.getPropValueFloat(Devices.Keys.XYSTAGE, Properties.Keys.STAGESCAN_MAX_MOTOR_SPEED);
    final double accelFactor =
        props_.getPropValueFloat(
            Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_STAGESCAN_ACCEL_FACTOR);
    return (10 + 100 * (motorSpeed / maxMotorSpeed)) * accelFactor;
  }

  private boolean getSkipScannerWarnings(Devices.Keys galvoDevice) {
    boolean ignoreMissingScanner =
        prefs_.getBoolean(
            MyStrings.PanelNames.SETTINGS.toString(),
            Properties.Keys.PREFS_IGNORE_MISSING_SCANNER,
            false);
  ...
  private boolean prepareControllerForAquisition_Side(
      final Devices.Sides side, final AcquisitionSettings settings) {
    ...
    Properties.Keys widthProp =
        (side == Devices.Sides.A)
            ? Properties.Keys.PLUGIN_SHEET_WIDTH_A
            : Properties.Keys.PLUGIN_SHEET_WIDTH_B;
    Properties.Keys offsetProp =
        (side == Devices.Sides.A)
            ? Properties.Keys.PLUGIN_SHEET_OFFSET_A
            : Properties.Keys.PLUGIN_SHEET_OFFSET_B;

    // save sheet width and offset which may get clobbered
    props_.setPropValue(
        Devices.Keys.PLUGIN,
        widthProp,
        props_.getPropValueFloat(galvoDevice, Properties.Keys.SA_AMPLITUDE_X_DEG),
        skipScannerWarnings);
    props_.setPropValue(
        Devices.Keys.PLUGIN,
        offsetProp,
        props_.getPropValueFloat(galvoDevice, Properties.Keys.SA_OFFSET_X_DEG),
        skipScannerWarnings);

    // if we are changing color slice by slice then set controller to do multiple slices per piezo
    // move
    ...
    props_.setPropValue(
        galvoDevice,
        Properties.Keys.SPIM_NUM_SLICES_PER_PIEZO,
        numSlicesPerPiezo,
        skipScannerWarnings);
    ...
    if (settings.hardwareTimepoints && settings.useTimepoints) {
      float volumeDurationMs =
          (float) ASIdiSPIM.getFrame().getAcquisitionPanel().computeActualVolumeDuration(settings);
    ...
    props_.setPropValue(
        galvoDevice, Properties.Keys.SPIM_DELAY_REPEATS, delayRepeats, skipScannerWarnings);
    props_.setPropValue(
        galvoDevice, Properties.Keys.SPIM_NUM_REPEATS, numVolumesPerTrigger, skipScannerWarnings);
    props_.setPropValue(
        galvoDevice,
    ...
    if (settings.isStageScanning && devices_.isValidMMDevice(piezoDevice)) {
      ...
      // this is basically required for interleaved mode (otherwise piezo would be moving every
      // slice)
      //    and by convention we'll do it for all stage scanning
      piezoCenter =
          props_.getPropValueFloat(piezoDevice, Properties.Keys.HOME_POSITION)
              * 1000; // *1000 to convert mm to um
    ...
    float sliceRate =
        prefs_.getFloat(
            MyStrings.PanelNames.SETUP.toString() + side.toString(),
            Properties.Keys.PLUGIN_RATE_PIEZO_SHEET,
            100);
    if (MyNumberUtils.floatsEqual(sliceRate, 0.0f)) {
      MyDialogUtils.showError(
          "Calibration slope for side "
              + side.toString()
    ...
    float sliceOffset =
        prefs_.getFloat(
            MyStrings.PanelNames.SETUP.toString() + side.toString(),
            Properties.Keys.PLUGIN_OFFSET_PIEZO_SHEET,
            0);
    ...
    final boolean oppositeDirections =
        prefs_.getBoolean(
    ...
    if (oppositeDirections) {
      props_.setPropValue(
          galvoDevice,
          Properties.Keys.SPIM_ALTERTATE_DIRECTIONS,
          Properties.Values.YES,
          skipScannerWarnings);
    } else {
      props_.setPropValue(
          galvoDevice,
          Properties.Keys.SPIM_ALTERTATE_DIRECTIONS,
          Properties.Values.NO,
          skipScannerWarnings);
    }
    props_.setPropValue(
        galvoDevice,
        Properties.Keys.SPIM_DURATION_SCAN,
        settings.sliceTiming.scanPeriod,
        skipScannerWarnings);
    props_.setPropValue(
        galvoDevice, Properties.Keys.SA_AMPLITUDE_Y_DEG, sliceAmplitude, skipScannerWarnings);
    props_.setPropValue(
        galvoDevice, Properties.Keys.SA_OFFSET_Y_DEG, sliceCenter, skipScannerWarnings);
    props_.setPropValue(
        galvoDevice, Properties.Keys.SPIM_NUM_SLICES, numSlices, skipScannerWarnings);
    props_.setPropValue(
        galvoDevice, Properties.Keys.SPIM_NUM_SIDES, settings.numSides, skipScannerWarnings);
    props_.setPropValue(
        galvoDevice,
        Properties.Keys.SPIM_FIRSTSIDE,
        settings.firstSideIsA ? "A" : "B",
        skipScannerWarnings);
    ...
    if (devices_.isValidMMDevice(piezoDevice)) {
      ...
      props_.setPropValue(piezoDevice, Properties.Keys.SA_AMPLITUDE, piezoAmplitude);
      props_.setPropValue(piezoDevice, Properties.Keys.SA_OFFSET, piezoCenter);
      props_.setPropValue(piezoDevice, Properties.Keys.SPIM_NUM_SLICES, numSlices);
      props_.setPropValue(piezoDevice, Properties.Keys.SPIM_STATE, Properties.Values.SPIM_ARMED);
    }
    ...
    if (settings.isStageScanning) {
      ...
      // for interleaved stage scanning there will never be "home" pulse and for normal stage
      // scanning
      //   the first side piezo will never get moved into position either so do both manually (for
      //   simplicity ignore fact that one of two is unnecessary for two-sided normal stage scan
      // acquisition)
      try {
        if (devices_.isValidMMDevice(piezoDevice)) {
    ...
    //    not to move piezos anywhere (i.e. maybe setting "home disable" to true doesn't have any
    // really effect)
    if (isInterleaved) {
      props_.setPropValue(
          galvoDevice,
          Properties.Keys.SPIM_PIEZO_HOME_DISABLE,
          Properties.Values.YES,
          skipScannerWarnings);
    } else {
      props_.setPropValue(
    ...
    if (isInterleaved) {
      props_.setPropValue(
          galvoDevice,
          Properties.Keys.SPIM_INTERLEAVE_SIDES,
          Properties.Values.YES,
          skipScannerWarnings); // make sure to check for errors
    } else {
      props_.setPropValue(
          galvoDevice,
          Properties.Keys.SPIM_INTERLEAVE_SIDES,
          Properties.Values.NO,
          true); // ignore errors b/c older firmware won't have it
    }
    ...
    if (settings.cameraMode == CameraModes.Keys.LIGHT_SHEET) {
      // adjust sheet width and offset to account for settle time where scan is going but we aren't
      // imaging yet
      final float settleTime =
      ...
      final float readoutTime =
          settings.sliceTiming.laserDuration - 0.25f; // -0.25 is for scanLaserBufferTime
      // offset should be decreased by half of the distance traveled during settle time (instead of
      // re-extracting slope use existing sheetWidth/readoutTime)
  ...
  public boolean cleanUpControllerAfterAcquisition(
      final int numSides, final boolean firstSideIsA, final boolean centerPiezos) {
    ...
    props_.setPropValue(
        Devices.Keys.PLOGIC,
        Properties.Keys.PLOGIC_PRESET,
        Properties.Values.PLOGIC_PRESET_2,
        true);
  ...
  private boolean cleanUpControllerAfterAcquisition_Side(
      final Devices.Sides side, final boolean movePiezo, final float piezoPosition) {
    ...
    props_.setPropValue(
        ...
        props_.getPropValueFloat(Devices.Keys.PLUGIN, widthProp),
        skipScannerWarnings);
    props_.setPropValue(
        Devices.Keys.PLUGIN,
        offsetProp,
    ...
    props_.setPropValue(galvoDevice, Properties.Keys.SPIM_STATE, Properties.Values.SPIM_IDLE, true);
  ...
  public float getSheetWidth(
      CameraModes.Keys cameraMode, Devices.Keys cameraDevice, Devices.Sides side) {
    float sheetWidth;
    if (cameraMode == CameraModes.Keys.LIGHT_SHEET) {
      final float sheetSlope =
          prefs_.getFloat(
              MyStrings.PanelNames.SETUP.toString() + side.toString(),
              Properties.Keys.PLUGIN_LIGHTSHEET_SLOPE,
              2000);
      Rectangle roi;
      try {
      ...
      final boolean autoSheet =
          prefs_.getBoolean(
              MyStrings.PanelNames.SETUP.toString() + side.toString(),
              Properties.Keys.PREFS_AUTO_SHEET_WIDTH,
              false);
      if (autoSheet) {
        Rectangle roi;
        try {
        ...
        final float sheetSlope =
            prefs_.getFloat(
                MyStrings.PanelNames.SETUP.toString() + side.toString(),
                Properties.Keys.PLUGIN_SLOPE_SHEET_WIDTH.toString(),
                2);
        sheetWidth =
            roi.height * sheetSlope / 1000f; // in millidegrees per pixel, convert to degrees
        // TODO add extra width to compensate for filter depending on sweep rate and filter freq
        // TODO calculation should account for sample exposure to make sure 0.25ms edges get
        // appropriately compensated for
        sheetWidth *= 1.1f; // 10% extra width just to be sure
      } else {
        final Properties.Keys widthProp =
            (side == Devices.Sides.A)
                ? Properties.Keys.PLUGIN_SHEET_WIDTH_EDGE_A
                : Properties.Keys.PLUGIN_SHEET_WIDTH_EDGE_B;
  ...
  public float getSheetOffset(CameraModes.Keys cameraMode, Devices.Sides side) {
    ...
    if (cameraMode == CameraModes.Keys.LIGHT_SHEET) {
      sheetOffset =
          prefs_.getFloat(
                  MyStrings.PanelNames.SETUP.toString() + side.toString(),
                  Properties.Keys.PLUGIN_LIGHTSHEET_OFFSET,
                  0)
              / 1000f; // in millidegrees, convert to degrees
    } else {
      final Properties.Keys offsetProp =
          (side == Devices.Sides.A)
              ? Properties.Keys.PLUGIN_SHEET_OFFSET_EDGE_A
              : Properties.Keys.PLUGIN_SHEET_OFFSET_EDGE_B;
  ...
  /**
   * Programs the PLogic card for hardware channel switching according to the selections in the
   * Multichannel subpanel
   *
   * @param settings
   * @return false if there is a fatal error, true if successful
   */
  public boolean setupHardwareChannelSwitching(final AcquisitionSettings settings) {
    ...
    final int laserTriggerAddress =
        10; // this should be set to (42 || 8) = (TTL1 || manual laser on)
    ...
    if ((settings.numChannels > 4)
      ...
      MyDialogUtils.showError(
          "PLogic card cannot handle more than 4 channels for hardware switching.");
      return false;
    }
    ...
    String editCellUpdates =
        props_.getPropValueString(Devices.Keys.PLOGIC, Properties.Keys.PLOGIC_EDIT_CELL_UPDATES);
    if (!editCellUpdates.equals(Properties.Values.NO.toString())) {
      props_.setPropValue(
          Devices.Keys.PLOGIC, Properties.Keys.PLOGIC_EDIT_CELL_UPDATES, Properties.Values.NO);
    }
    ...
    for (int cellNum = 13; cellNum <= 16; cellNum++) {
      ...
      props_.setPropValue(
          Devices.Keys.PLOGIC,
          Properties.Keys.PLOGIC_EDIT_CELL_TYPE,
          Properties.Values.PLOGIC_AND4);
      props_.setPropValue(
          Devices.Keys.PLOGIC, Properties.Keys.PLOGIC_EDIT_CELL_INPUT_2, laserTriggerAddress);
    ...
    for (int channelNum = 0; channelNum < settings.numChannels; channelNum++) {
      ...
      int outputNum =
          getPLogicOutputFromChannel(settings.channels[channelNum], settings.channelGroup);
      if (outputNum < 5) { // check for error in getPLogicOutputFromChannel()
        // restore update setting
        props_.setPropValue(
            Devices.Keys.PLOGIC, Properties.Keys.PLOGIC_EDIT_CELL_UPDATES, editCellUpdates);
      ...
      props_.setPropValue(
          Devices.Keys.PLOGIC, Properties.Keys.PLOGIC_POINTER_POSITION, outputNum + 8);
      props_.setPropValue(
          Devices.Keys.PLOGIC,
          Properties.Keys.PLOGIC_EDIT_CELL_INPUT_1,
          invertAddress); // enable this AND4
      // if we are doing per-volume switching with side B first then counter will start at 1 instead
      // of 0
      // the following lines account for this by incrementing the channel number "match" by 1 in
      // this special case
      ...
      final int in3 =
          (adjustedChannelNum & 0x01) > 0 ? counterLSBAddress : counterLSBAddress + invertAddress;
      final int in4 =
          (adjustedChannelNum & 0x02) > 0 ? counterMSBAddress : counterMSBAddress + invertAddress;
  ...
  public boolean triggerControllerStartAcquisition(
    ...
    switch (spimMode) {
      ...
      case STAGE_SCAN_UNIDIRECTIONAL:
        ...
        props_.setPropValue(
            Devices.Keys.XYSTAGE, Properties.Keys.STAGESCAN_STATE, Properties.Values.SPIM_RUNNING);
        break;
      ...
      case NO_SCAN:
        ...
        props_.setPropValue(
            ...
            getSkipScannerWarnings(galvoDevice));
        break;
      default:
  ...
  private int getPLogicOutputFromChannel(ChannelSpec channel, String channelGroup) {
    try {
      ...
      if (!configData.isPropertyIncluded(
          devices_.getMMDevice(Devices.Keys.PLOGIC),
          Properties.Keys.PLOGIC_OUTPUT_CHANNEL.toString())) {
        MyDialogUtils.showError(
            "Must include PLogic \"OutputChannel\" in preset for hardware switching");
        return 0;
      }
      String setting =
          configData
      ...
      if (setting.equals(Properties.Values.PLOGIC_CHANNEL_BNC5.toString())) {
        ...
        MyDialogUtils.showError(
            "Channel preset setting must use PLogic \"OutputChannel\" and be set to one of outputs 5-8 only");
        return 0;
      }
    } catch (Exception e) {
