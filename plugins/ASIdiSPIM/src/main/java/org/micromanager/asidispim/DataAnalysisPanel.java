...
/**
 * Panel in ASIdiSPIM plugin specifically for data analysis/processing For now, we provide a way to
 * export Micro-Manager datasets into a mipav compatible format mipav likes data in a folder as
 * follows: folder - SPIMA - name_SPIMA-0.tif, name_SPIMA-x.tif, name_SPIMA-n.tif - SPIMB -
 * name_SPIMB-0.tif, name_SPIMB-x.tif, name_SPIMB-n.tif
 *
 * <p>TODO: make mipav export work for single sided acquisitions TODO: make mipav export work for
...
public class DataAnalysisPanel extends ListeningJPanel {
  ...
  public DataAnalysisPanel(Studio gui, Prefs prefs, Properties props, Devices devices) {
    super(MyStrings.PanelNames.DATAANALYSIS.toString(), new MigLayout("", "[right]", "[]16[]"));
    gui_ = gui;
    prefs_ = prefs;
    props_ = props;
    devices_ = devices;
    ...
    exportPanel_ = new JPanel(new MigLayout("", "[right]4[center]4[left]", "[]8[]"));

    exportPanel_.setBorder(PanelUtils.makeTitledBorder("Export diSPIM data", exportPanel_));

    exportPanel_.add(new JLabel("Export directory:"), "");

    saveDestinationField_ = new JTextField();
    saveDestinationField_.setText(
        prefs_.getString(panelName_, Properties.Keys.PLUGIN_EXPORT_DATA_DIR, ""));
    saveDestinationField_.setColumns(textFieldWidth);
    saveDestinationField_.addActionListener(
        new ActionListener() {
          @Override
    ...
    browseToSaveDestinationButton.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(final ActionEvent e) {
    ...
    String transformOption =
        prefs_.getString(
            panelName_, Properties.Keys.PLUGIN_EXPORT_TRANSFORM_OPTION, TRANSFORMOPTIONS[1]);
    transformSelect.setSelectedItem(transformOption);
    transformSelect.addActionListener(
        new ActionListener() {
          @Override
    ...
    String exportFormatOption =
        prefs_.getString(panelName_, Properties.Keys.PLUGIN_EXPORT_FORMAT, EXPORTFORMATS[1]);
    exportFormatSelect.setSelectedItem(exportFormatOption);
    exportFormatSelect.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            prefs_.putString(
    ...
    splitChannels.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    deskewPanel_ = new JPanel(new MigLayout("", "[right]4[center]4[left]", "[]8[]"));
    ...
    deskewFactor_ =
        pu.makeFloatEntryField(panelName_, Properties.Keys.PLUGIN_DESKEW_FACTOR.toString(), 1.0, 5);
    deskewPanel_.add(deskewFactor_, "wrap");

    deskewInvert_ =
        pu.makeCheckBox(
            "Invert direction", Properties.Keys.PLUGIN_DESKEW_INVERT, panelName_, false);
    deskewPanel_.add(deskewInvert_, "left, span 2, wrap");

    deskewInterpolate_ =
        pu.makeCheckBox(
            "Interpolate", Properties.Keys.PLUGIN_DESKEW_INTERPOLATE, panelName_, false);
    deskewPanel_.add(deskewInterpolate_, "left, span 2, wrap");

    deskewAutoTest_ =
        pu.makeCheckBox(
            "Auto-deskew test acquisitions",
    ...
    deskewButton.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(final ActionEvent e) {
  ...
  public void runDeskew(final ListeningJPanel caller) {
    /**
     * Worker thread to execute deskew. Patterned after Nico's ExportTask SwingWorker code but
     * updating progress bar wasn't working and task is pretty quick so I removed that code.
     *
     * @author Jon
     */
      ...
      public Void doInBackground() throws Exception {
        ...
        if (currentWindow != null) {
          ...
          if (metadata.containsString("PixelSize_um") && metadata.containsString("z-step_um")) {
            // with test acquisitions ip.getCalibration() isn't correct for some reason so prefer
            // metadata
          ...
          ReportingUtils.logDebugMessage(
              "Deskew may be incorrect because don't have Micro-Manager dataset with metadata");
        }
        ...
        if (sc > 1) {
          ...
          IJ.run(
              "Duplicate...",
              "title=C1-" + title + " duplicate"); // make it named as 1st channel would be
        }
        String mergeCmd = "";
        int dir;
        ...
        ReportingUtils.logDebugMessage(
            "Deskew operation took "
                + (finishTime - startTime)
                + " milliseconds with total of "
                + (sc * ss)
                + " images");
  ...
  /**
   * Worker thread that executes file saving. Updates the progress bar using the setProgress method,
   * which results in a PropertyChangedEvent in attached listeners
   */
  class ExportTask extends SwingWorker<Void, Void> {
    ...
    ExportTask(String targetDirectory, String baseName, int transformIndex, int exportFormat) {
    ...
    protected Void doInBackground() throws Exception {
      ...
      if (exportFormat_ == 0) { // mipav
        ...
        boolean usesChannels =
            store.getNextIndex(Coords.CHANNEL) > 2; // if have channels besides two cameras
        String[] channelDirArray = new String[store.getNextIndex(Coords.CHANNEL)];
        if (usesChannels) {
          for (int c = 0; c < store.getNextIndex(Coords.CHANNEL); c++) {
            String chName = store.getSummaryMetadata().getUserData().getStringArray("ChNames")[c];
            String colorName =
                chName.substring(
                    chName.indexOf("-") + 1); // matches with AcquisitionPanel naming convention
            channelDirArray[c] =
                targetDirectory_
                    + File.separator
                    + baseName_
          ...
          channelDirArray[0] =
              targetDirectory_ + File.separator + baseName_ + File.separator + "SPIMA";
          channelDirArray[1] =
              targetDirectory_ + File.separator + baseName_ + File.separator + "SPIMB";
        }
        ...
        throw new SaveTaskException(
            "Should import Micro-Manager datasets "
  ...
  private void setSaveDestinationDirectory(JTextField rootField) {
    File result =
        FileDialogs.openDir(null, "Please choose a directory root for image data", EXPORT_DATA_SET);
    if (result != null) {
