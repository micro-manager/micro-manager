...
// known issue: quirky way of handling case where user removes the device that a joystick is
// currently associated with
//   without the plugin knowing the device, the joystick can't actually be disabled so will continue
// to function with that physical device
...
public final class JoystickSubPanel extends ListeningJPanel {
   ...
   * @param prefs
   */
  public JoystickSubPanel(
      Joystick joystick, Devices devices, String instanceLabel, Devices.Sides side, Prefs prefs) {
    super(
        MyStrings.PanelNames.JOYSTICK_SUBPANEL.toString() + instanceLabel,
        new MigLayout("", "[right]8[align center]", "[]16[]"));
  ...
  private JComboBox makeJoystickSelectionBox(Joystick.Keys jkey) {
    ...
    // intentionally set from prefs after adding listeners so programmatic change from prefs will be
    // handled like user change
    // we store string in prefs but have associative object as combo box items
    // could simplify the equals method of JSAxisData to only look at the display string but afraid
    // of unintended consequences
    // so we look through combo box strings and select a match
    String selectedItem =
        prefs_.getString(instanceLabel_, jkey.toString(), Joystick.Keys.NONE.toString());
  ...
  private class StageSelectionBoxListener
      implements ItemListener, ActionListener, DevicesListenerInterface {
    Joystick.Keys jkey_;
    JComboBox cb_;
    ...
    /**
     * This will be called whenever the user selects a new item, but also when the tab to which this
     * selectionbox belongs is selected (via gotSelected() method) Save the selection to preferences
     * every time (probably inefficient)
     *
    ...
    public void actionPerformed(ActionEvent ae) {
      ...
      prefs_.putString(
          instanceLabel_, jkey_.toString(), ((JSAxisData) cb_.getSelectedItem()).toString());
    }
    ...
    /**
     * Resets the items in the combo box based on current contents of device tab. Besides being
     * called on original combo box creation, it is called whenever something in the devices tab is
     * changed
     */
    private void updateStageSelections() {
      ...
      // get the appropriate list of strings (in form of JSAxisData array) depending on joystick
      // device type
  ...
  public void gotDeSelected() {
    ...
    joystick_.unsetJoystick(
        Joystick.Keys.RIGHT_WHEEL, (JSAxisData) rightWheelBox_.getSelectedItem());
