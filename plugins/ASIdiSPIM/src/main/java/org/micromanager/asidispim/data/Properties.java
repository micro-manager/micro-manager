...
/**
 * Contains data and methods related to getting and setting device properties. Ideally this is the
 * only place where MM properties are read and set. One instance of this class exists in the
 * top-level class.
 *
 * <p>Property "reads" ignore errors due to missing device or property (instead they return empty
 * string or zero); original functionality to catch those errors was unused and removed Jan 2015.
 *
 * <p>Property "writes" default to reporting errors due to missing device or property, but can be
 * called (and occasionally are) such as to ignore errors.
 *
 * <p>For the special case of the "PLUGIN" device which doesn't have properties we store the values
 * using preferences.
 *
 * @author Jon
 * @author nico
...
public class Properties {
  ...
  /**
   * List of all device adapter properties used. The enum value (all caps) is used in the Java code.
   * The corresponding string value (in quotes) is the value used by the device adapter.
   */
  public static enum Keys {
    ...
    PLUGIN_PIEZO_SHEET_INCREMENT(
        "PiezoSheetIncrement"), // piezo increment for moving piezo and galvo together
    PLUGIN_OFFSET_PIEZO_SHEET("OffsetPiezoSheet"), // Offset in piezo/sheet calibration
    PLUGIN_RATE_PIEZO_SHEET(
        "RatePiezoSheet"), // Rate in piezo/sheet calibration, really should be named "slice"
    ...
    PLUGIN_EXPORT_TRANSFORM_OPTION(
        "ExportTransformOption"), // Transform to be applied when exporting data
    ...
    PLUGIN_CAMERA_LIVE_EXPOSURE_FIRST(
        "CameraLiveExposureMs_First"), // used internally to save/restore live exposure time
    PLUGIN_CAMERA_LIVE_EXPOSURE_SECOND(
    ...
    if (device == Devices.Keys.PLUGIN) {
      ...
      try {
        ...
        MyDialogUtils.showError(
            ex, "Couldn't find property " + name.toString() + " in device " + mmDevice);
      }
    }
    return false;
  }
   ...
   * @param ignoreError false (default) will do error checking, true means ignores non-existing
   *     property
   */
  public void setPropValue(
      Devices.Keys device, Properties.Keys name, String strVal, boolean ignoreError) {
    if (device == Devices.Keys.PLUGIN) {
      ...
      try {
        mmDevice = devices_.getMMDeviceException(device);
        if (name.doForceSet() || !core_.getProperty(mmDevice, name.toString()).equals(strVal)) {
        ...
        if (ignoreError) {
          if (mmDevice != null) {
            ...
            ReportingUtils.logMessage(
                "Device " + mmDevice + " does not have property: " + name.toString());
          }
          // do nothing if ignoreError set and we didn't find the device at all
        } else {
          MyDialogUtils.showError(
              ex,
              "Error setting string property "
                  + name.toString()
                  + " to "
                  + strVal
                  + " in device "
   ...
   */
  public void setPropValue(
      Devices.Keys[] devices, Properties.Keys name, String strVal, boolean ignoreError) {
    for (Devices.Keys device : devices) {
   ...
   * @param device enum key for device
   * @param name enum key for property
   * @param val value in Properties.Values enum form, sent to core using setProperty() after
   *     toString() call
   * @param ignoreError false (default) will do error checking, true means ignores non-existing
   *     property
   */
  public void setPropValue(
      Devices.Keys device, Properties.Keys name, Properties.Values val, boolean ignoreError) {
   ...
   */
  public void setPropValue(
      Devices.Keys[] devices, Properties.Keys name, Properties.Values val, boolean ignoreError) {
    for (Devices.Keys device : devices) {
      setPropValue(device, name, val.toString(), ignoreError);
    }
  }
   ...
   */
  public void setPropValue(
      Devices.Keys device, Properties.Keys name, int intVal, boolean ignoreError) {
    if (device == Devices.Keys.PLUGIN) {
      ...
      try {
        ...
        if (name.doForceSet()
            || intVal
                != NumberUtils.coreStringToInt(core_.getProperty(mmDevice, name.toString()))) {
        ...
        if (ignoreError) {
          ...
          MyDialogUtils.showError(
              ex, "Error setting int property " + name.toString() + " in device " + mmDevice);
        }
      }
    }
  }
   ...
   */
  public void setPropValue(
      Devices.Keys[] devices, Properties.Keys name, int intVal, boolean ignoreError) {
    for (Devices.Keys device : devices) {
   ...
   */
  public void setPropValue(
      Devices.Keys device, Properties.Keys name, float floatVal, boolean ignoreError) {
    if (device == Devices.Keys.PLUGIN) {
      ...
      try {
        mmDevice = devices_.getMMDeviceException(device);
        if (name.forceSet
            || !MyNumberUtils.floatsEqual(
                floatVal,
                (float)
                    NumberUtils.coreStringToDouble(core_.getProperty(mmDevice, name.toString())))) {
        ...
        if (ignoreError) {
          ...
          MyDialogUtils.showError(
              ex, "Error setting float property " + name.toString() + " in device " + mmDevice);
        }
      }
    }
  }
   ...
   */
  public void setPropValue(
      Devices.Keys[] devices, Properties.Keys name, float floatVal, boolean ignoreError) {
    for (Devices.Keys device : devices) {
  ...
  /**
   * reads the property value from the device adapter using a core call, or empty string if it can't
   * find property.
   *
   * @param device enum key for device
   * @param name enum key for property
  ...
  /**
   * returns a string value for the specified property (assumes the caller knows the property
   * contains an string) Ignores missing device or property, returning empty string.
   *
   * @param device enum key for device
   * @param name enum key for property
   * @return
  ...
  /**
   * returns an integer value for the specified property (assumes the caller knows the property
   * contains an integer). If property isn't found, returns 0.
   *
   * @param device enum key for device
   * @param name enum key for property
   * @return
  ...
  /**
   * returns an float value for the specified property (assumes the caller knows the property
   * contains a float). If property isn't found, returns 0.
   *
   * @param device enum key for device
   * @param name enum key for property
   * @return
