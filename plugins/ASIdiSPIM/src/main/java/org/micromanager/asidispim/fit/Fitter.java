...
public class Fitter {
  ...
  /**
   * Find the index in the data series with an x value closest to the given searchValue
   *
  ...
  /**
   * Calculate the y value for a given function and x value Throws an IllegalArgumentException if
   * the pars do not match the function
   *
  ...
  public static FunctionType getFunctionTypeAsType(String key) {
    if (key.equals(NOFIT)) return FunctionType.NoFit;
    if (key.equals(GAUSSIAN)) return FunctionType.Gaussian;
