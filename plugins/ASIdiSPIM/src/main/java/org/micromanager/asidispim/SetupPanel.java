...
public final class SetupPanel extends ListeningJPanel {
  ...
  // used to store the start/stop positions of the single-axis moves for imaging piezo and
  // micromirror sheet move axis
  ...
  public SetupPanel(
    ...
    super(
        ...
        new MigLayout("", "8[center]8[center]0", "[]8[]8[]"));
    ...
    piezoDeltaField_ =
        pu.makeFloatEntryField(
            panelName_, Properties.Keys.PLUGIN_PIEZO_SHEET_INCREMENT.toString(), 5, 2);
    piezoDeltaField_.setToolTipText("Piezo increment used by up/down arrow buttons");

    JButton upButton = new JButton();
    upButton.setIcon(new ImageIcon(getClass().getResource("/org/micromanager/icons/arrow_up.png")));
    upButton.setText("");
    ...
    downButton.setIcon(
        new ImageIcon(getClass().getResource("/org/micromanager/icons/arrow_down.png")));
    downButton.setText("");
    ...
    calibrationPanel.add(
        new JLabel("<html><u>Piezo/Slice Calibration</u></html>"), "span 5, center");
    calibrationPanel.add(
    ...
    testAcqButton.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    sheetPanelContainer_ = new JPanel(new MigLayout("", "0[]0", "0[]0"));
    ...
    autoSheetWidth_ =
        pu.makeCheckBox("Automatic", Properties.Keys.PREFS_AUTO_SHEET_WIDTH, panelName_, false);
    widthPropNormal_ =
        (side == Devices.Sides.A)
            ? Properties.Keys.PLUGIN_SHEET_WIDTH_EDGE_A
            : Properties.Keys.PLUGIN_SHEET_WIDTH_EDGE_B;
    autoSheetWidth_.addActionListener(
        new ActionListener() {
          @Override
    ...
    sheetWidthSlope_ =
        pu.makeFloatEntryField(
            panelName_, Properties.Keys.PLUGIN_SLOPE_SHEET_WIDTH.toString(), 2, 5);
    sheetWidthSlope_.addPropertyChangeListener(
        "value",
    ...
    sheetIncButton_ =
        makeIncrementButton(micromirrorDeviceKey_, widthPropNormal_, "-", (float) -0.01);
    ...
    final JComponent[] manualSheetWidthComponents = {
      sheetIncButton_, sheetDecButton_, sheetWidthSlider_
    };
    ...
    autoSheetWidth_.addActionListener(
        new ActionListener() {
          ...
          public void actionPerformed(ActionEvent e) {
            PanelUtils.componentsSetEnabled(autoSheetWidthComponents, autoSheetWidth_.isSelected());
            PanelUtils.componentsSetEnabled(
                manualSheetWidthComponents, !autoSheetWidth_.isSelected());
          }
        });
    ...
    sheetPanelNormal_.add(tmp_but);
    sheetPanelNormal_.add(
        makeIncrementButton(Devices.Keys.PLUGIN, offsetProp, "-", (float) -0.01), "split 2");
    sheetPanelNormal_.add(makeIncrementButton(Devices.Keys.PLUGIN, offsetProp, "+", (float) 0.01));
    // make sure our internal-use property is initialized
    ...
    // we can't copy relevant elements from other panel because Java swing components can only have
    // one container
    //   (if added to a second container they are removed from the first) so first part is lots of
    // copy/paste
    sheetPanelLightSheet_ =
        new JPanel(
            new MigLayout(
                "",
                "[" + labelWidth + "px!,right]8[" + positionWidth + "px!,center]8[center]8[center]",
    ...
    tmp_but.addActionListener(
        new ActionListener() {
          ...
          public void actionPerformed(ActionEvent e) {
            if (devices_.isValidMMDevice(piezoIlluminationDeviceKey_)) {
              try {
                gui_.core().home(devices_.getMMDevice(piezoIlluminationDeviceKey_));
              } catch (Exception e1) {
                ReportingUtils.showError(e1, "Could not move piezo to home");
              }
            }
    ...
    illumPiezoHomeEnable2_ =
        pu.makeCheckBox(
            "Go home on tab activate",
            Properties.Keys.PREFS_ENABLE_ILLUM_PIEZO_HOME,
            panelName_,
            true);
    // below is hack to keep the two elements synchronized (can only have swing components in one
    // container)
    illumPiezoHomeEnable2_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    illumPiezoHomeEnable_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    sheetPanelLightSheet_.add(
        new JLabel("<html><u>Light Sheet Synchronization</u></html>"), "span 3, wrap");
    JFormattedTextField lightSheetSlope =
        pu.makeFloatEntryField(
            panelName_, Properties.Keys.PLUGIN_LIGHTSHEET_SLOPE.toString(), 2000, 5);
    lightSheetSlope.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            ...
            if (key == CameraModes.Keys.LIGHT_SHEET) {
              float width =
                  controller_.getSheetWidth(CameraModes.Keys.LIGHT_SHEET, cameraDeviceKey_, side_);
              props_.setPropValue(
                  micromirrorDeviceKey_,
                  Properties.Keys.SA_AMPLITUDE_X_DEG,
                  width,
                  true); // ignore missing device
            }
          }
        });
    JFormattedTextField lightSheetOffset =
        pu.makeFloatEntryField(
            panelName_, Properties.Keys.PLUGIN_LIGHTSHEET_OFFSET.toString(), 0, 5);
    lightSheetOffset.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            CameraModes.Keys key = getSPIMCameraMode();
    ...
    sheetPanelLightSheet_.add(new JLabel("Slope / offset:"));
    ...
    // initialize sheet controls per current setting stored in prefs; combobox hasn't been created
    // yet
    final int currentCameraMode =
        prefs_.getInt(
            MyStrings.PanelNames.SETTINGS.toString(), Properties.Keys.PLUGIN_CAMERA_MODE, 0);
    sheetPanelContainer_.add(
        currentCameraMode == 5
            ? // crude but couldn't figure out a way to refer to the code otherwise
            sheetPanelLightSheet_
            : sheetPanelNormal_);
  ...
  /**
   * Performs "1-point" calibration updating the offset (but not the slope) based on current
   * piezo/galvo positions. Does not perform any error/range checking except to ensure beam is on.
   */
    ...
    try {
      if (beamPanel_.isBeamAOn()) {
        ...
        ReportingUtils.logMessage(
            "updated offset for side " + side_ + "; new value is " + newOffset);
      }
    } catch (Exception ex) {
      MyDialogUtils.showError(ex);
    }
  ...
  /**
   * Update the calibration offset according to the autofocus score. Caller should make sure to
   * apply any required criteria before calling this. method because no error/range checking is done
   * here.
   *
  ...
  public void updateCalibrationOffset(AutofocusUtils.FocusResult score) {
    ...
    offsetField_.setValue((Double) newOffset);
    ReportingUtils.logMessage(
        "autofocus updated offset for side " + side_ + "; new value is " + newOffset);
  }

  /** Performs "2-point" calibration updating the offset and slope. Pops up a sub-window. */
  ...
  /**
   * Centers the piezo and micro-mirror. Doesn't do anything if the devices aren't assigned to
   * prevent spurious exceptions, but will move micro-mirror if piezo isn't assigned.
   *
   * @throws Exception
   */
  private void centerPiezoAndGalvo() {
    ...
    if (success || !devices_.isValidMMDevice(piezoImagingDeviceKey_)) {
      positions_.setPosition(
          micromirrorDeviceKey_, Directions.Y, computeGalvoFromPiezo(imagingCenterPos_));
    }
  }

  private JButton makeIncrementButton(
      Devices.Keys devKey, Properties.Keys propKey, String label, float incrementAmount) {
    class incrementButtonActionListener implements ActionListener {
      ...
      public void actionPerformed(ActionEvent e) {
        try {
          props_.setPropValue(
              devKey_,
              propKey_,
              incrementAmount_ + props_.getPropValueFloat(devKey_, propKey_),
              true);
      ...
      private incrementButtonActionListener(
          Devices.Keys devKey, Properties.Keys propKey, float incrementAmount) {
        devKey_ = devKey;
        propKey_ = propKey;
  ...
  /**
   * Restore camera and beam settings after an acquisition has been run (either autofocus or test
   * acquisition)
   */
  ...
  /**
   * Updates the controllers SAA setting for the scan axis based on the settings (slider setting for
   * "normal" mode, ROI and slope for light sheet mode
   */
  ...
  /**
   * Updates the controllers SAO setting for the scan axis based on the settings (slider setting for
   * "normal" mode, ROI and slope for light sheet mode
   */
  ...
  /**
   * @return CameraModes.Keys value from Camera panel (internal, edge, overlap, pseudo-overlap,
   *     light sheet)
   */
  ...
  public void updateStagePositions() {
    ...
    illuminationPiezoPositionLabel2_.setText(
        positions_.getPositionString(piezoIlluminationDeviceKey_));
    slicePositionLabel_.setText(positions_.getPositionString(micromirrorDeviceKey_, Directions.Y));
  }
  ...
  public void refreshSelected() {
    if (prefs_.getBoolean(
        ...
        Properties.Keys.PLUGIN_AUTOFOCUS_AUTOUPDATE_OFFSET,
        false)) {
      ...
      if (score.getFocusSuccess()) {
        ...
        double maxDelta =
            props_.getPropValueFloat(
                Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_AUTOFOCUS_MAXOFFSETCHANGE_SETUP);
  ...
  public void cameraModeChange() {
    ...
    sheetPanelContainer_.add(
        (key == CameraModes.Keys.LIGHT_SHEET) ? sheetPanelLightSheet_ : sheetPanelNormal_);
  ...
  private void updateKeyAssignments() {
    ...
    piezoIlluminationDeviceKey_ =
        Devices.getSideSpecificKey(Devices.Keys.PIEZOA, Devices.getOppositeSide(side_));
  ...
  public void setImagingCenter(double center) throws ASIdiSPIMException {
    if (MyNumberUtils.outsideRange(
      ...
      throw new ASIdiSPIMException(
          "Cannot set imaging center outside the range of the imaging piezo");
    }
  ...
  public void runAutofocus() {
    autofocus_.runFocus(
        this, side_, true, ASIdiSPIM.getFrame().getAcquisitionPanel().getSliceTiming(), true);
  }
