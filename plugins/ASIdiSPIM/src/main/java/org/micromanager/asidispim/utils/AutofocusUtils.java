...
public class AutofocusUtils {
  ...
  public class FocusResult {
    ...
    public FocusResult(
        boolean focusSuccess, double galvoPosition, double piezoPosition, double offsetDelta) {
      focusSuccess_ = focusSuccess;
      galvoPosition_ = galvoPosition;
      piezoPosition_ = piezoPosition;
      offsetDelta_ =
          offsetDelta; // amount in um that the offset will shift, could be positive or negative
    }

    public boolean getFocusSuccess() {
      return focusSuccess_;
    }

    public double getGalvoFocusPosition() {
      return galvoPosition_;
    }

    public double getPiezoFocusPosition() {
      return piezoPosition_;
    }
  ...
  /**
   * Acquires image stack by scanning the mirror, calculates focus scores Acquires around the
   * current piezo position. As a side effect, will temporarily set the center position to the
   * current position. If the autofocus is successful (based on the goodness of fit of the focus
   * curve), either the scanner slice position will be set to the in-focus position (for option "fix
   * piezo, sweep slice") or else the piezo position will be set to the in-focus position (for
   * option "fix slice, sweep piezo"). Immediately following if the user calls
   * updateCalibrationOffset() in the setup panel then the calibration offset will be changed to be
   * in-focus. If the goodness of fit is insufficient or else the best fit position is at the
   * extreme of the sweep range then the piezo and sheet will be returned to their original
   * positions.
   *
   * @param caller - calling panel, used to restore settings after focusing
   * @param side - A or B
   * @param centerAtCurrentZ - whether to focus around the current position (true), or around the
   *     imaging center set in the setup panel (false)
   ...
   *     actual result can be accessed via getLastFocusResult() in the caller's refreshSelected()
   *     method
   */
      ...
      protected FocusResult doInBackground() throws Exception {
        ...
        if (ASIdiSPIM.getFrame().getHardwareInUse()) {
          throw new ASIdiSPIMException(
              "Cannot run autofocus while another" + " autofocus or acquisition is ongoing.");
        }
        ...
        final String algorithmName =
            Fitter.getAlgorithmFromPrefCode(
                    prefs_.getInt(
                        ...
                        Fitter.Algorithm.VOLATH.getPrefCode()))
                .toString();
        ...
        final boolean showImages =
            prefs_.getBoolean(
                MyStrings.PanelNames.AUTOFOCUS.toString(),
                Properties.Keys.PLUGIN_AUTOFOCUS_SHOWIMAGES,
                true);
        final boolean showPlot =
            prefs_.getBoolean(
                MyStrings.PanelNames.AUTOFOCUS.toString(),
                Properties.Keys.PLUGIN_AUTOFOCUS_SHOWPLOT,
                true);
        float piezoStepSize =
        ...
        final float imagingCenter =
            prefs_.getFloat(
                ...
                0);
        final float minimumRSquare =
            props_.getPropValueFloat(
                Devices.Keys.PLUGIN, Properties.Keys.PLUGIN_AUTOFOCUS_MINIMUMR2);
        ...
        final boolean beamOff =
            props_
                .getPropValueString(galvoDevice, Properties.Keys.BEAM_ENABLED)
        ...
        final double originalGalvoPosition =
            positions_.getUpdatedPosition(galvoDevice, Directions.Y);
        final double piezoCenter = centerAtCurrentZ ? originalPiezoPosition : imagingCenter;

        posUpdater_.pauseUpdates(true);

        // start with current acquisition settings, then modify a few of them for the focus
        // acquisition
        AcquisitionSettings acqSettings =
            ASIdiSPIM.getFrame().getAcquisitionPanel().getCurrentAcquisitionSettings();
        ...
        acqSettings.spimMode =
            isPiezoScan
                ? AcquisitionModes.Keys.PIEZO_SCAN_ONLY
        ...
        final float calibrationRate =
            prefs_.getFloat(
                MyStrings.PanelNames.SETUP.toString() + side.toString(),
                Properties.Keys.PLUGIN_RATE_PIEZO_SHEET,
                100);
        final float calibrationOffset =
            prefs_.getFloat(
                MyStrings.PanelNames.SETUP.toString() + side.toString(),
        ...
        try {
          ...
          if (showImages) {
            ...
            ourWindow_.setDisplaySettings(dsb.colorModeGrayscale().build());
            SummaryMetadata.Builder smb = gui_.data().summaryMetadataBuilder();
            Coords dim = Coordinates.builder().z(nrImages).channel(1).stagePosition(1).t(1).build();
          ...
          boolean success =
              controller_.triggerControllerStartAcquisition(
                  AcquisitionModes.Keys.SLICE_SCAN_ONLY, side.equals(Devices.Sides.A));
          if (!success) {
          ...
          // timeout = Math.max(5000,
          // Math.round(1.2*controller_.computeActualVolumeDuration(sliceTiming)));
          while (gui_.core().getRemainingImageCount() == 0 && (now - startTime < timeout)) {
          ...
          while ((gui_.core().getRemainingImageCount() > 0 || gui_.core().isSequenceRunning(camera))
              && !done) {
            now = System.currentTimeMillis();
            if (gui_.core().getRemainingImageCount() > 0) { // we have an image to grab
              ...
              try {
                focusScores[counter] =
                    gui_.getAutofocusManager().getAutofocusMethod().computeScore(ip);
              } catch (Exception ex) {
                done = true;
                throw new ASIdiSPIMException(
                    "Selected autofocus device didn't return a focus score.");
              }
              imageStore[counter] = timg;
              ReportingUtils.logDebugMessage(
                  "Autofocus, image: " + counter + ", score: " + focusScores[counter]);
              ...
              if (showImages) {
                ...
                Coords coords = Coordinates.builder().z(counter).t(0).c(0).p(0).build();
          ...
          // if we are successful then stay at that position, otherwise restore the original
          // position
          //   (don't bother moving anything if we are doing this during acquisition)
          // the caller may (and usually will) apply further logic to decide whether or not this was
          // a
          ...
          if (isPiezoScan) {
            ...
            lastFocusResult_ =
                new FocusResult(focusSuccess, galvoPosition, bestPiezoPosition, focusDelta);
          } else { // slice scan
            ...
            lastFocusResult_ =
                new FocusResult(focusSuccess, bestGalvoPosition, piezoPosition, focusDelta);
          }
          ...
          if (showImages) {
            ...
            gui_.live().displayImage(gui_.data().convertTaggedImage(imageStore[highestIndex]));
          }
          ...
          try {
            ...
            if (runAsynchronously) { // proxy for "running from setup"
              if (isPiezoScan && devices_.isValidMMDevice(piezoDevice)) {
                positions_.setPosition(
                    piezoDevice, focusSuccess ? bestPiezoPosition : originalPiezoPosition);
              } else {
                if (devices_.isValidMMDevice(galvoDevice))
                  positions_.setPosition(
                      galvoDevice,
            ...
            throw new ASIdiSPIMException(
                ex, "Error while restoring hardware state after autofocus.");
          } finally {
        ...
        ReportingUtils.logMessage(
            "finished autofocus: "
                ...
                + " with galvo position "
                + lastFocusResult_.getGalvoFocusPosition()
                + " and piezo position "
                + lastFocusResult_.getPiezoFocusPosition());
        return lastFocusResult_;
      }
  ...
  public static ImageProcessor makeProcessor(TaggedImage taggedImage) throws ASIdiSPIMException {
    final JSONObject tags = taggedImage.tags;
    try {
      return makeProcessor(
          getIJType(tags), tags.getInt("Width"), tags.getInt("Height"), taggedImage.pix);
