...
public class Cameras {
  ...
  /**
   * associative class to store information for camera combo boxes. Contains string shown in combo
   * box, key of corresponding device
   */
  public static class CameraData {
    ...
    public int hashCode() {
      return (this.displayString.hashCode()
          + this.deviceKey.toString().hashCode()
          + this.side.hashCode());
    }
  ...
  public CameraData[] getCameraData() {
    ...
    list.add(
        new CameraData(
            devices_.getDeviceDisplay(Devices.Keys.CAMERAPREVIOUS),
            Devices.Keys.CAMERAPREVIOUS,
            Devices.Sides.NONE));
  ...
  public void setCamera(Devices.Keys key) {
    if (!Devices.CAMERAS.contains(key) || key == Devices.Keys.CAMERAPREVIOUS) {
      return;
    }
  ...
  /**
   * Get the device key of the selected camera after making sure the plugin's representation of the
   * current camera matches the core (core is master)
   *
  ...
  public final Devices.Keys getCurrentCamera() {
    ...
    if (currentCameraKey_ == null || !camera.equals(devices_.getMMDevice(currentCameraKey_))) {
      currentCameraKey_ = null;
      for (Devices.Keys camKey : Devices.CAMERAS) {
        if (devices_.isValidMMDevice(camKey) && devices_.getMMDevice(camKey).equals(camera)) {
  ...
  /**
   * Take care of low-level property setting to make internal trigger or appropriate external mode
   * depending on camera type (via the DeviceLibrary) currently HamamatsuHam, PCO_Camera, and Andor
   * sCMOS are supported
   *
   * @param devKey
    ...
    switch (camLibrary) {
      case HAMCAM:
        ...
        switch (mode) {
          case EDGE:
          case LIGHT_SHEET:
            props_.setPropValue(devKey, Properties.Keys.TRIGGER_ACTIVE, Properties.Values.EDGE);
            break;
          case LEVEL:
            props_.setPropValue(devKey, Properties.Keys.TRIGGER_ACTIVE, Properties.Values.LEVEL);
            break;
          case OVERLAP:
            props_.setPropValue(
                devKey, Properties.Keys.TRIGGER_ACTIVE, Properties.Values.SYNCREADOUT);
            break;
          default:
            break;
        }
      ...
      case PCOCAM:
        switch (mode) {
          ...
          case PSEUDO_OVERLAP:
            props_.setPropValue(
                devKey, Properties.Keys.TRIGGER_MODE_PCO, Properties.Values.EXTERNAL_LC);
            break;
          case LEVEL:
            props_.setPropValue(
                devKey, Properties.Keys.TRIGGER_MODE_PCO, Properties.Values.LEVEL_PCO);
            break;
          case INTERNAL:
            props_.setPropValue(
                devKey, Properties.Keys.TRIGGER_MODE_PCO, Properties.Values.INTERNAL_LC);
            break;
          default:
            break;
        }
      ...
      case ANDORCAM:
        ...
        //  to "normal" center out simultaneous but works if we always go through the in-between
        // mode
        if (props_.hasProperty(
            devKey, Properties.Keys.SENSOR_READOUT_MODE)) { // skip step if property is missing
          props_.setPropValue(
              devKey, Properties.Keys.SENSOR_READOUT_MODE, Properties.Values.BOTTOM_UP_SIM_ANDOR);
          props_.setPropValue(
              devKey,
        ...
        switch (mode) {
          ...
          case LIGHT_SHEET:
            props_.setPropValue(
                devKey, Properties.Keys.TRIGGER_MODE, Properties.Values.EXTERNAL_LC);
            props_.setPropValue(devKey, Properties.Keys.ANDOR_OVERLAP, Properties.Values.OFF);
            break;
          case INTERNAL:
            props_.setPropValue(
                devKey, Properties.Keys.TRIGGER_MODE, Properties.Values.INTERNAL_ANDOR);
            break;
          case LEVEL:
            props_.setPropValue(
                devKey, Properties.Keys.TRIGGER_MODE, Properties.Values.LEVEL_ANDOR);
            props_.setPropValue(devKey, Properties.Keys.ANDOR_OVERLAP, Properties.Values.OFF);
            break;
          case OVERLAP:
            props_.setPropValue(
                devKey, Properties.Keys.TRIGGER_MODE, Properties.Values.LEVEL_ANDOR);
            props_.setPropValue(devKey, Properties.Keys.ANDOR_OVERLAP, Properties.Values.ON);
            break;
          default:
            break;
        }
      ...
      case PVCAM:
        switch (mode) {
          ...
          case LIGHT_SHEET:
            props_.setPropValue(
                devKey, Properties.Keys.TRIGGER_MODE, Properties.Values.EDGE_TRIGGER, true);
            break;
          case INTERNAL:
            props_.setPropValue(
                devKey, Properties.Keys.TRIGGER_MODE, Properties.Values.INTERNAL_TRIGGER, true);
            break;
          default:
            break;
        }
  ...
  /**
   * Utility: calculates the number of rows that need to be read out for camera sensor split
  ...
  private int roiReadoutRowsSplitReadout(Rectangle roi, Rectangle sensor) {
    return Math.min(
        Math.abs(roiVerticalOffset(roi, sensor))
            + roi.height / 2, // if ROI overlaps sensor mid-line
  ...
  private boolean isZyla55(Devices.Keys camKey) {
    return props_
        .getPropValueString(camKey, Properties.Keys.CAMERA_NAME)
        .substring(0, 8)
        .equals("Zyla 5.5");
  }
  ...
  private boolean isEdge55(Devices.Keys camKey) {
    return props_.getPropValueString(camKey, Properties.Keys.CAMERA_TYPE).contains(" 5.5");
  }

  /**
   * Goes to properties and sees if this camera has slow readout enabled (which affects row transfer
   * speed and thus reset/readout time).
   *
   * @param camKey
   * @return
   */
  private boolean isSlowReadout(Devices.Keys camKey) {
    switch (devices_.getMMDeviceLibrary(camKey)) {
      ...
      case ANDORCAM:
        if (isZyla55(camKey)) {
          return props_
              .getPropValueString(camKey, Properties.Keys.PIXEL_READOUT_RATE)
              .substring(0, 3)
              .equals("200");
        } else {
          return props_
  ...
  /**
   * Tries to figure out binning by looking at first character of the camera's binning property of
   * the camera. There is not a uniform binning representation but hopefully this works.
   *
  ...
  /**
   * @return dimension/resolution of sensor. If binning is enabled then it is not reflected here
   *     because reset/readout time don't depend on binning.
   */
  private Rectangle getSensorSize(Devices.Keys camKey) {
    ...
    if (x == 0 || y == 0) {
      MyDialogUtils.showError(
          "Was not able to get sensor size of camera " + devices_.getMMDevice(camKey));
    }
  ...
  public double getRowReadoutTime(Devices.Keys camKey) {
    ...
    MyDialogUtils.showError(
        "Was not able to get per-row readout time of camera " + devices_.getMMDevice(camKey));
    return 1;
  }

  /**
   * Gets an estimate of a specific camera's time between trigger and global exposure, i.e. how long
   * it takes for reset. Will depend on whether we have/use global reset, the ROI size, etc. To
   * first order this is the same as the camera readout time in edge-trigger mode so we utilize that
    ...
    if (camMode == CameraModes.Keys.LIGHT_SHEET) {
      ...
      if (camLibrary == Devices.Libraries.PVCAM
          && props_
              ...
              .equals(Properties.Values.PRIME_95B_CHIPNAME)) {
        int trigToGlobal =
            props_.getPropValueInteger(camKey, Properties.Keys.PVCAM_POST_TIME)
                + props_.getPropValueInteger(camKey, Properties.Keys.PVCAM_READOUT_TIME);
        // it appears as of end-May 2017 that the clearing time is actually rolled into the
        // post-trigger
        //    time despite Photometrics documentation to the contrary
        // String clearMode = props_.getPropValueString(camKey,
        // Properties.Keys.PVCAM_CLEARING_MODE);
        // if (clearMode.equals(Properties.Values.PRE_EXPOSURE.toString())) {
        //   trigToGlobal += props_.getPropValueInteger(camKey,
        // Properties.Keys.PVCAM_CLEARING_TIME);
        // }
      } else {
        // all other cameras
        ...
        switch (camLibrary) {
          case HAMCAM:
            ...
            if (props_
                .getPropValueString(camKey, Properties.Keys.TRIGGER_ACTIVE)
  ...
  /**
   * Gets an estimate of a specific camera's readout time based on ROI and other settings. Returns 0
   * for overlap mode and 0.25ms pseudo-overlap mode (or camera-specific time for Photometrics)
   *
   * @param camKey device key for camera in question
   * @param camMode camera mode
  ...
  public float computeCameraReadoutTime(Devices.Keys camKey, CameraModes.Keys camMode) {
      ...
      case LEVEL:
        ...
        switch (camLibrary) {
          case HAMCAM:
            ...
            if (props_
                .getPropValueString(camKey, Properties.Keys.SENSOR_MODE)
          ...
          case PVCAM:
            int endGlobalToTrig =
                props_.getPropValueInteger(camKey, Properties.Keys.PVCAM_PRE_TIME)
                    + props_.getPropValueInteger(camKey, Properties.Keys.PVCAM_READOUT_TIME);
    ...
    ReportingUtils.logDebugMessage(
        "camera readout time computed as "
            + readoutTimeMs
            + " for camera "
            + devices_.getMMDevice(camKey));
  ...
  public void setCameraForAcquisition(Devices.Keys camKey, boolean acq) {
    if (acq) {
      ...
      //        int shutterSpeed = props_.getPropValueInteger(Devices.Keys.PLUGIN,
      // Properties.Keys.PLUGIN_LS_SHUTTER_SPEED);
      //    }
      //
  ...
  /**
   * Gets the camera ROI in actual pixels used (i.e. if binning is 4x with 2k sensor return 2k
   * instead of 512). This is because reset and readout times depend on actual pixels and only data
   * transfer time (usually less then readout time) is affected by binning (at least for Flash4v2
    ...
    try {
      ...
      if (binning > 1) {
        return new Rectangle(
            (roi.x < 0
                ? 0
                : roi.x * binning), // make sure isn't negative, some cameras seem to do this
            (roi.y < 0
                ? 0
                : roi.y * binning), // make sure isn't negative, some cameras seem to do this
            roi.width * binning,
            roi.height * binning);
      } else {
        return roi;
