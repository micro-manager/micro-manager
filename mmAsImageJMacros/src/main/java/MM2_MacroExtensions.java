...
public class MM2_MacroExtensions implements PlugIn, MacroExtension {
  ...
  private final ExtensionDescriptor[] extensions = {
    ExtensionDescriptor.newDescriptor("snap", this),
    ...
    ExtensionDescriptor.newDescriptor(
        "getDeviceProperties", this, ARG_STRING, ARG_STRING + ARG_OUTPUT),
    ExtensionDescriptor.newDescriptor(
        "setDeviceProperty", this, ARG_STRING, ARG_STRING, ARG_STRING),
  };
      ...
      case "getConfigGroups":
        {
          ...
          try {
            ((String[]) args[0])[0] =
                String.join(";", studio.core().getAvailableConfigGroups().toArray());
          } catch (Exception e) {
            e.printStackTrace();
          }
          break;
