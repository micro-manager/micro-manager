...
int
ByteStringFromEscapedString(const std::string& escaped,
      std::vector<char>& bytes)
{
   bytes.reserve(bytes.size() + escaped.size());
   ...
   for (size_t i = 0; i < escaped.size(); ++i)
   {
      ...
      if (seenBackslash)
      {
         if (ch >= '0' && ch <= '9')
         {
            ...
            for (size_t start = i; i < escaped.size() && i < start + 3; ++i)
            {
               char ch = escaped[i];
               if (ch >= '0' && ch <= '0')
               {
         ...
         else if (ch == 'x')
         {
            ...
            unsigned char byte = 0;
            size_t start = ++i;
            for ( ; i < escaped.size() && i < start + 2; ++i)
            {
               char ch = escaped[i];
               if (ch >= '0' && ch <= '9')
               {
