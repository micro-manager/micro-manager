...
//  Purpose:
//
//    GLOMIN seeks a global minimum of a function DoubleFunctionOfDouble(X) in an interval [A,B].
//
//  Discussion:
//
//    This function assumes that DoubleFunctionOfDouble(X) is twice continuously differentiable
//    over [A,B] and that DoubleFunctionOfDouble''(X) <= M for all X in [A,B].
//
//  Licensing:
//
...
//    absolute error in the evaluation of DoubleFunctionOfDouble(X) for any X in [A,B].
//
//    Input, double T, a positive error tolerance.
//
//    Input, double DoubleFunctionOfDouble ( double x ), the name of a user-supplied
//    function whose global minimum is being sought.
//
//    Output, double *X, the estimated value of the abscissa
//    for which DoubleFunctionOfDouble attains its global minimum value in [A,B].
//
//    Output, double GLOMIN, the value DoubleFunctionOfDouble(X).
//
{
  double a0;
  double a2;
  ...
  *x = a0;
  a2 = a;
  y0 = DoubleFunctionOfDouble ( b );
  yb = y0;
  y2 = DoubleFunctionOfDouble ( a );
  y = y2;
...
double local_min ( double a, double b, double eps, double t, double *x )

//****************************************************************************80
//
//  Purpose:
//
//    LOCAL_MIN seeks a local minimum of a function DoubleFunctionOfDouble(X) in an interval [A,B].
//
//  Discussion:
//
//    The method used is a combination of golden section search and
//    successive parabolic interpolation.  Convergence is never much slower
//    than that for a Fibonacci search.  If DoubleFunctionOfDouble has a continuous second
...
//    DoubleFunctionOfDouble is never evaluated at two points closer than TOL.  
//
//    If DoubleFunctionOfDouble is a unimodal function and the computed values of DoubleFunctionOfDouble are always
//    unimodal when separated by at least SQEPS * abs ( X ) + (T/3), then
//    LOCAL_MIN approximates the abscissa of the global minimum of DoubleFunctionOfDouble on the 
//    interval [A,B] with an error less than 3*SQEPS*abs(LOCAL_MIN)+T.  
//
//    If DoubleFunctionOfDouble is not unimodal, then LOCAL_MIN may approximate a local, but 
...
//    for which DoubleFunctionOfDouble attains a local minimum value in [A,B].
//
//    Output, double LOCAL_MIN, the value DoubleFunctionOfDouble(X).
//
{
  double c;
  double d;
...
//
      ...
      u = *x + d;
//
//  DoubleFunctionOfDouble must not be evaluated too close to A or B.
//
      if ( ( u - sa ) < t2 || ( sb - u ) < t2 )
      {
        if ( *x < m )
...
//
//  DoubleFunctionOfDouble must not be evaluated too close to X.
//
    if ( tol <= r8_abs ( d ) )
    {
      u = *x + d;
...
double local_min_rc ( double *a, double *b, int *status, double value, double tol )
...
//    specifed by the caller attains a minimum on the interval (A,B).
//
//    The method used is a combination of golden section search and
//    successive parabolic interpolation.  Convergence is never much
//    slower than that for a Fibonacci search.  If DoubleFunctionOfDouble has a continuous
...
//
//  Request value of DoubleFunctionOfDouble(U).
//
  arg = u;
  *status = *status + 1;
...
//  Purpose:
//
//    ZERO seeks the root of a function DoubleFunctionOfDouble(X) in an interval [A,B].
//
//  Discussion:
//
//    The interval [A,B] must be a change of sign interval for DoubleFunctionOfDouble.
//    That is, DoubleFunctionOfDouble(A) and DoubleFunctionOfDouble(B) must be of opposite signs.  Then
//    assuming that DoubleFunctionOfDouble is continuous implies the existence of at least
//    one value C between A and B for which DoubleFunctionOfDouble(C) = 0.
//
...
//    Input, double T, a positive error tolerance.
//
//    Input, member DoubleFunctionOfDouble

//
//    Output, double ZERO, the estimated value of a zero of
//    the function DoubleFunctionOfDouble.
//
{
  double c;
  double d;
...
//
  sa = a;
  sb = b;
  fa = DoubleFunctionOfDouble ( sa );
  fb = DoubleFunctionOfDouble ( sb );
...
//  Purpose:
//
//    ZERO_RC seeks the root of a user-specified function in an interval [A,B].
//
//  Discussion:
//
//    The interval [A,B] must be a change of sign interval 
...
//
//  Input STATUS = 1.
//  Receive F(A), request value at (B).
//
  else if ( *status == 1 )
  {
    fa = value;
