...
#pragma comment(lib, "CamUSB_API.lib")	// add ABS Camera support
...
// to load particular device from the library
const char* g_CameraDeviceNameBase = "ABSCam";
#define ThisCameraType ABSCamera
...
MODULE_API void InitializeModuleData()
{
   ...
   __try 
   {
      ...
      else
      {
         ...
         snprintf( szCameraDescription, MAX_PATH, "%s", "No ABS camera device connected!");
   ...
   __except (DelayLoadDllExceptionFilter(GetExceptionInformation())) 
   {
      ...
      snprintf( szDeviveName,		   MAX_PATH, "%s", g_CameraDeviceNameBase);
      snprintf( szCameraDescription, MAX_PATH, "%s", "CamUSB_API.dll not found or wrong version! Device won't work!");
...
int ABSCamera::createPixelType()
{
   ...
   pixelTypeHW = (bColor) ? g_sSupportedPixelTypesColor[0].szPixelTypeName : g_sSupportedPixelTypesMono[0].szPixelTypeName;
...
int ABSCamera::UpdatePixelTypes(void)
{
   ...
   for ( unsigned int p=0; p < this->pixelTypeCap->dwCount; p++ )
   {
      if (bColor)
      {
         for ( u32 n=0; n < CNT_ELEMENTS(g_sSupportedPixelTypesColor); n++ )
         {
            if( pixelTypeCap->dwPixelType[p] == g_sSupportedPixelTypesColor[n].dwPixelType )
            {
               if (nUsedBpp == GetUsedBpp(g_sSupportedPixelTypesColor[n].dwPixelType))
               {
                  pixelTypeHW = g_sSupportedPixelTypesColor[n].szPixelTypeName;
                  pixelType   = g_sSupportedPixelTypesColor[n].dwPixelType;
                  bPixelTypeAssigned = true;
               }
               pixelTypeValues.push_back( g_sSupportedPixelTypesColor[n].szPixelTypeName );             
               break;
            }
         }
      }
      ...
      {
         for ( u32 n=0; n < CNT_ELEMENTS(g_sSupportedPixelTypesMono); n++ )
         {
            if( pixelTypeCap->dwPixelType[p] == g_sSupportedPixelTypesMono[n].dwPixelType )
            {
               if (nUsedBpp == GetUsedBpp(g_sSupportedPixelTypesMono[n].dwPixelType))
               {
                  pixelTypeHW = g_sSupportedPixelTypesMono[n].szPixelTypeName;
                  pixelType   = g_sSupportedPixelTypesMono[n].dwPixelType;
                  bPixelTypeAssigned = true;
               }
               pixelTypeValues.push_back( g_sSupportedPixelTypesMono[n].szPixelTypeName );             
               break;
            }
         }
      }
   ...
   if (!bPixelTypeAssigned)
   {
      if (bColor)
      {
         pixelTypeHW = g_sSupportedPixelTypesColor[0].szPixelTypeName;
         pixelType   = g_sSupportedPixelTypesColor[0].dwPixelType;
      }
      else
      {
         pixelTypeHW = g_sSupportedPixelTypesMono[0].szPixelTypeName;
         pixelType   = g_sSupportedPixelTypesMono[0].dwPixelType;
      }
   }

   nRet = SetPixelType(pixelType);
   nRet = SetProperty(MM::g_Keyword_PixelType, pixelTypeHW.c_str()); // set allowed pixel types
...
int ABSCamera::createColorMode()
{
   ...
   if ( this->resolutionCap != NULL )
   {
      ...
      std::string strDefaultColorMode = colorValues.at( 0 );
      // create property
      nRet = CreateProperty(MM::g_Keyword_ColorMode, (char*)strDefaultColorMode.c_str(), MM::String, false, pAct);
      if (nRet == DEVICE_OK)
...
int ABSCamera::OnPixelType(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   ...
   if (eAct == MM::AfterSet) // property was written -> apply value to hardware
   {
      ...
      if (bColor)
      {
         for ( u32 n=0; n < CNT_ELEMENTS(g_sSupportedPixelTypesColor); n++ )
         {
            if ( pixelType.compare( g_sSupportedPixelTypesColor[n].szPixelTypeName ) == 0 ) 
            {
               dwPixelType = g_sSupportedPixelTypesColor[n].dwPixelType;
               break;
            }
         }
      }
      else
      {
         for ( u32 n=0; n < CNT_ELEMENTS(g_sSupportedPixelTypesMono); n++ )
         {
            if ( pixelType.compare( g_sSupportedPixelTypesMono[n].szPixelTypeName ) == 0 ) 
            {
               dwPixelType = g_sSupportedPixelTypesMono[n].dwPixelType;
               break;
            }
         }
      }
   ...
   else if (eAct == MM::BeforeGet) // property will be read -> update property with value from hardware
   {
      ...
      for ( u32 n=0; n < CNT_ELEMENTS(g_sSupportedPixelTypesMono); n++ )
      {
         if( pixType == g_sSupportedPixelTypesMono[n].dwPixelType )
         {
            pixelTypeHW = g_sSupportedPixelTypesMono[n].szPixelTypeName;          
            bFound = true;
            break;
      ...
      // pixel type found in mono pixel types
      if (bFound)  
      {
         if (bColor) // => change ColorMode to mono !!!
         {
            bColor = false;
            UpdatePixelTypes();
         }
      ...
      if (!bFound)
      {
         for ( u32 n=0; n < CNT_ELEMENTS(g_sSupportedPixelTypesColor); n++ )
         {
            if( pixType == g_sSupportedPixelTypesColor[n].dwPixelType )
            {
               pixelTypeHW = g_sSupportedPixelTypesColor[n].szPixelTypeName;          
               bFound = true;
               break;
      ...
      else // pixeltype found set it at gui
      {
