...
long QICamera::GetImageBufferSize() const
{
    ...
    long byteDepth = (m_bitDepth > 8) ? 2 : 1;
    long size = m_imageWidth * m_imageHeight * byteDepth;
    return size;
}
...
{
    return StartSequenceAcquisition(m_sthd->GetRemaining(), m_interval, stopOnOverflow_);
}
