...
{   
    ...
    nRet = CreateProperty(g_PropertySkyraPowerSetpoint, powerSetPoint_.c_str(),  MM::Float, true, pAct);
    if (DEVICE_OK != nRet)
        return nRet;
   ...
   double powerMaximum = std::atof(powerMaximum_.c_str());
    ...
    CreateProperty("Power Help", g_PropertySkyraPowerHelpOn, MM::String, true);
    ...
    nRet = CreateProperty(g_PropertySkyraCurrentSetpoint, g_Default_Float, MM::String, true, pAct);
    if (DEVICE_OK != nRet)
        return nRet;
    ...
    CreateProperty("Current Help", g_PropertySkyraCurrentHelpOn, MM::String, true);
    ...
    nRet = CreateProperty(g_PropertySkyraControlMode, controlMode_.c_str(), MM::String, false, pAct);
    if (nRet != DEVICE_OK)
        return nRet;

    commands.clear();
    commands.push_back(g_PropertySkyraControlModePower);
    commands.push_back(g_PropertySkyraControlModeCurrent);
    if (bModulation_ == true)  commands.push_back(g_PropertySkyraControlModeModulation);
    SetAllowedValues(g_PropertySkyraControlMode, commands);  
...
int Skyra::OnSerialCommand(MM::PropertyBase* pProp , MM::ActionType eAct)
{
    if (eAct == MM::AfterSet)
    {
        std::string answer;
...
int Skyra::OnSerialCommandResponse(MM::PropertyBase* pProp , MM::ActionType eAct)
{
    if (eAct == MM::BeforeGet)
    {
...
int Skyra::OnControlMode(MM::PropertyBase* pProp, MM::ActionType  eAct)
{
    ...
    else if (eAct == MM::AfterSet)
    {
        ...
        if (controlMode_.compare(answer) != 0) {
            controlMode_ = answer;
            if (controlMode_.compare(g_PropertySkyraControlModePower) == 0) {
                SerialCommand(ID_ + "cp");
                SetProperty(g_PropertySkyraModulationStatus, g_PropertyDisabled);
            }
            else if (controlMode_.compare(g_PropertySkyraControlModeCurrent) == 0) {
                SerialCommand(ID_ + "ci");
                SetProperty(g_PropertySkyraModulationStatus, g_PropertyDisabled);
            }
            if (controlMode_.compare(g_PropertySkyraControlModeModulation) == 0) {
...
int Skyra::OnPower(MM::PropertyBase* pProp, MM::ActionType  eAct)
{
    ...
    else if (eAct == MM::AfterSet)
    {
        ...
        if (controlMode_.compare(g_PropertySkyraControlModePower)  != 0) 
            SetProperty(g_PropertySkyraControlMode,g_PropertySkyraControlModePower);
        ...
        if (eAct == MM::AfterSet)
        {
            ...
            if (current <= std::atof(currentMaximum_.c_str())) {
               ...
               Current_ = strs.str();
                ...
                SetProperty(g_PropertySkyraControlMode,g_PropertySkyraControlModeCurrent);
                SetProperty(g_PropertySkyraModulationStatus, g_PropertyDisabled);
        ...
        if (answer.at(0) == '0')
            ...
            if (nSkyra_ > 0 ) {
                ...
                if (answer.at(0) == '3')
                    ...
                    operatingStatus_ = g_PropertySkyraControlModeModulation;
                    SetProperty(g_PropertySkyraControlMode,operatingStatus_.c_str());
                }
            }
        }
...
int Skyra::OnLaserHelp1(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    if (eAct == MM::BeforeGet)
    {
        if (laserStatus_.compare(g_PropertyOn) == 0) pProp->Set(g_PropertySkyraAutostartHelp3); 
        else if (laserStatus_.compare(g_PropertyOff) == 0) pProp->Set(g_PropertySkyraAutostartHelp1); 
    }
...
{	
    if (eAct == MM::BeforeGet)
    {
        if (laserStatus_.compare(g_PropertyOn) == 0) pProp->Set(g_PropertySkyraAutostartHelp4); 
        else if (laserStatus_.compare(g_PropertyOff) == 0) pProp->Set(g_PropertySkyraAutostartHelp2); 
    }
...
std::string Skyra::UpdateWaveLength(std::string id) 
{
    ...
    // RASP
    SetProperty(g_PropertySkyraCurrentSetpoint,currentSetPoint_.c_str());
    ...
    SetProperty(g_PropertySkyraPowerSetpoint,powerSetPoint_.c_str());
    ...
    // Control Mode
    SetProperty(g_PropertySkyraControlMode, controlMode_.c_str());
...
std::string Skyra::GetReadAll(std::string laserID) {
    ...
    if (!RA_.empty()) {	
        ...
        pch = strtok (NULL, " ");
        if (pch != NULL) {
            if (pch[0] == '0') controlMode_ = g_PropertySkyraControlModeCurrent;
            if (pch[0] == '1')  controlMode_= g_PropertySkyraControlModePower;
            if (pch[0] == '2')  controlMode_ = g_PropertySkyraControlModeModulation;
...
int Skyra::SetOpen(bool open)
{
    ...
    if (autostartStatus_.compare(g_PropertyDisabled)== 0) AllLasersOn((int) open);
        ...
        if (controlMode_.compare(g_PropertySkyraControlModeCurrent) !=0) { 
            SetProperty(g_PropertySkyraControlMode,g_PropertySkyraControlModeCurrent);
        }
...
std::string Skyra::SerialCommand (std::string serialCommand) {
    ...
    if (answer.compare ("Syntax error: only allowed when operating mode is \'Completed\'.") == 0) {
        SetProperty(g_PropertySkyraOperatingStatus,"Aborted: Complete Laser Start Required");
    }
