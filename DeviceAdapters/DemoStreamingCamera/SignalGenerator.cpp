...
DemoSignalGenerator::DemoSignalGenerator() : 
 ...
 */
void DemoSignalGenerator::GetName(char* name) const
{
 ...
 * Required by the MM::Device API.
 */
int DemoSignalGenerator::Initialize()
{
   if (initialized_)
      return DEVICE_OK;
   ...
   // stdDev
   CPropertyAction *pAct = new CPropertyAction (this, &DemoSignalGenerator::OnOutputDevice);
 ...
 * Required by the MM::Device API.
 */
int DemoSignalGenerator::Shutdown()
{
   initialized_ = false;
   return DEVICE_OK;
}

int DemoSignalGenerator::Process(unsigned char* buffer, unsigned width, unsigned height, unsigned byteDepth)
{
...
int DemoSignalGenerator::OnOutputDevice(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::AfterSet)
   {
