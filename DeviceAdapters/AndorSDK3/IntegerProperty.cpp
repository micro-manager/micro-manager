...
TIntegerProperty::TIntegerProperty(const std::string MM_name,
                                   IInteger* integer_feature,
                                   CAndorSDK3Camera* camera,
                                   MySequenceThread* thd,
                                   SnapShotControl* snapShotController,
                                   bool readOnly,
                                   bool limited) :
MM_name_(MM_name), integer_feature_(integer_feature), camera_(camera), thd_(thd),
...
void TIntegerProperty::Update(ISubject* /*Subject*/)
{
   ...
   if (limited_) {
      camera_->SetPropertyLimits(MM_name_.c_str(),
         (long)integer_feature_->Min(), (long)integer_feature_->Max());
   }
}
