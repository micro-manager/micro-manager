...
int CDECamera::SnapImage()
{
	...
	{
		// status of server camera no longer synced w/ client.  Try syncing the status.
		// Probably don't need the following anymore. Sunny 1/7/2011
		int syncRet = this->ResizeImageBuffer();
		return (DEVICE_OK == syncRet) ? DEVICE_INCONSISTENT_STATE : syncRet;
	}
	catch (const std::exception& e)
	{
		return BoostToMMError(e);
...
void CDECamera::SetupProperty(string label, PropertyHelper settings)
{
	...
	case PropertyHelper::Type::Allow_All:		
		switch (settings.GetProperty()){
		case PropertyHelper::PropertyType::String :
			CreateProperty(label.c_str(), "", MM::String, false, pAct);			
			break;
		case PropertyHelper::PropertyType::Float :
			CreateProperty(label.c_str(), "", MM::Float, false, pAct);			
			break;
		case PropertyHelper::PropertyType::Integer :
			CreateProperty(label.c_str(), "", MM::Integer, false, pAct);			
			break;
		default: 
			return;
		}
	...
	case PropertyHelper::Type::ReadOnly:		
		switch (settings.GetProperty()){
		case PropertyHelper::PropertyType::Float :
			CreateProperty(label.c_str(), "", MM::Float, true, pAct);			
			break;
		case PropertyHelper::PropertyType::String :
			CreateProperty(label.c_str(), "", MM::String, true, pAct);			
			break;
		case PropertyHelper::PropertyType::Integer :
			CreateProperty(label.c_str(), "", MM::Integer, true, pAct);			
			break;
		default: 
			return;
		}		
