...
const char* g_DeviceSiriusBLSUSBName = "Mightex_BLS(USB)";
//const char* g_DeviceSiriusBLSRS232Name = "Mightex_BLS(RS-232)";
...
//	RegisterDevice(g_DeviceSiriusSLCRS232Name, MM::ShutterDevice, "Mightex Sirius SLC LED Driver(RS-232)");
	RegisterDevice(g_DeviceSiriusBLSUSBName, MM::ShutterDevice, "Mightex Sirius BLS Control Module(USB)");
//	RegisterDevice(g_DeviceSiriusBLSRS232Name, MM::ShutterDevice, "Mightex Sirius BLS LED Driver(RS-232)");
}
...
//	}
	else if(strcmp(deviceName, g_DeviceSiriusBLSUSBName) == 0)
	{
		return new Mightex_Sirius_BLS_USB;
	}
//	else if(strcmp(deviceName, g_DeviceSiriusBLSRS232Name) == 0)
//	{
//		return new Mightex_Sirius_BLS_RS232;
//	}
...
bool Mightex_Sirius_SLC_USB::getLastError(int* err)
{
	...
	// add error to list
	dynErrlist_add(errCode + ERR_Mightex_Sirius_LED_OFFSET, errDescr);
	// publish the error code
	if(err) *err = errCode + ERR_Mightex_Sirius_LED_OFFSET;
	// return
	return true;
}
...
/*---------------------------------------------------------------------------
 Default constructor.
---------------------------------------------------------------------------*/
Mightex_Sirius_BLS_USB::Mightex_Sirius_BLS_USB() :
	dev_num(0),
	cur_dev(0),
	devHandle(-1),
	channels(-1),
   ...
   m_initialized(false)
{
	...
	// Name
	CreateProperty(MM::g_Keyword_Name, g_DeviceSiriusBLSUSBName, MM::String, true);

	// Description
	CreateProperty(MM::g_Keyword_Description, "Mightex Sirius BLS Control Module(USB)", MM::String, true);

	CPropertyAction* pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnDevices);
	CreateProperty("Devices", "", MM::String, false, pAct, true);
	...
	int dev_Handle;
	std::string s_devName;
	dev_num = MTUSB_BLSDriverInitDevices();
	for(int i = 0; i < dev_num; i++)
	{
		dev_Handle = MTUSB_BLSDriverOpenDevice(i);
		if(dev_Handle >= 0)
			if(HidGetDeviceName(dev_Handle, devName, sizeof(devName)) > 0)
				if(MTUSB_BLSDriverGetSerialNo(dev_Handle, (BYTE *)serialNum, sizeof(serialNum)) > 0)
				{
					sprintf(ledName, "%s:%s", devName, serialNum);
					AddAllowedValue( "Devices", ledName);
					s_devName = ledName;
		...
		MTUSB_BLSDriverCloseDevice(dev_Handle);
	}
}
...
/*---------------------------------------------------------------------------
 This function returns the device name ("Mightex Sirius BLS(USB)").
---------------------------------------------------------------------------*/
void Mightex_Sirius_BLS_USB::GetName(char* Name) const
{
	CDeviceUtils::CopyLimitedString(Name, g_DeviceSiriusSLCUSBName);
}
...
/*---------------------------------------------------------------------------
 This function initialize a Mightex_Sirius_BLS_USB device and creates the actions.
---------------------------------------------------------------------------*/
int Mightex_Sirius_BLS_USB::Initialize()
{
	...
	if(dev_num > 0)
	{
		devHandle = MTUSB_BLSDriverOpenDevice(cur_dev);
		if(devHandle >= 0)
		{
			channels = MTUSB_BLSDriverGetChannels(devHandle);
			devModuleType = MTUSB_BLSDriverGetModuleType(devHandle);
		}
	}

	// Initialize the Mode action
	CPropertyAction* pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnMode);
	nRet = CreateProperty("mode", "DISABLE", MM::String, false, pAct);
	if (DEVICE_OK != nRet)	return nRet;
	AddAllowedValue( "mode", "DISABLE");
	AddAllowedValue( "mode", "NORMAL");
	...
	pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnChannel);
	nRet = CreateProperty("channel", "1", MM::Integer, false, pAct);
	if (DEVICE_OK != nRet)	return nRet;
	if(channels > -1)
		SetPropertyLimits("channel", 1, channels);
	...
	pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnSetNormalCurrent);
	nRet = CreateProperty("normal_CurrentSet", "50", MM::Integer, false, pAct);
	if (DEVICE_OK != nRet)	return nRet;
	SetPropertyLimits("normal_CurrentSet", 0, 1000);

	pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnSetRepeatCnt);
	nRet = CreateProperty("repeatCnt", "1", MM::Integer, false, pAct);
	if (DEVICE_OK != nRet)	return nRet;

	pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnSetI1);
	nRet = CreateProperty("i1", "0", MM::Integer, false, pAct);
	if (DEVICE_OK != nRet)	return nRet;
	SetPropertyLimits("i1", 0, 1000);

	pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnSetI2);
	nRet = CreateProperty("i2", "50", MM::Integer, false, pAct);
	if (DEVICE_OK != nRet)	return nRet;
	SetPropertyLimits("i2", 0, 1000);

	pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnSetI3);
	nRet = CreateProperty("i3", "0", MM::Integer, false, pAct);
	if (DEVICE_OK != nRet)	return nRet;
	SetPropertyLimits("i3", 0, 1000);

	pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnSetT1);
	nRet = CreateProperty("t1", "500000", MM::Integer, false, pAct);
	if (DEVICE_OK != nRet)	return nRet;

	pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnSetT2);
	nRet = CreateProperty("t2", "500000", MM::Integer, false, pAct);
	if (DEVICE_OK != nRet)	return nRet;

	pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnSetT3);
	nRet = CreateProperty("t3", "500000", MM::Integer, false, pAct);
	if (DEVICE_OK != nRet)	return nRet;

	pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnSetPulse_Follow_Mode);
	...
	pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnSetION);
	nRet = CreateProperty("iON", "50", MM::Integer, false, pAct);
	if (DEVICE_OK != nRet)	return nRet;
	SetPropertyLimits("iON", 0, 1000);

	pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnSetIOFF);
	nRet = CreateProperty("iOFF", "0", MM::Integer, false, pAct);
	if (DEVICE_OK != nRet)	return nRet;
	SetPropertyLimits("iOFF", 0, 1000);

	pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnSetSoftStart);
	...
	// Initialize the status query action
	pAct = new CPropertyAction (this, &Mightex_Sirius_BLS_USB::OnStatus);
	nRet = CreateProperty("Status", "No Fault", MM::String, true, pAct);
	if (DEVICE_OK != nRet)	return nRet;
	...
	// init message
	std::ostringstream log;
	log << "Mightex Sirius BLS(USB) - initializied " << "S/N: " << m_serialNumber;
	LogMessage(log.str().c_str());
...
/*---------------------------------------------------------------------------
 This function sets the LED output to off in case the Mightex_Sirius_BLS_USB was initialized.
---------------------------------------------------------------------------*/
int Mightex_Sirius_BLS_USB::Shutdown()
{
   if (m_initialized)
   {
      LEDOnOff(false);
...
/*---------------------------------------------------------------------------
 This function returns true in case device is busy.
---------------------------------------------------------------------------*/
bool Mightex_Sirius_BLS_USB::Busy()
{
	return false;
}
...
/*---------------------------------------------------------------------------
 This function sets the LED output.
---------------------------------------------------------------------------*/
int Mightex_Sirius_BLS_USB::SetOpen(bool open)
{
	int val = 0;
...
/*---------------------------------------------------------------------------
 This function returns the LED output.
---------------------------------------------------------------------------*/
int Mightex_Sirius_BLS_USB::GetOpen(bool &open)
{
	m_LEDOn = "Undefined";
	int ret = DEVICE_OK;
...
/*---------------------------------------------------------------------------
 This function does nothing but is recommended by the shutter API.
---------------------------------------------------------------------------*/
int Mightex_Sirius_BLS_USB::Fire(double)
{
   return DEVICE_UNSUPPORTED_COMMAND;
}
...
/*---------------------------------------------------------------------------
 This function sets the LED output on or off.
---------------------------------------------------------------------------*/
int Mightex_Sirius_BLS_USB::LEDOnOff(int onoff)
{
	int ret = DEVICE_OK;
	...
	if (onoff == 0)
	{
		m_LEDOn = "Off";
		if(devHandle >= 0)
			MTUSB_BLSDriverSetMode(devHandle, m_channel, 0); //mode is "DISABLE"
	}
	else
	{
		m_LEDOn = "On";
		if(devHandle >= 0)
		{
			MTUSB_BLSDriverSetMode(devHandle, m_channel, mode);
...
int Mightex_Sirius_BLS_USB::OnDevices(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	std::string temp;
	if (eAct == MM::BeforeGet)
	{
...
int Mightex_Sirius_BLS_USB::OnMode(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	if (eAct == MM::BeforeGet)
	{
		pProp->Set(m_mode.c_str());
...
int Mightex_Sirius_BLS_USB::OnChannel(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	if (eAct == MM::BeforeGet)
	{
		pProp->Set(m_channel);
...
int Mightex_Sirius_BLS_USB::OnSetNormalCurrent(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	long temp;
	if (eAct == MM::BeforeGet)
	{
	...
	else if (eAct == MM::AfterSet)
	{
		pProp->Get(temp);
		if(devHandle >= 0)
			if(MTUSB_BLSDriverSetNormalCurrent(devHandle, m_channel, temp) == 0)
...
int Mightex_Sirius_BLS_USB::OnSetRepeatCnt(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	if (eAct == MM::BeforeGet)
	{
...
int Mightex_Sirius_BLS_USB::OnSetI1(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	long temp;
	if (eAct == MM::BeforeGet)
	{
...
int Mightex_Sirius_BLS_USB::OnSetI2(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	long temp;
	if (eAct == MM::BeforeGet)
	{
...
int Mightex_Sirius_BLS_USB::OnSetI3(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	long temp;
	if (eAct == MM::BeforeGet)
	{
...
int Mightex_Sirius_BLS_USB::OnSetT1(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	long temp;
	if (eAct == MM::BeforeGet)
	{
...
int Mightex_Sirius_BLS_USB::OnSetT2(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	long temp;
	if (eAct == MM::BeforeGet)
	{
...
int Mightex_Sirius_BLS_USB::OnSetT3(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	long temp;
	if (eAct == MM::BeforeGet)
	{
...
int Mightex_Sirius_BLS_USB::OnSetPulse_Follow_Mode(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	std::string temp;
	if (eAct == MM::BeforeGet)
	{
...
int Mightex_Sirius_BLS_USB::OnSetION(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	if (eAct == MM::BeforeGet)
	{
		pProp->Set(m_iON);
...
int Mightex_Sirius_BLS_USB::OnSetIOFF(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	if (eAct == MM::BeforeGet)
	{
		pProp->Set(m_iOFF);
...
int Mightex_Sirius_BLS_USB::OnSetSoftStart(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	std::string temp;
	if (eAct == MM::BeforeGet)
	{
...
/*---------------------------------------------------------------------------
 This function is the callback function for the "status" query.
---------------------------------------------------------------------------*/
int Mightex_Sirius_BLS_USB::OnStatus(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	int ret = DEVICE_OK;
...
/*---------------------------------------------------------------------------
 This function clears the dynamic error list.
---------------------------------------------------------------------------*/
bool Mightex_Sirius_BLS_USB::dynErrlist_free(void)
{
	for(unsigned int i = 0; i < m_dynErrlist.size(); i++)
	{
		delete m_dynErrlist[i];
...
/*---------------------------------------------------------------------------
 ...
 found. FALSE(0) otherwise.
---------------------------------------------------------------------------*/
bool Mightex_Sirius_BLS_USB::dynErrlist_lookup(int err, std::string* descr)
{
	for(unsigned int i = 0; i < m_dynErrlist.size(); i++)
	{
		if(m_dynErrlist[i]->err == err)
...
/*---------------------------------------------------------------------------
 ...
 is already known, it returns true.
---------------------------------------------------------------------------*/
bool Mightex_Sirius_BLS_USB::dynErrlist_add(int err, std::string descr)
{
	// when the error is already in list return true
	if((dynErrlist_lookup(err, NULL)) == true)	return true;
...
---------------------------------------------------------------------------*/
bool Mightex_Sirius_BLS_USB::getLastError(int* err)
{
	std::string errDescr = "";
	int errCode = 0;
...
/*---------------------------------------------------------------------------
 This function creates the static read only properties.
---------------------------------------------------------------------------*/
int Mightex_Sirius_BLS_USB::CreateStaticReadOnlyProperties(void)
{
	int nRet = DEVICE_OK;
	char serialNum[32] = "";
	...
	if(devHandle >= 0)
		MTUSB_BLSDriverGetSerialNo(devHandle, (BYTE *)serialNum, sizeof(serialNum));
	m_serialNumber = serialNum;
	nRet = CreateProperty("Serial Number", m_serialNumber.c_str(), MM::String, true);
	if (DEVICE_OK != nRet)	return nRet;
