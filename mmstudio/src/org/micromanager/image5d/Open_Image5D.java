    ...
    public void run(String arg) {
        ...
        if (name!=null) {
            ...
            if (imp!=null) {                
                ...
                // sort image data into Image5D
                Image5D i5d = new Image5D(title, type, width, height, nChannels, nSlices, nFrames, false);       
                for (int c=0; c<nChannels; ++c) {
                    for (int z=0; z<nSlices; ++z) {
                        for (int t=0; t<nFrames; ++t) {
                            i5d.setCurrentPosition(0,0,c, z, t);
                            imp.setSlice(t*(nSlices*nChannels)+z*nChannels+c+1);
                            // Hand over references to pixel arrays
                            i5d.setPixels(imp.getProcessor().getPixels());                            
                            // Copy slice labels
                            i5d.getStack().setSliceLabel(imp.getStack().getSliceLabel(z+1), z+1);
                        }
                    }
                }               
