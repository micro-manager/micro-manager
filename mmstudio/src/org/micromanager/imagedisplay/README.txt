...
ImagePlus is backed by an ImageStack, which provides image data; we provide an
MMVirtualStack for this.  When told to draw, the ImagePlus will request images
using a flat index, which is translated into Z, Time, and Channel coordinates
(ImageJ doesn't know about any other kind of coordinate axis). The
MMVirtualStack must then request the appropriate image from the Datastore.
ImageJ does not react well to no such image being available, which is
problematic because it can easily request images that don't actually exist yet
-- in which case we are forced to make something up, or else risk irrevocably
breaking our display. See MMVirtualStack for comments on how we handle this.
