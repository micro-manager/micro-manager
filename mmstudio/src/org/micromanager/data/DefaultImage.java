 ...
 * ImageCoords instance).
 */
public class DefaultImage extends org.micromanager.api.data.Image {
   ...
   public DefaultImage(Object pixels, int width, int height, int bytesPerPixel) 
      ...
      metadata_ = new Metadata.MetadataBuilder().build();
      coords_ = new ImageCoords();
   ...
   private ImgPlus generateImgPlusFromPixels(Object pixels, int width, 
      ...
      if (pixels instanceof short[]) {
         ...
         // Assume grayscale.
         pixels_ = new ImgPlus<UnsignedShortType>(
               ArrayImgs.unsignedShorts((short[]) pixels, dims));
      }
         ...
         if (bytesPerPixel == 4) {
            ...
            pixels_ = new ImgPlus<ARGBType>(
                  ArrayImgs.argbs((int[]) pixels, dims));
         }
         else { // Grayscale image
            pixels_ = new ImgPlus<UnsignedByteType>(
                  ArrayImgs.unsignedBytes((byte[]) pixels, dims));
         }
      }
      else {
