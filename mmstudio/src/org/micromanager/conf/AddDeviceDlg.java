      ...
      for (int idd = 0; idd < allDevs_.length; ++idd) {
         ...
         if (0 != thisLibrary.compareTo(allDevs_[idd].getLibrary())) {
            ...
            node = new TreeNodeShowsDeviceAndDescription(
                  allDevs_[idd].getLibrary());
            root.add(node);
            thisLibrary = allDevs_[idd].getLibrary(); // remember which library
                                                      // we are processing
         }
         Object[] userObject = { allDevs_[idd].getLibrary(),
               allDevs_[idd].getAdapterName(), allDevs_[idd].getDescription() };
