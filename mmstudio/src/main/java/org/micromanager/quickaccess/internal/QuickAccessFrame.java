...
public final class QuickAccessFrame extends JFrame {
  ...
  public QuickAccessFrame(Studio studio, JSONObject config) {
    ...
    configureButton_ = new JToggleButton(IconLoader.getIcon("/org/micromanager/icons/gear.png"));
    configureButton_.setToolTipText("Open the configuration UI for this panel");
    configureButton_.addActionListener(
        new ActionListener() {
          @Override
    ...
    configurePanel_ = new ConfigurationPanel(config.optString("title", DEFAULT_TITLE));
  ...
  private void loadConfig(JSONObject config) {
    if (config.length() == 0) {
      ...
      setTitle(
          ((DefaultQuickAccessManager) studio_.quickAccess()).getUniqueTitle(this, DEFAULT_TITLE));
    ...
    try {
      ...
      setTitle(
          ((DefaultQuickAccessManager) studio_.quickAccess())
              .getUniqueTitle(this, config.optString("title", DEFAULT_TITLE)));
      configurePanel_.updateSpinners(numCols_, numRows_);
      JSONArray cells = config.getJSONArray("cells");
      for (int i = 0; i < cells.length(); ++i) {
        addControl(ControlCell.fromJSON(cells.getJSONObject(i), studio_, this));
      }
      ...
      if (openMode.equals(OPEN_ALWAYS) || (wasOpen && openMode.equals(OPEN_REMEMBER))) {
        setVisible(true);
      }
  ...
  public void paint(Graphics g) {
    ...
    if (draggedIcon_ != null) {
      ...
      g.drawImage(draggedIcon_.getIcon().getImage(), p.x - iconOffsetX_, p.y - iconOffsetY_, null);
    }
  }

  /**
   * Start dragging the specified DraggableIcon, which was clicked on according to the provided
   * MouseEvent.
   *
  ...
  /**
   * Drop the specified plugin into the controlsPanel_ at the current mouse location, creating a new
   * control (and configuring it if necessary).
   *
  ...
  public void dropPlugin(QuickAccessPlugin plugin) {
    ...
    if (plugin != null) {
      ...
      try {
        ...
        ControlCell cell = new ControlCell(studio_, this, plugin, config, rect);
        addControl(cell);
      } catch (Exception e) {
  ...
  /**
   * Add a control to the grid of controls at the specified location and dimensionality. This
   * requires removing and then re-adding all components because the size of the MigLayout entity
   * may have changed.
   */
  ...
  /**
   * Return true if there is room for a control at the desired rect. Optionally include a control
   * being moved, so we don't have to worry about self-intersections.
   */
  public boolean canFitRect(Rectangle rect, ControlCell source) {
    if (rect.x < 0
        || rect.x + rect.width > numCols_
        || rect.y < 0
        || rect.y + rect.height > numRows_) {
  ...
  private void setGridSize(int cols, int rows) {
    ...
    for (ControlCell control : controls_) {
      ...
      boolean isVisible = (r.x + r.width <= numCols_ && r.y + r.height <= numRows_);
  ...
  /**
   * Map the current mouse position (relative to the window) into a cell location (in the
   * controlsPanel_). Returns null if the location is not valid.
   */
  ...
  private String updateTitle(String title) {
    String newTitle =
        ((DefaultQuickAccessManager) studio_.quickAccess()).getUniqueTitle(this, title);
    setTitle(newTitle);
  ...
  /**
   * These panels have a fixed size based on the number of rows/columns. Depending on the boolean
   * isConfigurePanel_, they may also draw a semitransparent grid over their contents (which in turn
   * are expected to be DraggableIcons instead of normal controls).
   */
  private class GridPanel extends JPanel {
    ...
    public GridPanel(boolean isConfigurePanel) {
      super(new SparseGridLayout(QuickAccessPlugin.CELL_WIDTH, QuickAccessPlugin.CELL_HEIGHT));
    ...
    /**
     * Return the Divider that is closest to the mouse event, or null if no divider is close enough.
     */
    private Divider getDivider(MouseEvent e) {
      ...
      if (columnDist < rowDist) {
        // Horizontal divider.
        return new Divider(new Point(column, row), new Point(column, row + 1));
      } else {
        // Vertical divider.
        return new Divider(new Point(column, row), new Point(column + 1, row));
      }
    }
    ...
    public void paint(Graphics g) {
      ...
      g.fillRect(0, 0, numCols_ * cellWidth, numRows_ * cellHeight);
      if (draggedIcon_ != null) {
        ...
        if (p != null) {
          Rectangle rect =
              new Rectangle(p.x, p.y, draggedIcon_.getSize().width, draggedIcon_.getSize().height);
      ...
      for (int i = 0; i < numCols_ + 1; ++i) {
        g.drawLine(i * cellWidth, 0, i * cellWidth, cellHeight * numRows_);
      }
      ...
      if (curDivider_ != null) {
        g.setColor(dividers_.contains(curDivider_) ? Color.RED : Color.BLUE);
        ((Graphics2D) g).setStroke(new BasicStroke(4));
        curDivider_.paint(g);
        ((Graphics2D) g).setStroke(new BasicStroke(1));
      }
    ...
    public Dimension getPreferredSize() {
      return new Dimension(
          numCols_ * QuickAccessPlugin.CELL_WIDTH, numRows_ * QuickAccessPlugin.CELL_HEIGHT);
    }
  }

  /** This class exposes a GUI for adding and removing controls from the main window. */
  private class ConfigurationPanel extends JPanel {
    ...
    /**
     * We take the title as a parameter because, at the time that this constructor is invoked, we
     * haven't loaded the correct title for the window yet, so getTitle() returns the wrong value.
     * And anyway, getTitle() includes any modifications from the getUniqueTitle call.
     */
    public ConfigurationPanel(String title) {
      ...
      colsControl_ = new JSpinner(new SpinnerNumberModel(numCols_, 1, 99, 1));
      subPanel.add(colsControl_);
      colsControl_.addChangeListener(
          new ChangeListener() {
            @Override
      ...
      rowsControl_ = new JSpinner(new SpinnerNumberModel(numCols_, 1, 99, 1));
      subPanel.add(rowsControl_, "wrap");
      rowsControl_.addChangeListener(
          new ChangeListener() {
            @Override
            public void stateChanged(ChangeEvent e) {
              updateSize();
      ...
      openSelect_ = new JComboBox(new String[] {OPEN_NEVER, OPEN_ALWAYS, OPEN_REMEMBER});
      ...
      subPanel.add(
          new JLabel(
              "<html>Drag controls into the grid above to add them to the panel.<br>Click on grid lines to add or remove dividers. Right-click<br>on a control in the grid to customize its icon (when possible).</html>"),
          "span, wrap, gaptop 10");

      add(subPanel, "span, wrap");

      JPanel buttonsPanel = new JPanel(new MigLayout("flowx, wrap " + numCols_));
    ...
    /** Receive a new grid size. Used to initialize the GUI when the frame is created. */
  ...
  /**
   * Simple class representing a pair of Points in the grid that have been highlighted to create a
   * visual divider.
   */
  private class Divider {
    ...
    private int clampX(int x) {
      return Math.max(2, Math.min(numCols_ * QuickAccessPlugin.CELL_WIDTH - 2, x));
    }

    /** As above, but for Y. */
    private int clampY(int y) {
      return Math.max(2, Math.min(numRows_ * QuickAccessPlugin.CELL_HEIGHT - 2, y));
    }
    ...
    /**
     * We want any two identical Dividers to hash the same in the dividers_ structure, hence we
    ...
    public boolean equals(Object obj) {
      ...
      return (p1_.x == alt.p1_.x && p1_.y == alt.p1_.y && p2_.x == alt.p2_.x && p2_.y == alt.p2_.y);
    }

    @Override
    public String toString() {
      return String.format("<Divider from (%d, %d) to (%d, %d)>", p1_.x, p1_.y, p2_.x, p2_.y);
    }
  }
