...
/**
 * This class represents an icon that can be dragged around the window, for adding or removing
 * controls.
 */
public final class DraggableIcon extends JLabel {
  /**
   * Paths to images inside the JAR that we can provide as custom icons. Each of these will have
   * "/org/micromanager/icons/" prepended and ".png" postpended when they are actually loaded.
   */
  private static final String[] JAR_ICONS =
      new String[] {
  ...
  /**
   * icon and plugin are both optional. If isReified is true, then we assume this references an
   * instantiated control which can be removed from the controlsPanel_ when dragged out of the grid.
   * Otherwise, we want to create a new control when the icon is dragged into the grid. If icon is
   * null, then we generate an icon from the JComponent.
   *
            ...
            if (parentCell_ != null) {
              ...
              if (p == null) {
                ...
                frame_.removeControl(parentCell_, true);
                  ...
                  new Rectangle(p.x, p.y, getSize().width, getSize().height), parentCell_)) {
  ...
  private Image resizeToFit(Image image) {
    ...
    double scale = Math.min((double) width / maxWidth, (double) height / maxHeight);
    return image.getScaledInstance(
        (int) (width * scale), (int) (height * scale), Image.SCALE_DEFAULT);
  }
  ...
  /**
   * Grid of icons the user can pick from, that we can embed into a popup menu. This is used for
   * customizing icons.
   */
  private class IconGrid extends JPanel implements MenuElement {
    public IconGrid(JPopupMenu menu) {
      ...
      addItem(
          ...
          "Color Swatch...",
          "wrap",
          DefaultQuickAccessManager.createSwatch(Color.RED, 16),
          1,
          new Runnable() {
            @Override
            public void run() {
      ...
      for (int i = 0; i < JAR_ICONS.length; ++i) {
        ...
        String iconPath = "/org/micromanager/icons/" + iconName + ".png";
        addItem(
            menu,
            null,
            "",
    ...
    private void loadIconFromFile() {
      ...
      try {
        ...
        iconJson.put(DefaultQuickAccessManager.ICON_TYPE, DefaultQuickAccessManager.CUSTOM_FILE);
        iconJson.put(DefaultQuickAccessManager.ICON_PATH, imageFile.getAbsolutePath());
        config =
            config
                .copyBuilder()
        ...
        frame_.addControl(new ControlCell(studio_, frame_, plugin_, config, parentCell_.getRect()));
      } catch (IOException e) {
    ...
    private void makeColorSwatch() {
      Color color = JColorChooser.showDialog(frame_, "Choose a color for the icon.", Color.WHITE);
      PropertyMap config = parentCell_.getConfig();
      try {
        JSONObject iconJson = new JSONObject();
        iconJson.put(DefaultQuickAccessManager.ICON_TYPE, DefaultQuickAccessManager.COLOR_SWATCH);
     ...
     *
     * @param name The filename, minus ".png", and not including the "/org/micromanager/icons/" bit
     *     either.
     */
    private void loadJarIcon(String name) {
      ...
      try {
        JSONObject iconJson = new JSONObject();
        iconJson.put(DefaultQuickAccessManager.ICON_TYPE, DefaultQuickAccessManager.JAR_ICON);
