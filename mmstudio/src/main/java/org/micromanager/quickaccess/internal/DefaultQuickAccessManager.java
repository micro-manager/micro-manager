...
public final class DefaultQuickAccessManager implements QuickAccessManager {
  ...
  public void onStartupComplete(StartupCompleteEvent event) {
    try {
      String configStr =
          studio_.profile().getSettings(QuickAccessManager.class).getString(SAVED_CONFIG, null);
      if (configStr == null) {
        // Nothing saved.
        return;
      }
  ...
  public void onShutdownCommencing(InternalShutdownCommencingEvent event) {
    ...
    studio_.profile().getSettings(QuickAccessManager.class).putString(SAVED_CONFIG, config);
  }
  ...
  public Icon getCustomIcon(PropertyMap config, Icon defaultIcon) {
    String configString = config.getString(WidgetPlugin.CUSTOM_ICON_STRING, null);
    ...
    try {
      ...
      if (iconType.equals(CUSTOM_FILE)) {
        ...
        return IconLoader.getIcon("/org/micromanager/icons/" + json.getString(ICON_PATH) + ".png");
      } else {
  ...
  /**
   * Dynamically generate an icon that's a square of the specified color with the specified edge
   * length.
   */
  ...
  public void promptToDelete(JFrame panel) {
    if (!(panel instanceof QuickAccessFrame)) {
      ...
      studio_.logs().logError("Asked to delete JFrame that isn't a QuickAccessFrame.");
    }
  ...
  /**
   * Provides a unique string for the provided panel, based on the provided base title and the
   * titles of the other panels.
   *
