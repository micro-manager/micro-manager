 ...
 *
 * <p>A property map can store string-keyed values of various primitive types, nested property maps,
 * and uniformly typed (ordered) collections of values. Because it is immutable, it can be passed
 * around between objects without worrying about thread safety. Null values cannot be stored.
 *
 * <p>To create a PropertyMap, use {@link PropertyMaps#builder()}.
 *
 * <p>Property maps can be converted to a specific JSON format and back in a type-preserving manner.
 *
 * <p>Property maps are similar in concept to {@link java.util.Properties} but stores typed values
 * rather than just strings, and supports storage of nested property maps.
 *
 ...
 * </code></pre>
 *
 * Note that, for the collections (plural) get methods, the returned value is an array ({@code
 * long[]} if the default value was given as an array or varargs, whereas it is a list ({@code
 * List<long>}) if the default value was given as any collection or iterable. The same pattern
 * applies to {@code boolean}, {@code byte}, {@code short}, {@code int}, {@code long}, {@code
 * float}, and {@code double}. (No {@code char} or {@code void}, this is intentional.)
 *
 * <p><strong>Methods to access primitive types</strong><br>
 *
 * <pre><code>
 ...
 * </code></pre>
 *
 * This differs from the case of primitive types in that the collection (plural) get methods return
 * a {@code List} regardless of the type of the default value. The same pattern applies to {@code
 * String}, {@code Color}, {@code AffineTransform}, {@code Rectangle}, {@code Dimension}, {@code
 * Point}, and nested {@code PropertyMap}.
 *
 * <p><strong>Methods to access enum types</strong><br>
 * Enum values can be stored by automatically converting to {@code String}. See {@link
 * #containsStringForEnum}, {@link #containsStringListForEnumList}, {@link #getStringAsEnum}, and
 * {@link #getStringListAsEnumList}. This is useful for storing multiple-choice settings
 *
 * <p>For how to insert values of the various types, see {@link PropertyMap.Builder}.
 */
public interface PropertyMap {
  /**
   * A value that can be stored in a property map.
   *
   * <p>This is only used for interchange purposes, for example when performing bulk operations on
   * property maps.
   */
   ...
   *
   * <p>See {@link PropertyMaps#builder()} for a usage example.
   */
   ...
   *
   * <p>This method is intended for special code that handles serialization or conversion of
   * property maps. In most cases, code that uses PropertyMap as a container for settings or data
   * should use a fixed, pre-determined type for each key.
   *
   * <p>For object (non-primitive) types, the class is returned (e.g. {@code String.class}, {@code
   * PropertyMap.class}).
   *
   * <p>For primitive types, the primitive type class is returned (e.g. {@code int.class}, {@code
   * float.class}). Note that {@code int.class != Integer.class}.
   *
   * <p>For collection (plural) types, the corresponding array type is returned (e.g. {@code
   * int[].class}, {@code String[].class}).
   *
   * <p>Keys stored as "EnumAsString" return {@code String.class}.
   *
   * @param key
   ...
   *
   * <p>This method may be useful for providing backward compatibility (for example, if a value
   * currently stored as a {@code long} used to be stored as an {@code int}). It should not be used
   * when the type-specific get method can be used.
   *
   * <p>Code that calls this method should be prepared to handle any of the following types
   * correctly: {@code Byte, Short, Integer, Long, Float, Double}.
   *
   * @param key
   * @param aDefault
   * @return
   ...
   *
   * <p>The property map does not record the specific class of enum values. It is the caller's
   * responsibility to specify the correct enum class.
   *
   * <p>If the value stored in the property map is not one of the allowed values for {@code
   * enumType}, {@code aDefault} will be returned.
   *
   * @param <E> the enum class
   * @param key the property key
   * @param enumType the enum class
   * @param aDefault a default value to return if the key is missing or the stored value is an enum
   *     but not a known value for the given enum class.
   ...
   * responsibility to specify the correct enum class.
   *
   * <p>Unless all of the values stored in the property map are allowed values of {@code enumType},
   * {@code defaults} will be returned.
   *
   * @param <E> the enum class
   * @param key the property key
   * @param enumType the enum class
   ...
   *
   * <p>To create a property map back from the JSON representation, see {@link
   ...
   *
   * <p>To create a property map back from the saved file, see {@link PropertyMaps#loadJSON}.
   *
   * @param file the file to write to
   * @param overwrite if false and file exists, don't write and return false
   * @param createBackup if true and overwriting, first rename the existing file by appending "~" to
   *     its name
