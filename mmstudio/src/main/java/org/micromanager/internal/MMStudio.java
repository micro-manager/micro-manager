...
public final class MMStudio implements Studio {
   ...
   *
   * @param startAsImageJPlugin Indicates if we're running from "within" ImageJ, which governs our
   *     behavior when we are closed.
   */
   ...
   * @param startAsImageJPlugin Indicates if we're running from "within" ImageJ, which governs our
   *     behavior when we are closed.
   * @param profileNameAutoStart The name of a user profile. This profile and its most recently used
   *     hardware configuration will be to automatically loaded.
   */
  @SuppressWarnings("LeakingThisInConstructor")
  public MMStudio(boolean startAsImageJPlugin, String profileNameAutoStart) {
    ...
    daytimeNighttimeManager_ =
        DaytimeNighttime.create(
            studio_); // Essential GUI settings in preparation of the intro dialog
    ...
    uiMovesStageManager_ =
        new UiMovesStageManager(this); // used by Snap/Live Manager and StageControlFrame
    ...
    quickAccess_ =
        new DefaultQuickAccessManager(
            studio_); // The tools menu depends on the Quick-Access Manager.
    ...
    events()
        .registerForEvents(
            this); // We need to be subscribed to the global event bus for plugin loading
    ...
    try {
      if (profileNameAutoStart != null) {
        for (Map.Entry<UUID, String> entry :
          ...
          if (name.equals(profileNameAutoStart)) {
            ...
            sysConfigFile_ =
                HardwareConfigurationManager.getRecentlyUsedConfigFilesFromProfile(profile).get(0);
            break;
          }
        }
      ...
      ReportingUtils.showError(
          ex, "Error accessing user profiles"); // TODO We should fall back to virtual profile
    }

    core_.enableDebugLog(
        OptionsDlg.isDebugLoggingEnabled(
            studio_)); // Profile may have been switched in Intro Dialog, so reflect its setting
    ...
    coreCallback_ =
        new CoreEventCallback(
            studio_, acqEngine_); // This entity is a class property to avoid garbage collection.
    ...
    if (sysConfigFile_ != null) { // we do allow running Micro-Manager without a config file!
      if (!loadSystemConfiguration()) {
        ReportingUtils.showErrorOn(
            false); // TODO Do we still need to turn errors off to prevent spurious error messages?
      }
    }

    acquisitionManager_ =
        new DefaultAcquisitionManager(this, acqEngine_, ui_.getAcquisitionWindow());
    ...
    ReportingUtils.showErrorOn(
        true); // Switch error reporting back on TODO See above where it's turned off
    ...
    events()
        .post(new DefaultStartupCompleteEvent()); // Give plugins a chance to initialize their state
  ...
  private void initializeLogging(CMMCore core) {
    ...
    if (settings().getShouldDeleteOldCoreLogs()) {
      LogFileManager.deleteLogFilesDaysOld(settings().getCoreLogLifetimeDays(), logFileName);
    }
  ...
  /**
   * Spawn a new thread to load the acquisition engine jar, because this takes significant time.
   * Measured as ~1.3 seconds.
   */
  private void prepAcquisitionEngine() {
    acquisitionEngine2010LoadingThread_ =
        new Thread("Pipeline Class loading thread") {
          @Override
          public void run() {
            try {
              acquisitionEngine2010Class_ =
                  Class.forName("org.micromanager.internal.AcquisitionEngine2010");
  ...
  protected void changeBinning(String mode) {
    ...
    try {
      ...
      if (core_
          .getProperty(cache().getCameraLabel(), MMCoreJ.getG_Keyword_Binning())
          .equals(mode)) {
  ...
  public void runZMQServer() {
    if (zmqServer_ == null) {
      ...
      Function<Class, Object> instanceGrabberFunction =
      ...
      try {
        // It appears that every plugin has its own ClassLoader. Need to extract all of these and
        // pass to
        // ZMQServer, so that knows where to search for classes to load. If we don't do this, and
        // just create
        // new ClassLoaders to instantiate objects, static varibles will not be shared across
        // instances
        ...
        zmqServer_ =
            new ZMQServer(
                ...
                new String[] {"org.micromanager.internal"},
                new Consumer<String>() {
                  @Override
        ...
        logs()
            .logMessage("Initialized ZMQ Server on port: " + ZMQServer.DEFAULT_MASTER_PORT_NUMBER);
   ...
   * @return Whether or not cleanup was successful. Shutdown should abort on failure.
   */
  ...
  public synchronized boolean closeSequence(boolean quitInitiatedByImageJ) {
    ...
    if (afMgr_ != null && afMgr_.getAutofocusMethod() != null) {
      profile()
          .getSettings(MMStudio.class)
          .putString(AUTOFOCUS_DEVICE, afMgr_.getAutofocusMethod().getName());
    }
  ...
  private void executeStartupScript() {
    ...
    if (!f.exists()) {
      logs().logMessage("Startup script (" + f.getAbsolutePath() + ") not present");
      return;
    }

    ReportingUtils.logMessage("Running startup script (" + f.getAbsolutePath() + ")...");
  ...
  public boolean loadSystemConfiguration() {
    ...
    final WaitDialog waitDlg = new WaitDialog("Loading system configuration, please wait...");
    ...
    try {
      ...
      ReportingUtils.showError(err, "Failed to load hardware configuration", null);
      result = false;
    } finally {
      waitDlg.closeDialog();
      if (ui_.frame() != null) {
  ...
  public class MMSettings {
    ...
    private static final String CIRCULAR_BUFFER_SIZE =
        "size, in megabytes of the circular buffer used to temporarily store images before they are written to disk";

    public boolean getShouldDeleteOldCoreLogs() {
      return profile().getSettings(MMStudio.class).getBoolean(SHOULD_DELETE_OLD_CORE_LOGS, false);
    }

    public void setShouldDeleteOldCoreLogs(boolean shouldDelete) {
      profile().getSettings(MMStudio.class).putBoolean(SHOULD_DELETE_OLD_CORE_LOGS, shouldDelete);
    }

    public boolean getShouldRunZMQServer() {
      return profile().getSettings(MMStudio.class).getBoolean(SHOULD_RUN_ZMQ_SERVER, false);
    }

    public void setShouldRunZMQServer(boolean shouldRun) {
      profile().getSettings(MMStudio.class).putBoolean(SHOULD_RUN_ZMQ_SERVER, shouldRun);
    }

    public int getCoreLogLifetimeDays() {
      return profile().getSettings(MMStudio.class).getInteger(CORE_LOG_LIFETIME_DAYS, 7);
    }

    public void setCoreLogLifetimeDays(int days) {
      profile().getSettings(MMStudio.class).putInteger(CORE_LOG_LIFETIME_DAYS, days);
    }
    ...
    public int getCircularBufferSize() {
      ...
      return profile().getSettings(MMStudio.class).getInteger(CIRCULAR_BUFFER_SIZE, defaultVal);
    }

    public void setCircularBufferSize(int newSize) {
      profile().getSettings(MMStudio.class).putInteger(CIRCULAR_BUFFER_SIZE, newSize);
    }
  }
}
