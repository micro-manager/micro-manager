...
public final class PluginFinder {
  /**
   * Recursively seek through the directory structure under the specified root and generate a list
   * of files that match the given extension. Just a passthrough to the actual recursive method.
   */
  ...
  /**
   * Find all jars under the given root, check them for the META-INF file that indicates that
   * they're annotated with the @Plugin annotation, and return a list of the corresponding annotated
   * classes.
   */
    ...
    for (String jarPath : findPaths(root, ".jar")) {
      ...
      try {
        PluginClassLoader loader =
            new PluginClassLoader(jarURL, MMStudio.getInstance().getClass().getClassLoader());
   ...
   *
   * <p>The only difference from URLClassLoader is that it allows temporary blockage of resource
   * enumeration and loading from the parent loader.
   */
