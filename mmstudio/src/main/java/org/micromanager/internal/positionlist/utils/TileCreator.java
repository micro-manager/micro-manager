...
public final class TileCreator {
  ...
  public static enum OverlapUnitEnum {
    UM,
    PX,
    PERCENT
  };
   ...
   */
  public PositionList createTiles(
      double overlap,
      OverlapUnitEnum overlapUnit,
      MultiStagePosition[] endPoints,
      double pixelSizeUm,
      String labelPrefix,
      String xyStage,
      StrVector zStages,
    ...
    // bounding box size accounting for the fact that the edges of the image extend past the max/min
    // values set.
    ...
    if (nrImagesX < 1 || nrImagesY < 1) {
      ReportingUtils.showError(
          "Zero or negative number of images requested. "
    ...
    // a position that is outside of the stage's range of motion. This causes issues when it comes
    // time to stitch.
    // This approach could also result in damage if the user specified a region that is near
    // something delicate and then
      ...
      for (int x = 0; x < nrImagesX; x++) {
        ...
        if (zGen != null) {
          ...
          for (int a = 0; a < zStages.size(); a++) {
            StagePosition newSP =
                StagePosition.create1D(zStages.get(a), zGen.getZ(X, Y, zStages.get(a)));
            msp.add(newSP);
          }
        }
  ...
  private boolean isSwappedXY() {
    // Returns true if the the camera device adapter indicates that it's x and y axis should be
    // swapped.
  ...
  public double[] getTileSize(double overlap, OverlapUnitEnum overlapUnit, double pixSizeUm) {
    ...
    if (overlapUnit == OverlapUnitEnum.UM) overlapUmX = overlapUmY = overlap;
  ...
  public double[] getImageSize(double pixSizeUm) {
    ...
    boolean swapXY = isSwappedXY();
    double imageSizeXUm =
        swapXY ? pixSizeUm * core_.getImageHeight() : pixSizeUm * core_.getImageWidth();
    double imageSizeYUm =
        swapXY ? pixSizeUm * core_.getImageWidth() : pixSizeUm * core_.getImageHeight();
