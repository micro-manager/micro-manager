...
/**
 * The PositionListDlg class provides a convenient UI to generate, edit, load, and save PositionList
 * objects.
 */
public class PositionListDlg extends JFrame implements MouseListener, ChangeListener {
  ...
  public PositionListDlg(Studio studio, PositionList posList) {
    ...
    int posCol0Width = profile.getSettings(PositionListDlg.class).getInteger(POS_COL0_WIDTH, 75);
    ...
    int axisCol0Width = profile.getSettings(PositionListDlg.class).getInteger(AXIS_COL0_WIDTH, 75);
    ...
    markButton_.setIcon(
        new ImageIcon(MMStudio.class.getResource("/org/micromanager/icons/flag_green.png")));
    ...
    upButton.setIcon(
        new ImageIcon(MMStudio.class.getResource("/org/micromanager/icons/arrow_up.png")));
    upButton.setText(""); // "Up"
    upButton.setToolTipText(
        "Move currently selected position up list (positions higher on list are acquired earlier)");
    ...
    downButton.setIcon(
        new ImageIcon(MMStudio.class.getResource("/org/micromanager/icons/arrow_down.png")));
    downButton.setText(""); // "Down"
    downButton.setToolTipText(
        "Move currently selected position down list (lower positions on list are acquired later)");
    ...
    mergeButton.setIcon(
        new ImageIcon(MMStudio.class.getResource("/org/micromanager/icons/asterisk_orange.png")));
    mergeButton.setText("Merge");
    mergeButton.setToolTipText(
        "Select an axis, and set the selected positions' value along that axis to the current stage position.");
    add(mergeButton);
    ...
    gotoButton.setIcon(
        new ImageIcon(MMStudio.class.getResource("/org/micromanager/icons/resultset_next.png")));
    ...
    refreshButton.setIcon(
        new ImageIcon(MMStudio.class.getResource("/org/micromanager/icons/arrow_refresh.png")));
    ...
    removeButton.setIcon(
        new ImageIcon(MMStudio.class.getResource("/org/micromanager/icons/cross.png")));
    ...
    setOriginButton.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent arg0) {
            calibrate(); // setOrigin();
    ...
    setOriginButton.setIcon(
        new ImageIcon(MMStudio.class.getResource("/org/micromanager/icons/empty.png")));
    setOriginButton.setText("Set Origin");
    setOriginButton.setToolTipText(
        "Drives X and Y stages back to their original positions and zeros their position values");
    add(setOriginButton);
    ...
    removeAllButton.setIcon(
        new ImageIcon(MMStudio.class.getResource("/org/micromanager/icons/delete.png")));
  ...
  public void addListeners() {
    // This method should be called after the constructor finishes to fully register all needed
    // listeners.
  ...
  public void markPosition(boolean shouldOverwrite) {
    ...
    if (selMsp == null) {
      ...
      msp.setLabel(getPositionList().getPosition(posTable_.getSelectedRow() - 1).getLabel());
      int selectedRow = posTable_.getSelectedRow();
      getPositionList().replacePosition(posTable_.getSelectedRow() - 1, msp);
      updatePositionData();
  ...
  /**
   * Displays a popup menu to let the user select an axis, which will then invoke
   * mergePositionsAlongAxis, below.
   *
  ...
  /**
   * Given a device name, change all currently-selected positions so that their positions for that
   * device match the current stage position for that device.
   *
  ...
  /** Update display of the current stage position. Go out to the hardware to get new positions */
    ...
    try {
      ...
      for (int i = 0; i < stages.size(); i++) {
        if (axisList_.use(stages.get(i))) {
          StagePosition sp =
              StagePosition.create1D(stages.get(i), core_.getPosition(stages.get(i)));
          msp.add(sp);
        }
      }
      ...
      for (int i = 0; i < stages2D.size(); i++) {
        if (axisList_.use(stages2D.get(i))) {
          ...
          StagePosition sp =
              StagePosition.create2D(
                  stageName, core_.getXPosition(stageName), core_.getYPosition(stageName));
          msp.add(sp);
        }
      }
    } catch (Exception e) {
  ...
  private void calibrate() {
    ...
    JOptionPane.showMessageDialog(
        this,
        "ALERT! Please REMOVE objectives! It may damage lens!",
        "Calibrate the XY stage",
    ...
    if (JOptionPane.YES_OPTION
        != JOptionPane.showOptionDialog(
            this,
            "Really calibrate your XY stage?",
            "Are you sure?",
  ...
  public void mouseClicked(MouseEvent e) {
    ...
    if (rowIndex >= 0) {
      if (rowIndex == posTable_.getSelectedRow() && rowIndex == lastRowClicked_) {
        posTable_.clearSelection();
        lastRowClicked_ = -1;
      } else lastRowClicked_ = rowIndex;
    }
    updateMarkButtonText();
  }

  /**
   * Generate a dialog that will call our offsetSelectedSites() function with a set of X/Y/Z offsets
   * to apply.
   */
  ...
  /**
   * Given a device (either a StageDevice or XYStageDevice) and a Vector of floats, apply the given
   * offsets to all selected positions for that particular device.
   *
   * @param deviceName
   * @param offsets
   */
  public void offsetSelectedSites(String deviceName, ArrayList<Float> offsets) {
    ...
    for (int rowIndex : posTable_.getSelectedRows()) {
      ...
      for (int posIndex = 0; posIndex < multiPos.size(); ++posIndex) {
        ...
        if (subPos.getStageDeviceLabel().equals(deviceName)) {
          if (subPos.is1DStagePosition()) {
            subPos.set1DPosition(
                subPos.getStageDeviceLabel(), subPos.get1DPosition() + offsets.get(0));
          } else if (subPos.is2DStagePosition()) {
            subPos.set2DPosition(
                subPos.getStageDeviceLabel(),
