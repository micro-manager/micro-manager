...
public final class GraphPanel extends JPanel {
  ...
  protected void drawGraph(Graphics2D g, Rectangle box) {
    ...
    if (bounds_.getRangeY() == 0.0) {
      if (bounds_.yMax > 0.0) bounds_.yMin = 0.0;
    ...
    for (int i = 0; i < data_.getSize(); i++) {
      ...
      Point2D.Float pt =
          getDevicePoint(new Point2D.Float((float) tmp.x, (float) tmp.y), box, xUnit, yUnit);
  ...
  public Point2D.Float getPositionPoint(int x, int y) {
    ...
    Point2D.Float posPt =
        new Point2D.Float(
            ...
            (float)
                ((((box.y + box.height) - y) / (float) box.height)
  ...
  private void drawGrid(Graphics2D g, Rectangle box) {
    ...
    if (textVisible_) {
      ...
      g.drawString(
          fmt.format(bounds_.xMax).toString(),
          box.x + box.width,
          box.y + box.height + TEXT_OFFSET_Y);
