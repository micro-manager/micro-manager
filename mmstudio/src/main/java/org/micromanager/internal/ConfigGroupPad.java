...
public final class ConfigGroupPad extends JScrollPane {
  ...
  public void initialize() {
    ...
    table_.addColumn(new TableColumn(0, 200, new StateGroupCellRenderer(studio_), null));
    table_.addColumn(
        new TableColumn(1, 200, new StatePresetCellRenderer(studio_), new StatePresetCellEditor()));

    int colWidth = studio_.profile().getSettings(this.getClass()).getInteger(COLUMN_WIDTH, 0);
    if (colWidth > 0) {
  ...
  public void saveSettings() {
    if (table_ != null) {
      studio_
          ...
          .putInteger(COLUMN_WIDTH, table_.getColumnModel().getColumn(0).getWidth());
    }
  }
          ...
          try {
            ...
            if (item.singleProp) {
              if (item.hasLimits && item.isInteger()) {
                core_.setProperty(
                    item.device, item.name, NumberUtils.intStringDisplayToCore(value));
              } else if (item.hasLimits && !item.isInteger()) {
                core_.setProperty(
                    item.device, item.name, NumberUtils.doubleStringDisplayToCore(value));
              } else {
            ...
            if (item.group.equals(core_.getChannelGroup())) {
              core_.setExposure(
                  studio_
                      .app()
                      .getChannelExposureTime(item.group, value.toString(), core_.getExposure()));
            }
          ...
          if (item.allowed.length == 1) {
            ...
            if (cfg.size() == 1) {
              ...
              if (item.hasLimits || !itemHasAllowedValues) {
                ...
                item.singlePropAllowed =
                    core_.getAllowedPropertyValues(item.device, item.name).toArray();
              }
            }
          }
