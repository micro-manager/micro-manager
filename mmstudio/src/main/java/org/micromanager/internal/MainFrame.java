...
import org.micromanager.alerts.internal.*;
...
public final class MainFrame extends JFrame {
  ...
  public MainFrame(MMStudio mmStudio, CMMCore core) {
    ...
    super.setTitle(String.format("%s %s", MICRO_MANAGER_TITLE, MMVersion.VERSION_STRING));
  ...
  private JButton createButton(String text, String iconPath, String help, final Runnable action) {
    Icon icon = null;
    if (iconPath != null) {
  ...
  private static JLabel createLabel(String text, boolean big) {
    ...
    label.setFont(new Font("Arial", big ? Font.BOLD : Font.PLAIN, big ? 11 : 10));
    label.setText(text);
    return label;
  }

  private JPanel createImagingSettingsWidgets() {
    JPanel subPanel = new JPanel(new MigLayout("flowx, fillx, insets 1, gap 0"));
    ...
    textFieldExp_.addFocusListener(
        new FocusAdapter() {
          @Override
    ...
    textFieldExp_.addActionListener(
        (ActionEvent e) -> {
          mmStudio_.app().setExposure(getDisplayedExposureTime());
        });
    ...
    chanGroupSelect_.addActionListener(
        (ActionEvent e) -> {
    ...
    comboBinning_.addActionListener(
        (ActionEvent e) -> {
    ...
    // Shutter device.
    JPanel shutterPanel = new JPanel(new MigLayout("fillx, flowx, insets 0, gap 0"));
  ...
  private JPanel createConfigurationControls() {
    JPanel subPanel = new JPanel(new MigLayout("fill, flowy, insets 1, gap 0"));
    subPanel.add(createLabel("Configuration settings", true), "flowx, growx, pushy 0, split 2");
    ...
    subPanel.add(saveConfigButton_, "pushy 0, gapleft push, alignx right, w 88!, h 20!");
    ...
    subPanel.add(configPad_, "grow, pushy 100, alignx center, w min::9999, h min::9999, span");
    subPanel.add(configPadButtonPanel_, "growx, pushy 0, alignx left, w 320!, h 20!, span");
    return subPanel;
  }

  /** Generate the "Snap", "Live", "Snap to album", "MDA", and "Refresh" buttons. */
  private JPanel createCommonActionButtons() {
    JPanel subPanel = new JPanel(new MigLayout("flowy, insets 1, gap 1"));
    snapButton_ =
        (JButton)
        ...
        (JButton)
            QuickAccessFactory.makeGUI(
                mmStudio_
                    .plugins()
                    .getQuickAccessPlugins()
                    .get("org.micromanager.quickaccess.internal.controls.CloseAllButton"));
  ...
  private JPanel createComponents() {
    ...
    labelImageDimensions_.setBorder(new MatteBorder(1, 0, 0, 0, new Color(200, 200, 200)));
    overPanel.add(labelImageDimensions_, "growx, pushy 0, span, gap 2 0 2 0");
    return overPanel;
  }

  private JPanel createConfigProfileLine() {
    JPanel subPanel = new JPanel(new MigLayout("flowx, insets 0 1 0 1, gap 0, fill"));
    subPanel.setBorder(new MatteBorder(0, 0, 1, 0, new Color(200, 200, 200)));
  ...
  private JPanel createUtilityButtons() {
    ...
    roiPanel.add(createLabel("ROI", true), "span 2, alignx center, growx, wrap");
    setRoiButton_ =
        createButton(
            null,
            "shape_handles.png",
    ...
    stagePanel.add(createLabel("Stage", true), "span 3, alignx center, growx, wrap");
    // This icon is the public-domain icon at
    // https://openclipart.org/detail/198011/mono-move
    AbstractButton moveButton =
        createButton(
            null,
            "move.png",
    ...
    // https://openclipart.org/detail/198011/mono-move
    handMovesButton_ =
        new JToggleButton(IconLoader.getIcon("/org/micromanager/icons/move_hand.png"));
    ...
    handMovesButton_.addActionListener(
        (ActionEvent e) -> {
    ...
    autoPanel.add(createLabel("Autofocus", true), "span 2, alignx center, growx, wrap");
  ...
  /**
   * Recreate the contents and current selection of the chanGroupSelect_ combobox. We have to
   * temporarily disable its action listener so it doesn't try to change the current channel group
   * while we do this.
   */
  ...
  private void setHandMovesButton(boolean state) {
    ...
    handMovesButton_.setIcon(IconLoader.getIcon("/org/micromanager/icons/" + icon));
  ...
  public void configureBinningComboForCamera(String cameraLabel) {
    ...
    try {
      StrVector binSizes =
          core_.getAllowedPropertyValues(cameraLabel, MMCoreJ.getG_Keyword_Binning());
  ...
  public void savePrefs() {
    mmStudio_
        .profile()
        .getSettings(MainFrame.class)
        .putString(MAIN_EXPOSURE, textFieldExp_.getText());
  }
