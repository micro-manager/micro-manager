...
public final class DefaultAlbum implements Album {
  ...
  public boolean addImage(Image image) throws IOException {
    ...
    String curChannel = "";
    try {
      curChannel = studio_.core().getCurrentConfig(studio_.core().getChannelGroup());
    } catch (Exception e) {
    ...
    */
    if (mustCreateNew) {
      ...
      try {
        SummaryMetadata.Builder smb =
            studio_.acquisitions().generateSummaryMetadata().copyBuilder();
        smb.channelGroup(studio_.core().getChannelGroup());
        // TODO: can there be other axes than T?
        smb.channelNames(curChannel).axisOrder(Coords.T, Coords.C, Coords.Z, Coords.P);
      ...
      if (ds == null) {
        ds =
            DefaultDisplaySettings.builder().colorMode(DisplaySettings.ColorMode.GRAYSCALE).build();
      }
    ...
    try {
      synchronized (pipelineLock_) {
        ...
        try {
          ...
          studio_.logs().showError(e, "An error occurred while processing images.");
  ...
  public Coords createAlbumCoords(Image image) throws IOException {
    ...
    if (curTime_ == null) {
      ...
      Coords matcher =
          Coordinates.builder().channel(image.getCoords().getChannel()).t(curTime_).build();
  ...
  private void saveDisplaySettings() {
    if (display_.getDisplaySettings() instanceof DefaultDisplaySettings) {
      ((DefaultDisplaySettings) display_.getDisplaySettings())
          .saveToProfile(studio_.profile(), PropertyKey.ALBUM_DISPLAY_SETTINGS.key());
    }
  }
}
