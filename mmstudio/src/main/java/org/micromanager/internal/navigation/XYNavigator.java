...
public class XYNavigator {
  ...
  /**
   * Central interface to move the default XY stage in the direction desired by the user. Facilities
   * to correct movement as seen on the screen to movement of the XY stage are included.
   *
   * <p>The XY stage to be moved can be specified, however, the movement corrections are likely only
   * valid for the default XY Stage
   *
   * <p>Each stage runs on its own executor. These will only be created after movement for a stage
   * has been requested.
   *
  ...
  private void getOrientation() {
    ...
    try {
      ...
      try {
        ...
        if (Math.abs(pixelSize - AffineUtils.deducePixelSize(affineTransform_)) > 0.1 * pixelSize) {
  ...
  /**
   * Converts Camera-Pixel Space to Stage-micron space Use the affine transform when available,
   * otherwise uses pixelSize and orientation booleans, determined using the getOrientation
   * function.
   *
  ...
  /**
   * Move the XYStage in such a manner that the sample as displayed in the viewer will move in an
   * absolute Carthesian coordinate system. For instance, moving 1 micron in x will move the stage
   * so that the image on the screen will move 1 micron to the right, which may involve XYStage
  ...
  public void moveSampleOnDisplayUm(final double tmpXUm, final double tmpYUm) {
    ...
    if (!(pixSizeUm > 0.0)) {
      JOptionPane.showMessageDialog(
          null, "Please provide pixel size calibration data before using this function");
      return;
    }
    ...
    Point2D stagePos = toStageSpace(pixSizeUm, tmpXUm / pixSizeUm, tmpYUm / pixSizeUm);
    moveXYStageUm(studio_.core().getXYStageDevice(), stagePos.getX(), stagePos.getY());
  }

  /**
   * Move the XYStage in such a manner that the sample as displayed in the viewer will move in an
   * absolute Carthesian coordinate system. For instance, moving 1 pixel in x will move the stage so
   * that the image on the screen will move 1 pixels to the right, which may involve XYStage
   * movement in both x and y.
   *
  ...
  public void moveSampleOnDisplayPixels(final double tmpXPixels, final double tmpYPixels) {
    ...
    Point2D stagePos = toStageSpace(pixSizeUm, tmpXPixels, tmpYPixels);
    moveXYStageUm(studio_.core().getXYStageDevice(), stagePos.getX(), stagePos.getY());
  }

  /**
   * Moves the XYStage the desired distance in microns without corrections Creates an XYStageTask if
   * the stage is unknown, otherwise adds the desired movement to the task. If a task is still
   * running, it will add the movement to its future movement task.
   *
  ...
  /**
   * Helper class that executes stage movement, and pools movement requests if they come in while
   * the stage is busy.
   *
   * <p>This should reduce the number of commands send to the stage when the UI is firing requests
   * faster than the stage can execute them.
   */
    ...
    public void run() {
      ...
      try {
        while (moveMemoryX_.get() != 0 || moveMemoryY_.get() != 0.0) {
          ...
          studio_.events().post(new DefaultXYStagePositionChangedEvent(xyStage_, xs[0], ys[0]));
        }
      } catch (Exception ex) {
