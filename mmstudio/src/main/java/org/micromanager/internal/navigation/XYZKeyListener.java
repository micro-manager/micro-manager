...
public final class XYZKeyListener {
  ...
  /**
   * The XYZKeyListener receives settings from the StageControl plugin by means of the user profile.
   *
   * <p>Array sizes, etc. match those of the StageControl plugin, so changes there may break things
   * here.
   *
   * <p>Movement requests funnel to the single instance of the XYNavigator. The XYNavigator send the
   * actual movement comman to the XY stage using its own executor. Movements can be send even while
   * the stage is moving. Once the stage stops moving, all (added) movement requests will be
   * combined into a single stage movement.
   *
  ...
  public XYZKeyListener(Studio studio, XYNavigator xyNavigator, ZNavigator zNavigator) {
    ...
    settings_ =
        studio.profile().getSettings(org.micromanager.internal.dialogs.StageControlFrame.class);
  }
  ...
  public void IncrementZ(double micron) {
    ...
    if (zStage == null || zStage.length() == 0) return;
