...
public final class PipelineTableModel extends AbstractTableModel {
  ...
  public void moveConfigurator(ConfiguratorWrapper configurator, int offset) {
   ...
   *
   * @param isLiveMode if true, select configurators enabled for live, if false, select
   *     generally-enabled configurators.
   */
  ...
  */
  public List<ConfiguratorWrapper> getEnabledConfigurators(boolean isLiveMode) {
    ...
    for (ConfiguratorWrapper config : pipelineConfigs_) {
      if ((isLiveMode && config.isEnabledInLive()) || (!isLiveMode && config.isEnabled())) {
        result.add(config);
      }
    }
    return result;
  ...
  /** Record the current pipeline to the user's profile, so it can be restored later. */
  public void savePipelineToProfile(Studio studio) {
    ...
    studio
        ...
        .putStringList(SAVED_PIPELINE, serializedConfigs);
  }

  /** Restore the pipeline from the user's profile. Return true if we actually updated the model. */
  public boolean restorePipelineFromProfile(Studio studio) {
    List<String> serializedConfigs =
        studio
            .profile()
            .getSettings(PipelineTableModel.class)
    ...
    for (String configString : serializedConfigs) {
      ConfiguratorWrapper config = ConfiguratorWrapper.fromString(configString, studio);
      if (config.isEnabled()) {
        didUpdate = true;
      }
