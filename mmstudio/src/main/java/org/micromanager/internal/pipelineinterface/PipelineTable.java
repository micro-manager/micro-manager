...
public final class PipelineTable extends JTable {
  ...
  private class ConfigureButtonCellEditor extends AbstractCellEditor
    ...
    public Component getTableCellEditorComponent(
        JTable table, Object cellValue, boolean isSelected, int row, int column) {
  ...
  PipelineTable() {
    ...
    setDefaultRenderer(ConfiguratorWrapper.class, new ConfigureButtonCellRenderer());
    setDefaultEditor(ConfiguratorWrapper.class, new ConfigureButtonCellEditor());

    // Shrink the checkbox columns down to size.
    for (int columnID :
        new int[] {PipelineTableModel.ENABLED_COLUMN, PipelineTableModel.ENABLED_LIVE_COLUMN}) {
  ...
  ConfiguratorWrapper getSelectedConfigurator() {
    ...
    if (i >= 0) {
      Object cellValue = getModel().getValueAt(i, PipelineTableModel.CONFIGURE_COLUMN);
