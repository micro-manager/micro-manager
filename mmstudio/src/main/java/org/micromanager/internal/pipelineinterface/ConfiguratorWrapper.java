 ...
 * without having any logic in the plugin side of things. It also contains some backreferences to
 * the plugin that created the Configurator, for ease of use.
 */
public final class ConfiguratorWrapper {
  ...
  public ConfiguratorWrapper(
      ProcessorPlugin plugin, ProcessorConfigurator configurator, String name) {
    plugin_ = plugin;
  ...
  public static ConfiguratorWrapper fromString(String contents, Studio studio) {
    try {
      ...
      ProcessorPlugin plugin =
          studio.plugins().getProcessorPlugins().get(json.getString("pluginName"));
      ...
      ConfiguratorWrapper result =
          new ConfiguratorWrapper(plugin, configurator, json.getString("name"));
