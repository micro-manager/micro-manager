...
public final class PipelineFrame extends JFrame implements ListSelectionListener {
  ...
  public PipelineFrame(Studio studio) {
    ...
    pipelineScrollPane_.setMinimumSize(new Dimension(370, pipelineTable_.getRowHeight()));
    ...
    addButton.setIcon(
        new ImageIcon(MMStudio.class.getResource("/org/micromanager/icons/plus.png")));
    ...
    removeButton_.setIcon(
        new ImageIcon(MMStudio.class.getResource("/org/micromanager/icons/minus.png")));
    ...
    replayButton_.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    final Dimension frameSize =
        new Dimension(contentSize.width + widthDelta, contentSize.height + heightDelta);
    final Dimension minFrameSize =
        new Dimension(minSize.width + widthDelta, minSize.height + heightDelta);
  ...
  private void updateEditButtonStatus(ListSelectionModel model) {
    ...
    moveUpButton_.setEnabled(enableEditButtons && model.getMaxSelectionIndex() > 0);
    moveDownButton_.setEnabled(
        enableEditButtons && model.getMinSelectionIndex() < pipelineTable_.getRowCount() - 1);
  }

  /**
   * A new ProcessorPlugin was registered; re-load our list of registered processors.
   *
   * @param event
   */
  @Subscribe
  ...
  /**
   * When shutdown starts, we record the current processing pipeline, so it can be restored later.
   */
  @Subscribe
  ...
  /**
   * Re-acquire the entire list of registered processors. We do things this way because we have no
   * idea when we are created vs. when all the plugins are registered (they happen in different
   * threads), thus we have no idea currently how many processors are registered that we don't know
   * about.
   */
  ...
  private void moveSelectedConfigurator(int offset) {
    ...
    pipelineTable_.getSelectionModel().setSelectionInterval(i + offset, i + offset);
  }
  ...
  public void addAndConfigureProcessor(ProcessorPlugin plugin) {
    ...
    ProcessorConfigurator configurator = plugin.createConfigurator(PropertyMaps.builder().build());
    getTableModel()
        .addConfigurator(new ConfiguratorWrapper(plugin, configurator, plugin.getName()));
    setVisible(true);
    configurator.showGUI();
  }

  /** Add a new entry to the list of processors, which has already been configured. */
  public void addConfiguredProcessor(ProcessorConfigurator config, ProcessorPlugin plugin) {
    getTableModel().addConfigurator(new ConfiguratorWrapper(plugin, config, plugin.getName()));
    setVisible(true);
    config.showGUI();
  }

  /**
   * Generate a list of ProcessorFactories based on the currently-enabled configurators and their
   * settings.
   */
