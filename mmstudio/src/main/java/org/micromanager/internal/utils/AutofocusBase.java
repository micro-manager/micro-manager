...
public abstract class AutofocusBase implements AutofocusPlugin {
  ...
  /**
   * Add property to the list of device properties. This is the most general method where all
   * property features can be controlled.
   *
  ...
  /**
   * Sets value for a given property name. This method will not check if the value is allowed or
   * not, or whether it conforms to the property limits. It is assumed that the caller will take
   * care of that using appropriate property information (see Property class)
   */
  @Override
  ...
  public void saveSettings() {
    ...
    for (int i = 0; i < properties_.size(); i++) {
      profile
          ...
          .putString(properties_.get(i).name, properties_.get(i).value);
    }
  }
