...
public final class TextUtils {
  ...
  /**
   * Given a filename, truncate it by a) removing file extension, and b) replacing inner content by
   * ellipses, to meet the provided max length.
   */
  public static String truncateFilename(String name, int maxLen) {
    ...
    if (name.length() > maxLen) {
      ...
      if (name.length() > maxLen) {
        int len = name.length();
        name = name.substring(0, maxLen / 2 - 3) + "..." + name.substring(len - maxLen / 2, len);
      }
    }
    return name;
  }
  ...
  /**
   * Given an input string (presumably formatted by one of our formatters), remove the leading
   * negative sign if the numerals are all zero. E.g. turns "-0.0" into "0.0". See
