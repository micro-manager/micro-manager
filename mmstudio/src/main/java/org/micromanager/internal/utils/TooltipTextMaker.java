...
public final class TooltipTextMaker {
  ...
  public static String addHTMLBreaksForTooltip(String text) {
    ...
    for (int i = 0; i < numLines; i++) {
      ...
      while (text.charAt(actualBreakIndex) != ' ') actualBreakIndex--;
      result.append(
          text.substring(i * numCharsPerLine, actualBreakIndex)
              + "<br>"
              + text.substring(actualBreakIndex, estimatedBreakIndex + 1).trim());
    }
