...
public final class DaytimeNighttime implements ApplicationSkin {
  ...
  private static final String[] LIGHTER_BACKGROUND_COLOR_KEYS =
      new String[] {
        "CheckBoxMenuItem", "ComboBox", "Menu", "MenuItem", "PopupMenu", "RadioButtonMenuItem"
      };
  ...
  private static final String[] DISABLED_TEXT_COLOR_KEYS =
      new String[] {"Button", "CheckBox", "RadioButton", "ToggleButton"};
  ...
  private DaytimeNighttime(Studio studio) {
    ...
    background_.put(SkinMode.DAY, new ColorUIResource(java.awt.SystemColor.control));
    background_.put(SkinMode.NIGHT, new ColorUIResource(new Color(64, 64, 64)));

    disabledBackground_ = new HashMap<>();
    disabledBackground_.put(SkinMode.DAY, new ColorUIResource(Color.LIGHT_GRAY));
    disabledBackground_.put(SkinMode.NIGHT, new ColorUIResource(new Color(32, 32, 32)));

    lightBackground_ = new HashMap<>();
    lightBackground_.put(SkinMode.DAY, new ColorUIResource(java.awt.SystemColor.control));
    // 37.5% gray; dodging both the OSX checkmark (25% gray) and disabled
    // text (50% gray).
    lightBackground_.put(SkinMode.NIGHT, new ColorUIResource(new Color(96, 96, 96)));

    padBackground_ = new HashMap<>();
    padBackground_.put(SkinMode.DAY, new ColorUIResource(Color.white));
    padBackground_.put(SkinMode.NIGHT, new ColorUIResource(java.awt.SystemColor.control));

    enabledTextColor_ = new HashMap<>();
    enabledTextColor_.put(SkinMode.DAY, new ColorUIResource(20, 20, 20));
    enabledTextColor_.put(SkinMode.NIGHT, new ColorUIResource(200, 200, 200));

    disabledTextColor_ = new HashMap<>();
    disabledTextColor_.put(SkinMode.DAY, new ColorUIResource(100, 100, 100));
    disabledTextColor_.put(SkinMode.NIGHT, new ColorUIResource(120, 120, 120));
  }
  ...
  /**
   * This version of the function allows us to specify whether or not the UI should be updated after
   * changing modes. Not updating is only generally wanted in cases where a one-off component must
   * be created that doesn't adhere to our custom look and feel; see suspendToMode() below.
   */
  ...
  /**
   * If the specified mode is not currently active, then we switch to that mode without updating the
   * UI. Useful if a component must be generated with a nonstandard look-and-feel.
   *
  ...
  private void storeSkin(SkinMode mode) {
    studio_
        ...
        .putString(BACKGROUND_MODE, mode.getDesc());
  }
  ...
  public SkinMode getSkin() {
    return SkinMode.fromString(
        studio_
            .profile()
            .getSettings(DaytimeNighttime.class)
            .getString(BACKGROUND_MODE, SkinMode.NIGHT.getDesc()));
  }
  ...
  /**
   * Because tables don't obey look and feel color commands, we have this custom table that picks up
   * its background based on the current mode.
   */
