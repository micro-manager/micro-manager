...
public class ClearCLNioConverters {
   ...
   * @param dimensions Dimensions of the image. Should be 2D or 3D (higher?) Mismatch with the input
   *     data may be disastrous
   * @return ClearCLBuffer containing a copy of the input data
   */
  public static ClearCLBuffer convertNioTiClearCLBuffer(
      ClearCLContext context, Buffer source, long[] dimensions) {
  ...
  public static ClearCLImage convertNioTiClearCLImage(
      ClearCLContext context, Buffer source, long[] dimensions) {
    ClearCLImage target;
  ...
  /**
   * Copies Intensity data from ClearCLBuffer to Java Direct Buffer Input is either ClearCLBuffer or
   * ClearCLImage Caller will also need dimensions: source.getDimensions()
   *
    ...
    if (null != source.getNativeType())
      switch (source.getNativeType()) {
        case UnsignedByte:
          buffer =
              ByteBuffer.allocate(
                  (int) (source.getSizeInBytes() / source.getNativeType().getSizeInBytes()));
