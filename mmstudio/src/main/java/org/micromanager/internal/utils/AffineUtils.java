 ...
 * translation between the different data formats for affine transform
 *
 * @author nico
 */
public class AffineUtils {
  ...
  public static final AffineTransform doubleToAffine(DoubleVector atf) {
    ...
    double[] flatMatrix = {atf.get(0), atf.get(3), atf.get(1), atf.get(4), atf.get(2), atf.get(5)};
    return new AffineTransform(flatMatrix);
  }
  ...
  public static double[] affineToMeasurements(AffineTransform transform) {
    try {
      ...
      at.getMatrix(newMat);
      double xScale =
          Math.sqrt(newMat[0] * newMat[0] + newMat[1] * newMat[1]) * (newMat[0] > 0 ? 1.0 : -1.0);
      double yScale =
          Math.sqrt(newMat[2] * newMat[2] + newMat[3] * newMat[3]) * (newMat[3] > 0 ? 1.0 : -1.0);
