...
public class PropertyItem {
  ...
  public void dump() {
    ...
    for (int i = 0; i < allowed.length; i++) ReportingUtils.logMessage("   " + allowed[i]);
  }

  public void setValueFromCoreString(String coreValue) {
    try {
      if (isInteger()) value = NumberUtils.intStringCoreToDisplay(coreValue);
      else if (isFloat()) value = NumberUtils.doubleStringCoreToDisplay(coreValue);
      else value = coreValue;
    } catch (Exception e) {
  ...
  public String getValueInCoreFormat() {
    try {
      if (isInteger()) return NumberUtils.intStringDisplayToCore(value);
      else if (isFloat()) return NumberUtils.doubleStringDisplayToCore(value);
      else return value;
    } catch (Exception e) {
      ReportingUtils.logError(e);
      return value;
  ...
  public String toString() {
    return String.format(
        "<PropertyItem %s-%s (%s)%s>", device, name, value, readOnly ? " (read-only)" : "");
  }
}
