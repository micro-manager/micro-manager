 ...
 * summary metadata. Ideally all tags that have any effect on program flow would only be accessed by
 * way of this module; the eventual goal being to promote those tags to being proper member fields
 * and deprecate the corresponding bits of JSON.
 *
 * <p>By using this module, type safety is enforced, redundant tags can be identified (e.g. "Frame"
 * vs. "FrameIndex"), and it becomes much easier to track which bits of code are relying on which
 * tags.
 *
...
public final class MDUtils {
  ...
  public static int getNumPositions(JSONObject map) throws JSONException {
    if (map.has("Positions")) return map.getInt("Positions");
  ...
  public static String getPixelType(JSONObject map) throws JSONException, IllegalArgumentException {
    try {
      if (map != null) return map.getString("PixelType");
    } catch (JSONException e) {
      try {
  ...
  public static UUID getUUID(JSONObject map) throws JSONException {
    if (map.has("UUID")) return UUID.fromString(map.getString("UUID"));
    else return null;
  }
  ...
  public static int getSingleChannelType(JSONObject map)
      throws JSONException, IllegalArgumentException {
  ...
  public static int getNumberOfComponents(JSONObject map)
    ...
    String pixelType = getPixelType(map);
    if (pixelType.contentEquals("GRAY8")) return 1;
    else if (pixelType.contentEquals("GRAY16")) return 1;
    else if (pixelType.contentEquals("GRAY32")) return 1;
    else if (pixelType.contentEquals("RGB32")) return 3;
    else if (pixelType.contentEquals("RGB64")) return 3;
    else {
  ...
  public static String getLabel(JSONObject md) {
    try {
      return generateLabel(
          getChannelIndex(md), getSliceIndex(md), getFrameIndex(md), getPositionIndex(md));
  ...
  public static String generateLabel(int channel, int slice, int frame, int position) {
    return NumberUtils.intToCoreString(channel)
        + "_"
        + NumberUtils.intToCoreString(slice)
        + "_"
  ...
  public static Rectangle getROI(JSONObject tags) throws IllegalArgumentException, JSONException {
  ...
  public static void setROI(JSONObject tags, Rectangle r) throws JSONException {
    String roiString =
        String.format(
            "%d-%d-%d-%d", (int) r.getX(), (int) r.getY(), (int) r.getWidth(), (int) r.getHeight());
  ...
  public static int getNumFrames(JSONObject tags) throws JSONException {
    if (tags.has("Summary")) {
      ...
      if (summary.has("Frames")) return Math.max(1, summary.getInt("Frames"));
    }
    if (tags.has("Frames")) return Math.max(1, tags.getInt("Frames"));
    return 1;
  }
  ...
  public static int getNumSlices(JSONObject tags) throws JSONException {
    if (tags.has("Summary")) {
      JSONObject summary = tags.getJSONObject("Summary");
      if (summary.has("Slices")) return Math.max(1, summary.getInt("Slices"));
    }
    if (tags.has("Slices")) return Math.max(1, tags.getInt("Slices"));
    return 1;
  }
  ...
  public static int getNumChannels(JSONObject tags) throws JSONException {
    if (tags.has("Summary")) {
      JSONObject summary = tags.getJSONObject("Summary");
      if (summary.has("Channels")) return Math.max(1, summary.getInt("Channels"));
    }
    if (tags.has("Channels")) return Math.max(1, tags.getInt("Channels"));
    return 1;
  }
