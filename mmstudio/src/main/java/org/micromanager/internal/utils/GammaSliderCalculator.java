...
public final class GammaSliderCalculator {
  ...
  public GammaSliderCalculator(int low, int high) {
    ...
    for (int i = low; i <= high; i++) {
      try {
        double gamma =
            NumberUtils.displayStringToDouble(NumberUtils.doubleToDisplayString(calculateGamma(i)));
  ...
  public int gammaToSlider(double gamma) {
    if (gamma < min_) gamma = min_;
    if (gamma > max_) gamma = max_;
    try {
    ...
    while (diff > 1 && i < 10) {
      ...
      lastGuess = guess;
      if (gamma > gammas_.get(guess)) guess += diff / 2;
      else guess -= diff / 2;
      i++;
    }
    return guess;
