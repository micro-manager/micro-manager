...
public final class ColorPalettes {
  ...
  public static List<Color> getColorblindFriendlyPalette() {
    return Collections.unmodifiableList(Arrays.asList(COLORBLIND_FRIENDLY_COLORS));
  }
  ...
  private static final Color[] COLORBLIND_FRIENDLY_COLORS =
      new Color[] {
  ...
  private static final Color[] PRIMARY_COLORS =
      new Color[] {
  ...
  private static final Map<String, Float> WAVELENGTHS = new LinkedHashMap<String, Float>();
  ...
  private static final Pattern NM_PATTERN =
      Pattern.compile(
          // 3-digit number
          "(^|\\D)(\\d\\d\\d)(\\D|$)");
  ...
  public static Color colorForMonochromaticWavelenth(double lambdaNm) {
    ...
    Color xyzColor =
        new Color(
            ColorSpace.getInstance(ColorSpace.CS_CIEXYZ), new float[] {n * x, n * y, n * z}, 1.0f);
    float[] rgb =
        xyzColor.getColorComponents(ColorSpace.getInstance(ColorSpace.CS_sRGB), new float[3]);
   ...
   * Chris Wyman, Peter-Pike Sloan, and Peter Shirley, 2013. Simple Analytic Approximations to the
   * CIE XYZ Color Matching Functions. <a href="http://jcgt.org/published/0002/02/01/">Journal of
   * Computer Graphics Techniques, 2(2):1-11</a>.
   *
   * <p>The 1964 (rather than 1931) approximations are used here since they are better than the
   * simple 1931 approximations but simpler than the piecewise approximations for 1931.
   */
  ...
  public static void main(String[] args) {
    ...
    for (int lambda = 380; lambda <= 680; ++lambda) {
      ...
      float[] hsv = Color.RGBtoHSB(c.getRed(), c.getGreen(), c.getBlue(), new float[3]);
    ...
    JFreeChart chart =
        ChartFactory.createScatterPlot(
            "CIE 1964", "lambda (nm)", null, data, PlotOrientation.VERTICAL, true, true, false);
