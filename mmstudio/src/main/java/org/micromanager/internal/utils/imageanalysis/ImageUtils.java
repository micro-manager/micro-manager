...
public final class ImageUtils {
  ...
  public static ImageProcessor makeProcessor(TaggedImage taggedImage) {
    ...
    try {
      return makeProcessor(
          MDUtils.getIJType(tags),
          MDUtils.getWidth(tags),
          MDUtils.getHeight(tags),
          taggedImage.pix);
    } catch (Exception e) {
  ...
  public static ImageProcessor subtractFloatProcessors(FloatProcessor proc1, ByteProcessor proc2) {
    return new FloatProcessor(
        proc1.getWidth(),
        proc1.getHeight(),
        subtractPixelArrays((float[]) proc1.getPixels(), (byte[]) proc2.getPixels()),
        null);
  }
  ...
  public static ImageProcessor subtractFloatProcessors(FloatProcessor proc1, ShortProcessor proc2) {
    return new FloatProcessor(
        proc1.getWidth(),
        proc1.getHeight(),
        subtractPixelArrays((float[]) proc1.getPixels(), (short[]) proc2.getPixels()),
        null);
  }
  ...
  private static ShortProcessor subtractShortByteProcessors(
      ShortProcessor proc1, ByteProcessor proc2) {
  ...
  private static ShortProcessor subtractShortFloatProcessors(
      ShortProcessor proc1, FloatProcessor proc2) {
    return new ShortProcessor(
        proc1.getWidth(),
        proc1.getHeight(),
  ...
  private static int findArrayMax(Object pix) {
    if (pix instanceof byte[]) return findArrayMax((byte[]) pix);
    if (pix instanceof int[]) return findArrayMax((int[]) pix);
    if (pix instanceof short[]) return findArrayMax((short[]) pix);
    if (pix instanceof float[]) return findArrayMax((float[]) pix);
    else return -1;
  }
  ...
  /**
   * Converts a sequence of 4 bytes into an int. Note that this function assumes bytes to be signed.
   * Most data coming from MMCore will have unsigned bytes, so used the appropriate function for
   * that conversion.
   *
   * @param pixels
   * @return
  ...
  /**
   * Used to convert unsigned bytes coming from the C++ later into ints that can be used by ImageJ.
   *
   * @param pixels
   * @return
   */
  ...
  public static byte[] singleChannelFromRGB32(byte[] pixels, int channel) {
    if (channel != 0 && channel != 1 && channel != 2) return null;
  ...
  /**
   * Generate a new TaggedImage off of the provided one, with copied metadata so that changes
   * elsewhere in the program won't affect this one.
   */
  ...
  /**
   * Executes ImageJ-based Fourier space cross-correlation This should have the exact same result as
   * the ImageJ FFT - FD Math - Correlate command
   *
  ...
  public static ImageProcessor crossCorrelate(ImageProcessor proc1, ImageProcessor proc2) {
    ...
    */
    ...
    result
        .swapQuadrants(); // This needs to be done after transforming. to get back to the original.
