...
public abstract class ConfigDialog extends JDialog {
  ...
  public ConfigDialog(String groupName, String presetName, Studio studio, boolean newItem) {
    super();
  ...
  /**
   * Override this method to insert any GUI elements between initializeWidgets() and
   * initializePropertyTable(). Used for affine transform in pixel editors.
   */
  ...
  protected void initializeWidgets() {
    ...
    JPanel topMidPanel = new JPanel(new MigLayout("flowx, insets 0 6 0 0, gap 2"));
    ...
    // nameField_.setFont(new Font("Arial", Font.BOLD, 16));  // should consider increasing font
    // size for entry field for readability, but would want to do it in multiple places for
    // consistency
  ...
  public void initializePropertyTable() {
    ...
    if (numColumns_ == 3) {
      ...
      table_.addColumn(
          new TableColumn(
              1, 75, new PropertyUsageCellRenderer(studio_), new PropertyUsageCellEditor()));
      table_.addColumn(
          new TableColumn(
              2, 200, new PropertyValueCellRenderer(studio_), new PropertyValueCellEditor(true)));
    } else if (numColumns_ == 2) {
      table_.addColumn(new TableColumn(0, 200, new PropertyNameCellRenderer(studio_), null));
      table_.addColumn(
          new TableColumn(
              1, 200, new PropertyValueCellRenderer(studio_), new PropertyValueCellEditor(false)));
    }
  }
