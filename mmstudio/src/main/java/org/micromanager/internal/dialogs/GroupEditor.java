...
public final class GroupEditor extends ConfigDialog {
  ...
  public GroupEditor(String groupName, String presetName, Studio studio, boolean newItem) {
    ...
    data_ =
        ptdb.groupName(groupName)
            .presetName(presetName)
            .propertyValueColumn(2)
            .propertyUsedColumn(1)
            .groupOnly(false)
            .allowChangingProperties(false)
            .allowChangesOnlyWhenUsed(false)
            .isPixelSizeConfig(false)
            .build();
    initializeData();
  ...
  public void okChosen() {
    ...
    for (PropertyItem item : data_.getProperties()) {
      try {
        if (item.confInclude
          ...
          if ((item.allowed[0].contentEquals("0") && item.allowed[1].contentEquals("1"))
              || (item.allowed[0].contentEquals("1") && item.allowed[1].contentEquals("0"))) {
            shutters.add(item);
          }
        }
      } catch (Exception e) {
        studio_
            .logs()
            .logError(
                e, "Error getting information on device/property " + item.device + "/" + item.name);
      }
    }
    ...
    if (shutters.size() > 0
      ...
      JCheckBox neverAgain = new JCheckBox("Do not show me this warning again.");
      ...
      studio_
          .profile()
          .getSettings(GroupEditor.class)
          .putBoolean(DISPLAY_SHUTTER_WARNING, !neverAgain.isSelected());
      if (selection == 2) {
        // User cancelled.
        return;
      } else if (selection == 1) {
              ...
              if (item.confInclude) {
                if (first.isPropertyIncluded(item.device, item.name)
                  ...
                  if (!first
                      ...
                      .getPropertyValue()
                      .contentEquals(
                          second.getSetting(item.device, item.name).getPropertyValue())) {
                    same = false;
                  }
                }
              }
        ...
        for (PropertyItem item : data_.getProperties()) {
          if (item.confInclude) {
            if (itemsIncludedCount == 1 && item.allowed.length > 0) {
              ...
              core_.defineConfig(
                  newName, "NewPreset", item.device, item.name, item.getValueInCoreFormat());
            }
          }
        }
      ...
      try {
        ...
        for (PropertyItem item : data_.getProperties()) {
          if (!item.confInclude && unionCfg.isPropertyIncluded(item.device, item.name)) {
            ...
            // If some presets don't have this property when they should, add it with current
            // values.
            for (int i = 0; i < cfgs.size(); i++) {
              cfg = cfgs.get(i);
              if (!core_
                  .getConfigData(groupName_, cfg)
                  .isPropertyIncluded(item.device, item.name)) {
                core_.defineConfig(
                    newName, cfg, item.device, item.name, item.getValueInCoreFormat());
              }
            }
          }
        }
