...
public class PixelPresetEditor extends ConfigDialog implements PixelSizeProvider {
  ...
  public PixelPresetEditor(
      String pixelSizeConfigName, CalibrationListDlg parent, String pixelSize, boolean newItem) {
    ...
    data_ =
        ptdb.groupName(groupName_)
            .presetName(presetName_)
            .propertyValueColumn(1)
            .propertyUsedColumn(2)
            .groupOnly(true)
            .allowChangingProperties(true)
            .allowChangesOnlyWhenUsed(true)
            .isPixelSizeConfig(true)
            .build();
  ...
  public void okChosen() {
    try {
      AffineTransform affineTransform =
          AffineUtils.doubleToAffine(affineEditorPanel_.getAffineTransform());
    ...
    try {
        ...
        for (PropertyItem item : data_.getProperties()) {
          if (item.confInclude)
            if (otherPreset.isPropertyIncluded(item.device, item.name))
              if (!item.getValueInCoreFormat()
                  .contentEquals(otherPreset.getSetting(item.device, item.name).getPropertyValue()))
                same = false;
        }

        if (same) {
          showMessageDialog(
              "This combination of properties is already found in the \""
                  + otherPresetName
      ...
      for (PropertyItem item : data_.getProperties()) {
        if (item.confInclude) {
          ...
          if (!same) {
            core_.definePixelSizeConfig(
                newName, item.device, item.name, item.getValueInCoreFormat());
          }
          // if fails to edit an existing preset due to the duplicate presets, keep the old preset
            ...
            core_.definePixelSizeConfig(
                ...
                item.device,
                item.name,
                oldPreset.getSetting(item.device, item.name).getPropertyValue());
          }
        }
      }
