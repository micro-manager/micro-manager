...
public final class OptionsDlg extends JDialog {
  ...
  public OptionsDlg(CMMCore core, MMStudio mmStudio) {
    ...
    final JCheckBox alwaysUseDefaultProfileCheckBox =
        new JCheckBox("Always use the default user profile");
    ...
    alwaysUseDefaultProfileCheckBox.addActionListener(
        (ActionEvent e) -> {
          ...
          if (checked) {
            startupSettings.setSkipConfigSelectionAtStartup(askForConfigFileCheckBox.isSelected());
          } else {
    ...
    askForConfigFileCheckBox.addActionListener(
        (ActionEvent arg0) -> {
          startupSettings.setSkipConfigSelectionAtStartup(!askForConfigFileCheckBox.isSelected());
        });
    askForConfigFileCheckBox.setSelected(!startupSettings.shouldSkipConfigSelectionAtStartup());
    ...
    deleteLogCheckBox.addActionListener(
        (ActionEvent e) -> {
    ...
    deleteLogFilesButton.addActionListener(
        (final ActionEvent e) -> {
    ...
    clearPreferencesButton.addActionListener(
        (final ActionEvent e) -> {
          int answer =
              JOptionPane.showConfirmDialog(
                  OptionsDlg.this,
    ...
    bufSizeField_ =
        new JTextField(Integer.toString(mmStudio_.settings().getCircularBufferSize()), 5);
    ...
    metadataFileWithMultipageTiffCheckBox.addActionListener(
        (ActionEvent arg0) -> {
    ...
    separateFilesForPositionsMPTiffCheckBox.addActionListener(
        (ActionEvent arg0) -> {
    ...
    syncExposureMainAndMDA.addActionListener(
        (ActionEvent arg0) -> {
    ...
    hideMDAdisplay.addActionListener(
        (ActionEvent arg0) -> {
    ...
    super.setLayout(new net.miginfocom.swing.MigLayout("fill, insets dialog", "[fill]"));
    ...
    super.add(deleteLogFilesButton, "split 3, gapleft push, gapright push, wrap");
    ...
    super.add(clearPreferencesButton, "split 2, sizegroup bottomBtns, gapright unrelated");
  ...
  public static boolean isDebugLoggingEnabled(Studio studio) {
    return studio.profile().getSettings(OptionsDlg.class).getBoolean(IS_DEBUG_LOG_ENABLED, false);
  }

  public static void setDebugLoggingEnabled(Studio studio, boolean isEnabled) {
    studio.profile().getSettings(OptionsDlg.class).putBoolean(IS_DEBUG_LOG_ENABLED, isEnabled);
  }

  public static boolean getShouldCloseOnExit(Studio studio) {
    return studio.profile().getSettings(OptionsDlg.class).getBoolean(SHOULD_CLOSE_ON_EXIT, true);
  }

  public static void setShouldCloseOnExit(Studio studio, boolean shouldClose) {
    studio.profile().getSettings(OptionsDlg.class).putBoolean(SHOULD_CLOSE_ON_EXIT, shouldClose);
  }
}
