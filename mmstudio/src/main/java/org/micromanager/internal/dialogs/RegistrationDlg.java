...
public final class RegistrationDlg extends JDialog {
  ...
  public static final String SHOULD_NEVER_REGISTER =
      "this user never wants to be prompted to register";
  public static final String REGISTRATION_ATTEMPTS =
      "number of times we've shown the registration dialog to this user";
  ...
  /**
   * Display the registration dialog, if the user has not registered and not opted out of
   * registering.
   *
   * @param studio Uses studio.app().skin() and studio.profile(), so make sure those are initialized
  ...
  /**
   * Display the registration dialog, For debugging purposes only
   *
   * @param studio Uses studio.app().skin() and studio.profile(), so make sure those are initialized
   *     before calling this function
   */
  ...
  public RegistrationDlg(Studio studio) {
    ...
    welcomeTextArea_.setBackground(
        new Color(studio_.app().skin().getDisabledBackgroundColor().getRGB()));
        ...
        (ActionEvent arg0) -> {
          if (name_.getText().length() == 0 || email_.getText().length() == 0) {
            JOptionPane.showMessageDialog(
                RegistrationDlg.this, "Name and email fields can't be empty.");
          } else {
            try {
              URL url;
              InputStream is;
              ...
              MutablePropertyMapView settings =
                  studio_.profile().getSettings(RegistrationDlg.class);
              ...
              String regText =
                  "http://valelab.ucsf.edu/micro-manager-registration.php?Name="
                      + name1
                      + "&Institute="
                      + inst
                      + "&email="
              ...
              if (response.compareTo("SUCCESS") != 0) {
                JOptionPane.showMessageDialog(
                    RegistrationDlg.this,
                    "Registration did not succeed. You will be prompted again next time.");
                dispose();
                return;
              }
              ...
              ReportingUtils.showError(
                  e,
                  "Registration did not succeed. You are probably not connected to the Internet.\n"
    ...
    skipButton.addActionListener(
        (ActionEvent arg0) -> {
          JOptionPane.showMessageDialog(
              RegistrationDlg.this,
              "You choose to postpone registration.\n"
    ...
    if (getNumRegistrationAttempts(studio_) > 1) {
      ...
      neverButton.addActionListener(
          (ActionEvent arg0) -> {
            JOptionPane.showMessageDialog(
                RegistrationDlg.this,
                "You have chosen never to register. \n"
  ...
  public static void setHaveRegistered(Studio studio, boolean haveRegistered) {
    studio.profile().getSettings(RegistrationDlg.class).putBoolean(HAVE_REGISTERED, haveRegistered);
  }
  ...
  private static int getNumRegistrationAttempts(Studio studio) {
    return studio.profile().getSettings(RegistrationDlg.class).getInteger(REGISTRATION_ATTEMPTS, 0);
  }
}
