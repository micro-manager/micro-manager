...
/**
 * Time-lapse, channel and z-stack acquisition setup dialog. This dialog specifies all parameters
 * for the MDA acquisition.
 *
 * <p>TODO: GUI settings and acquisition engine settings are continuously synchronized. This causes
 * a lot of overhead and a lot of room for bugs. Separate these better, and only synchronize when
 * really needed.
 */
public final class AcqControlDlg extends JFrame
    implements PropertyChangeListener, AcqSettingsListener, TableModelListener {
  ...
  /**
   * Acquisition control dialog box. Specification of all parameters required for the acquisition.
   *
  ...
  public AcqControlDlg(AcquisitionWrapperEngine acqEng, MMStudio mmStudio) {
    ...
    super.setLayout(new MigLayout("fill, flowy, gap 2, insets 6", "[grow, fill]", "[][grow][][]"));
    ...
    JPanel topPanel =
        new JPanel(new MigLayout("fill, insets 0", "[grow, fill]6[grow, fill]6[]", "[grow, fill]"));

    JPanel topLeftPanel =
        new JPanel(new MigLayout("fill, flowy, insets 0", "[grow, fill]", "[]push[]push[]"));
    JPanel topMiddlePanel =
        new JPanel(new MigLayout("fill, flowy, insets 0", "[grow, fill]", "[]push[]push[]"));
    JPanel topRightPanel = new JPanel(new MigLayout("flowy, insets 0", "[]", "10[]10[]10[]push"));
    ...
    acqOrderBox_.addActionListener(
        (ActionEvent e) -> {
    ...
    for (int k = 0; k < columnCount; k++) {
      columnWidth_[k] = settings_.getInteger(ACQ_COLUMN_WIDTH + k, ACQ_DEFAULT_COLUMN_WIDTH);
  ...
  /**
   * Create the panel for showing the timepoints settings. This one can have its contents overridden
   * by the custom time intervals system, in which case its normal contents get hidden.
   */
  private JPanel createTimePoints() {
    framesPanel_ = createCheckBoxPanel("Time Points");
    framesPanel_.setLayout(
        new MigLayout(PANEL_CONSTRAINT + ", hidemode 3", "[grow, fill]", "[grow, fill]"));

    defaultTimesPanel_ =
        new JPanel(new MigLayout("fill, gap 2, insets 0", "push[][][]push", "[][][]"));
    framesPanel_.add(defaultTimesPanel_, "grow");

    customTimesPanel_ = new JPanel(new MigLayout("fill, gap 0, insets 0"));
    ...
    disableCustomIntervalsButton.addActionListener(
        (ActionEvent e) ->
            acqEng_.setSequenceSettings(
                acqEng_.getSequenceSettings().copyBuilder().useCustomIntervals(false).build()));
  ...
  private JPanel createMultiPositions() {
    ...
    positionsPanel_.setLayout(new MigLayout(PANEL_CONSTRAINT, "[grow]", "[grow, fill]"));
    listButton_ = new JButton("Edit Position List...");
    listButton_.setToolTipText("Open XY list dialog");
    listButton_.setIcon(IconLoader.getIcon("/org/micromanager/icons/application_view_list.png"));
  ...
  private JPanel createZStacks() {
    ...
    slicesPanel_.setLayout(new MigLayout("fillx, gap 2, insets 2", "push[][][][]push", ""));
    ...
    slicesPanel_.add(zValCombo_, "skip 1, spanx, gaptop 4, gapbottom 0, alignx left, wrap");
    ...
    stackKeepShutterOpenCheckBox_.addActionListener(
        (final ActionEvent e) -> applySettingsFromGUI());
    slicesPanel_.add(stackKeepShutterOpenCheckBox_, "skip 1, spanx, gaptop 0, alignx left");

    slicesPanel_.addActionListener(
        (final ActionEvent e) -> {
  ...
  private JPanel createAcquisitionOrder() {
    ...
    acquisitionOrderPanel_.setLayout(new MigLayout(PANEL_CONSTRAINT + ", flowy"));
  ...
  private JPanel createAutoFocus() {
    ...
    afButton.setIcon(IconLoader.getIcon("/org/micromanager/icons/wrench_orange.png"));
    ...
    afSkipFrame1.setToolTipText(
        "How many frames to skip between running autofocus. Autofocus is always run at new stage positions");
    ...
    afSkipInterval_.addChangeListener(
        (ChangeEvent e) -> {
          applySettingsFromGUI();
          afSkipInterval_.setValue(acqEng_.getSequenceSettings().skipAutofocusCount());
        });
  ...
  private JPanel createChannelsPanel() {
    ...
    channelsPanel_.setLayout(new MigLayout("fill, gap 2, insets 2", "[grow][]", "[][grow]"));
  ...
  private JPanel createRunButtons() {
    ...
    acquireButton.addActionListener(
        (ActionEvent e) -> {
          AbstractCellEditor ae = (AbstractCellEditor) channelTable_.getCellEditor();
          if (ae != null) {
            ae.stopCellEditing();
  ...
  private JPanel createSavePanel() {
    ...
    savePanel_.setLayout(new MigLayout(PANEL_CONSTRAINT, "[][grow, fill][]", "[][][]"));
    ...
    singleButton_.addActionListener(
        (ActionEvent e) ->
            DefaultDatastore.setPreferredSaveMode(
                mmStudio_, Datastore.SaveMode.SINGLEPLANE_TIFF_SERIES));
    savePanel_.add(singleButton_, "spanx, split");

    multiButton_ = new JRadioButton("Image stack file");
    multiButton_.setFont(DEFAULT_FONT);
    multiButton_.addActionListener(
        (ActionEvent e) ->
            DefaultDatastore.setPreferredSaveMode(mmStudio_, Datastore.SaveMode.MULTIPAGE_TIFF));
  ...
  private JPanel createCommentsPanel() {
    ...
    commentsPanel.setLayout(new MigLayout(PANEL_CONSTRAINT, "[grow, fill]", "[]"));
    ...
    commentScrollPane.setBorder(
        BorderFactory.createCompoundBorder(
            BorderFactory.createLoweredBevelBorder(), BorderFactory.createEtchedBorder()));
  ...
  private void createToolTips() {
    ...
    acquisitionOrderPanel_.setToolTipText(
        TooltipTextMaker.addHTMLBreaksForTooltip(
    ...
    channelsPanel_.setToolTipText(
        "Lets you acquire images in multiple channels (groups of "
            + "properties with multiple preset values");
    savePanel_.setToolTipText(
        TooltipTextMaker.addHTMLBreaksForTooltip(
  ...
  private void updateAcquisitionOrderText() {
    if (acquisitionOrderText_ != null
        && acqOrderBox_ != null
        && acqOrderBox_.getSelectedItem() != null) {
      acquisitionOrderText_.setText(((AcqOrderMode) (acqOrderBox_.getSelectedItem())).getExample());
    }
  }
  ...
  public void onChannelColorEvent(ChannelColorEvent event) {
    ...
    ChannelDisplaySettings newCDS =
        RememberedDisplaySettings.loadChannel(
                mmStudio_, event.getChannelGroup(), event.getChannel(), null)
            .copyBuilder()
            .color(event.getColor())
            .build();
    RememberedDisplaySettings.storeChannel(
        mmStudio_, event.getChannelGroup(), event.getChannel(), newCDS);
  }
    ...
    if (getShouldSyncExposure()) {
      try {
        ...
        if (model_.hasChannel(channelGroup, channel)) {
          double exposure = model_.getChannelExposureTime(channelGroup, channel, 10.0);
          mmStudio_.app().setChannelExposureTime(channelGroup, channel, exposure);
        }
      } catch (Exception e) {
  ...
  public void onChannelGroupChanged(ChannelGroupChangedEvent event) {
    acqEng_.setSequenceSettings(
        acqEng_
            .getSequenceSettings()
            .copyBuilder()
            .channelGroup(event.getNewChannelGroup())
            .build());
  ...
  public final synchronized SequenceSettings loadAcqSettingsFromProfile() {
    ...
    ssb.root(
        settings_.getString(ACQ_ROOT_NAME, System.getProperty("user.home") + "/AcquisitionData"));
    ...
    ssb.keepShutterOpenChannels(settings_.getBoolean(ACQ_CHANNELS_KEEP_SHUTTER_OPEN, false));
    ssb.keepShutterOpenSlices(settings_.getBoolean(ACQ_STACK_KEEP_SHUTTER_OPEN, false));
    ...
    ssb.useCustomIntervals(settings_.getBoolean(ACQ_ENABLE_CUSTOM_INTERVALS, false));
  ...
  /**
   * Updates the dialog (with acqEngine settings) and only returns once this is actually executed
   * (on the EDT).
   */
    ...
    if (!SwingUtilities.isEventDispatchThread()) {
      ...
      interval_.setText(
          numberFormat_.format(
              convertMsToTime(sequenceSettings.intervalMs(), timeUnitCombo_.getSelectedIndex())));
      ...
      for (AcqOrderMode mode : acqOrderModes_) {
        mode.setEnabled(
            ...
            positionsPanel_.isSelected(),
            slicesPanel_.isSelected(),
  ...
  protected void setRootDirectory() {
    File result =
        FileDialogs.openDir(
            this, "Please choose a directory root for image data", FileDialogs.MM_DATA_SET);
    if (result != null) {
      ...
      acqEng_.setSequenceSettings(
          acqEng_
              .getSequenceSettings()
              .copyBuilder()
              .root(result.getAbsolutePath().trim())
              .build());
  ...
  protected void saveAcqSettingsToFile() {
    saveAcqSettingsToProfile();
    File file =
        FileDialogs.save(this, "Save the acquisition settings file", FileDialogs.ACQ_SETTINGS_FILE);
    if (file != null) {
      try {
        SequenceSettings settings = acqEng_.getSequenceSettings();
        mmStudio_.acquisitions().saveSequenceSettings(settings, file.getAbsolutePath());
      } catch (IOException e) {
  ...
  /**
   * Asks acqEngine to estimate memory usage, so use this method only after settings have been send
   * to acqEngine. Prompt the user if there may not be enough memory
   *
  ...
  private boolean warnMemoryMayNotBeSufficient() {
    ...
    if (acqTotalBytes > 0.8 * freeRam) {
      ...
      int answer =
          JOptionPane.showConfirmDialog(this, ep, "Not enough memory", JOptionPane.YES_NO_OPTION);
  ...
  public Datastore runAcquisition() {
    if (acqEng_.isAcquisitionRunning()) {
      JOptionPane.showMessageDialog(
          this, "Cannot start acquisition: previous acquisition still in progress.");
      return null;
    }
    ...
    try {
      ...
      if (acqEng_.getSequenceSettings().useChannels() && model.duplicateChannels()) {
        JOptionPane.showMessageDialog(
            this, "Cannot start acquisition using the same channel twice");
        return null;
      }
      ...
      if (badChannels.size() > 0 && getShouldCheckExposureSanity()) {
        ...
        String message =
            String.format(
                "I found unusually long exposure times for %s. Are you sure you want to run this acquisition?",
                channelString);
  ...
  private void applySettingsFromGUI() {
    ...
    try {
      ...
      ssb.skipAutofocusCount(NumberUtils.displayStringToInt(afSkipInterval_.getValue().toString()));
    ...
    if (singleButton_.isSelected()) {
      DefaultDatastore.setPreferredSaveMode(mmStudio_, Datastore.SaveMode.SINGLEPLANE_TIFF_SERIES);
  ...
  public static class ComponentTitledPanel extends JPanel {
    ...
    /**
     * HACK: when the look and feel changes, the background colors of all components are supposed to
     * get updated. For some reason, our custom titled components don't receive this update, so we
     * have to propagate it manually.
     *
  ...
  public class CheckBoxPanel extends ComponentTitledPanel {
    ...
    CheckBoxPanel(String title) {
      ...
      compTitledBorder =
          new ComponentTitledBorder(checkBox, this, BorderFactory.createEtchedBorder());
      super.setBorder(compTitledBorder);
      borderSet_ = true;
  ...
  public static boolean getShouldSyncExposure() {
    return MMStudio.getInstance()
        .profile()
        .getSettings(AcqControlDlg.class)
        .getBoolean(SHOULD_SYNC_EXPOSURE, true);
  }
  ...
  public boolean getShouldCheckExposureSanity() {
    return profile_.getSettings(AcqControlDlg.class).getBoolean(SHOULD_CHECK_EXPOSURE_SANITY, true);
  }

  public void setShouldCheckExposureSanity(boolean shouldCheck) {
    profile_.getSettings(AcqControlDlg.class).putBoolean(SHOULD_CHECK_EXPOSURE_SANITY, shouldCheck);
  }
}
