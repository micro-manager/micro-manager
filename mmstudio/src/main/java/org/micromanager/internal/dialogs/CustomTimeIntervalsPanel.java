...
/**
 * Creates the custom time point interval.  Original design by Henry Pinkard.
 */
public final class CustomTimeIntervalsPanel extends JPanel {
   ...
   private void createInfoPanel() {
      ...
      JLabel info = new JLabel("<html>This tab allows you to set custom time intervals between "
            + "successive frames. The above table shows the list of time intervals currently "
            + "supplied to Micromanager, as well as their indices and the expected elapsed time "
            + "at the start of collection of each frame.  Each time interval represents the "
            + "minimum delay from the start of the previous frame to the start of the current one. "
            + " The interval corresponding to frame 0 is the delay before the start of "
            + " acquisition, and should be set to 0 ms unless such a delay is desired.  Note "
            + " that the actual intervals between frames and the actual elapsed times may "
            + " exceed the values listed here, depending on the speed with which the hardware "
   ...
   private void createButtons() {
      ...
      removeAllButton.addActionListener(arg0 -> intervalTableModel_.removeAll());
      ...
      createLinearTimePointsButton.addActionListener(e -> {
      ...
      createLogTimePointsButton.addActionListener(e -> {
      ...
      useIntervalsCheckBox_.addActionListener(
            e -> acqEng_.setSequenceSettings(acqEng_.getSequenceSettings().copyBuilder()
   ...
   private class LogTimeDialog extends JDialog {
      private JComboBox<String> creationTypeCombo_;
      private JFormattedTextField aValue_;
      private JFormattedTextField rValue_;
      private JFormattedTextField tValue_;
      private JComboBox<String> aCombo_;
      private JComboBox<String> tCombo_;
      ...
      private double convertToMS(double num, JComboBox<String> unitCombo) {
         double result = num;
      ...
      private void createCheckBoxes() {
         ...
         ActionListener boxesListener = e -> {
            int checkCount = 0;
            if (aCheck_.isSelected()) {
               checkCount++;
            }
         ...
         SpinnerModel sModel = new SpinnerNumberModel(3, 3, null, 1);
         nValue_ = new JSpinner(sModel);
         nValue_.setPreferredSize(new Dimension(80, 22));
         ((JSpinner.DefaultEditor) nValue_.getEditor()).getTextField().setFont(
         ...
         aValue_.addCaretListener(e -> {
            if (aValue_.isEnabled()) {
               updateParameterValues();
            }
         });
         tValue_.addCaretListener(e -> {
            if (tValue_.isEnabled()) {
               updateParameterValues();
            }
         });
         rValue_.addCaretListener(e -> {
            if (rValue_.isEnabled()) {
               updateParameterValues();
            }
         });
         nValue_.addChangeListener(e -> {
            if (nValue_.isEnabled()) {
               updateParameterValues();
            }
         });
         ((JSpinner.DefaultEditor) nValue_.getEditor()).getTextField().addCaretListener(e -> {
            if (nValue_.isEnabled()) {
      ...
      private void createCreationComboAndButtons() {
         ...
         cancelButton.addActionListener(arg0 -> closeLogDialog());
         ...
         createButton.addActionListener(arg0 -> createButtonAction());
   ...
   private class LinearTimeDialog extends JDialog {
      ...
      private void initComponents() {
         ...
         SpinnerModel sModel = new SpinnerNumberModel(2, 2, null, 1);
         numFrames_ = new JSpinner(sModel);
         numFrames_.setPreferredSize(new Dimension(80, 22));
         ((JSpinner.DefaultEditor) numFrames_.getEditor()).getTextField().setFont(
               new Font("Arial", Font.PLAIN, 14));
         row1.add(numFrames_);
         ...
         cancelButton.addActionListener(arg0 -> closeLinearDialog());
   ...
   private class IntervalTableCellRenderer extends DefaultTableCellRenderer {
      ...
      public Component getTableCellRendererComponent(JTable table, Object value,
         ...
         return super.getTableCellRendererComponent(table,
               formatter.format(value), isSelected, hasFocus, row, column);
      }
   }
   ...
   private class TimeIntervalTableModel extends AbstractTableModel {
      ...
      public TimeIntervalTableModel() {
         super();
         timeIntervals_ = new ArrayList<>();
         syncIntervalsFromAcqEng();
         this.addTableModelListener(e -> syncCheckBoxFromAcqEng());
      }
      ...
      public boolean isCellEditable(int row, int col) {
         return col == 1 && row < timeIntervals_.size();
      }
