 ...
 * @author nico
 * @author Jon
 *     <p>TODO: The XYZKeyListener now gets the active z Stage and the amount each keypress should
 *     move each stage from this dialog. The Dialog should also show which keys do what, and
 *     possibly provide the option to change these keys.
 */
public final class StageControlFrame extends JFrame {
  ...
  public static final String[] X_MOVEMENTS =
      new String[] {"SMALLMOVEMENT", "MEDIUMMOVEMENT", "LARGEMOVEMENT"};
  public static final String[] Y_MOVEMENTS =
      new String[] {"SMALLMOVEMENT_Y", "MEDIUMMOVEMENT_Y", "LARGEMOVEMENT_Y"};
  public static final String SMALL_MOVEMENT_Z =
      "SMALLMOVEMENTZ"; // used both by individual ZPanels (with id) and in general
  public static final String MEDIUM_MOVEMENT_Z = "MEDIUMMOVEMENTZ";
  public static final String SELECTED_Z_DRIVE =
      "SELECTED_Z_DRIVE"; // used to keep track of the "active" Z Drive
  private static final String CURRENT_Z_DRIVE =
      "CURRENTZDRIVE"; // used by Drive selection combo boxes
  ...
  private JFormattedTextField[] xStepTexts_ =
      new JFormattedTextField[] {
        new JFormattedTextField(NumberFormat.getNumberInstance()),
        new JFormattedTextField(NumberFormat.getNumberInstance()),
        new JFormattedTextField(NumberFormat.getNumberInstance())
      };
  private JFormattedTextField[] yStepTexts_ =
      new JFormattedTextField[] {
        new JFormattedTextField(NumberFormat.getNumberInstance()),
        new JFormattedTextField(NumberFormat.getNumberInstance()),
        new JFormattedTextField(NumberFormat.getNumberInstance())
  ...
  /**
   * Initialized GUI components based on current hardware configuration Can be called at any time to
   * adjust display (for instance after hardware configuration change). Also called when user
   * requests window to be shown.
   */
  public final void initialize() {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      xStepTexts_[i].setText(NumberUtils.doubleToDisplayString(xStepSizes[i]));
      xStepTexts_[i].addPropertyChangeListener(
          "value",
      ...
      yStepTexts_[i].setText(NumberUtils.doubleToDisplayString(yStepSizes[i]));
      yStepTexts_[i].addPropertyChangeListener(
          "value",
          new PropertyChangeListener() {
            @Override
            public void propertyChange(PropertyChangeEvent evt) {
    ...
    if (haveZ) {
      ...
      for (int idx = MAX_NUM_Z_PANELS - 1; idx > -1; idx--) {
        ...
        ItemListener[] zDriveItemListeners = zDriveSelect_[idx].getItemListeners();
        ...
        int cbIndex =
            model.getIndexOf(
                settings_.getString(CURRENT_Z_DRIVE + idx, "")); // returns -1 if not found
        if (cbIndex < 0) {
          cbIndex =
              model.getIndexOf(
                  (idx < nrZDrives) ? zDrives.get(idx) : ""); // returns -1 if not found
        }
        if (cbIndex < 0) {
          cbIndex = 0;
        }
        zDriveSelect_[idx].setSelectedIndex(
            -1); // needed to make sure setSelectedIndex fires an ItemListener for index 0
        zDriveSelect_[idx].setSelectedIndex(cbIndex);
        if (Objects.equals(
            zDriveSelect_[idx].getSelectedItem(), settings_.getString(SELECTED_Z_DRIVE, " "))) {
        ...
        new MouseAdapter() {
          ...
          public void mouseExited(MouseEvent e) {
            for (int i = 0; i < X_MOVEMENTS.length && i < Y_MOVEMENTS.length; i++) {
              try {
                settings_.putDouble(
                    X_MOVEMENTS[i], NumberUtils.displayStringToDouble(xStepTexts_[i].getText()));
                settings_.putDouble(
                    Y_MOVEMENTS[i], NumberUtils.displayStringToDouble(yStepTexts_[i].getText()));
              } catch (ParseException pex) {
              }
            }
          }
  ...
  /**
   * Called during constructor and never again. Creates GUI components and adds them to JPanel, but
   * they may be turned visible/invisible during operation.
   */
  ...
  private JPanel createXYPanel() {
    ...
    result.add(new JLabel("XY Stage", JLabel.CENTER), "span, alignx center, wrap");
      ...
      button.addActionListener(
          (ActionEvent e) -> {
            ...
            try {
              ...
              if (dx == 0) {
                increment = NumberUtils.displayStringToDouble((yStepTexts_[stepIndex].getText()));
              }
    ...
    for (int i = 0; i < 3; ++i) {
      JLabel indicator =
          new JLabel(
              IconLoader.getIcon(
                  "/org/micromanager/icons/stagecontrol/arrowhead-" + stepSizes[i] + "r.png"));
  ...
  /**
   * NOTE: this method makes assumptions about the layout of the XY panel. In particular, it is
   * assumed that each chevron button is 30px tall, that the step size controls are 20px tall, and
   * that there is a 20px gap between the chevrons and the step size controls.
   */
  private JPanel createZPanel(final int idx) {
    ...
    zDriveActiveButtons_[idx].addItemListener(
        e -> {
          if (e.getStateChange() == ItemEvent.SELECTED) {
            String activeZDrive = (String) zDriveSelect_[idx].getSelectedItem();
            // push to profile
            settings_.putString(SELECTED_Z_DRIVE, activeZDrive);
            settings_.putDouble(SMALL_MOVEMENT_Z, settings_.getDouble(SMALL_MOVEMENT_Z + idx, 1.1));
            settings_.putDouble(
                MEDIUM_MOVEMENT_Z, settings_.getDouble(MEDIUM_MOVEMENT_Z + idx, 11.1));
          }
        });
    ...
    zDriveSelect_[idx].addItemListener(
        (ItemEvent e) -> {
          if (e.getStateChange() == ItemEvent.SELECTED) {
            settings_.putString(
                CURRENT_Z_DRIVE + idx, zDriveSelect_[idx].getSelectedItem().toString());
            try {
    ...
    result.add(
        zDriveActiveButtons_[idx], "height 22!, gaptop 4, gapbottom 4, hidemode 0, alignx center");
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      button.addActionListener(
          (ActionEvent e) -> {
            ...
            JFormattedTextField text =
                (index == 0 || index == 3) ? zStepTextsMedium_[idx] : zStepTextsSmall_[idx];
            try {
      ...
      if (i == 1) {
        ...
        result.add(zPositionLabel_[idx], "height 30!, width 100:, alignx center, growx");
      }
    }
    ...
    plusButtons_[idx].addActionListener(
        (ActionEvent arg0) -> {
          String sysConfigFile = ((MMStudio) studio_).getSysConfigFile(); // TODO add method to API
          int nrZPanels = settings_.getInteger(NR_Z_PANELS + sysConfigFile, 0);
    ...
    timer_.scheduleAtFixedRate(
        new TimerTask() {
          ...
          public void run() {
            ...
            if (staticFrame_ != null
                && staticFrame_.isVisible()) { // don't update if stage control is hidden
  ...
  private JPanel createSettingsPanel() {
    ...
    enableRefreshCB_.addItemListener(
        (ItemEvent e) -> {
  ...
  private JPanel createErrorPanel() {
    ...
    JLabel noDriveLabel = new javax.swing.JLabel("No XY or Z drive found.  Nothing to control.");
  ...
  private void setRelativeXYStagePosition(double x, double y) {
    ...
    uiMovesStageManager_.getXYNavigator().moveXYStageUm(core_.getXYStageDevice(), x, y);
  }
  ...
  private void setZPosLabel(double z, int idx) {
    zPositionLabel_[idx].setText(
        TextUtils.removeNegativeZero(NumberUtils.doubleToDisplayString(z)) + " \u00B5m");
  }
  ...
  private boolean confirmLargeMovementSetting(double movementUm) {
    int response =
        JOptionPane.showConfirmDialog(
            this,
            String.format(
                NumberUtils.doubleToDisplayString(movementUm, 0)
