 ...
 *
 * <p>The only public interface is start().
 *
 * <p>All methods must be called on the EDT.
 */
public final class ProblemReportController {
  ...
  /**
   * Prompt user to reopen an interrupted Problem Report if one exists. Does nothing if the Problem
   * Report window is currently being displayed.
   */
  ...
  /**
   * Activates the Problem Report window. If one is already open, brings it to the front; if not,
   * creates the window.
   */
  ...
  void showFrame(boolean forceReopen, mmcorej.CMMCore core) {
    ...
    if (frame_ == null) {
      int reopenAnswer = forceReopen ? JOptionPane.YES_OPTION : checkForInterruptedReport(true);
  ...
  void setName(String name) {
    if (report_ != null) report_.setUserName(name);
  }
  ...
  void setOrganization(String organization) {
    if (report_ != null) report_.setUserOrganization(organization);
  }
  ...
  void setEmail(String email) {
    if (report_ != null) report_.setUserEmail(email);
  }
    ...
    if (hasUnsentContent_) {
      int result =
          JOptionPane.showConfirmDialog(
              frame_, "Discard this report?", "Cancel Problem Report", JOptionPane.YES_NO_OPTION);
    ...
    new SwingWorker<Object, Object>() {
      ...
      public void done() {
        ...
        if (frame_ != null) {
          ...
          panel.setInstructions(
              "<html><b>Please perform the steps that reproduce the problem.</b></html>");
  ...
  void sendRequested() {
    ...
    if (!isContactInfoValid()) {
      JOptionPane.showMessageDialog(
          frame_, "Please enter your name, organization, and valid email address.");
      return;
    }
    ...
    new SwingWorker<Result, Object>() {
      ...
      public void done() {
        ...
        try {
          ...
          JOptionPane.showMessageDialog(
              frame_, "Failed to generate or send report:\n" + e.getMessage());
          panel.setUIMode(SendReportControlPanel.UIMode.UNSENT);
  ...
  private boolean confirmEmailAddress() {
    ...
    if (!confirmEmail.equals(getEmail())) {
      JOptionPane.showMessageDialog(
          frame_, "Email address does not match; please check your typing.");
      return false;
    }
    return true;
  }
