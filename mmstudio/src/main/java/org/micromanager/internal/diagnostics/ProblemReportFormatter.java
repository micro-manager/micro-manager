...
public final class ProblemReportFormatter {
  ...
  public String format(ProblemReport report) {
    ...
    if (report.hasStartingConfig()) {
      sb.append(
          fileSection(
              "Starting Config (" + report.getStartingConfigFileName() + ")",
              report.getStartingConfig()));
    }
    if (report.configChangedDuringLogCapture() && report.hasEndingConfig()) {
      sb.append(
          fileSection(
              "Ending Config (" + report.getEndingConfigFileName() + ")",
    ...
    if (report.hasHotSpotErrorLog()) {
      sb.append(
          fileSection(
              "HotSpot Error Log (" + report.getHotSpotErrorLogFileName() + ")",
  ...
  private String formatPreamble(ProblemReport report) {
    ...
    sb.append(
        preambleKeyValue("User Description", description.isEmpty() ? "" : "\n" + description));
