...
public final class ProblemReport {
   ...
   *
   * <p>The report will not be backed by persistent storage, and therefore will not be crash-proof.
   *
   * <p>Note that although core is used for logging control, but the information logged may come
   * from global state (the system information classes obtain the Core from the MMStudio singleton).
   *
   * @param core the Core.
   */
   ...
   * from global state (the system information classes obtain the Core from the MMStudio singleton).
   *
   * <p>If there are any errors writing to storageDirectory, they are silently ignored (and the
   * report will behave as if it were non-persistent).
   *
   * @param core the Core.
  ...
  public boolean isUsefulReport() {
    if (metadata_ == null) return false;
    if (capturedLogContent_ != null && !capturedLogContent_.isEmpty()) return true;
    return false;
  }
   ...
   *
   * <p>This is for the name entered by the user, not the login name.
   *
   * @param name the name.
   */
   ...
   *
   * <p>This is the human-readable name entered by the user, not the login name.
   *
   * @return the name.
   */
   ...
   *
   * <p>Save the current hardware configuration file, then start capturing the CoreLog.
   */
   ...
   *
   * <p>After stopping CoreLog capture, the current hardware configuration file is also recorded.
   */
   ...
   *
   * <p>This is intended to be called after startCapturingLog().
   *
  ...
  private void loadReport(File directory) {
    ...
    if (metadata_.startCfgFilename != null) {
      startCfg_ =
          new NamedTextFile(metadata_.startCfgFilename, new File(directory, START_CFG_FILENAME));
    }
