...
class JVMDeadlockedThreadInfoSection implements SystemInfo.SystemInfoSection {
  ...
  public String getReport() {
    ...
    if (deadlockedThreadIds != null && deadlockedThreadIds.length > 0) {
      sb.append("Deadlocked Java threads: ")
          .append(Integer.toString(deadlockedThreadIds.length))
          .append(" detected\n");
      ...
      for (ThreadInfo tInfo : deadlockedInfos) {
        sb.append("Deadlocked Java thread: id ")
        ...
        sb.append("  Blocked waiting to lock ")
            .append(blockingLock.getClassName())
            .append(' ')
            .append(Integer.toString(blockingLock.getIdentityHashCode()))
            .append('\n');
          ...
          for (java.lang.management.MonitorInfo monitor : monitors) {
            if (monitor.getLockedStackFrame().equals(frame)) {
              sb.append("      where monitor was locked: ")
                  .append(monitor.getClassName())
                  .append(' ')
                  .append(Integer.toString(monitor.getIdentityHashCode()))
                  .append('\n');
            }
          }
        }
        for (java.lang.management.LockInfo sync : synchronizers) {
          sb.append("  Ownable synchronizer is locked: ")
              .append(sync.getClassName())
              .append(' ')
              .append(Integer.toString(sync.getIdentityHashCode()))
              .append('\n');
        }
      }
    } else {
  ...
  private void createDeadlockedThreadPair() {
    ...
    Thread th0 =
        new Thread("DeadLockTestThread0") {
          public void run() {
            try {
              synchronized (a) {
                Thread.sleep(100);
    ...
    Thread th1 =
        new Thread("DeadLockTestThread1") {
          public void run() {
            try {
              synchronized (b) {
                Thread.sleep(100);
