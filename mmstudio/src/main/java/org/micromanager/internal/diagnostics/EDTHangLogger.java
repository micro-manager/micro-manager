 ...
 *
 * <p>Note: Failure to call stop() before destructing the Core will result in a crash.
 */
public final class EDTHangLogger {
  ...
  public EDTHangLogger(CMMCore core, long heartbeatTimeoutMs, long hangCheckIntervalMs) {
    ...
    logMessage(
        "Started monitoring of EDT hangs\n"
            + "[heartbeat timeout = "
            + heartbeatTimeoutMs_
            + " ms, hang check interval = "
            + hangCheckIntervalMs_
            + " ms]");
  }
  ...
  private synchronized void heartbeat() {
    ...
    if (missedHeartbeat_) {
      long elapsedSinceTimebaseMs = (lastHeartbeatNs_ - heartbeatTimebaseNs_) / MS_PER_NS;
      logMessage("First heartbeat after miss (" + elapsedSinceTimebaseMs + " ms since timebase)");
    }
    logDebug(2, "Heartbeat after " + (lastHeartbeatNs_ - heartbeatTimebaseNs_) + " ns");
  }
  ...
  private synchronized void checkForHang(boolean firstCheck) {
    ...
    if (firstCheck) {
      ...
      logMessage(
          "Event handling has exceeded at least "
              + elapsedTimeMs
              + " ms (currently "
              + elapsedSinceTimebaseMs
  ...
  private void formatThreadStackTrace(StringBuilder sb, Thread thread, StackTraceElement[] trace) {
    sb.append("\n");
    sb.append(
        "Thread " + thread.getId() + " [" + thread.getName() + "] " + thread.getState().toString());
