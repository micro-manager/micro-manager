...
public final class ToolsMenu {
  ...
  public ToolsMenu(MMStudio studio, JMenuBar menuBar) {
    ...
    centerAndDragMenuItem_ =
        GUIUtils.addCheckBoxMenuItem(
            toolsMenu_,
            ...
            () -> {
              ...
              centerAndDragMenuItem_.setIcon(IconLoader.getIcon("/org/micromanager/icons/" + icon));
            },
            getMouseMovesStage());
    ...
    GUIUtils.addMenuItem(
        ...
        () -> {
          ...
          if (oldBufsize != mmStudio_.settings().getCircularBufferSize()) {
            try {
              core_.setCircularBufferMemoryFootprint(mmStudio_.settings().getCircularBufferSize());
            } catch (Exception exc) {
  ...
  private void populateQuickAccessMenu() {
    ...
    for (final String title : titles) {
      GUIUtils.addMenuItem(
          ...
          "Delete this panel",
          () ->
              ((DefaultQuickAccessManager) mmStudio_.quickAccess())
                  .promptToDelete(titleToFrame.get(title)));
    }
    ...
    for (final String title : titles) {
      GUIUtils.addMenuItem(
          quickAccessMenu_, title, "", () -> titleToFrame.get(title).setVisible(true));
    }
  ...
  public boolean getMouseMovesStage() {
    return mmStudio_.profile().getSettings(ToolsMenu.class).getBoolean(MOUSE_MOVES_STAGE, false);
  }

  public void setMouseMovesStage(boolean doesMove) {
    mmStudio_.profile().getSettings(ToolsMenu.class).putBoolean(MOUSE_MOVES_STAGE, doesMove);
  }
}
