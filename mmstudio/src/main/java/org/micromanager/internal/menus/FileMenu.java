...
public final class FileMenu {
  ...
  /** Fill in the contents of the menu, including the "open recent" dynamic menus. */
    ...
    new Thread(
            new Runnable() {
              ...
              public void run() {
                try {
                  Datastore store =
                      studio_.data().promptForDataToLoad(studio_.app().getMainWindow(), isVirtual);
                  if (store == null) {
                    // User cancelled.
                    return;
                  }
  ...
  private void openSciFIO() {
    File file =
        FileDialogs.openFile(null, "Please select an image data set", FileDialogs.SCIFIO_DATA);
    if (file != null) {
      new Thread(
              new Runnable() {
                @Override
                public void run() {
                  try {
                    SciFIODataProvider sdp =
                        new SciFIODataProvider(studio_, file.getAbsolutePath());
  ...
  private JMenu makeOpenRecentMenu(final boolean isVirtual) {
    JMenu result = new JMenu(String.format("Open Recent (%s)", isVirtual ? "Virtual" : "RAM"));
                          ...
                          try {
                            ...
                            if (!ioe.getMessage().equals("User Canceled")) {
                              ReportingUtils.showError(
                                  ioe, "There was an error while opening data");
                            }
                          }
                        }
                      })
  ...
  /**
   * Whenever a file is opened, add it to our history of recently-opened files.
   *
