...
class PropertyTableModel extends AbstractTableModel implements MMPropertyTableModel {
  ...
  public void updateValues(MicroscopeModel model, int mode, Device dev) {
    model_ = model;
    if (dev == null) {
      if (mode == COMPORT) devices_ = model.getAvailableSerialPorts();
      else devices_ = model.getDevices();
    } else {
      ...
      for (int j = 0; j < devices_[i].getNumberOfProperties(); j++) {
        ...
        if (mode == PREINIT) {
          if (!p.readOnly && p.preInit && !devices_[i].isSerialPort() && !p.readOnly) {
            ...
            if (setupProp != null) p.value = setupProp.value;
          }
  ...
  public Object getValueAt(int rowIndex, int columnIndex) {
    ...
    if (columnIndex == 0) return devNames_[rowIndex];
    else if (columnIndex == 1) return props_[rowIndex].name;
    else return props_[rowIndex].value;
  }
  ...
  public boolean isCellEditable(int nRow, int nCol) {
    if (nCol == 2 && !props_[nRow].readOnly) return true;
    else return false;
  }
