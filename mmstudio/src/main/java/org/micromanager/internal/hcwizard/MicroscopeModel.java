...
public final class MicroscopeModel {
  ...
  public MicroscopeModel() {
    ...
    Device coreDev =
        new Device(MMCoreJ.getG_Keyword_CoreDevice(), "Default", "MMCore", "Core controller");
  ...
  public void loadAvailableDeviceList(CMMCore core) {
    try {
          ...
          try {
            ...
            for (int j = 0; j < devs.length; j++) {
              ReportingUtils.logMessage(
                  "   " + devs[j].getAdapterName() + ", " + devs[j].getDescription());
              if (!devs[j].isSerialPort()) {
                ...
                if (!ports.contains(devs[j])) ports.add(devs[j]);
              }
              good = true;
            }
          } catch (Exception e) {
            ...
            ReportingUtils.logError(
                null, "Unable to load " + libs.get(i) + " library: " + e.getMessage());
          }
        }
        if (!good) badLibraries_.add(libs.get(i));
      }
      ...
      for (int i = 0; i < devsTotal.size(); i++) {
        ...
        if (availableDevices_[i].isHub()) hubs.add(availableDevices_[i]);
      }
  ...
  public Device[] getAvailableDevicesCompact() {
    ...
    for (int i = 0; i < availableDevices_.length; i++) {
      ...
      if (include) compactList.add(availableDevices_[i]);
    }
  ...
  void useSerialPort(int portIndex, boolean use) {
    if (use)
      comPortInUse_.put(availableComPorts_[portIndex].getName(), availableComPorts_[portIndex]);
    else comPortInUse_.remove(availableComPorts_[portIndex].getName());
  }

  void useSerialPort(Device dev, boolean use) {
    if (use) comPortInUse_.put(dev.getName(), dev);
    else comPortInUse_.remove(dev.getName());
  }
  ...
  /**
   * Copy the configuration presets from the hardware and override the current setup data.
   *
      ...
      for (int i = 0; i < curGroups.size(); i++) {
        ...
        for (int j = 0; j < presets.size(); j++) {
          ...
          for (int k = 0; k < cfg.size(); k++) {
            ...
            Setting s =
                new Setting(ps.getDeviceLabel(), ps.getPropertyName(), ps.getPropertyValue());
            p.addSetting(s);
          }
  ...
  public void updateLabelsInPreset(String deviceName, String oldLabel, String newLabel) {
    for (Enumeration<ConfigGroup> e = configGroups_.elements(); e.hasMoreElements(); ) {
    ...
    try {
      ...
      while ((line = input.readLine()) != null) {
        ...
        if (tokens[0].contentEquals(new StringBuffer().append(MMCoreJ.getG_CFGCommand_Device()))) {
          ...
          Device dev =
              new Device(
                  tokens[1], tokens[2], tokens[3], getDeviceDescription(tokens[2], tokens[3]));
          // ReportingUtils.logMessage("Adding: " + tokens[1] + "," +
          // tokens[2] + "," + tokens[3]);
          // get description
          devices_.add(dev);
        } else if (tokens[0].contentEquals(
            new StringBuffer().append(MMCoreJ.getG_CFGCommand_Property()))) {
          ...
          if (tokens[1].contentEquals(
              new StringBuffer().append(MMCoreJ.getG_Keyword_CoreDevice()))) {

            // core device processing
            // ----------------------
            if (tokens[2].contentEquals(
                new StringBuffer().append(MMCoreJ.getG_Keyword_CoreInitialize()))) {
            ...
            new StringBuffer().append(MMCoreJ.getG_CFGCommand_Label()))) {
          // -------------------------------------------------------------
          // "Label" command
          // -------------------------------------------------------------
          if (tokens.length != 4) {
          ...
          addSetupLabel(tokens[1], lab);
            ...
            new StringBuffer().append(MMCoreJ.getG_CFGCommand_ImageSynchro()))) {
          // -------------------------------------------------------------
          ...
          synchroDevices_.add(tokens[1]);
            ...
            new StringBuffer().append(MMCoreJ.getG_CFGCommand_ConfigGroup()))) {
          // -------------------------------------------------------------
          ...
          if (tokens.length == 6) {
            ...
            new StringBuffer().append(MMCoreJ.getG_CFGCommand_ConfigPixelSize()))) {
          // -------------------------------------------------------------
          ...
          pixelSizeGroup_.addConfigSetting(tokens[1], tokens[2], tokens[3], tokens[4]);
            ...
            new StringBuffer().append(MMCoreJ.getG_CFGCommand_PixelSize_um()))) {
          // -------------------------------------------------------------
          // "PixelSize" commands
          // -------------------------------------------------------------
          if (tokens.length == 3) {
            ...
            new StringBuffer().append(MMCoreJ.getG_CFGCommand_PixelSizeAffine()))) {
          if (tokens.length == 8) {
            ConfigPreset cp = pixelSizeGroup_.findConfigPreset(tokens[1]);
            if (cp != null) {
            ...
            new StringBuffer().append(MMCoreJ.getG_CFGCommand_Delay()))) {
          // -------------------------------------------------------------
          // "Delay" commands
          // -------------------------------------------------------------
          if (tokens.length != 3) {
  ...
  public void saveToFile(String path) throws MMConfigFileException {
    try {
      ...
      for (int i = 0; i < availableComPorts_.length; i++) {
        ...
        if (isPortInUse(dev)) {
          out.write(
              MMCoreJ.getG_CFGCommand_Device()
                  + ","
                  + dev.getName()
                  + ","
                  + dev.getLibrary()
      ...
      for (int i = 0; i < devices_.size(); i++) {
        ...
        for (int j = 0; j < dev.getNumberOfSetupProperties(); j++) {
          ...
          if (prop.preInit) {
            out.write(
                MMCoreJ.getG_CFGCommand_Property()
                    + ","
                    + dev.getName()
                    + ","
                    + prop.name
      ...
      for (int i = 0; i < devices_.size(); i++) {
        ...
        if (!(parentID.length() == 0)) {
          out.write(MMCoreJ.getG_CFGCommand_ParentID() + "," + dev.getName() + "," + parentID);
          out.newLine();
        }
      }
      out.newLine();
      ...
      for (Device dev : devices_) {
        if (dev.isStage()) {
          ...
          out.write(
              MMCoreJ.getG_CFGCommand_FocusDirection() + "," + dev.getName() + "," + direction);
          out.newLine();
        }
      }
      out.newLine();
      ...
      for (int i = 0; i < groups.length; i++) {
        ...
        for (int j = 0; j < presets.length; j++) {
          ...
          for (int k = 0; k < presets[j].getNumberOfSettings(); k++) {
            ...
            // write setting
            out.write(
                MMCoreJ.getG_CFGCommand_ConfigGroup()
                    + ","
                    + group.getName()
                    + ","
                    + presets[j].getName()
      ...
      for (int j = 0; j < presets.length; j++) {
        ...
        for (int k = 0; k < presets[j].getNumberOfSettings(); k++) {
          ...
          // write setting
          out.write(
              MMCoreJ.getG_CFGCommand_ConfigPixelSize()
                  + ","
                  + presets[j].getName()
                  + ","
                  + s.deviceName_
        ...
        out.write(
            MMCoreJ.getG_CFGGroup_PixelSizeUm()
                + ","
                + presets[j].getName()
                + ","
        ...
        if (aft != null) {
          out.write(MMCoreJ.getG_CFGCommand_PixelSizeAffine() + "," + presets[j].getName());
  ...
  public void removeDevice(String devName) {
    ...
    if (dev != null) {
      ...
      if (!(port.length() == 0)) {
        ...
        if (!inUse) comPortInUse_.remove(port);
      }
  ...
  public void addDevice(Device dev) throws MMConfigFileException {
    ...
    if (findDevice(dev.getName()) != null) {
      throw new MMConfigFileException(
          dev.getName() + " already defined, please choose a different name.");
    }
    devices_.add(dev);
    modified_ = true;
  }
  ...
  public void setDeviceSetupProperty(String devName, String propName, String value)
      throws MMConfigFileException {
    Device c = findDevice(devName);
    if (c == null) {
  ...
  /**
   * Remove configurations which refer to non existent devices. This situation may occur if some
   * devices are removed while still referred to by the config group.
   */
  ...
  public void AddSelectedPeripherals(
      CMMCore c, Vector<Device> pd, Vector<String> hubs, Vector<Boolean> sel) {
    for (int idit = 0; idit < pd.size(); ++idit) {
      if (sel.get(idit)) {
        Device newDev =
            new Device(
                pd.get(idit).getName(),
                pd.get(idit).getLibrary(),
                pd.get(idit).getAdapterName(),
                pd.get(idit).getDescription());
  ...
  /**
   * This method attempts to initialize all devices in a model, simulating what MMCore does upon
   * loading configuration file
   *
   * @param core_
   * @param amLoading Flag used to synchoinize with Loading Devices GUI elemnt Since we may display
   *     a dialog in this function, we need a way to get rid of the Loading Devices message
   */
    ...
    for (String key : comPortInUse_.keySet()) {
      try {
        ...
        for (int i = 0; i < portDev.getNumberOfSetupProperties(); i++) {
          ...
          if (pi.preInit) core_.setProperty(portDev.getName(), pi.name, pi.value);
        }
    ...
    for (Device d : new ArrayList<>(devices_)) {
      if (!d.isInitialized() && !d.isCore()) {
        try {
          ...
          if (!(parentHub.length() == 0)) core_.setParentLabel(d.getName(), parentHub);
