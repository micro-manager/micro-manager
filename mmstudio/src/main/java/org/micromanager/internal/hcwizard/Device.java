...
/** Data structure describing a general MM device. Part of the MicroscopeModel. */
public final class Device {
  ...
  public String[] getPreInitProperties() {
    ...
    for (PropertyItem p : properties_) {
      if (p.preInit) piProps.add(p.name);
    }
  ...
  public String getPropertyValue(String propName) throws MMConfigFileException {
    ...
    if (p == null) throw new MMConfigFileException("Property " + propName + " is not defined");
    return p.value;
  }

  public void setPropertyValue(String name, String value) throws MMConfigFileException {
    PropertyItem p = findProperty(name);
    if (p == null) throw new MMConfigFileException("Property " + name + " is not defined");
    p.value = value;
  }
  ...
  public PropertyItem findProperty(String name) {
    for (int i = 0; i < properties_.length; i++) {
      ...
      if (p.name.contentEquals(new StringBuffer().append(name))) return p;
    }
    return null;
  }
  ...
  public void updateSetupProperties() {
    ...
    for (int i = 0; i < properties_.length; i++) {
      setupProperties_.add(
          new PropertyItem(properties_[i].name, properties_[i].value, properties_[i].preInit));
    }
  }
