...
/**
 * This class is responsible for all logic surrounding live mode and the "snap image" display (which
 * is the same display as that used for live mode).
 *
...
public final class SnapLiveManager extends DataViewerListener
  ...
  private final PerformanceMonitor perfMon_ = PerformanceMonitor.createWithTimeConstantMs(1000.0);
  ...
  private class DisplayInfo {
    ...
    public void setImageInfo(
        final int width, final int height, final int numComponents, final int bytesPerPixel) {
      width_ = width;
      height_ = height;
  ...
  /**
   * If live mode needs to temporarily stop for some action (e.g. changing the exposure time), then
   * clients can blindly call setSuspended(true) to stop it and then setSuspended(false) to
   * resume-only-if-necessary. Note that this function will not notify listeners. We need to handle
   * the case where we get nested calls to setSuspended, hence the reference count. And if we *are*
   * suspended and someone tries to start live mode (even though it wasn't running when we started
   * the suspension), then we should automatically start live mode when the suspension ends. Thus,
   * isLiveOn_ tracks the "nominal" state of live mode, irrespective of whether or not it is
   * currently suspended. When suspendCount_ hits zero, we match up the actual state of live mode
          ...
          new Runnable() {
            ...
            public void run() {
              ...
              synchronized (SnapLiveManager.this) {
                if (scheduledGrab_ == null || liveModeStartCount_ != liveModeCount) {
                  return;
                }
              }
              ...
              if (display_ != null) {
                displayIntervalLowQuantileMs =
                    display_.getDisplayIntervalQuantile(DISPLAY_INTERVAL_ESTIMATE_Q);
              } else {
              ...
              synchronized (SnapLiveManager.this) {
                ...
                scheduledGrab_ = scheduler_.schedule(this, delayMs, TimeUnit.MILLISECONDS);
              }
  ...
  private static long computeGrabDelayMs(
      double exposureMs, double displayIntervalMs, double alreadyElapsedMs) {
  ...
  private void stopLiveMode() {
    ...
    try {
      ...
      ReportingUtils.showError(
          e, "Failed to stop sequence acquisition. Double-check shutter status.");
    }
  }

  /** This method takes images out of the Core and inserts them into our pipeline. */
  private void grabAndAddImages(String camName, final long liveModeCount) {
    try {
      ...
      for (int c = 0; c < 6 * numCameraChannels_; ++c) {
        ...
        perfMon_.sample("Image missing ImageNumber (%)", seqNr == null ? 100.0 : 0.0);
        Coords newCoords = image.getCoords().copyBuilder().t(0).c(imageChannel).build();
        // Generate a new UUID for the image, so that our histogram
        // update code realizes this is a new image.
        Metadata newMetadata = image.getMetadata().copyBuilderWithNewUUID().build();
  ...
  private void createDisplay() {
    DisplayWindowControlsFactory controlsFactory = (DisplayWindow display) -> createControls();
    display_ =
        new DisplayController.Builder(store_).controlsFactory(controlsFactory).build(mmStudio_);
    DisplaySettings ds =
        DefaultDisplaySettings.restoreFromProfile(
            mmStudio_.profile(), PropertyKey.SNAP_LIVE_DISPLAY_SETTINGS.key());
    if (ds == null) {
      ds = DefaultDisplaySettings.builder().colorMode(DisplaySettings.ColorMode.GRAYSCALE).build();
    }
    for (int ch = 0; ch < store_.getSummaryMetadata().getChannelNameList().size(); ch++) {
      ds =
          ds.copyBuilderWithChannelSettings(
                  ch,
  ...
  private List<Component> createControls() {
    ...
    JComponent snapButton =
        QuickAccessFactory.makeGUI(
            mmStudio_
                .plugins()
                .getQuickAccessPlugins()
                .get("org.micromanager.quickaccess.internal.controls.SnapButton"));
    snapButton.setPreferredSize(buttonSize);
    snapButton.setMinimumSize(buttonSize);
    controls.add(snapButton);

    JComponent liveButton =
        QuickAccessFactory.makeGUI(
            mmStudio_
                .plugins()
                .getQuickAccessPlugins()
                .get("org.micromanager.quickaccess.internal.controls.LiveButton"));
    liveButton.setPreferredSize(buttonSize);
    liveButton.setMinimumSize(buttonSize);
    controls.add(liveButton);

    JButton toAlbumButton =
        new JButton("Album", IconLoader.getIcon("/org/micromanager/icons/camera_plus_arrow.png"));
    ...
    toAlbumButton.addActionListener(
        (ActionEvent event) -> {
          ...
          for (int i = 0; i < store_.getNextIndex(Coords.CHANNEL); ++i) {
            builder.channel(i);
            try {
              mmStudio_.album().addImages(store_.getImagesMatching(builder.build()));
              hadChannels = true;
            } catch (IOException e) {
          ...
          try {
            if (!hadChannels) {
              mmStudio_.album().addImages(store_.getImagesMatching(Coordinates.builder().build()));
            }
          } catch (IOException e) {
  ...
  /**
   * Display the provided image. Due to limitations of ImageJ, if the image's parameters (width,
   * height, or pixel type) change, we have to recreate the display and datastore.
   *
  ...
  public void displayImage(final Image image) {
      ...
      for (int camCh = 0; camCh < numCameraChannels_; ++camCh) {
        ...
        if (channelNames == null || camCh >= channelNames.size()) {
          ...
          if (display_ != null && !display_.isClosed()) {
            ...
            ChannelDisplaySettings newCD =
                RememberedDisplaySettings.loadChannel(
                    mmStudio_, core_.getChannelGroup(), name, null);
            display_.setDisplaySettings(
                display_.getDisplaySettings().copyBuilderWithChannelSettings(camCh, newCD).build());
          }
          channelNames.set(camCh, name);
          store_.setSummaryMetadata(
              store_
                  .getSummaryMetadata()
                  .copyBuilder()
    ...
    try {
      DefaultImage newImage =
          new DefaultImage(
              image, image.getCoords(), mmStudio_.acquisitions().generateMetadata(image, true));
      ...
      if ((displayInfo_ != null)
        ...
        if (prevSeqNr != null && newSeqNr != null) {
          if (Objects.equals(prevSeqNr, newSeqNr)) {
            perfMon_.sample("Image rejected based on ImageNumber (%)", 100.0);
            return; // Already displayed this image
          }
          perfMon_.sample(
              "Frames dropped at sequence buffer exit (%)",
      ...
      synchronized (displayInfoLock_) {
        ...
        displayInfo_.setImageInfo(
            newImage.getWidth(),
            newImage.getHeight(),
            newImage.getNumComponents(),
      ...
      synchronized (pipelineLock_) {
        try {
          ...
          mmStudio_
              .logs()
              .showError(e, "Unable to insert image into pipeline; this should never happen.");
        } catch (PipelineErrorException e) {
          // Notify the user, and halt live.
          mmStudio_.logs().showError(e, "An error occurred while processing images.");
          stopLiveMode();
  ...
  private void createOrResetDatastoreAndDisplay() {
    ...
    try {
      ...
      try {
        ...
        ReportingUtils.logError(e, "Unable to update store summary metadata");
      }
    } catch (Exception e) {
  ...
  /**
   * Make a name for the given channel/camera number combination. This tries to replicate the code
   * in the acquisition engine. TODO: Combine code in acq engine and this function
   */
