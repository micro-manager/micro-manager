...
public class StagePosition {
  /**
   * For two-axis stages, the X position; for one-axis stages, the only stage position. For example,
   * if using a Z focus drive, its position would be given by the "x" parameter.
   *
   ...
   *
   * @deprecated Use {@link #set1DPosition} or {@link #set2DPosition} to set; use {@link
   *     #getStageDeviceLabel} to access.
   */
  @Deprecated public String stageName;

  /**
   * The number of stage axes. Must be 1 or 2.
   *
   * @deprecated Use {@link #set1DPosition} or {@link #set2DPosition} to set; use {@link
   *     #getNumberOfStageAxes}, {@link #is1DStagePosition}, or {@link #is2DStagePosition} to
   *     access.
   */
  @Deprecated public int numAxes;
   ...
   * @return StagePosition object based on the input
   */
  public static StagePosition create2D(String stageDeviceLabel, double x, double y) {
    StagePosition ret = new StagePosition();
  ...
  public String getVerbose() {
    if (numAxes == 1) return stageName + "(" + NumberUtils.doubleToDisplayString(x) + ")";
    else if (numAxes == 2)
      return stageName
          + "("
          + NumberUtils.doubleToDisplayString(x)
          + ","
  ...
  /**
   * Compare us against the provided StagePosition and return true only if we are equal in all
   * respects.
   *
  ...
  public static StagePosition fromPropertyMap(PropertyMap pmap) {
    ...
    switch (n) {
      case 1:
        return StagePosition.create1D(
            pmap.getString(PropertyKey.STAGE_POSITION__DEVICE.key(), null),
            pmap.getDoubleList(PropertyKey.STAGE_POSITION__POSITION_UM.key())[0]);
      case 2:
        return StagePosition.create2D(
            pmap.getString(PropertyKey.STAGE_POSITION__DEVICE.key(), null),
            pmap.getDoubleList(PropertyKey.STAGE_POSITION__POSITION_UM.key())[0],
            pmap.getDoubleList(PropertyKey.STAGE_POSITION__POSITION_UM.key())[1]);
      default:
        throw new IllegalArgumentException(
            String.format("Invalid stage position (%d-axis stage not supported)", n));
    }
  }
}
