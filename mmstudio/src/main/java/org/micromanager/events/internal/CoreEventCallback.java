 ...
 * EventManager's event bus. Callbacks are all issued on the EDT to avoid deadlock
 */
public final class CoreEventCallback extends MMEventCallback {
  ...
  public void onPropertiesChanged() {
    ...
    if (engine_ != null && engine_.isAcquisitionRunning()) {
      ...
      SwingUtilities.invokeLater(() -> studio_.events().post(new DefaultPropertiesChangedEvent()));
    }
  }
  ...
  public void onPropertyChanged(String deviceName, String propName, String propValue) {
    ...
    SwingUtilities.invokeLater(
        () ->
            studio_
                .events()
                .post(new DefaultPropertyChangedEvent(deviceName, propName, propValue)));
  }

  @Override
  public void onChannelGroupChanged(String newChannelGroupName) {
    SwingUtilities.invokeLater(
        () -> studio_.events().post(new DefaultChannelGroupChangedEvent(newChannelGroupName)));
  }
  ...
  public void onConfigGroupChanged(String groupName, String newConfig) {
    // see OnPropertyChanged for reasons to run this on the EDT
    SwingUtilities.invokeLater(
        () -> studio_.events().post(new DefaultConfigGroupChangedEvent(groupName, newConfig)));
  }
  ...
  public void onPixelSizeChanged(double newPixelSizeUm) {
    // see OnPropertyChanged for reasons to run this on the EDT
    SwingUtilities.invokeLater(
        () -> studio_.events().post(new DefaultPixelSizeChangedEvent(newPixelSizeUm)));
  }

  @Override
  public void onPixelSizeAffineChanged(
      double npa0, double npa1, double npa2, double npa3, double npa4, double npa5) {
    double[] flatMatrix = {npa0, npa1, npa2, npa3, npa4, npa5};
    AffineTransform newPixelSizeAffine = new AffineTransform(flatMatrix);
    // see OnPropertyChanged for reasons to run this on the EDT
    SwingUtilities.invokeLater(
        () -> studio_.events().post(new DefaultPixelSizeAffineChangedEvent(newPixelSizeAffine)));
  }
  ...
  public void onStagePositionChanged(String deviceName, double pos) {
    // see OnPropertyChanged for reasons to run this on the EDT
    SwingUtilities.invokeLater(
        () -> studio_.events().post(new DefaultStagePositionChangedEvent(deviceName, pos)));
  }
  ...
  public void onXYStagePositionChanged(String deviceName, double xPos, double yPos) {
    ...
    SwingUtilities.invokeLater(
        () ->
            studio_.events().post(new DefaultXYStagePositionChangedEvent(deviceName, xPos, yPos)));
  }
  ...
  public void onExposureChanged(String deviceName, double exposure) {
    // see OnPropertyChanged for reasons to run this on the EDT
    SwingUtilities.invokeLater(
        () -> studio_.events().post(new DefaultExposureChangedEvent(deviceName, exposure)));
  }
  ...
  public void onSLMExposureChanged(String deviceName, double exposure) {
    // see OnPropertyChanged for reasons to run this on the EDT
    SwingUtilities.invokeLater(
        () -> studio_.events().post(new DefaultSLMExposureChangedEvent(deviceName, exposure)));
  }
