 ...
 *
 * <p><strong>Important</strong>: Custom Datastore implementations are not supported (consider
 * creating a custom storage instead).
 */
public interface Datastore extends DataProvider {
  ...
  /**
   * Insert an image into the Datastore. Posts a NewImageEvent to the event bus.
   *
   * @param image Micro-Manager Image object
   * @throws java.io.IOException if an IO error occurs
   * @throws DatastoreFrozenException if the freeze() method has been called.
   * @throws DatastoreRewriteException if an Image with the same coordinates already exists in the
   *     Datastore.
  ...
  /**
   * Set the SummaryMetadata. Posts a NewSummaryMetadataEvent to the event bus. This method may only
   * be called once for a given Datastore.
   *
   * @param metadata Object representing the summary metadata
   * @throws java.io.IOException if an IO error occurs
   * @throws DatastoreFrozenException if the freeze() method has been called.
   * @throws DatastoreRewriteException if the Datastore already has SummaryMetadata.
   */
   ...
   *
   * <p>In the current Micro-Manager file format, this path is used as the enclosing directory.
   *
   ...
   *
   * <p>If this is an in-memory datastore, returns whatever was set by {@link #setSavePath}. If this
   * is an on-disk datastore, returns the actual path.
   *
  ...
  /**
   * @param parent Window on top of which to display the dialog prompt; may be null.
  ...
  /**
   * Saves the datastore to an interactively determined path. Opens a file dialog prompting user for
   * a storage location
   *
   * @param parent Window on top of which to display a dialog prompting the user for a location to
   *     save. After displaying
   * @param blocking if true will return after saving, otherwise will return quickly and continue
   *     saving on another thread
  ...
  /**
   * Saves the datastore to the given path using the given format (SaveMode) Will save synchronously
   * (i.e. this function will block)
   *
   ...
   * @param mode File format to save to
   * @param path File path used to save the data
   * @param blocking when true, will block while saving data, otherwise will return immediately
   * @throws java.io.IOException if an IO error occurs
   */
