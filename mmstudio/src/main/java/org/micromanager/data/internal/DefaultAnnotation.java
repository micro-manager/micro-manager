...
public final class DefaultAnnotation implements Annotation {
    ...
    if (target.exists()) {
      try {
        ...
        if (data.containsKey(GENERAL_KEY)) {
          generalAnnotation_ = data.getPropertyMap(GENERAL_KEY, PropertyMaps.builder().build());
        }
  ...
  /**
   * Utility function to get the path at which an Annotation's data should reside, if it exists.
   *
   * @param store
   * @param filename
   * @return
  ...
  /**
   * Return true if there's an existing annotation for this datastore, on disk where the datastore's
   * own data is stored.
   *
   * @param store
   * @param filename
