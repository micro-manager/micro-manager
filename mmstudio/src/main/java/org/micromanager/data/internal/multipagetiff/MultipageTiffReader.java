...
public final class MultipageTiffReader {
   ...
   * @param firstImage
   */
  public MultipageTiffReader(
      StorageMultipageTiff masterStorage,
      SummaryMetadata summaryMD,
  ...
  /**
   * HACK: this version is only used when fixing index maps. Ideally said fixing would be done
   * without needing to create a new MultipageTiffReader object, but that would require
   * disentangling the file reading code that does some setup before fixIndexMap() is called.
   */
  ...
  public static boolean isMMMultipageTiff(String directory) throws IOException {
    ...
    if (children == null) {
      throw new IOException(
          directory + " does not appear to be a directory; is this a \u00B5Manager dataset?");
    }
    File testFile = null;
    for (File child : children) {
      ...
      } else if ((!child.getName().startsWith("._")) && child.getName().endsWith(".tif")
    ...
    if (testFile == null) {
      throw new IOException(
          "Unable to find any .tif files in " + directory + "; is this a \u00B5Manager dataset?");
    }
    RandomAccessFile ra;
    try {
  ...
  public Set<Coords> getIndexKeys() {
    if (coordsToOffset_ == null) return null;
  ...
  private void readSummaryMD() throws IOException {
    ...
    imageFormatReadFromSummary_ = NonPropertyMapJSONFormats.imageFormat().fromGson(summaryGson);
    summaryMetadata_ =
  ...
  /**
   * Read the comments block from the end of the file. This should be a JSONObject containing two
   * potential types of comment: a summary comment and per-image comments. They're stored under the
   * "Summary" key for the summary comment, and under coordinate strings for the per-image comments.
   * In the event that we find comments here *and* there is no existing Annotation storing comment
   * data for this Datastore, we should convert any comments we find here into an Annotation.
   */
  private void readComments() throws IOException {
    ...
    try {
      ...
      for (String key : MDUtils.getKeys(comments)) {
        ...
        builder.channel(indices[0]).z(indices[1]).time(indices[2]).stagePosition(indices[3]);
        CommentsHelper.setImageComment(store, builder.build(), comments.getString(key));
      }
      if (didCreate) {
  ...
  private void readIndexMap() throws IOException, InvalidIndexMapException {
    ...
    for (int i = 0; i < numMappings; i++) {
      ...
      builder.channel(channel).z(slice).t(frame).stagePosition(position);
  ...
  private IFDData readIFD(long byteOffset) throws IOException {
    ...
    if (data.pixelOffset == 0
        || data.bytesPerImage == 0
        || data.mdOffset == 0
        || data.mdLength == 0) {
      throw new IOException("Failed to read image from file at offset " + byteOffset);
    }
  ...
  private Image readImage(IFDData data) throws IOException {
    ...
    try {
      PropertyMap formatPmap = NonPropertyMapJSONFormats.imageFormat().fromGson(mdGson);
      Coords coords =
          DefaultCoords.fromPropertyMap(NonPropertyMapJSONFormats.coords().fromGson(mdGson));
      Metadata metadata =
          DefaultMetadata.fromPropertyMap(NonPropertyMapJSONFormats.metadata().fromGson(mdGson));
      ...
      PixelType pixelType =
          formatPmap.getStringAsEnum(PropertyKey.PIXEL_TYPE.key(), PixelType.class, null);
      if (pixelType == null) {
        pixelType =
            imageFormatReadFromSummary_.getStringAsEnum(
        ...
        formatPmap =
            formatPmap
                .copyBuilder()
                .putEnumAsString(PropertyKey.PIXEL_TYPE.key(), pixelType)
                .build();
      }
      ...
      switch (pixelType) {
        case GRAY8:
          return new DefaultImage(pixelBuffer.array(), formatPmap, coords, metadata);
        case GRAY16:
  ...
  private void fixIndexMap(long firstIFD, String fileName) throws IOException {
    ...
    while (filePosition > 0) {
      try {
        ...
        if (((DefaultMetadata) image.getMetadata())
            .toPropertyMap()
            .equals(new DefaultMetadata.Builder().build())) {
    ...
    filePosition += writeDisplaySettings(DefaultDisplaySettings.builder().build(), filePosition);
  ...
  private int writeDisplaySettings(DisplaySettings settings, long filePosition) throws IOException {
    ...
    int numReservedBytes =
        settingsJSON.length() * MultipageTiffWriter.DISPLAY_SETTINGS_BYTES_PER_CHANNEL;
      ...
      for (String axis : coords.getAxes()) {
        if (!ALLOWED_AXES.contains(axis)) {
          ReportingUtils.logError(
              "Axis " + axis + " is ignored because it is not one of " + ALLOWED_AXES.toString());
        }
      }
  ...
  private class IFDData {
    ...
    public String toString() {
      return String.format(
          "<IFDData offset %d, bytes %d, metadata offset %d, metadata length %d, next %d, next offset %d>",
          pixelOffset, bytesPerImage, mdOffset, mdLength, nextIFD, nextIFDOffsetLocation);
    }
  }
  ...
  private class IFDEntry {
    ...
    public String toString() {
      return String.format(
          "<IFDEntry tag 0x%s, type 0x%s, count %d, value %d>",
          Integer.toHexString((int) tag), Integer.toHexString((int) type), count, value);
    }
  }
}
