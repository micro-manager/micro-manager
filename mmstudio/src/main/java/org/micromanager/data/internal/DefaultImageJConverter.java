...
public final class DefaultImageJConverter implements ImageJConverter {
  ...
  /**
   * Creates an Image ImageProcessor with the pixel type and dimensions of the input Image, but all
   * pixel values set to 0
   *
  ...
  @Override
  public ImageProcessor createProcessorFromComponent(Image image, int component) {
    int numComponents = image.getNumComponents();
    if (numComponents == 1) {
    ...
    // currently.
    if (bytesPerPixel == 4 && numComponents == 3 && pixels instanceof byte[]) {
      byte[] subPixels = ImageUtils.singleChannelFromRGB32((byte[]) pixels, component);
      return new ByteProcessor(image.getWidth(), image.getHeight(), subPixels);
    } else {
  ...
  @Override
  public Image createImage(ImageProcessor processor, Coords coords, Metadata metadata) {
