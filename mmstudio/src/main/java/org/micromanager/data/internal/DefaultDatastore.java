...
public class DefaultDatastore implements Datastore {
  ...
  private static final FileFilter SINGLEPLANEFILTER = new SaveFileFilter(SINGLEPLANE_TIFF_SERIES);
  private static final FileFilter MULTIPAGEFILTER = new SaveFileFilter(MULTIPAGE_TIFF);
  ...
  /**
   * Copy all data from the source Datastore into ourselves. The optional ProgressMonitor can be
   * used to keep callers appraised of our progress.
   *
      ...
      for (String axis : coords.getAxes()) {
        if (!ourAxes.contains(axis) && coords.getIndex(axis) > 0) {
          throw new IllegalArgumentException(
              "Invalid image coordinate axis " + axis + "; allowed axes are " + ourAxes);
        }
      }
    }
  ...
  public boolean hasAnnotation(String tag) {
    return (annotations_.containsKey(tag) || DefaultAnnotation.isAnnotationOnDisk(this, tag));
  }

  public Annotation createNewAnnotation(String filename) {
    if (hasAnnotation(filename)) {
      throw new IllegalArgumentException(
          "Annotation \"" + filename + "\" for datastore at " + savePath_ + " already exists");
    }
    try {
  ...
  public void close() throws IOException {
    freeze();
    studio.events().post(new DefaultDatastoreClosingEvent(this));
    if (storage_ != null) {
      storage_.close();
      System.gc();
    }
  ...
  public String save(Component parent, boolean blocking) throws IOException {
    ...
    if (filter == SINGLEPLANEFILTER) {
      ...
      ReportingUtils.showError("Unrecognized file format filter " + filter.getDescription());
      return null;
    }
    ...
    // saving all the data
    DefaultDataSaver ds = new DefaultDataSaver(studio, this, mode, file.getAbsolutePath());
    if (!blocking) {
      final ProgressBar pb = new ProgressBar(parent, "Saving..", 0, 100);
      ds.addPropertyChangeListener(
  ...
  public static Datastore.SaveMode getPreferredSaveMode(Studio studio) {
    String modeStr =
        studio
            .profile()
            .getSettings(DefaultDatastore.class)
            .getString(PREFERRED_SAVE_FORMAT, MULTIPAGE_TIFF);
  ...
  public static void setPreferredSaveMode(Studio studio, Datastore.SaveMode mode) {
    ...
    studio.profile().getSettings(DefaultDatastore.class).putString(PREFERRED_SAVE_FORMAT, modeStr);
  }
}
