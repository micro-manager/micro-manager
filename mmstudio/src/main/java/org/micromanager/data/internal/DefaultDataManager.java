...
public final class DefaultDataManager implements DataManager {
  ...
  @Override
  public RewritableDatastore createRewritableRAMDatastore(Datastore storeToCopy) {
    throw new UnsupportedOperationException();
  }

  @Override
  public Datastore createMultipageTIFFDatastore(
    ...
    result.setStorage(
        new StorageMultipageTiff(
            null, result, directory, true, shouldGenerateSeparateMetadata, shouldSplitPositions));
    return result;
  }
  ...
  public Datastore createSinglePlaneTIFFSeriesDatastore(String directory) throws IOException {
    DefaultDatastore result = new DefaultDatastore(studio_);
    result.setStorage(new StorageSinglePlaneTiffSeries(result, directory, true));
    return result;
  }

  @Override
  public Datastore createSinglePlaneTIFFSeriesDatastore(Datastore storeToCopy, String directory)
      throws IOException {
    throw new UnsupportedOperationException();
  }
  ...
  public Datastore promptForDataToLoad(Window parent, boolean isVirtual) throws IOException {
    File file =
        FileDialogs.openDir(parent, "Please select an image data set", FileDialogs.MM_DATA_SET);
    if (file == null) {
      return null;
    }
  ...
  public Datastore loadData(Component parent, String directory, boolean isVirtual)
    ...
    if (isMultipageTiff) {
      ...
      result.setStorage(new StorageSinglePlaneTiffSeries(result, directory, false));
    }
    if (!isVirtual) {
      ...
      ProgressMonitor monitor =
          new ProgressMonitor(parent, "Loading images into RAM...", null, 0, result.getNumImages());
  ...
  public Image createImage(
    ...
    return new DefaultImage(
        pixelClone, width, height, bytesPerPixel, numComponents, coords, metadata);
  }
  ...
  public Image wrapImage(
      ...
      Coords coords,
      Metadata metadata) {
    return new DefaultImage(pixels, width, height, bytesPerPixel, numComponents, coords, metadata);
  }
  ...
  @Override
  public Image convertTaggedImage(TaggedImage tagged, Coords coords, Metadata metadata)
  ...
  public Pipeline createPipeline(
      List<ProcessorFactory> factories, Datastore store, boolean isSynchronous) {
  ...
  @Override
  public Pipeline copyApplicationPipeline(Datastore store, boolean isSynchronous) {
    return createPipeline(
        studio_.uiManager().getPipelineFrame().getPipelineFactories(), store, isSynchronous);
  }

  @Override
  public Pipeline copyLivePipeline(Datastore store, boolean isSynchronous) {
    return createPipeline(
        studio_.uiManager().getPipelineFrame().getLivePipelineFactories(), store, isSynchronous);
  }
  ...
  public void addAndConfigureProcessor(ProcessorPlugin plugin) {
    studio_.uiManager().getPipelineFrame().addAndConfigureProcessor(plugin);
  }
