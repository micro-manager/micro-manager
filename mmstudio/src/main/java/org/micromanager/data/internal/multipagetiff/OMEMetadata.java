...
public final class OMEMetadata {
  ...
  private void startSeriesMetadata(int seriesIndex, String baseFileName) {
    ...
    MetadataTools.populateMetadata(
        ...
        numSlices_,
        numChannels_,
        mptStorage_.getIntendedSize(Coords.T),
        1);
    ...
    try {
      String summaryComment = CommentsHelper.getSummaryComment(mptStorage_.getDatastore());
    ...
    for (int channel = 0; channel < mptStorage_.getIntendedSize(Coords.CHANNEL); channel++) {
        ...
        for (int channel = 0; channel < numChannels_; channel++) {
          ...
          if (tiffDataIndex == null) {
            ...
            while (tiffDataIndex == null) {
              tiffDataIndex =
                  tiffDataIndexMap_.get(
                      MDUtils.generateLabel(channel, s, frameSearchIndex, position));
              if (tiffDataIndex != null) {
                break;
              }
              ...
              if (backIndex >= 0) {
                tiffDataIndex =
                    tiffDataIndexMap_.get(
                        MDUtils.generateLabel(channel, backIndex, frameSearchIndex, position));
                if (tiffDataIndex != null) {
                  break;
                }
                backIndex--;
              ...
              if (forwardIndex < numSlices_) {
                tiffDataIndex =
                    tiffDataIndexMap_.get(
                        MDUtils.generateLabel(channel, forwardIndex, frameSearchIndex, position));
                if (tiffDataIndex != null) {
                  break;
                }
                forwardIndex++;
            ...
            metadata_.setTiffDataFirstZ(
                new NonNegativeInteger(slice), position, indices.tiffDataIndex_);
            metadata_.setTiffDataFirstC(
                new NonNegativeInteger(channel), position, indices.tiffDataIndex_);
            metadata_.setTiffDataFirstT(
                new NonNegativeInteger(frame), position, indices.tiffDataIndex_);
            ...
            metadata_.setTiffDataPlaneCount(
                new NonNegativeInteger(1), position, indices.tiffDataIndex_);
  ...
  public void addImageTagsToOME(
    ...
    if (!seriesIndices_.containsKey(position)) {
      ...
      try {
        ...
        if (imageTime != null) {
          ...
          String reformattedDate = DateTools.formatDate(imageTime, "yyyy-MM-dd HH:mm:ss Z", true);
          if (reformattedDate == null) {
            reformattedDate = DateTools.formatDate(imageTime, "yyyy-MM-dd E HH:mm:ss Z", true);
          }
          if (reformattedDate != null) {
            metadata_.setImageAcquisitionDate(new Timestamp(reformattedDate), position);
          }
        }
      } catch (IllegalArgumentException e) {
        ReportingUtils.logError(
            e, "Problem adding System state cache metadata to OME Metadata: " + e);
    ...
    metadata_.setUUIDValue(uuid, position, indices.tiffDataIndex_);
    tiffDataIndexMap_.put(
        MDUtils.generateLabel(channel, slice, frame, position), indices.tiffDataIndex_);
    ...
    if (exposureMs != null) {
      metadata_.setPlaneExposureTime(new Time(exposureMs, UNITS.MS), position, indices.planeIndex_);
    }
    Double xPositionUm = metadata.getXPositionUm();
    if (xPositionUm != null) {
      final Length xPosition = new Length(xPositionUm, UNITS.MICROM);
    ...
    if (yPositionUm != null) {
      final Length yPosition = new Length(yPositionUm, UNITS.MICROM);
    ...
    if (zPositionUm != null) {
      metadata_.setPlanePositionZ(
          new Length(zPositionUm, UNITS.MICROM), position, indices.planeIndex_);
    }
    double elapsedTimeMs = metadata.getElapsedTimeMs(-1.0);
    if (elapsedTimeMs >= 0.0) {
      metadata_.setPlaneDeltaT(new Time(elapsedTimeMs, UNITS.MS), position, indices.planeIndex_);
    }
