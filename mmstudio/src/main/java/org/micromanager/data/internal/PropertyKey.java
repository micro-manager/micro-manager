 ...
 * Micro-Manager file format.
 *
 * <p>This is the single-source-of-truth for all JSON metadata keys. All knowledge about what
 * (structure of data) is stored under each key should live here. The affiliation of each key to
 * data structures is defined by the subclasses of {@link NonPropertyMapJSONFormats}.
 *
 * <p>To each key is associated knowledge of how to read and (if still used) write the key to the
 * non-property-map JSON formats. The canonical key strings are also used in modern property maps.
 *
 * <p>When adding new keys, the key string should be camel-cased, starting with a capital letter. As
 * an exception, physical units should be suffixed with an underscore. An example of an existing key
 * following this convention is "ElapsedTime_ms".
 *
  ...
  */
  ...
  CAMERA("Camera", Metadata.class) {
    ...
    public boolean extractFromGsonObject(JsonObject jo, PropertyMap.Builder dest) {
      ...
      if (super.extractFromGsonObject(jo, dest) && !dest.build().getString(key(), "").isEmpty()) {
        return true;
      }
  ...
  CHANNELS("Channels", SummaryMetadata.class) { // See INTENDED_DIMENSIONS
    ...
    @Override
    protected JsonElement convertToGson(PropertyMap pmap) {
      PropertyMap intendedDims =
          pmap.getPropertyMap(INTENDED_DIMENSIONS.key(), PropertyMaps.emptyPropertyMap());
  ...
  IJ_TYPE("IJType", Image.class) {
    ...
    @Override
    protected JsonElement convertToGson(PropertyMap pmap) {
      PixelType pixelType = pmap.getStringAsEnum(PIXEL_TYPE.key(), PixelType.class, null);
      if (pixelType == null) {
        return null;
      }
  ...
  INTERVAL_MS("Interval_ms", "IntervalMs", "interval_ms", "WaitInterval", SummaryMetadata.class) {
    @Override
    protected void convertFromGson(JsonElement je, PropertyMap.Builder dest) {
      dest.putDouble(key(), je.getAsDouble());
    }
  ...
  KEEP_SHUTTER_OPEN_CHANNELS(
      "KeepShutterOpenChannels", "keepShutterOpenChannels", SummaryMetadata.class) {
    @Override
    protected void convertFromGson(JsonElement je, PropertyMap.Builder dest) {
  ...
  KEEP_SHUTTER_OPEN_SLICES(
      "KeepShutterOpenSlices", "keepShutterOpenSlices", SummaryMetadata.class) {
    @Override
    protected void convertFromGson(JsonElement je, PropertyMap.Builder dest) {
      dest.putBoolean(key(), je.getAsBoolean());
    }
  ...
  MULTI_STAGE_POSITION__DEFAULT_XY_STAGE(
      "DefaultXYStage", "DEFAULT_XY_STAGE", "defaultXYStage", MultiStagePosition.class) {
    @Override
    protected void convertFromGson(JsonElement je, PropertyMap.Builder dest) {
      dest.putString(key(), je.getAsString());
    }
  ...
  MULTI_STAGE_POSITION__DEFAULT_Z_STAGE(
      "DefaultZStage", "DEFAULT_Z_STAGE", "defaultZStage", MultiStagePosition.class) {
    @Override
    protected void convertFromGson(JsonElement je, PropertyMap.Builder dest) {
      dest.putString(key(), je.getAsString());
    }
  ...
  MULTI_STAGE_POSITION__GRID_ROW(
      "GridRow", "GRID_ROW", "gridRow", "GridRowIndex", MultiStagePosition.class) {
    @Override
    protected void convertFromGson(JsonElement je, PropertyMap.Builder dest) {
      dest.putInteger(key(), je.getAsInt());
    }
  ...
  MULTI_STAGE_POSITION__GRID_COLUMN(
      "GridCol", "GRID_COL", "gridCol", "GridColumnIndex", MultiStagePosition.class) {
    @Override
    protected void convertFromGson(JsonElement je, PropertyMap.Builder dest) {
      dest.putInteger(key(), je.getAsInt());
    }
  ...
  MULTI_STAGE_POSITION__PROPERTIES(
      "Properties", "PROPERTIES", "properties", MultiStagePosition.class) {
    @Override
    protected void convertFromGson(JsonElement je, PropertyMap.Builder dest) {
  ...
  /**
   * It is unclear what is meant with this key. Positions in the positionlist are multi stage
   * positions. These have Label, DefaultXYSTage, DefaultZStage, GridRow , GridCol, and
   * DevicePositions fields. So, it seems this keyword catches the DevicePositions field within
   * Multi_Stage_Positions However, the code in here refers to
   * NonPropertyMapJSONFormats.multiStagePosition() which goes through all of the fields listed
   * above So, handling of the DevicePositions field is a complete hack, as it is unclear from the
   * original design what the intention was, and this whole construction going back and forth
   * between different encodings still gives me headaches even after having stared at it for way too
   * many days....
   */
    ...
    protected void convertFromGson(JsonElement je, PropertyMap.Builder dest) {
      ...
      if (je.isJsonArray()) {
        for (JsonElement e : je.getAsJsonArray()) {
          ...
          PropertyMap msp = NonPropertyMapJSONFormats.multiStageDevicePosition().fromGson(jo);
  ...
  ROI("ROI", Metadata.class) {
    ...
    protected JsonElement convertToGson(PropertyMap pmap) {
      ...
      String hyphenated = Joiner.on("-").join(rect.x, rect.y, rect.width, rect.height);
  ...
  SCOPE_DATA_KEYS("ScopeDataKeys", "scopeDataKeys", "StateCache-keys", Metadata.class) {
    @Override
    public String getDescription() {
    ...
    protected JsonElement convertToGson(PropertyMap pmap) {
      ...
      List<String> keys = Lists.newArrayList(pmap.getPropertyMap(SCOPE_DATA.key(), null).keySet());
      JsonArray ja = new JsonArray();
      for (String key : keys) {
  ...
  private PropertyKey(String canonical, Object... historicalStringsAndAffiliations) {
  ...
  public final String getDisplayName() {
    ...
    for (String unitSuffix : UNITS.keySet()) {
      if (name.endsWith(" " + unitSuffix)) {
        name =
            name.substring(0, name.length() - unitSuffix.length())
                + "("
  ...
  /**
   * Parse the value for this key given as a JSON element and place it in a property map builder.
   *
   * <p>Implementations should throw (sensible) unchecked exceptions if the value is invalid.
   *
   * <p>The parsed value must be placed in {@code destination} under the key returned by {@code
   * getCanonicalKey}.
   *
   * @param element the Gson element
   * @param destination the property map builder into which the parsed value should be placed
   */
  protected void convertFromGson(JsonElement element, PropertyMap.Builder destination) {
   ...
   *
   * <p>In most cases we just examine all keys (default implementation), but in some cases this
   * method can be overridden to add special behavior, such as looking under different keys.
   *
   * @param source non-property-map JSON object from which to extract the value for this key
   * @param destination property map builder to which the value should be stored
   * @return true if value was found and placed in {@code destination}; false otherwise
   */
  public boolean extractFromGsonObject(JsonObject source, PropertyMap.Builder destination) {
   ...
   *
   * @param source property map in which to find the value for this key
   * @param destination non-property-map JSON object to which the value should be added under this
   *     key
   * @return true if the key was found in {@code source} and was added to {@code destination}; false
   *     otherwise
   */
  public boolean storeInGsonObject(PropertyMap source, JsonObject destination) {
