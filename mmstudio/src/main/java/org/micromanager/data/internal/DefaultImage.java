 ...
 * metadata (in the form of a Metadata instance), and the image's index as part of a larger dataset
 * (in the form of a Coords instance).
 *
 * <p>We store the image data in a ByteBuffer or ShortBuffer. However, ImageJ wants to work with
 * image data in the form of byte[], short[], or int[] arrays (depending on pixel type).
 * getRawPixels(), the method exposed in the Image interface to access pixel data, returns an
 * ImageJ-style array, while getPixelBuffer (which is not exposed in the API) returns the raw
 * buffer.
 */
public final class DefaultImage implements Image {
  ...
  /**
   * Generate a DefaultImage from a TaggedImage. Note that this method will result in Micro-Manager
   * assuming the image data came from the microscope that is currently running (i.e. this is a bad
   * method to use for loading saved images). If you want to avoid that, then you need to manually
   * reconstruct the Metadata for the TaggedImage and use the constructor that this method calls.
   *
  ...
  /**
   * Generate a DefaultImage from a TaggedImage using the supplied coords and metadata.
   *
   * <p>PixelData from the TaggedImage will be used directly, i.e., they are not copied.
   *
   * @param tagged A TaggedImage to base the Image on. Pixels are not copied.
   * @param coords Coords to be used for this new Image. When null, tags in input image are
   *     interpreted to best guess the desired Coord.
   * @param metadata Metadata to be used for this new Image. When null, tags from input image are
  ...
  public DefaultImage(TaggedImage tagged, Coords coords, Metadata metadata)
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Failed to parse JSON created from TaggedImage tags", unlikely);
    }

    if (metadata == null) {
      try {
        metadata =
            DefaultMetadata.fromPropertyMap(NonPropertyMapJSONFormats.metadata().fromGson(je));
      } catch (Exception e) {
  ...
  /**
   * Generates a DefaultImage from pixels, image info in a PropertyMap and the supplied coords and
   * metadata.
   *
   ...
   * @throws IllegalArgumentException
   */
  public DefaultImage(Object pixels, PropertyMap format, Coords coords, Metadata metadata)
      throws IllegalArgumentException {
    this(
        pixels,
        ...
        format
            .getStringAsEnum(PropertyKey.PIXEL_TYPE.key(), PixelType.class, null)
            .getBytesPerPixel(),
        format
            .getStringAsEnum(PropertyKey.PIXEL_TYPE.key(), PixelType.class, null)
  ...
  /**
   * Generates a DefaultImage from pixels, minimal image info, and the supplied coords and metadata.
   *
   * <p>Input pixels will be used directly (i.e., they are not copied).
   *
   * @param pixels Image pixels. Should be a Java array of bytes or shorts (not null).
  ...
  public DefaultImage(
    ...
    metadata_ =
        metadata == null ? new DefaultMetadata.Builder().build() : (DefaultMetadata) metadata;
    coords_ = coords == null ? Coordinates.builder().build() : coords;

    int bpc;
    if (pixels instanceof byte[]) bpc = 1;
    else if (pixels instanceof short[]) bpc = 2;
    else if (pixels instanceof int[] && bytesPerPixel == 4 && numComponents == 3) {
      bpc = 1;
  ...
  /**
   * Creates a new image object that shares the pixels of the source image Attaches the provided
   * coords and metadata
   *
  ...
  public DefaultImage(Image source, Coords coords, Metadata metadata) {
    ...
    pixelType_ = PixelType.valueFor(source.getBytesPerPixel(), bpc, source.getNumComponents());
  }
  ...
  /**
   * Note this returns a byte[], short[], or int[] array, not a ByteBuffer, ShortBuffer, or
   * IntBuffer. Use getPixelBuffer() for that.
   */
  @Override
  ...
  public TaggedImage legacyToTaggedImage() {
    ...
    NonPropertyMapJSONFormats.imageFormat().addToGson(jo, formatToPropertyMap());
    NonPropertyMapJSONFormats.coords().addToGson(jo, ((DefaultCoords) coords_).toPropertyMap());
    NonPropertyMapJSONFormats.metadata().addToGson(jo, metadata_.toPropertyMap());
  ...
  public String toString() {
    return String.format(
        "<%dx%dx%d image (%d bytes per pixel) at %s>",
        getWidth(), getHeight(), getNumComponents(), getBytesPerPixel(), getCoords());
  }
}
