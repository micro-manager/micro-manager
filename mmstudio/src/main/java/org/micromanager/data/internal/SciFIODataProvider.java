...
public class SciFIODataProvider implements DataProvider {
  ...
  /**
   * Initializes the reader and creates Micro-Manager's summaryMetData It appears that most SciFIO
   * datasources have a single image and multiple Planes. Conversion of plane index to Multi-D index
   * is carried out by the SciFIO utility function FormatTools.rasterToPosition(). Data are always
   * returned as Bytes.
   *
  ...
  public Image planeToImage(Plane plane, final Coords coords) throws IOException {
    ...
    Object pixels =
        Bytes.makeArray(
            plane.getBytes(), bytesPerPixel, false, plane.getImageMetadata().isLittleEndian());
  ...
  public Image planeToImage(final Plane plane, final long[] rasterPosition) throws IOException {
  ...
  public final Coords rasterPositionToCoords(
      final ImageMetadata im, final long[] rasterPosition, Coords.Builder cb) {
    List<CalibratedAxis> axes = im.getAxes();
  ...
  public Image getAnyImage() throws IOException {
    try {
      ...
      final long[] rasterPosition =
          FormatTools.rasterToPosition(IMAGEINDEX, planeIndex, reader_.getMetadata());
      return planeToImage(reader_.openPlane(IMAGEINDEX, planeIndex), rasterPosition);
      ...
      if (tmpC.equals(coords)) {
        ...
        if (im.isMultichannel() && channelAxisIndex_ == 0) {
          for (int c = 0; c < im.getAxisLength(channelAxisIndex_); c++) {
            result.add(planeToImage(plane, coords.copyBuilder().c(c).build()));
          }
        } else {
