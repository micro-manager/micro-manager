...
public final class MultipageTiffWriter {
  ...
  public MultipageTiffWriter(
      final StorageMultipageTiff masterStorage, final Image firstImage, final String filename)
      throws IOException {
    ...
    summaryPmap =
        augmentWithImageMetadata(summaryPmap, (DefaultImage) masterStorage_.getAnyImage());
    summaryPmap = augmentWithDisplaySettings(summaryPmap, DefaultDisplaySettings.builder().build());
    reader_ = new MultipageTiffReader(masterStorage_, summary, summaryPmap, firstImage);
  ...
  /**
   * Insert certain fields into the provided JSONObject that are stored in the Image or its
   * Metadata.
   */
  private PropertyMap augmentWithImageMetadata(PropertyMap summary, DefaultImage image) {
    return summary
        .copyBuilder()
        .putEnumAsString(PropertyKey.PIXEL_TYPE.key(), image.getPixelType())
  ...
  /**
   * Insert certain fields into the provided JSONObject that are stored in the given
   * DisplaySettings.
   */
  private PropertyMap augmentWithDisplaySettings(PropertyMap summary, DisplaySettings settings) {
    return summary
        .copyBuilder()
        .putPropertyMap(
            PropertyKey.DISPLAY_SETTINGS.key(), ((DefaultDisplaySettings) settings).toPropertyMap())
        .build();
  }
  ...
  private static ByteBuffer allocateByteBuffer(int capacity) {
    ByteBuffer b =
        capacity >= BUFFER_DIRECT_THRESHOLD
  ...
  /**
   * Called when there is no more data to be written. Write null offset after last image in
   * accordance with TIFF specification and set number of index map entries for backwards reading
   * capability. A file that has been finished should have everything it needs to be properly
   * reopened in MM or by a basic TIFF reader
   *
   * @throws IOException
   */
  ...
  /**
   * Called when entire set of files (i.e. acquisition) is finished. Adds in all the extra (but
   * nonessential) stuff--comments, display settings, OME/IJ metadata, and truncates the file to a
   * reasonable length
   *
  ...
  public void close(String omeXML, String ijDescriptionString) throws IOException {
    String summaryComment = CommentsHelper.getSummaryComment(masterStorage_.getDatastore());
    ...
    for (String axis : coords.getAxes()) {
      if (!MultipageTiffReader.ALLOWED_AXES.contains(axis)) {
        ReportingUtils.logError(
            "Axis "
                + axis
                + " is ignored because it is not one of "
                + MultipageTiffReader.ALLOWED_AXES.toString());
      }
    }
  ...
  private void writeIFD(Image img) {
    ...
    NonPropertyMapJSONFormats.imageFormat()
        .addToGson(jo, ((DefaultImage) img).formatToPropertyMap());
    NonPropertyMapJSONFormats.coords()
        .addToGson(jo, ((DefaultCoords) img.getCoords()).toPropertyMap());
    NonPropertyMapJSONFormats.metadata()
        .addToGson(jo, ((DefaultMetadata) img.getMetadata()).toPropertyMap());
    ...
    // 2 bytes for number of directory entries, 12 bytes per directory entry, 4 byte offset of next
    // IFD
    // 6 bytes for bits per sample if RGB, 16 bytes for x and y resolution, 1 byte per character of
    // MD string
    // number of bytes for pixels
    int totalBytes =
        2 + numEntries * 12 + 4 + (rgb_ ? 6 : 0) + 16 + mdBytes.length + bytesPerImagePixels_;
  ...
  private void writeIFDEntry(
      ByteBuffer buffer, CharBuffer cBuffer, char tag, char type, long count, long value) {
  ...
  /**
   * writes channel LUTs and display ranges for composite mode Could also be expanded to write ROIs,
   * file info, slice labels, and overlays
   */
  private void writeImageJMetadata(int numChannels, String summaryComment) {
    ...
    // nTypes is number actually written among: fileInfo, slice labels, display ranges, channel
    // LUTS,
    ...
    if (ds == null) {
      for (int ch = 0; ch < numChannels; ch++) {
        ...
        ChannelDisplaySettings cds =
            RememberedDisplaySettings.loadChannel(MMStudio.getInstance(), channelGroup, name, null);
        // Display Ranges: For each channel, write min then max
        // TODO: doesn't handle multi-component images.
        mdBuffer.putDouble(
            bufferPosition, (double) cds.getComponentSettings(0).getScalingMinimum());
        bufferPosition += 8;
        mdBuffer.putDouble(
            bufferPosition, (double) cds.getComponentSettings(0).getScalingMinimum());
        bufferPosition += 8;
  ...
  private void writeComments() throws IOException {
    ...
    String summaryComments = CommentsHelper.getSummaryComment(masterStorage_.getDatastore());
    comments.putString("Summary", summaryComments);
    for (Coords coords : masterStorage_.getUnorderedImageCoords()) {
      String imageComments = CommentsHelper.getImageComment(masterStorage_.getDatastore(), coords);
  ...
  private void writeBlankIFD() {
    ...
    char numEntries =
        (char) ((firstIFD_ ? ENTRIES_PER_IFD + 2 : ENTRIES_PER_IFD) + (firstIFD_ ? 2 : 0));
