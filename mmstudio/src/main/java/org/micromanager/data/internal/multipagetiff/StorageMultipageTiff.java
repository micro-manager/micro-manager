...
/**
 * This class provides image storage in the form of a single TIFF file that contains all image data
 * in the dataset. Adapted from the TaggedImageStorageMultipageTiff module.
 *
 * <p>TODO: The org.micromanager.data package should not depend on internal code (or any code
 * outside of the packae) so that it can be re-used elsewhere.
 */
public final class StorageMultipageTiff implements Storage {
  ...
  private String summaryMetadataString_ =
      NonPropertyMapJSONFormats.summaryMetadata().toJSON(summaryMetadata_.toPropertyMap());
  ...
  private final ConcurrentHashMap<Coords, Image> coordsToPendingImage_ = new ConcurrentHashMap<>();
  ...
  public StorageMultipageTiff(Component parent, Datastore store, String dir, Boolean amInWriteMode)
      throws IOException {
    this(
        parent,
        store,
   ...
   * @param separateFilesForPositions If true, will store positions in separate files, otherwise all
   *     data will go into a single file
   * @throws java.io.IOException
   */
  public StorageMultipageTiff(
      Component parent,
      Datastore store,
      String dir,
      boolean amInWriteMode,
      boolean separateMDFile,
      boolean separateFilesForPositions)
      throws IOException {
  ...
  /**
   * Indicator of Acquisition order. This function is difficult to name. "First" means that the axis
   * comes before another axis in the ordered axes list. Note, however, that the list of ordered
   * axes is exactly the inverse of the Acquisition Order as presented to the user
   *
  ...
  private void openExistingDataSet() {
    ...
    if (!GraphicsEnvironment.isHeadless()) {
      progressBar = new ProgressBar(parent_, "Reading " + directory_, 0, dir.listFiles().length);
    }
    int numRead = 0;
    if (progressBar != null) {
  ...
  private MultipageTiffReader loadFile(File f) {
    ...
    try {
      ...
      for (Coords coords : readerCoords) {
        ...
        lastFrameOpenedDataSet_ = Math.max(coords.getT(), lastFrameOpenedDataSet_);
        if (firstImage_ == null) {
  ...
  /**
   * This method is a wrapper around startWritingTask, that primarily concerns itself with ensuring
   * that coordsToPendingImage_ is kept up-to-date. That structure, in turn, ensures that we do not
   * have to rely on MultipageTiffReader.readImage() returning a coherent (i.e. finished-writing)
   * image if our getImage() method is called before writing is completed.
   */
  ...
  /**
   * This method handles starting the process of writing images (which means that it ultimately
   * submits a task to writingExecutor_).
   */
  private void startWritingTask(DefaultImage image) throws MMException, IOException {
    ...
    if (writingExecutor_ == null) {
      writingExecutor_ =
          new ThreadPoolExecutor(
              1, 1, 0, TimeUnit.NANOSECONDS, new LinkedBlockingQueue<java.lang.Runnable>());
    }
    int fileSetIndex = 0;
    ...
    if (!positionToFileSet_.containsKey(fileSetIndex)) {
      positionToFileSet_.put(
          fileSetIndex,
          new FileSet(image, this, omeMetadata_, splitByXYPosition_, separateMetadataFile_));
    }
  ...
  public synchronized void finished() {
    ...
    ProgressBar progressBar = null;
    if (!GraphicsEnvironment.isHeadless()) {
      progressBar = new ProgressBar(parent_, "Finishing Files", 0, positionToFileSet_.size());
    }
    try {
      int count = 0;
      if (progressBar != null) {
  ...
  private void setSummaryMetadata(DefaultSummaryMetadata summary, boolean showProgress) {
    summaryMetadata_ = summary;
    summaryMetadataString_ =
        NonPropertyMapJSONFormats.summaryMetadata().toJSON(summary.toPropertyMap());
    ...
    if (showProgress && !GraphicsEnvironment.isHeadless()) {
      ProgressBar progressBar =
          new ProgressBar(parent_, "Building image location map", 0, oldImageMap.keySet().size());
  ...
  /**
   * This function provides the ImageJ "Properties" information when opening in ImageJ/Fiji.
   *
   * <p>It also defines the image orde when creating Hyperstacks from sequences of images
   *
   * @return
   */
  ...
  /**
   * Since we do no longer want to pass through DisplaySettings to the saving code, but still need
   * access to the, to enable saving metadata used by 3rdparty applications (such as ImageJ/Fiji) to
   * display the data the same way we did in MM, we need some ugly heuristics to figure out what
   * DisplaySettings were used.
   *
   * @return DisplaySettings of a DataViewer that used this store for data. Will be null when no
   *     such DataViewer was found.
   */
  ...
  /**
   * We can not simply test if the viewer's store is the same as ours, since we may be saving a
   * "copy" (i.e., saving a RAMM data set on disk) This is bad, but seems to work alright for now.
   */
    ...
    if (maxIndices_ == null) {
      ...
      for (Coords coords : coordsToReader_.keySet()) {
        for (String axis : coords.getAxes()) {
          if (!maxIndices.containsKey(axis) || coords.getIndex(axis) > maxIndices.get(axis)) {
  ...
  public Integer getIntendedSize(String axis) {
    ...
    return Math.max(summaryMetadata_.getIntendedDimensions().getIndex(axis), 1);
  }
  ...
  public boolean hasImage(Coords coords) {
    return coordsToPendingImage_.containsKey(coords) || coordsToReader_.containsKey(coords);
  }
  ...
  public static boolean getShouldGenerateMetadataFile() {
    return MMStudio.getInstance()
        .profile()
        .getSettings(StorageMultipageTiff.class)
