...
public interface DataProvider extends Closeable {
  ...
  /**
   * Returns the highest index plus 1 along the given axis. If the axis is not (yet) represented in
   * the data set, it will return 0.
   *
   * <p>If the axis is present in the data set, but no Coords with that axes are found (Coords for
   * an axis can not be zero), it will return 1.
   *
   * <p>Missing images for any given axis are ignored, e.g. if the DataProvider has images 0, 1, 3,
   * 4 for a given axis, it will return 5. Do not expect all images along a given axis to be present
   * in the DataProvider.
   *
  ...
  /**
   * Returns the highest index along the given axis plus 1. If the axis is not (yet) represented in
   * the data set, it will return 0.
   *
   * <p>If the axis is present in the data set, but no Coords with that axes are found (Coords for
   * an axis can not be zero), it will return 1.
   ...
   *
   * @param coords specification of multi-dimensional index that needs to be present in the Coords
   *     of the images that will be returned
  ...
  /**
   * Returns a list of image in the DataProvider's collection that have identical coords after
   * removing the given axes from both source and target.
   *
   * @param coords Coords to look for in the provider's collection
   * @param ignoreTheseAxes Axes that will be removed from copy of Coords in the collection before
  ...
  /**
   * Provides total number of images that can be accessed through this DataProvider TODO: does this
   * include blank images/empty Coords?
   *
