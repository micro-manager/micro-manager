 ...
 *
 * <p>This is typically used to represent the time point, stage position, z slice, and channel
 * indices of images.
 *
 * <p>The {@code Coords} object is a mapping from axes (represented by strings) to positive integer
 * indices. Because an arbitrary number of axes may be included in the Coords, the index zero is
 * meaningless and will be omitted. Therefore, the axis index for any index not included in the
 * Coords will be zero. The Coords of an image that is zero for all axes is an empty Coords.
 *
 * <p>The equals operator can be relied upon to establish whether two different Coords contain the
 * same axes and the same indices to those axes.
 *
 * <p>Although methods are included to support any arbitrary axes, such usage is not yet fully
 * supported. <strong>If you do use custom axes, give them names that include upper case
 * letters</strong> so that they do not clash with standard axes added in the future.
 *
 * <p>{@code Coords} objects are immutable.
 *
...
public interface Coords {
   ...
   *
   * @deprecated Use discouraged because it reads like a physical time rather than the time point
   *     index that it is.
   */
  @Deprecated String TIME = TIME_POINT;

  /** Axis label for the time point (frame) axis (short form). Same as {@code TIME_POINT}. */
  String T = TIME_POINT;

  /** Axis label for the stage position axis. */
  String STAGE_POSITION = "position";

  /** Axis label for the stage position axis (short form). Same as {@code STAGE_POSTITION}. */
  String P = STAGE_POSITION;

  /** Axis label for the Z slice axis. */
  String Z_SLICE = "z";

  /** Axis label for the Z slice axis (short form). Same as {@code Z_SLICE}. */
  String Z = Z_SLICE;
  ...
  interface Builder extends CoordsBuilder {
    ...
    /**
     * Set the channel index. Equivalent to {@code index(Coords.CHANNEL, channel)}.
     *
    ...
    /**
     * Set the time point (frame) index. Equivalent to {@code index(Coords.TIME_POINT, frame)}.
     *
    ...
    /**
     * Set the Z slice index. Equivalent to {@code index(Coords.Z_SLICE, slice)}.
     *
    ...
    /**
     * Shorthand for {@link #zSlice(int)}. Set the Z slice index. Equivalent to {@code
    ...
    /**
     * Set the stage position index. Equivalent to {@code index(Coords.STAGE_POSITION, index)}.
     *
    ...
    /**
     * Shorthand for {@link #stagePosition(int) stagePosition}. Set the stage position index.
     ...
     *
     * <p>If you set a negative value, the axis will be removed.
     *
     ...
     * @throws IndexOutOfBoundsException if applying the offset would result in a negative index.
     */
    @Override
   ...
   *
   * @param axis coordinate axis such as {@code Coords.CHANNEL}
   * @return index along {@code axis}, or {@code 0} if {@code axis} does not exist
   */
   ...
   *
   * <p>Equivalent to {@code getIndex(Coords.CHANNEL)}.
   *
   * @return channel index, or {@code 0} if this {@code Coords} doesn't contain a channel index.
   */
  int getChannel();
   ...
   *
   * <p>Equivalent to {@code getIndex(Coords.TIME_POINT)}.
   *
   * @return time point index, or {@code 0} if this {@code Coords} doesn't contain a time point
   *     index.
   */
  int getTimePoint();
   ...
   * @deprecated Due to looking like the physical time rather than an index. Use {@link
   *     #getTimePoint() getTmePoint}
   */
  @Deprecated
  int getTime();
   ...
   *
   * <p>Equivalent to {@code getIndex(Coords.Z_SLICE)}.
   *
   * @return Z slice index, or {@code 0} if this {@code Coords} doesn't contain a Z slice index.
   */
  int getZSlice();
   ...
   *
   * <p>Equivalent to {@code getIndex(Coords.STAGE_POSITION)}.
   *
   * @return stage position index, or {@code 0} if this {@code Coords} doesn't contain a stage
   *     position index.
   */
  ...
  /**
   * Name of this function is very unclear. It seems that its functionality is to provide a copy of
   * the given Coords, but only for the axes provided in the input strings. A more useful name may
   * be: copyProvidedAxes, or copyAxes
   *
