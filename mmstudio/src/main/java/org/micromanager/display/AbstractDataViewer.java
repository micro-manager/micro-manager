 ...
 *
 * <p>Custom data viewer classes should extend this class, rather than directly implement {@code
 * DataViewer}.
 *
...
public abstract class AbstractDataViewer implements DataViewer {
  ...
  private final ExecutorService asyncEventPoster_ =
      Executors.newSingleThreadExecutor(
          ThreadFactoryFactory.createThreadFactory("AbstractDataViewer Pool"));
   ...
   *
   * <p>Implementations should call this method to post required notification events.
   *
   * <p>Some standard viewer events require that they be posted on the Swing/AWT event dispatch
   * thread. Make sure you are on the right thread when posting such events.
   *
   ...
   * <p>{@inheritDoc}
   *
   * <p>Implementations must override {@code handleNewDisplaySettings} in order to respond to new
   * display settings.
   */
  @Override
  public final void setDisplaySettings(final DisplaySettings settings) {
    ...
    synchronized (this) {
      ...
      asyncEventPoster_.submit(
          () -> {
            postEvent(
                DefaultDisplaySettingsChangedEvent.create(
   ...
   *
   * <p>Override this method to apply new display settings.
   *
   * <p>This method is called in a thread-synchronized context, so you should avoid time-consuming
   * actions. It is safe to call {@code getDisplaySettings} and compare its return value with {@code
   * requestedSettings}. However, calling {@code setDisplaySettings} will result in infinite
   * recursion; if you need to adjust {@code requestedSettings} before accepting it, do so by
   * returning a modified copy from this method. This returned settings is what subsequent calls to
   * {@code getDisplaySettings} will return.
   *
   * <p>Typically, the implementation should record all information needed to make the changes and
   * arrange to apply the changes at a later time (usually on the Swing/AWT event dispatch thread).
   *
   * @param requestedSettings the new display settings requested
   * @return adjusted display settings that will actually apply
   */
  protected abstract DisplaySettings handleDisplaySettings(DisplaySettings requestedSettings);
   ...
   * <p>{@inheritDoc}
   *
   * <p>Implementations must override {@code handleNewDisplayPosition} in order to respond to new
   * display positions.
   */
  @Override
  public final void setDisplayPosition(final Coords position, boolean forceRedisplay) {
    if (position == null) {
    ...
    synchronized (this) {
      ...
      if (!position.equals(oldPosition)) {
        asyncEventPoster_.submit(
            () -> {
              postEvent(
                  DefaultDisplayPositionChangedEvent.create(
   ...
   */
  @Override
  public final boolean compareAndSetDisplayPosition(
      final Coords oldPosition, final Coords newPosition, boolean forceRedisplay) {
    if (newPosition == null) {
      throw new NullPointerException("Position must not be null");
    }
    synchronized (this) {
  ...
  @Override
  public final boolean compareAndSetDisplayPosition(Coords oldPosition, Coords newPosition) {
   ...
   *
   * <p>This is not a "close" method because the {@code DataViewer} interface may apply to viewers
   * that are not windows (e.g. an embeddable component).
   *
   * <p>This method should be called at an appropriate time to release non-memory resources used by
   * {@code AbstractDataViewer}.
   */
  ...
  /**
   * Implements {@code setDisplayedImageTo} by calling {@code setDisplayPosition}.
   *
   * <p>{@inheritDoc}
   *
   * @param coords
