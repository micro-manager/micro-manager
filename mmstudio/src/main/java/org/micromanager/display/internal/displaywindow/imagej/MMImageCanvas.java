...
public final class MMImageCanvas extends ImageCanvas
  ...
  private MMImageCanvas(ImageJBridge parent) {
    ...
    // Remove existing key listeners.  Ours will send unprocessed keyEvents through to ImageJ (which
    // handles keyEvents)
  ...
  /**
   * The ImageJ canvas knows best what its current size is, Adjust our display to the current size
   */
  ...
  public void setSize(int newWidth, int newHeight) {
    ...
    if (parent_ != null) {
      setSourceRect(
          parent_.computeSourceRectForCanvasSize(
  ...
  public void setMagnification(double factor) {
    ...
    // scaled image
    setMinimumSize(
        new Dimension(Math.min(84, getMaximumSize().width), Math.min(84, getMaximumSize().height)));
   ...
   * rectangle smaller and center it on the position in the canvas where the cursor was when zooming
   * started
   *
