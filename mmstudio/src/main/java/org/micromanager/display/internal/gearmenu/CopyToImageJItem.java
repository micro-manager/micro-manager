...
public final class CopyToImageJItem implements DisplayGearMenuPlugin, SciJavaPlugin {
  ...
  public void onPluginSelected(DisplayWindow display) {
    ...
    if (dp.getNumImages() == 1) {
      ...
      try {
        ImageStack imgStack =
            new ImageStack(dp.getAnyImage().getWidth(), dp.getAnyImage().getHeight());
        ...
        for (int t = 0; t < dp.getNextIndex(Coords.T); t++) {
          for (int z = 0; z < dp.getNextIndex(Coords.Z); z++) {
            for (int c = 0; c < dp.getNextIndex(Coords.C); c++) {
              ...
              if (image != null) {
                iProc = DefaultImageJConverter.createProcessor(image, copy);
              } else { // handle missing images - should be handled by MM
                // so remove this code once this is done nicely in MM
                iProc = DefaultImageJConverter.createBlankProcessor(dp.getAnyImage());
              }
        ...
        iPlus.setStack(
            imgStack,
            dp.getNextIndex(Coords.C),
            dp.getNextIndex(Coords.Z),
            dp.getNextIndex(Coords.T));
        ...
        for (int c = 0; c < dp.getNextIndex(Coords.C); c++) {
          ci.setChannelLut(
              LUT.createLutFromColor(display.getDisplaySettings().getChannelColor(c)), c + 1);
        }
