...
class AxisLinker {
  ...
  public static AxisLinker create(LinkManager linkManager, DisplayController viewer, String axis) {
    AxisLinker instance = new AxisLinker(linkManager, viewer, axis);
    viewer.registerForEvents(instance);
    return instance;
  }

  private AxisLinker(LinkManager linkManager, DisplayController viewer, String axis) {
  ...
  void updatePopupMenu(JPopupMenu menu) {
    ...
    for (LinkAnchor<Integer> peer : anchor_.getLinkablePeers()) {
      ...
      JCheckBoxMenuItem item = new JCheckBoxMenuItem(anchor.getViewer().getName());
