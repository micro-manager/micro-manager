...
public final class DefaultImageExporter implements ImageExporter {
  ...
  /** This recursive structure represents the "nested loop" approach to exporting. */
  public static class ExporterLoop {
    ...
    /**
     * Insert a new innermost ExporterLoop -- recursively propagates the provided child to the end
     * of the list.
     *
     * @param child
     */
    ...
    /**
     * Iterate over our specified axis, while running any inner loop(s), determining which images
     * will be exported. Use the provided base coordinates to cover for any coords we aren't
     * iterating over.
     *
     * @param baseCoords
     * @param result
     */
    public void selectImageCoords(Coords baseCoords, ArrayList<Coords> result) {
  ...
  /**
   * This method gets called twice for each image we export: once for the display responding to our
   * request to set the image coordinates, and then once for the display painting to our provided
   * Graphics object.
   *
   * @param event
   */
    ...
    if (event.getDataViewer() instanceof DisplayController) {
      ...
      try {
        if (dc.getUIController().getIJImageCanvas().getGraphics() != currentGraphics_) {
          ...
          currentImage_ =
              new BufferedImage(canvasSize.width, canvasSize.height, BufferedImage.TYPE_INT_RGB);
          ...
          if (format_ == OutputFormat.OUTPUT_IMAGEJ) {
            if (stack_ == null) {
              ...
              stack_ = new ImageStack(currentImage_.getWidth(), currentImage_.getHeight());
            }
  ...
  /**
   * Run sanity checks prior to exporting images. Determine how many images will be exported. Ensure
   * that each image will not overwrite any existing file. Return the list of image coordinates to
   * be exported.
   */
  private ArrayList<Coords> prepAndSanityCheck() throws IOException, IllegalArgumentException {
    if (format_ == null) {
    ...
    outerLoop_.selectImageCoords(displayedImages.get(0).getCoords(), coords);
    if (coords.isEmpty()) {
      // Nothing to do.
      return coords;
    }
  ...
  /** Check to make certain that outputting the nth image will not overwrite an existing file. */
  ...
  /**
   * Export images according to the user's setup. Iterate over each axis, setting the displayed
   * image to the desired coordinates, drawing it, saving the drawn image to disk, and then moving
   * on. This method is synchronized, which doesn't mean a whole lot because the actual export
   * process happens on separate threads. However, it calls waitForExport() as its first action,
   * which will block if another export is in progress.
   */
  @Override
                    ...
                    while (drawFlag_.get()) {
                      try {
                        ...
                        ReportingUtils.logError(
                            "Interrupted while waiting for drawing to complete.");
                        return;
                      }
                    }
                  }
