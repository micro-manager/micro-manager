 ...
 *
 * <p>This class manages our customized ImageJ objects, which we use to plug into ImageJ's concept
 * of an image viewer window.
 *
 * <p>The {@code ImageJBridge} facade object manages an {@code ImagePlus} (ImageJ's wrapper for an
 * image data set), an {@code MMVirtualStack} (our customized proxy for ImageJ's data provider
 * object), a {@code ProxyImageWindow} (our proxy subclass of ImageJ's {@code ImageWindow} (actually
 * {@code StackWindow}), which is never displayed but used to trick ImageJ into thinking it's
 * communicating with a window), and controls an {@code MMImageCanvas} (our extended version of
 * ImageJ's {@code ImageCanvas}).
 *
...
public final class ImageJBridge {
  ...
  @MustCallOnEDT
  public static ImageJBridge create(final DisplayUIController parent, final ImagesAndStats images) {
  ...
  private ImageJBridge(DisplayUIController parent, ImagesAndStats images) {
    ...
    if (images != null
        && images.getRequest().getImages().size() > 0
  ...
  @MustCallOnEDT
  private void mm2ijSetDisplayAxisExtents(int nChannels, int nZSlices, int nTimePoints) {
    int oldNChannels = ((IMMImagePlus) imagePlus_).getNChannelsWithoutSideEffect();
    ...
    ((IMMImagePlus) imagePlus_).setDimensionsWithoutUpdate(nChannels, nZSlices, nTimePoints);
  ...
  public void mm2ijEnsureDisplayAxisExtents() {
    ...
    int oldNChannels = ((IMMImagePlus) imagePlus_).getNChannelsWithoutSideEffect();
    int oldNSlices = ((IMMImagePlus) imagePlus_).getNSlicesWithoutSideEffect();
    int oldNFrames = ((IMMImagePlus) imagePlus_).getNFramesWithoutSideEffect();

    if (newNChannels > oldNChannels || newNSlices > oldNSlices || newNFrames > oldNFrames) {
      mm2ijSetDisplayAxisExtents(
          Math.max(newNChannels, oldNChannels),
  ...
  private void applyColorMode(ColorModeStrategy strategy) {
    Preconditions.checkState((strategy instanceof RGBColorModeStrategy) == isIJRGB());
  ...
  void paintMMOverlays(Graphics2D g, int canvasWidth, int canvasHeight, Rectangle sourceRect) {
    Rectangle canvasBounds = new Rectangle(0, 0, canvasWidth, canvasHeight);
    Rectangle2D.Float viewPort =
        new Rectangle2D.Float(sourceRect.x, sourceRect.y, sourceRect.width, sourceRect.height);
  ...
  private Image makeBlankImage(Coords coords) {
    ...
    return new DefaultImage(
        ...
        template.getWidth(),
        template.getHeight(),
        template.getBytesPerPixel(),
        template.getNumComponents(),
        coords,
        new DefaultMetadata.Builder().build());
  }

  /**
   * Tell the ImagePlus about certain properties of our data that it doesn't otherwise know how to
   * access.
   */
  @MustCallOnEDT
  ...
  public boolean isIJZoomedAllTheWayIn() {
    return canvas_.getMagnification() >= IJ_ZOOM_LEVELS.get(IJ_ZOOM_LEVELS.size() - 1) - 0.001;
  }
  ...
  public void mm2ijSetZoom(double factor) {
    ...
    Dimension newIdealCanvasSize =
        computeIdealCanvasSizeAfterZoom(
            factor,
            originalFactor,
            getMMWidth(),
            getMMHeight(),
            canvas_.getSize().width,
            canvas_.getSize().height);

    // Center the new source rect where the precious source rect was, to the
    // extent it fits in the image
    Rectangle newSourceRect =
        computeSourceRect(
            factor,
            getMMWidth(),
            getMMHeight(),
            newIdealCanvasSize.width,
            newIdealCanvasSize.height,
            canvas_.getSrcRect());
  ...
  Rectangle computeSourceRectForCanvasSize(
      double zoomRatio, int width, int height, Rectangle oldSourceRect) {
    return computeSourceRect(zoomRatio, getMMWidth(), getMMHeight(), width, height, oldSourceRect);
  }
  ...
  private static Rectangle computeSourceRect(
      double zoomRatio,
      int imageWidth,
      int imageHeight,
      int dstWidth,
    ...
    return computeSourceRect(
        zoomRatio, centerX, centerY, imageWidth, imageHeight, dstWidth, dstHeight);
  }
  ...
  private static Rectangle computeSourceRect(
    ...
    ret.width = Math.min(imageWidth, (int) Math.ceil((dstWidth + 1) / zoomRatio));
    ret.height = Math.min(imageHeight, (int) Math.ceil((dstHeight + 1) / zoomRatio));
  ...
  void ij2mmMouseClicked(MouseEvent e) {
    uiController_.mouseEventOnImage(
        e, computeImageRectForCanvasPoint(e.getPoint()), ij.gui.Toolbar.getToolId());
  }
  ...
  private BoundsRectAndMask makeBoundsAndMaskFromIJRoi(Roi ijRoi) {
    ...
    if (maskProc != null
        && (maskProc.getWidth() != bounds.width || maskProc.getHeight() != bounds.height)) {
      return BoundsRectAndMask.unselected();
    }
