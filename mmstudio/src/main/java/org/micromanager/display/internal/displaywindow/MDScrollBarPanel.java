 ...
 *
 * <p>This is a passive UI component and does not (and should not) contain any application logic.
 *
...
public class MDScrollBarPanel extends JPanel implements AdjustmentListener {
  public interface Listener {
    void scrollBarPanelHeightWillChange(MDScrollBarPanel panel, int currentHeight);

    void scrollBarPanelHeightDidChange(MDScrollBarPanel panel, int oldHeight, int newHeight);
  ...
  @MustCallOnEDT
  static MDScrollBarPanel create(
      ControlsFactory leftControlsFactory, ControlsFactory rightControlsFactory) {
    return new MDScrollBarPanel(leftControlsFactory, rightControlsFactory);
  }

  @MustCallOnEDT
  private MDScrollBarPanel(
      ControlsFactory leftControlsFactory, ControlsFactory rightControlsFactory) {
   ...
   *
   * <p>State of existing axes (based on string name matching) is preserved, even if axes are
   * reordered.
   *
   * @param axes
   */
  ...
  void setAxes(List<String> axes) {
    ...
    try {
      ...
      if (isRowCountChanging) {
        ...
        listeners_.fire().scrollBarPanelHeightDidChange(this, oldHeight, newHeight);
      }
    } finally {
  ...
  void setAxisLength(String axis, int length) {
    ...
    try {
      ...
      scrollModel.setRangeProperties(
          scrollModel.getValue(), (length > 0 ? 1 : 0), 0, length, false);
    } finally {
      shouldSuppressAdjustmentEvents_ = false;
    }
  }
  ...
  private JScrollBar getScrollBarForAxis(String axis) {
    int index = axes_.indexOf(axis);
    if (index < 0) {
      throw new NoSuchElementException(getClass().getSimpleName() + " has no axis " + axis);
    }
  ...
  private JPanel makeRowPanel(String axis, int length) {
    JPanel rowPanel = new JPanel(new MigLayout(new LC().insets("0").gridGap("0", "0").fillX()));

    if (leftControlsFactory_ != null) {
      JComponent leftControls = leftControlsFactory_.getControlsForAxis(axis, ROW_HEIGHT);
      rowPanel.add(leftControls, new CC());
    }

    JScrollBar scrollBar =
        new JScrollBar(JScrollBar.HORIZONTAL, 0, (length > 0 ? 1 : 0), 0, length);
    scrollBar.addAdjustmentListener(this);
    rowPanel.add(scrollBar, new CC().growX().pushX());

    if (rightControlsFactory_ != null) {
      JComponent rightControls = rightControlsFactory_.getControlsForAxis(axis, ROW_HEIGHT);
