...
public class DataViewerMousePixelInfoChangedEvent {
  ...
  public static DataViewerMousePixelInfoChangedEvent fromImage(int x, int y, Image image) {
    Preconditions.checkNotNull(image);
    return fromAxesAndImages(x, y, new String[] {}, Collections.singletonList(image));
  }
  ...
  public static DataViewerMousePixelInfoChangedEvent fromAxesAndImages(
    ...
    for (Image image : images) {
      ...
      for (String axis : indexingAxes) {
        if (!c.hasAxis(axis)) {
          throw new IllegalArgumentException("Image coords do not contain the requested axis");
        }
  ...
  public static DataViewerMousePixelInfoChangedEvent create(
      int x, int y, String[] indexingAxes, List<Coords> coords, List<long[]> componentValues) {
    ...
    if (indexingAxes.length == 0) {
      Preconditions.checkArgument(coords.size() <= 1, "Indexing axes required for multiple coords");
    }
    return new DataViewerMousePixelInfoChangedEvent(x, y, indexingAxes, coords, componentValues);
  }

  public static DataViewerMousePixelInfoChangedEvent createUnavailable() {
    return new DataViewerMousePixelInfoChangedEvent(
        -1, -1, new String[] {}, Collections.<Coords>emptyList(), Collections.<long[]>emptyList());
  }

  private DataViewerMousePixelInfoChangedEvent(
      int x, int y, String[] indexingAxes, List<Coords> coords, List<long[]> componentValues) {
  ...
  public String getComponentValuesStringForCoords(Coords coords) {
    ...
    return "("
        + Joiner.on(", ").join(ArrayUtils.toObject(getComponentValuesForCoords(coords)))
        + ")";
  }
}
