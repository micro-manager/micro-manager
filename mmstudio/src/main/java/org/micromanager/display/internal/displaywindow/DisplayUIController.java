 ...
 *
 * <p>This is a controller object that coordinates and routes activity. It also creates and destroys
 * the actual JFrame.
 *
 * <p>This is the object that holds the state describing which images are currently displayed (as
 * opposed to {@code DisplayController}'s notion of the current display position, which may update
 * faster than the actual UI.
 *
 * @author Mark A. Tsuchida
 */
public final class DisplayUIController
    implements Closeable, WindowListener, MDScrollBarPanel.Listener {
  ...
  private final List<Map.Entry<String, JToggleButton>> axisAnimationButtons_ = new ArrayList<>();
  private final List<Map.Entry<String, PopupButton>> axisPositionButtons_ = new ArrayList<>();
  private final List<Map.Entry<String, PopupButton>> axisLinkButtons_ = new ArrayList<>();
  private final List<Map.Entry<String, JButton>> axisLockButtons_ = new ArrayList<>();
  ...
  private static final Icon UNLOCKED_ICON =
      IconLoader.getIcon("/org/micromanager/icons/lock_open.png");
  // TODO: Make Icons same size so that they are not moving around when changing
  private static final Icon BLACK_LOCKED_ICON =
      IconLoader.getIcon("/org/micromanager/icons/lock_locked.png");
  private static final Icon RED_LOCKED_ICON =
      IconLoader.getIcon("/org/micromanager/icons/lock_super.png");
  ...
  private final ScheduledExecutorService scheduledExecutor_ =
      Executors.newSingleThreadScheduledExecutor(
          ThreadFactoryFactory.createThreadFactory("DisplayUIController"));
  ...
  private final AtomicBoolean repaintScheduledForNewImages_ = new AtomicBoolean(false);
  private final AtomicReference<TimeIntervalRunningQuantile> displayIntervalEstimator_ =
      new AtomicReference<>(TimeIntervalRunningQuantile.create(DISPLAY_INTERVAL_SMOOTH_N_SAMPLES));
  ...
  @MustCallOnEDT
  static DisplayUIController create(
      Studio studio,
      DisplayController parent,
      DisplayWindowControlsFactory controlsFactory,
      AnimationController<Coords> animationController) {
    DisplayUIController instance =
        new DisplayUIController(studio, parent, controlsFactory, animationController);
  ...
  private JFrame makeFrame(boolean fullScreen) {
    ...
    if (!fullScreen) {
      ...
      frame.setBounds(GUIUtils.getFullScreenBounds(frame.getGraphicsConfiguration()));
  ...
  /**
   * Not ideal, but the Image Exporter needs access to the canvas to grab images
   *
   * @return
   */
  ...
  private JPanel buildInitialUI() {
    ...
    contentPanel.setLayout(
        new MigLayout("insets 1, gap 1 1, fill", "[grow, fill]", "[] 0 [grow, fill] related []"));
    ...
    int minWidth =
        Math.max(
            topControlPanel_.getMinimumSize().width, bottomControlPanel_.getMinimumSize().width);
    int minHeight =
  ...
  private void setupDisplayUI(ImagesAndStats images) {
    ...
    canvasBorderPanel_.setBorder(BorderFactory.createLineBorder(Color.BLACK, BORDER_THICKNESS));
  ...
  private JComponent buildTopControls() {
    JPanel panel = makeValidationRootJPanel(new MigLayout("insets 0, gap 1 1, fillx"));
    ...
    fullScreenButton_.addActionListener(
        (ActionEvent e) -> {
    ...
    zoomInButton_ = new JButton(IconLoader.getIcon("/org/micromanager/icons/zoom_in.png"));
    zoomInButton_.setToolTipText("Zoom in");
    zoomInButton_.addActionListener(
        (ActionEvent e) -> {
          zoomIn();
        });
    ...
    zoomOutButton_ = new JButton(IconLoader.getIcon("/org/micromanager/icons/zoom_out.png"));
    zoomOutButton_.setToolTipText("Zoom out");
    zoomOutButton_.addActionListener(
        (ActionEvent e) -> {
          zoomOut();
        });
  ...
  private JComponent buildBottomControls() {
    JPanel panel =
        makeValidationRootJPanel(new MigLayout(new LC().insets("1").gridGap("0", "0").fillX()));
    ...
    playbackFpsSpinner_.addChangeListener(
        (ChangeEvent e) -> {
    ...
    int width =
        24
            + playbackFpsButton_
                .getFontMetrics(playbackFpsButton_.getFont())
                .stringWidth("Playback: 9999.0 fps");
    Dimension fpsButtonSize = new Dimension(width, pixelInfoLabel_.getPreferredSize().height + 12);
    ...
    playbackFpsButton_.addPopupButtonListener(
        (PopupButton button) -> {
    ...
    JPanel customControlsPanel = new JPanel(new MigLayout(new LC().insets("0").gridGap("1", "0")));
    ...
    newImageIndicator_.setFont(
        newImageIndicator_.getFont().deriveFont(10.0f).deriveFont(Font.BOLD));
  ...
  private JComponent makeScrollBarLeftControls(final String axis, final int height) {
    JPanel ret = new JPanel(new MigLayout(new LC().insets("0").gridGap("0", "0").fillX()));
    ...
    if (animateButton == null) {
      ...
      int fontWidth = animateButton.getFontMetrics(animateButton.getFont()).stringWidth("z");
      int offset = 28;
      ...
      axisAnimationButtons_.add(new AbstractMap.SimpleEntry<>(axis, animateButton));
    }
    ...
    if (positionButton == null) {
      ...
      int width =
          offset
              + positionButton.getFontMetrics(positionButton.getFont()).stringWidth("99999/99999");
      ...
      axisPositionButtons_.add(new AbstractMap.SimpleEntry<>(axis, positionButton));
    }
  ...
  private JComponent makeScrollBarRightControls(final String axis, int height) {
    ...
    if (linkButton == null) {
      final AxisLinker linker =
          AxisLinker.create(displayController_.getLinkManager(), displayController_, axis);
      final JPopupMenu linkPopup = new JPopupMenu();
      linkButton =
          PopupButton.create(IconLoader.getIcon("/org/micromanager/icons/linkflat.png"), linkPopup);
      linkButton.addPopupButtonListener(
          (PopupButton button) -> {
    ...
    if (lockButton == null) {
      ...
      axisLockButtons_.add(new AbstractMap.SimpleEntry<>(axis, lockButton));
    }
  ...
  void expandDisplayedRangeToInclude(Collection<Coords> coords) {
    ...
    for (int i = 0; i < scrollableAxes.size(); ++i) {
      ...
      scrollBarPanel_.setAxisLength(currentAxis, scrollableLengths.get(currentAxis));
      updateAxisPositionIndicator(currentAxis, -1, scrollableLengths.get(currentAxis));
    }
  ...
  /**
   * Creates the string in the bottom of the viewer showing the time, z-height, channel and position
   * the image shown was taken
   *
   * @param images
   * @return
   */
        ...
        switch (displayedAxes_.get(i)) {
          ...
          case Coords.T:
            ...
            if (elapsedTimeMs < 0) {
              ...
              sb.append(NumberUtils.doubleToDisplayString((elapsedTimeMs / 1000), 1)).append("s ");
            } else {
          ...
          case Coords.Z:
            ...
            if (zPositionUm != null) {
              sb.append(NumberUtils.doubleToDisplayString(zPositionUm, 2)).append("um ");
            }
            break;
          case Coords.C:
            int channelIndex = nominalCoords.getC();
            sb.append(displayController_.getChannelName(nominalCoords.getC())).append(" ");
            break;
          default:
            break;
        }
  ...
  public void applyDisplaySettings(DisplaySettings settings) {
    ...
    boolean autostretch = settings.isAutostretchEnabled() && displayedImages_ != null;
    ...
    if (!isRGB) {
      ...
      for (int i = 0; i < nChannels; ++i) {
        ChannelDisplaySettings channelSettings = settings.getChannelSettings(i);
        ...
        ChannelDisplaySettings rememberedSettings =
            RememberedDisplaySettings.loadChannel(
                studio_, channelSettings.getGroupName(), channelSettings.getName(), null);
      ...
      for (int chNr = 0; chNr < nChannels; chNr++) {
        /**
         * Note: Since the UI currently manipulates all components identically, and setting a
         * component results in many calculations and redrawing the image, for performance reasons
         * we only set one component. Setting components differently in a performant way will need a
         * bit of re-architecting
         */
        ...
        int max = Math.min(Integer.MAX_VALUE, (int) componentSettings.getScalingMaximum());
        int min = Math.max(1, (Math.min(max - 1, (int) componentSettings.getScalingMinimum())));
  ...
  private void applyAutostretch(ImagesAndStats images, DisplaySettings settings) {
    ...
    for (int i = 0; i < nChannels; ++i) {
      ...
      if (images.getResult().size() > statsIndex) {
        ...
        long max =
            Math.min(Integer.MAX_VALUE, stats.getComponentStats(0).getAutoscaleMaxForQuantile(q));
        ...
        DefaultComponentDisplaySettings dcds =
            (DefaultComponentDisplaySettings)
        ...
        ReportingUtils.logMessage(
            "DisplayUICOntroller: Received request to "
  ...
  private void updateAxisPositionIndicator(
      final String axis, final int position, final int length) {
    ...
    for (Map.Entry<String, PopupButton> e : axisPositionButtons_) {
      if (axis.equals(e.getKey())) {
        e.getValue().setText(String.format("% 5d/% 5d", checkedPosition + 1, checkedLength));
  ...
  void setFullScreenMode(boolean fullScreen) {
    if (fullScreen) {
      ...
      fullScreenButton_.setIcon(IconLoader.getIcon("/org/micromanager/icons/windowed.png"));
      ...
      fullScreenButton_.setIcon(IconLoader.getIcon("/org/micromanager/icons/fullscreen.png"));
  ...
  /**
   * Callback for the ImageJ code. Do not call directly. Used to update the Micro-Manager code of
   * the new zoom factor.
   *
  ...
  public void uiDidSetZoom(double factor) {
    updateZoomUIState();
    displayController_.setDisplaySettings(
        displayController_.getDisplaySettings().copyBuilder().zoomRatio(factor).build());
    updateTitle();
    canvasDidChangeSize();
  }
  ...
  public void canvasDidChangeSize() {
    ...
    if (isFullScreenMode()) {
      ...
      int newCanvasWidth =
          Math.min(
              canvasMaxSize.width,
              fullScreenFrame_.getWidth() - frameInsets.left - frameInsets.right - MARGIN);
      int newCanvasHeight = Math.min(canvasMaxSize.height, canvasPanel_.getHeight() - MARGIN);
      ijBridge_.getIJImageCanvas().setPreferredSize(new Dimension(newCanvasWidth, newCanvasHeight));
      ...
      Rectangle screenBounds =
          Geometry.insettedRectangle(
              gConfig.getBounds(), Toolkit.getDefaultToolkit().getScreenInsets(gConfig));
  ...
  /**
   * a key was pressed on the display Canvas. Post as an event. Let the caller know if any of the
   * event handlers consumed the keypress
   *
   ...
   *
   * <p>If {@code imageLocation} is null or empty, the indicator is hidden. The {@code
   * imageLocation} parameter can be a rectangle containing more than one pixel, for example if the
   * point comes from a zoomed-out canvas.
   *
   * @param e MouseEvent that occurred on the Canvas. Use its getId() function to discover what kind
   *     of Mouse Event happened.
   * @param imageLocation the image coordinates of the pixel for which information should be
   *     displayed (in image coordinates)
   * @param ijToolId ID of tool selected in ImageJ tool-bar
   */
  public void mouseEventOnImage(
      final MouseEvent e, final Rectangle imageLocation, final int ijToolId) {
    displayController_.postDisplayEvent(new DisplayMouseEvent(e, imageLocation, ijToolId));
    switch (e.getID()) {
      case MouseEvent.MOUSE_MOVED:
      case MouseEvent.MOUSE_ENTERED:
      case MouseEvent.MOUSE_EXITED:
        if (imageLocation == null || imageLocation.width == 0 || imageLocation.height == 0) {
          if (mouseLocationOnImage_ == null) {
            return;
          }
  ...
  private void updatePixelInformation() {
    if (displayedImages_ == null || mouseLocationOnImage_ == null) {
      displayController_.postDisplayEvent(DataViewerMousePixelInfoChangedEvent.createUnavailable());
      return;
    }

    List<Image> images = new ArrayList<>(displayedImages_.getRequest().getImages());
    if (images.isEmpty()) {
      displayController_.postDisplayEvent(DataViewerMousePixelInfoChangedEvent.createUnavailable());
      return;
    }
    ...
    if (images.get(0).getCoords().hasAxis(Coords.CHANNEL)) {
      try {
        displayController_.postDisplayEvent(
            DataViewerMousePixelInfoChangedEvent.fromAxesAndImages(
                center.x, center.y, new String[] {Coords.CHANNEL}, images));
        ...
        ReportingUtils.logError(
            "Request to display mousePixel Info failed for these images : " + coordString);
        displayController_.postDisplayEvent(
            DataViewerMousePixelInfoChangedEvent.createUnavailable());
      }
    } else {
      displayController_.postDisplayEvent(
          DataViewerMousePixelInfoChangedEvent.fromImage(center.x, center.y, images.get(0)));
    }
  }

  public void paintOverlays(Graphics2D g, Rectangle destRect, Rectangle2D.Float viewPort) {
    ...
    for (Overlay overlay : overlays) {
      if (overlay.isVisible()) {
        overlay.paintOverlay(g, destRect, displaySettings, images, primaryImage, viewPort);
      }
    }
  }
  ...
  public void paintDidFinish() {
    ...
    // the former case.
    boolean countAsNewDisplayedImage = repaintScheduledForNewImages_.compareAndSet(true, false);
    if (countAsNewDisplayedImage) {
      ...
      if (displayController_.isAnimating()) {
        adjustAnimationTickIntervalMs(displayIntervalEstimator_.get().getQuantile(0.5));
      }
      perfMon_.sample(
          "Display interval 25th percentile", displayIntervalEstimator_.get().getQuantile(0.25));
      perfMon_.sample(
          "Display interval 50th percentile", displayIntervalEstimator_.get().getQuantile(0.5));
      perfMon_.sample(
          "Display interval 75th percentile", displayIntervalEstimator_.get().getQuantile(0.75));
      showFPS();
    }
  }
  ...
  private void adjustAnimationTickIntervalMs(double achievedMedianMs) {
    ...
    double lowJitterFPS = animationFPS * Math.max(1.0, Math.floor(60.0 / animationFPS));
    ...
    targetMs = (int) Math.max(0.75 * currentSettingMs, Math.min(1.25 * currentSettingMs, targetMs));
  ...
  /**
   * NS, 20190515: Display fps is confusing to the user. It is unclear what it means. It also can
   * easily be confused with Playback fps and camera fps, which are more useful messages. I leave
   * the old code below for future use, but no longer call it Showing camera fps instead. Display
   * fps is set by the user, and the code should make sure that it is correct. It is surprisingly
   * difficult to estimate camera fps correctly with the information we get from the core. We are
   * using the imageNumber, inserted by the Circular Buffer to count images produced by the camera.
   * However, when the circular buffer is full, it will reset, so we need to deduce if that event
   * took place, and calculate how many images we missed when that happened. When the camera runs
   * very fast, and the circular buffer is small, it may happen that the buffer overflows before we
   * ever see an image (or possibly, the first image we see has a higher image number than the one
   * we remembered from the last iteration, which is how we check for overflows), resulting in poor
   * fps estimates. The Core is really much better suited to keep track of camera fps. I'll open a
   * ticket, but until then, this is teh best I can come up with.
   */
      ...
      if (displayedImages != null && displayedImages.size() > 0) {
        ...
        if (nr != null) {
          if (nr < lastImageNumber_) {
            ...
            long missingImages = studio_.core().getBufferTotalCapacity() - lastImageNumber_ + nr;
          ...
          if (durationMs_ > 0.0) {
            ...
            if (fps_ < 2.0) {
              cameraFpsLabel_.setText(String.format("Camera: %.3g fps", fps_));
            } else {
              cameraFpsLabel_.setText(String.format("Camera: %d fps", (int) fps_));
            }
          }
        } else {
          cameraFpsLabel_.setText(" ");
  ...
  public void resetDisplayIntervalEstimate() {
    displayIntervalEstimator_.set(
        TimeIntervalRunningQuantile.create(DISPLAY_INTERVAL_SMOOTH_N_SAMPLES));
  }
  ...
  /** @return Human readable String explaining the pixel Type TODO: Formalize these Strings */
  ...
  /**
   * Generates the info string in the top right of the viewer showing: - x-y size in microns - x-y
   * size in pixels - pixelType - Size in k/MB
   *
   * <p>This function is only called when the viewer is created or when the pixel size changes
   *
   * @param images
   * @return
   */
  public String getInfoString(ImagesAndStats images) {
    ...
    if (pixelSize != null && pixelSize != 0.0) {
      ...
      infoStringB
          .append(NumberUtils.doubleToDisplayString(widthUm))
          .append("x")
  ...
  private void handleAxisAnimateButton(ActionEvent event) {
    ...
    displayController_.setPlaybackAnimationAxes(animatedAxes.toArray(new String[] {}));
  }
  ...
  private void handleAxisSpinner(String axis, JSpinner spinner, ChangeEvent event) {
    ...
    displayController_.setDisplayPosition(
        displayController_.getDisplayPosition().copyBuilder().index(axis, newPosition).build());
  }

  private void handlePlaybackFpsSpinner(ChangeEvent event) {
    double fps = (Double) playbackFpsSpinner_.getValue();
    displayController_.setDisplaySettings(
        displayController_.getDisplaySettings().copyBuilder().playbackFPS(fps).build());
  }
  ...
  @Override
  public void scrollBarPanelHeightWillChange(MDScrollBarPanel panel, int currentHeight) {
    panel.setVisible(false);
  }

  @Override
  public void scrollBarPanelHeightDidChange(MDScrollBarPanel panel, int oldHeight, int newHeight) {
    if (isFullScreenMode()) {
      ...
      frame_.setSize(frame_.getWidth(), frame_.getHeight() - oldHeight + newHeight);
      panel.setVisible(true);
      frame_.validate();
  ...
  public void onEvent(DataViewerMousePixelInfoChangedEvent e) {
    ...
    pixelInfoLabel_.setText(String.format("%s = %s", e.getXYString(), valuesString));
    if (pixelInfoLabel_.getSize().width > pixelInfoLabel_.getMinimumSize().width) {
      pixelInfoLabel_.setMinimumSize(new Dimension(pixelInfoLabel_.getSize().width, 10));
    }
  }
    ...
    if (!channelColorEvent.equals(channelColorEvent_)) {
      if (channelColorEvent.getColor() != null) {
        ...
        do {
          ...
          newDisplaySettings =
              oldDisplaySettings
                  .copyBuilderWithChannelSettings(
                      index,
                      channelSettings.copyBuilder().color(channelColorEvent.getColor()).build())
                  .build();
