...
/**
 * A proxy object providing an interface between our image display system and ImageJ's system.
 *
 * <p>ImageJ expects its images to be displayed in {@code ij.gui.ImageWindow} (and its subclass
 * {@code ij.gui.StackWindow}), which descend from {@code java.awt.Frame}. Since MMStudio's image
 * display windows are {@code JFrame}s, it is not practical to get ImageJ to directly interact with
 * them. This class acts as an intermediary; it is never actually displayed on screen and simply
 * forwards the relevant requests from ImageJ to MMStudio's display controller.
 *
...
final class ProxyImageWindow extends StackWindow {
   ...
   *
   * @param parent the parent {@code ImageJBridge} object, which must be ready to provide a valid
   *     {@code ImagePlus} and {@code ImageCanvas}.
  ...
  public ImageCanvas getCanvas() {
    ...
    if (canvas == null) {
      throw new NullPointerException("Programming Error: ImageJLink returned null ImageCanvas");
    }
    return canvas;
  }
