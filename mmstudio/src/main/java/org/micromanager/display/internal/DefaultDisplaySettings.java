...
public final class DefaultDisplaySettings implements DisplaySettings {
  ...
  private DefaultDisplaySettings(Builder builder) {
    ...
    channelSettings_ = new ArrayList<>(builder.channelSettings_);
  }
  ...
  /**
   * Retrieve the display settings that have been saved in the preferences. Note: we explicitly
   * don't cache these settings, to ensure that displays don't end up with copies of the same
   * settings.
   *
   * @param key String for storing settings under different locations, so different "types" of
   *     displays can have different default settings.
  ...
  public static DefaultDisplaySettings getStandardSettings(String key) {
    ...
    Color[] defaultColors =
        new Color[] {
          Color.YELLOW, Color.CYAN, Color.MAGENTA, Color.RED, Color.GREEN, Color.BLUE, Color.ORANGE
        };
    ...
    // name change.
    builder.animationFPS(settings.getDouble(key + ANIMATION_FPS_DOUBLE, 10.0));
    builder.channelColorMode(
        DisplaySettings.ColorMode.fromInt(
            settings.getInteger(
                key + CHANNEL_COLOR_MODE, DisplaySettings.ColorMode.COMPOSITE.getIndex())));
  ...
  /**
   * Saves these DisplaySettings in the UserProfile. Implementers are free to save copies of the
   * settings themselves.
   *
   * @param profile Profile to which these settings should be saved
   * @param key Key under which the settings will be stored. Will be pre-pended with PROFILEKEY
   */
   ...
   * @param key Key use to retrieve the DisplaySettings Will be pre-pended with PROFILEKEY
  ...
  /**
   * Return the current color mode setting in the profile, or the provided default value. This is
   * specifically available to allow the Snap/Live Manager to default to grayscale instead of the
   * normal default (returned from getStandardSettings) of composite mode.
   *
   * @param key Profile key to use, as per [get|set]StandardSettings.
   */
  public static ColorMode getStandardColorMode(String key, DisplaySettings.ColorMode defaultVal) {
    UserProfile profile = MMStudio.getInstance().profile();
    key = key + "_";
    Integer mode =
        profile.getSettings(DefaultDisplaySettings.class).getInteger(CHANNEL_COLOR_MODE, -1);
    if (mode == -1) {
      return defaultVal;
    }
  ...
  /** Convert the provided array of Colors to an array of Integers, in RGB order. */
  ...
  public static class DefaultContrastSettings implements DisplaySettings.ContrastSettings {
     ...
     */
    public DefaultContrastSettings(
    ...
    public Integer getSafeContrastMin(int component, Integer defaultVal) {
      if (component < 0 || contrastMins_ == null || contrastMins_.length <= component) {
        return defaultVal;
      }
    ...
    public Integer getSafeContrastMax(int component, Integer defaultVal) {
      if (component < 0 || contrastMaxes_ == null || contrastMaxes_.length <= component) {
        return defaultVal;
      }
    ...
    public Double getSafeContrastGamma(int component, Double defaultVal) {
      if (component < 0 || gammas_ == null || gammas_.length <= component) {
        return defaultVal;
      }
    ...
    public boolean equals(Object obj) {
      ...
      if ((contrastMins_ != null && !Arrays.deepEquals(contrastMins_, altMins))
          || (contrastMaxes_ != null && !Arrays.deepEquals(contrastMaxes_, altMaxes))
    ...
    public String toString() {
      ...
      for (int i = 0; i < getNumComponents(); ++i) {
        result +=
            String.format(
                "(%d, %d @ %.2f)",
                getSafeContrastMin(i, -1),
                getSafeContrastMax(i, -1),
                getSafeContrastGamma(i, -1.0));
      }
      if (isVisible_ != null) {
  ...
  public static class LegacyBuilder extends Builder
    ...
    public DisplaySettingsBuilder channelColors(Color[] channelColors) {
      for (int i = 0; i < channelColors.length; ++i) {
        ...
        channel(i, getChannelSettings(i).copyBuilder().color(channelColors[i]).build());
      }
      return this;
    }

    @Override
    public DisplaySettingsBuilder safeUpdateChannelColor(Color newColor, int channelIndex) {
      channel(channelIndex, getChannelSettings(channelIndex).copyBuilder().color(newColor).build());
      return this;
    }
    ...
    public DisplaySettingsBuilder safeUpdateContrastSettings(
      ...
      ChannelDisplaySettings.Builder channelBuilder = channelSettings.copyBuilder();
      for (int j = 0; j < legacySettings.getNumComponents(); ++j) {
        ...
        if (legacySettings.getContrastMins() != null
          ...
          componentBuilder = componentBuilder.scalingMinimum(legacySettings.getContrastMins()[j]);
        }
        if (legacySettings.getContrastMaxes() != null
            && legacySettings.getContrastMaxes()[j] != null) {
          componentBuilder = componentBuilder.scalingMaximum(legacySettings.getContrastMaxes()[j]);
        }
        if (legacySettings.getContrastGammas() != null
            && legacySettings.getContrastGammas()[j] != null) {
          componentBuilder = componentBuilder.scalingGamma(legacySettings.getContrastGammas()[j]);
        }
  ...
  public ContrastSettings getSafeContrastSettings(int index, ContrastSettings defaultVal) {
    ...
    for (int j = 0; j < nComponents; ++j) {
      ComponentDisplaySettings componentSettings = channelSettings.getComponentSettings(j);
    ...
    return new DefaultContrastSettings(mins, maxes, gammas, channelSettings.isVisible());
  }
  ...
  @Override
  public DisplaySettings.Builder copyBuilderWithChannelSettings(
      int channel, ChannelDisplaySettings settings) {
    ...
    try {
      ...
      if (minsArr != null) {
        ...
        for (int i = 0; i < minsArr.length; ++i) {
          ...
          contrastSettings.add(new DefaultContrastSettings(min, max, 1.0, true));
        }
        builder.channelContrastSettings(contrastSettings.toArray(new DefaultContrastSettings[] {}));
      }

      if (tags.has(CHANNEL_COLOR_MODE)) {
        builder.channelColorMode(ColorMode.fromInt(tags.getInt(CHANNEL_COLOR_MODE)));
      }
  ...
  /**
   * Store these displaySettings in a propertyMap NS: Should this be in the API?
   *
  ...
  /**
   * Restore DisplaySettings from a PropertyMap NS: Should this be in the api?
   *
   * @param pMap PropertyMap to be restored to DisplaySettings
   * @return restored DisplaySettings. Any missing component will be replaced with the (Builder's)
   *     default
   */
  public static DisplaySettings fromPropertyMap(PropertyMap pMap) {
    ...
    if (pMap != null) {
      ...
      if (pMap.containsStringForEnum(PropertyKey.COLOR_MODE.key(), ColorMode.class)) {
        ddsb.colorMode(
            pMap.getStringAsEnum(PropertyKey.COLOR_MODE.key(), ColorMode.class, ddsb.mode_));
      }
      if (pMap.containsBoolean(PropertyKey.UNIFORM_CHANNEL_SCALING.key())) {
        ddsb.uniformChannelScaling(
            pMap.getBoolean(
      ...
      if (pMap.containsDouble(PropertyKey.AUTOSCALE_IGNORED_QUANTILE.key())) {
        ddsb.autoscaleIgnoredQuantile(
            pMap.getDouble(PropertyKey.AUTOSCALE_IGNORED_QUANTILE.key(), ddsb.extremaQuantile_));
      }
      if (pMap.containsPropertyMapList(PropertyKey.CHANNEL_SETTINGS.key())) {
        List<PropertyMap> propertyMapList =
            pMap.getPropertyMapList(
  ...
  /**
   * Saves the current displaySettings to the indicated File DisplaySettings are saves as a JSON
   * encoded PropertyMap
   *
  ...
  /**
   * Saves the current displaySettings to a file in the provided path. This file will be named using
   * a common convention (DisplaySettings.json, defined in PropertyKey.DISPLAY_SETTINGS_FILE_NAME).
   *
   * @param path path under which to create the DisplaySettings file
   */
  public void save(String path) {
    // TODO: test for sanity of input path?
    File displaySettingsFile =
        new File(path + File.separator + PropertyKey.DISPLAY_SETTINGS_FILE_NAME.key());
