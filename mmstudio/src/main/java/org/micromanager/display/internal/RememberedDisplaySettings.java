 ...
 * channelDisplaySettings for a given channel)
 *
 * <p>Contains code to stay backwards compatible with RememberedChannelSettins mechanism to store
 * settings. Remove this code after 9/2020
 *
 * @author nico
 */
public class RememberedDisplaySettings {
   ...
   */
  public static void storeChannel(
      Studio studio, String channelGroup, String channelName, ChannelDisplaySettings cds) {
    String key = genKey(channelGroup, channelName);
    MutablePropertyMapView settings = studio.profile().getSettings(RememberedDisplaySettings.class);
  ...
  /**
   * Loads ChannelDisplaySettings from profile. For backwards compatibility, will load from
   * RememberChannelSettings if this class' stored settings are not found. Returns defaults if
   * nothing is found.
   *
   * @param studio Object used to get access to profile
   * @param channelGroup Group to which the channel belongs
   * @param channelName Channel name
   * @param defaultColor If nothing was found, use this color as the default color will be ignored
   *     when null
   * @return Stored or Default ChannelDisplaySetting
   */
  public static ChannelDisplaySettings loadChannel(
      Studio studio, String channelGroup, String channelName, Color defaultColor) {
    String key = genKey(channelGroup, channelName);
    MutablePropertyMapView settings = studio.profile().getSettings(RememberedDisplaySettings.class);
  ...
  /**
   * Given the channels in summary metadata, constructs a DisplaySettings object with the
   * ChannelDisplaySettings appropriate for each channel Note: the "overall" DisplaySettings (i.e.
   * everything unrelated to channels) are not stored (yet).
   *
   ...
   */
  public static DisplaySettings loadDefaultDisplaySettings(Studio studio, SummaryMetadata summary) {
  ...
  /**
   * Some MM versions have stored DisplaySettings without storing ChannelNames This causes the
   * channel names not to be shown in the Intensity Inspector Panels. Fix this here.
   *
   ...
   * @return Copy of the input with Channelgroup and Channelnames from the summary metadata if they
   *     were empty in the input.
   */
