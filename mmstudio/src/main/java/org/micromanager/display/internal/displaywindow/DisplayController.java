 ...
 *
 * <p>This is also the implementation for the DisplayWindow API, for now (it might make sense to
 * refactor and separate API support from controller implementation).
 *
...
public final class DisplayController extends DisplayWindowAPIAdapter
  ...
  private final TreeMap<Integer, DataViewerListener> listeners_ = new TreeMap<>();
    ...
    if (initialDisplaySettings == null) {
      initialDisplaySettings =
          RememberedDisplaySettings.loadDefaultDisplaySettings(
              studio, builder.dataProvider_.getSummaryMetadata());
    }
    if (initialDisplaySettings == null) {
  ...
  private void initialize() {
    ...
    uiController_ =
        DisplayUIController.create(studio_, this, controlsFactory_, animationController_);
          ...
          public void run() {
            ...
            if (displayedImages_ != null
              ...
              for (int i = 0; i < images.getRequest().getNumberOfImages(); ++i) {
                if (images.getRequest().getImage(i) != displayedImages_.getRequest().getImage(i)) {
                  imagesDiffer = true;
                  break;
                }
              }
            ...
            if (imagesDiffer
                || getDisplaySettings().isAutostretchEnabled()
                || getDisplaySettings().getColorMode() != DisplaySettings.ColorMode.COMPOSITE) {
              uiController_.displayImages(images);
            } else if (getDisplaySettings().getColorMode() == DisplaySettings.ColorMode.COMPOSITE) {
            ...
            postEvent(
                DefaultDisplayDidShowImageEvent.create(
                    DisplayController.this, images.getRequest().getImages(), primaryImage));
  ...
  protected Coords handleDisplayPosition(Coords position) {
    ...
    runnablePool_.invokeLaterWithCoalescence(new ExpandDisplayRangeCoalescentRunnable(position));

    // Always compute stats for all channels
    List<Image> images;
    try {
      images =
          dataProvider_.getImagesIgnoringAxes(
              position.copyRemovingAxes(Coords.CHANNEL), Coords.CHANNEL);
    } catch (IOException e) {
    ...
    computeQueue_.submitRequest(ImageStatsRequest.create(position, images, selection));
  ...
  public List<Overlay> getOverlays() {
    if (!SwingUtilities.isEventDispatchThread()) {
      RunnableFuture<List<Overlay>> edtFuture = new FutureTask<>(() -> getOverlays());
  ...
  public void setPlaybackAnimationAxes(String... axes) {
    ...
    if (axes.length > 0) {
      ...
      int initialTickIntervalMs =
          Math.max(17, Math.min(500, (int) Math.round(1000.0 / getPlaybackSpeedFps())));
  ...
  public void setPlaybackSpeedFps(double fps) {
    ...
    int initialTickIntervalMs = Math.max(17, Math.min(500, (int) Math.round(1000.0 / fps)));
    animationController_.setTickIntervalMs(initialTickIntervalMs);
  ...
  /**
   * A coalescent runnable to avoid excessively frequent update of the data coords range in the UI
   */
  private class ExpandDisplayRangeCoalescentRunnable implements CoalescentRunnable {
    ...
    public CoalescentRunnable coalesceWith(CoalescentRunnable another) {
      coords_.addAll(((ExpandDisplayRangeCoalescentRunnable) another).coords_);
      return this;
    }
  ...
  public ImagePlus getImagePlus() {
    if (!SwingUtilities.isEventDispatchThread()) {
      RunnableFuture<ImagePlus> edtFuture = new FutureTask<>(() -> getImagePlus());
      SwingUtilities.invokeLater(edtFuture);
      try {
        return edtFuture.get();
      } catch (InterruptedException notUsedByUs) {
  ...
  public boolean requestToClose() {
    if (!SwingUtilities.isEventDispatchThread()) {
      RunnableFuture<Boolean> edtFuture = new FutureTask<>(() -> requestToClose());
      SwingUtilities.invokeLater(edtFuture);
      try {
        return edtFuture.get();
      } catch (InterruptedException notUsedByUs) {
  ...
  public Window getWindow() throws IllegalStateException {
    if (!SwingUtilities.isEventDispatchThread()) {
      RunnableFuture<Window> edtFuture = new FutureTask<>(() -> getWindow());
      SwingUtilities.invokeLater(edtFuture);
      try {
        return edtFuture.get();
      } catch (InterruptedException notUsedByUs) {
    ...
    if (uiController_ == null) {
      throw new IllegalStateException("Display has closed; no window available");
    }
