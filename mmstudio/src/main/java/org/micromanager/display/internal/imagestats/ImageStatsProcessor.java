...
public final class ImageStatsProcessor {
  ...
  public ImagesAndStats process(
      final long sequenceNumber, final ImageStatsRequest request, boolean interruptible)
  ...
  private ImageStats computeStats(Image image, ImageStatsRequest request, int index)
    ...
    int bitDepth = boxedBitDepth == null ? 8 * bytesPerSample : boxedBitDepth;
    ...
    if (bytesPerSample == 1) {
      Img<UnsignedByteType> img =
          ArrayImgs.unsignedBytes(
              (byte[]) image.getRawPixels(), nComponents, image.getWidth(), image.getHeight());
      result =
          compute(
      ...
      Img<UnsignedShortType> img =
          ArrayImgs.unsignedShorts(
              (short[]) image.getRawPixels(), nComponents, image.getWidth(), image.getHeight());
      result =
          compute(
              clipToRect(img, nComponents, statsBounds),
              mask,
  ...
  private <T extends IntegerType<T>> ImageStats compute(
    ...
    BinMapper1d<T> binMapper = PowerOf2BinMapper.create(sampleBitDepth, binCountPowerOf2);
    ...
    IntegerComponentStats[] componentStats = new IntegerComponentStats[nComponents];
    for (int component = 0; component < nComponents; ++component) {
      componentStats[component] =
          IntegerComponentStats.builder()
              .histogram(
                  histograms.get(component).toLongArray(),
  ...
  private <T extends IntegerType<T>> IterableInterval<T> clipToRect(
    ...
    return Views.interval(
        ...
        Intervals.createMinSize(
            0, statsBounds.x, statsBounds.y, nComponents, statsBounds.width, statsBounds.height));
  }
  ...
  private IterableInterval<UnsignedByteType> wrapROIMask(
    ...
    IntervalView<UnsignedByteType> mask =
        Views.translate(
            ArrayImgs.unsignedBytes(rawMask, maskBounds.width, maskBounds.height),
            maskBounds.x,
            maskBounds.y);
    ...
    long[] max = {
      nComponents - 1, statsBounds.x + statsBounds.width - 1, statsBounds.y + statsBounds.height - 1
    };
    MixedTransform t = new MixedTransform(3, 2);
    t.setComponentMapping(new int[] {1, 2});
    return Views.iterable(
        Views.interval(new MixedTransformView<UnsignedByteType>(mask, t), min, max));
  }
}
