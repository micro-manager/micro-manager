...
public final class DefaultChannelDisplaySettings implements ChannelDisplaySettings {
  ...
  private static final class Builder implements ChannelDisplaySettings.Builder {
    ...
    private final List<ComponentDisplaySettings> componentSettings_ = new ArrayList<>();
  ...
  /**
   * Helper function for overloaded versions of fromPropertyMap Restores everything form the
   * propertymap except for Channelgroup and ChannelName
   *
   * @param pMap
   * @return Builder with everything useful in the propertymap except for channelGroup and
   *     channelName
   */
  private static ChannelDisplaySettings.Builder partialBuilderFromPropertyMap(PropertyMap pMap) {
    ...
    if (pMap.containsPropertyMapList(PropertyKey.COMPONENT_SETTINGS.key())) {
      List<PropertyMap> componentMapList =
          pMap.getPropertyMapList(PropertyKey.COMPONENT_SETTINGS.key(), new ArrayList<>());
      for (int i = 0; i < componentMapList.size(); i++) {
        ComponentDisplaySettings cds =
            DefaultComponentDisplaySettings.fromPropertyMap(componentMapList.get(i));
        b.component(i, cds);
      }
    }
    if (pMap.containsInteger(PropertyKey.HISTOGRAM_BIT_DEPTH.key())) {
      b.histoRangeBits(pMap.getInteger(PropertyKey.HISTOGRAM_BIT_DEPTH.key(), b.histoRangeBits_));
    }
    if (pMap.containsBoolean(PropertyKey.USE_CAMERA_BIT_DEPTH.key())) {
      b.useCameraHistoRange(
          pMap.getBoolean(PropertyKey.USE_CAMERA_BIT_DEPTH.key(), b.useCameraRange_));
    }
  ...
  /**
   * Restores ChannelDisplaySettings from a PropertyMap, but uses the input channelGroup and
   * channelName rather than the ones in the PropertyMap Needed to gracefully update propertymaps
   * that did not yet store the channelGroup
   *
   ...
   */
  public static ChannelDisplaySettings fromPropertyMap(
      PropertyMap pMap, String channelGroup, String channelName) {
