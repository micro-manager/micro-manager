...
class RGBColorModeStrategy implements ColorModeStrategy {
  ...
  /**
   * For now, we use one LUT for Red, Green, and Blue. This behavior can (and should) be changed in
   * the future
   */
  ...
  public void applyModeToImagePlus(ImagePlus imagePlus) {
    ...
    Preconditions.checkArgument(imagePlus.getProcessor() instanceof ColorProcessor);
