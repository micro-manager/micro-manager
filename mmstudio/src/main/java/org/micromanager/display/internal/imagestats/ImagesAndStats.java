...
public final class ImagesAndStats {
  ...
  public static ImagesAndStats create(
      long sequenceNumber, ImageStatsRequest input, ImageStats... stats) {
    return new ImagesAndStats(sequenceNumber, input, stats);
  }

  private ImagesAndStats(long sequenceNumber, ImageStatsRequest input, ImageStats... stats) {
    this(sequenceNumber, input, input, stats);
  }

  private ImagesAndStats(
      long sequenceNumber,
      ImageStatsRequest request,
      ImageStatsRequest originalInput,
      ImageStats... stats) {
  ...
  public ImagesAndStats copyForRequest(ImageStatsRequest request) {
    return new ImagesAndStats(
        statsSequenceNumber_, request, request_, stats_.toArray(new ImageStats[] {}));
  }
  ...
  /**
   * Returns whether the stats were computed from the nominal request wrapped in this object (as
   * opposed to adopted or recycled from another request).
   *
   * @return true if the stats were computed from the request returned by {@code getRequest}
   */
