 ...
 *
 * <p>Unlike the default {@code Integer1dBinMapper} provided by ImgLib2, which only allows size-1
 * bins, this allows histograms to have size 1, 2, 4, 8, ... bins. For example, a 256-bin histogram
 * can be created for a 16-bit image.
 *
...
public class PowerOf2BinMapper<T extends IntegerType<T>> implements BinMapper1d<T> {
   ...
   * @return
   */
  public static <T extends IntegerType<T>> PowerOf2BinMapper<T> create(
      int sampleDepthPowerOf2, int binCountPowerOf2) {
