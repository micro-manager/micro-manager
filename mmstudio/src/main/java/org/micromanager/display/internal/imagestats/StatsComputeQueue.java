...
public final class StatsComputeQueue {
  ...
  private final ExecutorService computeExecutor_ =
      Executors.newSingleThreadExecutor(
          ThreadFactoryFactory.createThreadFactory("Stats Compute Queue Compute"));

  private final ExecutorService bypassExecutor_ =
      Executors.newSingleThreadExecutor(
          ThreadFactoryFactory.createThreadFactory("Stats Compute Queue Bypass"));

  private final ExecutorService resultExecutor_ =
      Executors.newSingleThreadExecutor(
          ThreadFactoryFactory.createThreadFactory("Stats Compute Queue Result"));

  // Outstanding compute tasks by priority
  // Guarded by monitor on this
  private final List<Future<?>> computeFutures_ = new ArrayList<Future<?>>();

  // Outstanding bypassed data by priority
  // Guarded by monitor on this
  private final List<Future<?>> bypassFutures_ = new ArrayList<Future<?>>();
  ...
  // Guarded by monitor on this
  private final List<Deque<Future<?>>> resultFutures_ = new ArrayList<Deque<Future<?>>>();
  ...
  // By priority.
  // Guarded by monitor on this
  private final List<ImagesAndStats> storedStats_ = new ArrayList<ImagesAndStats>();
  ...
  public synchronized void submitRequest(ImageStatsRequest request) {
    ...
    if (updateIntervalNs_ < Long.MAX_VALUE) {
      final long waitTargetNs =
          updateIntervalNs_ == Long.MAX_VALUE
              ? Long.MAX_VALUE
  ...
  private void submitBypass(
      final long sequenceNumber, final int priority, final ImageStatsRequest request) {
  ...
  private synchronized void submitResult(
      long sequenceNumber, final int priority, final ImagesAndStats result) {
    ...
    for (int p = priority; p >= 0; --p) {
      ...
      if (p == priority && perfMon_ != null) {
        perfMon_.sample(
            "Compute result queue size at priority " + priority,
            ...
            resultExecutor_.submit(
                new Runnable() {
                  ...
                  public void run() {
                    ...
                    if (perfMon_ != null) {
                      perfMon_.sample(
                          "Compute result pre-wait (ms)", Math.max(0, waitNs / 1000000));
                    }
                    try {
                      if (waitNs > 0) {
