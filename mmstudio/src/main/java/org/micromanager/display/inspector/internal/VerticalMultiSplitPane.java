 ...
 *
 * <p>This is more than just nested {@code JSplitPane}s: it ensures that each component can be
 * resized without affecting the size of the other components. An extra draggable divider is added
 * below the last (bottom) component to allow resizing the latter.
 *
 * <p>Caveat: The vertical scroll bar of a containing scroll panel will change length when the mouse
 * enters the split pane dividers. This is due to a hack used to get the desired behavior from a set
 * of nested {@code JSplitPane}s. Also, using the scroll wheel when the cursor is positioned on a
 * divider will cause odd effects. Both of these issues could be mitigated by notifying the scroll
 * pane and disabling the scroll bar and mouse wheel.
 *
 * <p>If used without a scroll pane, a {@code ComponentListener} should be added so that the
 * container can appropriately resize.
 *
...
public class VerticalMultiSplitPane extends JPanel implements Scrollable {
  ...
  public static VerticalMultiSplitPane create(int numChildren, boolean continuousLayout) {
  ...
  private VerticalMultiSplitPane(int numChildren, boolean continuousLayout) {
    ...
    for (int i = 0; i < numChildren; ++i) {
      final JSplitPane splitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT, continuousLayout);
  ...
  public Dimension getPreferredSize() {
    ...
    for (JSplitPane splitPane : splitPanes_) {
      ...
      Dimension componentPrefSize =
          component.isValid() ? component.getSize() : component.getPreferredSize();
  ...
  /**
   * Resize this multi-split pane so that every component is at its preferred height.
   *
  ...
  public void resizeToFitPreferredSizes() {
    ...
    List<JSplitPane> reversedSplitPanes = new ArrayList<JSplitPane>(splitPanes_);
  ...
  private void preferCurrentSizes() {
    for (JSplitPane splitPane : splitPanes_) {
      splitPane
          .getTopComponent()
          .setPreferredSize(new Dimension(splitPane.getTopComponent().getSize()));
      if (debug) {
  ...
  private void prepareForDividerDrag(JSplitPane splitPane) {
    ...
    splitPane.setSize(splitPane.getWidth(), splitPane.getHeight() + HUGE_HEIGHT);
  ...
  public int getScrollableBlockIncrement(Rectangle visibleRect, int orientation, int direction) {
    ...
    if (direction > 0) { // Scroll down
      ...
      for (JSplitPane splitPane : splitPanes_) {
        ...
        splitPanePos += splitPane.getTopComponent().getHeight() + splitPane.getDividerSize();
      }
  ...
  private static void test() {
    final VerticalMultiSplitPane multiSplitPane = VerticalMultiSplitPane.create(3, true);
