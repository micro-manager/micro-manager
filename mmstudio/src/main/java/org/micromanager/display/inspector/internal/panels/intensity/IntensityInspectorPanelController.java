...
/** @author mark */
public class IntensityInspectorPanelController extends AbstractInspectorPanelController {
  ...
  private final JMenu gearMenuPaletteSubMenu_ = new JMenu("Channel Color Palette");
  ...
  private final Map<String, colorMenuItem> colorMenuMap_ = new LinkedHashMap<>(3);
  private final JMenu gearMenuUpdateRateSubMenu_ = new JMenu("Histogram Update Rate");
  private final Map<String, Double> histogramMenuMap_ = new LinkedHashMap<>(6);
  ...
  private final List<ChannelIntensityController> channelControllers_ = new ArrayList<>();
  ...
  private IntensityInspectorPanelController(Studio studio) {
    ...
    panel_.setLayout(new MigLayout(new LC().flowY().fill().insets("0").gridGap("0", "0")));
  ...
  private void setUpGearMenu() {
    ...
    colorMenuMap_.put(
        ...
        new colorMenuItem(
            new JCheckBoxMenuItem(COLOR_BLIND_FRIENDLY),
            ColorPalettes.getColorblindFriendlyPalette()));
    colorMenuMap_.put(
        RGBCMYW,
        new colorMenuItem(new JCheckBoxMenuItem(RGBCMYW), ColorPalettes.getPrimaryColorPalette()));
    colorMenuMap_.put(CUSTOM, new colorMenuItem(new JCheckBoxMenuItem(CUSTOM), customPalette_));
    ...
    final String defaultUpdateFrequency =
        studio_
            .profile()
            .getSettings(IntensityInspectorPanelController.class)
            .getString(HISTOGRAM_UPDATE_FREQUENCY, "Every Displayed Image");
      ...
      jbmi.addActionListener(
          (ActionEvent e) -> {
            ...
            studio_
                .profile()
                .getSettings(IntensityInspectorPanelController.class)
                .putString(HISTOGRAM_UPDATE_FREQUENCY, jbmi.getText());
          });
    ...
    gearMenuLogYAxisItem_.addActionListener(
        (ActionEvent e) -> handleHistogramLogYAxis(gearMenuLogYAxisItem_.isSelected()));
    gearMenuUseROIItem_.addActionListener(
        (ActionEvent e) -> handleHistogramUseROI(gearMenuUseROIItem_.isSelected()));
  }
  ...
  private void setUpGeneralControlPanel() {
    ...
    percentileSpinner_.setModel(new SpinnerNumberModel(0.0, 0.0, 49.9, 0.1));
    percentileSpinner_.addChangeListener(
        (ChangeEvent e) -> {
    ...
    generalControlPanel_.setLayout(new MigLayout(new LC().fill().insets("0").gridGap("0", "0")));

    generalControlPanel_.add(new JLabel("Color Mode:"), new CC().gapBefore("rel").split(2));
    generalControlPanel_.add(colorModeComboBox_, new CC().gapAfter("push").wrap());

    generalControlPanel_.add(autostretchCheckBox_, new CC().split(4).gapAfter("related"));
  ...
  private void setUpChannelHistogramsPanel(int numChannels) {
    ...
    channelHistogramsPanel_.setLayout(
        new MigLayout(
    ...
    for (int i = 0; i < numChannels; ++i) {
      channelHistogramsPanel_.add(
          new JSeparator(JSeparator.HORIZONTAL), new CC().span(2).growX().wrap());

      if (viewer_ != null) {
        ChannelIntensityController chanController = ChannelIntensityController.create(viewer_, i);
        channelControllers_.add(chanController);
        channelHistogramsPanel_.add(chanController.getChannelPanel(), new CC().growY());
        channelHistogramsPanel_.add(chanController.getHistogramPanel(), new CC().grow().wrap());
      } else {
      ...
      displaySettings =
          displaySettings
              .copyBuilderWithChannelSettings(
                  i, allChannelSettings.get(i).copyBuilder().color(colors.get(i)).build())
              .build();
    }
          ...
          public void run() {
            for (int i = 0; i < channelControllers_.size(); ++i) {
              ...
              for (ImageStats channelStats : stats.getResult()) {
                ...
                if (stats.getRequest().getNumberOfImages() > channelStats.getIndex()) {
                  channel =
                      stats.getRequest().getImage(channelStats.getIndex()).getCoords().getChannel();
                }
                if (channel == i) {
  ...
  public void attachDataViewer(DataViewer viewer) {
    ...
    SwingUtilities.invokeLater(
        () -> {
          ...
          setUpChannelHistogramsPanel(viewer_.getDataProvider().getNextIndex(Coords.CHANNEL));
