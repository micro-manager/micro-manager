...
final class InspectorSectionController implements InspectorPanelListener {
  ...
  public static InspectorSectionController create(
      InspectorController inspectorController, InspectorPanelController panelController) {
    return new InspectorSectionController(inspectorController, panelController);
  }

  private InspectorSectionController(
      InspectorController inspectorController, InspectorPanelController panelController) {
    ...
    headerPanel_ = new JPanel(new MigLayout(new LC().fillX().insets("0").gridGap("0", "0")));

    headerLabel_ =
        new JLabel(
            panelController.getTitle(),
            UIManager.getIcon(
                panelController.initiallyExpand() ? "Tree.expandedIcon" : "Tree.collapsedIcon"),
            SwingConstants.LEFT);
    ...
    gearButton_ = PopupButton.create(IconLoader.getIcon("/org/micromanager/icons/gear.png"));
    if (panelController_.getGearMenu() != null) {
      gearButton_.addPopupButtonListener(
    ...
    gearButton_.setVisible(
        panelController.initiallyExpand() && panelController_.getGearMenu() != null);
    ...
    panel_ = new JPanel(new MigLayout(new LC().fill().insets("0").gridGap("0", "0")));
  ...
  private void setExpanded(boolean expanded) {
    ...
    headerLabel_.setIcon(UIManager.getIcon(expanded ? "Tree.expandedIcon" : "Tree.collapsedIcon"));
