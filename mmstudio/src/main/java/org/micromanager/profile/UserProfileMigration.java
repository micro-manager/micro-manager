 ...
 * <p>
 * As code evolves and gets refactored, the need sometimes arises to change how
 * a particular setting is stored in the user profiles.
 * <p>
 * Objects implementing this interface can be registered with the user profile
 * system, so that migration of settings is performed automatically.
 * <p>
 * Typically, in a class that uses the user profile, you should define the
 * profile keys as a static internal enum, like this:
 * <pre><code>
 * class MyClass {
 ...
 * </code></pre>
 * <p>
 * Although this practice is not strictly required (you could use plain string
 ...
 * </code></pre>
 * <p>
 * Suppose the setting that is now represented by MY_SETTING_1 used to live in
 ...
 * with an added static initialization to register the migration:
 * <pre><code>
 ...
 * </code></pre>
 * <p>
 * That's all you need to do. The user profile system will do the rest of the
 * work, so that by the time you access the setting, it will have been
 * migrated to the new owner class and key.
 * <p>
 * The above example showed a simple 1-to-1 migration (from
 ...
 * <p>
 * The {@code migrate} method is only ever called if the current setting
 ...
 * <p>
 * All of the migrations for a given owner class must be registered before any
 ...
 * <p>
 * The {@code legacy} property map passed to the {@code migrate} method
...
public interface UserProfileMigration {
    ...
    * <p>
    * If you are using an enum class to implement {@code ProfileMigration},
    * this method will be automatically available (see {@link Enum#name()}).
    * <p>
    * If you are implementing a migration as a plain object (not recommended),
