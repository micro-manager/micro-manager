...
public final class UserProfileAdmin {
  ...
  private final ScheduledExecutorService saverExecutor_ =
      Executors.newSingleThreadScheduledExecutor(
          ThreadFactoryFactory.createThreadFactory("User Profile Saver"));
  ...
  private ProfileWriteLock acquireWriteLock() {
    ...
    try {
      lock = ProfileWriteLock.tryCreate(new File(getAppDataDirectory(), WRITE_LOCK_FILE));
      if (lock == null) {
   ...
   *
   * <p>It is not necessary to call this method unless you want to learn whether migration took
   * place. If you access the profiles first, migration will happen automatically.
   *
   ...
   *
   * <p>Make sure to call {@code syncToDisk} on each autosaving profile before calling this method.
   */
   ...
   * @throws IOException if there was an error reading the profiles, or migrating legacy profiles
   */
  ...
  /**
   * @param uuid The unique ID number associated with the profile you want to get.
   * @param errorHandler If an exception occurs during the autosave process the exception will be
   *     passed to this object's `exceptionThrown` method.
   * @return A `User Profile` instance that will routinely save to file in case the program crashes.
   * @throws IOException
   */
  public UserProfile getAutosavingProfile(UUID uuid, final ExceptionListener errorHandler)
      throws IOException {
    return getProfileImpl(uuid, true, errorHandler);
  }

  private UserProfile getProfileImpl(
      UUID uuid, boolean autosaving, final ExceptionListener errorHandler) throws IOException {
    for (IndexEntry entry : getIndex().getEntries()) {
      if (entry.getUUID().equals(uuid)) {
        final String filename = entry.getFilename();
        Profile profile = readFile(filename);
        final DefaultUserProfile uProfile =
            DefaultUserProfile.create(this, uuid, profile.getSettings());
  ...
  public UserProfile getNonSavingGlobalProfile() throws IOException {
    ...
    return DefaultUserProfile.create(this, null, getGlobalSettingsWithoutMigration());
  }
   ...
   *
   * <p>This does not check for duplicate names. If duplicate names should be avoided, the caller is
  ...
  public UUID duplicateProfile(UUID originalUUID, String newName) throws IOException {
  ...
  public void renameProfile(UUID uuid, String newName) throws IOException {
    ...
    while (i.hasNext()) {
      ...
      if (entry.getUUID().equals(uuid)) {
        updated = new IndexEntry(uuid, newName, entry.getFilename());
        i.set(updated);
        break;
      }
    }
   ...
   * @param ignoreProfileReadOnly If the `READ_ONLY` setting of the profile is `true` then this
   *     method will do not save unless this argument is `true`.
   * @throws IOException
   */
   ...
   *
   * @return true if migration occurred; false if the profile index files was not found or could not
   *     be read.
  ...
  private boolean migrateProfiles() throws IOException {
    ...
    try {
      legacyIndex =
          MM1JSONSerializer.fromJSON(Files.toString(getLegacyIndexFile(), Charsets.UTF_8));
    } catch (IOException e) {
      return false;
    }
  ...
  private static PropertyMap migrateProfile(PropertyMap legacy) {
    ...
    for (String legacyKey : legacy.keySet()) {
      ...
      settings.get(owner).putOpaqueValue(key, legacy.getAsOpaqueValue(legacyKey));
    }
   ...
   *
   * <p>This is used once when creating the profile. After that, the filename remains constant even
   * if the profile is renamed.
   *
