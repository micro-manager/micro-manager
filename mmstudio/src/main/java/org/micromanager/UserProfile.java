 ...
 *
 * <p>In MMStudio and its plugins, <em>all</em> user preferences should be stored in the current
 * user profile, <em>not in the standard Java preferences</em>.
 *
 * <p>This works similarly to {@link java.util.prefs.Preferences}, except that it is possible to
 * switch between multiple profiles within a single operating system user account. This is provided
 * because sharing an OS account by all users is a widespread practice on computers attached to
 * scientific equipment.
 *
 * <p>The user profile stores the same kinds of values as {@link PropertyMap}, but is mutable and
 * periodically saves any modifications (if thus set up by the system).
 *
...
public interface UserProfile {
   ...
   *
   * <p>This is the main interface for accessing values in the user profile.
   *
   ...
   *
   * <p>Do not confuse with {@code getSettings(owner).clear()}!
   */
