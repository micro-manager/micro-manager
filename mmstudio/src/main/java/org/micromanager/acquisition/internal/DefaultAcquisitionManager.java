...
public final class DefaultAcquisitionManager implements AcquisitionManager {
  ...
  public DefaultAcquisitionManager(
      Studio studio, AcquisitionWrapperEngine engine, AcqControlDlg mdaDialog) {
    studio_ = studio;
    engine_ = engine;
  ...
  @Override
  public Datastore runAcquisitionWithSettings(SequenceSettings settings, boolean shouldBlock)
  ...
  public SequenceSettings loadSequenceSettings(String path) throws IOException {
    return SequenceSettings.fromJSONStream(Files.toString(new File(path), Charsets.UTF_8));
  }
  ...
  public Metadata generateMetadata(Image image, boolean includeHardwareState) throws Exception {
    ...
    try {
      String binning = studio_.core().getPropertyFromCache(camera, "Binning");
    ...
    if (includeHardwareState) {
      ...
      for (long i = 0; i < config.size(); ++i) {
        ...
        scopeBuilder.putString(
            setting.getDeviceLabel() + "-" + setting.getPropertyName(), setting.getPropertyValue());
      }
