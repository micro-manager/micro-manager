...
public final class AcquisitionWrapperEngine implements AcquisitionEngine {
  ...
  protected Datastore runAcquisition(SequenceSettings sequenceSettings) {
    ...
    try {
      ...
      MMAcquisition acq = new MMAcquisition(studio_, summaryMetadata_, this, shouldShow);
      curStore_ = acq.getDatastore();
      curPipeline_ = acq.getPipeline();

      studio_
          .events()
          .post(new DefaultAcquisitionStartedEvent(curStore_, this, acquisitionSettings));

      // Start pumping images through the pipeline and into the datastore.
      DefaultTaggedImageSink sink =
          new DefaultTaggedImageSink(
  ...
  private int getNumSlices() {
    ...
    return 1
        + (int)
            Math.abs(
                (sequenceSettings_.sliceZTopUm() - sequenceSettings_.sliceZBottomUm())
  ...
  private void updateChannelCameras() {
    ...
    for (int row = 0; row < channels.size(); row++) {
      camChannels.add(
          row, channels.get(row).copyBuilder().camera(getSource(channels.get(row))).build());
    }
  ...
  public void setChannel(int row, ChannelSpec sp) {
    ...
    channels.add(row, sp);
    sequenceSettings_ =
        (new SequenceSettings.Builder(sequenceSettings_)).channels(channels).build();
  }
  ...
  /**
   * Sets the channel group in the core Replies on callbacks to update the UI as well as
   * sequenceSettings (SequenceSettings are updated in the callback function in AcqControlDlg)
   *
  ...
  public boolean setChannelGroup(String group) {
    ...
    if (!(group != null && (curGroup == null || !curGroup.contentEquals(group)))) {
  ...
  public String getVerboseSummary() {
    ...
    String txt =
        "Number of time points: "
            + (!sequenceSettings_.useCustomIntervals()
                ? numFrames
                : sequenceSettings_.customIntervalsMs().size())
            + "\nNumber of positions: "
            + numPositions
            + "\nNumber of slices: "
            + numSlices
  ...
  /**
   * Find out if the configuration is compatible with the current group. This method should be used
   * to verify if the acquisition protocol is consistent with the current settings.
   *
  ...
  /**
   * The name of this function is a bit misleading Every channel group name provided as an argument
   * will return true, unless the group exists and only contains a single property with
   * propertylimits (i.e. a slider in the UI)
   *
   * @param group channel group name to be tested
   * @return false if the group exists and only has a single property that has propertylimits, true
   *     otherwise
   */
