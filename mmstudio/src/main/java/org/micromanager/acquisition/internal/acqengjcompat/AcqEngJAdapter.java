...
import org.micromanager.acquisition.internal.*;
 ...
 *
 * AcquisitionEngine implements a subset of the functionality of AcqEngJ,
...
public class AcqEngJAdapter implements AcquisitionEngine {
    ...
    private class RunnablePlusIndices {
        int channel_, slice_, frame_, position_;
        Runnable runnable_;
    ...
    private Datastore runAcquisition(SequenceSettings sequenceSettings) {
        ...
        try {
            ...
            BlockingQueue<TaggedImage> engineOutputQueue = diverter.getQueue();
     ...
     */
    private void addMMSummaryMetadata(JSONObject summaryMetadata, SequenceSettings acqSettings) throws JSONException {
...
//                imageMD.put(PropertyKey.CAMERA_CHANNEL_INDEX.key(), AcqEngMetadata.(imageMD));
        ...
        for (RunnablePlusIndices r : runnables_) {
            currentAcquisition_.addHook(new AcquisitionHook() {
                ...
                public AcquisitionEvent run(AcquisitionEvent event) {
                    ...
                    boolean cMatch = event.getChannelConfig() == null ||
                            acquisitionSettings.channels().get(r.channel_).config().equals(event.getChannelConfig());
            ...
            // TODO: does current API expect this to be before or after hardware? after camera? during event generation?
        };
    }
    ...
    private Iterator<AcquisitionEvent> createAcqEventIterator(SequenceSettings acquisitionSettings) {
        ...
        if (acquisitionSettings.useFrames()) {
            timelapse = MDAAcqEventModules.timelapse(acquisitionSettings.numFrames(), acquisitionSettings.intervalMs());
        ...
        return new AcquisitionEventIterator(baseEvent, acqFunctions, acqEventMonitor(acquisitionSettings));

    }

    /**
     * This function monitors acquisition events as they are dyanmically created
     * @return
     */
