 ...
 * through a Pipeline to the Datastore. It's also responsible for posting the AcquisitionEndedEvent,
 * which it recognizes when it receives the TaggedImageQueue.POISON object. Functionally this is
 * just glue code between the old acquisition engine and the 2.0 API.
 *
...
public final class DefaultTaggedImageSink {
  ...
  public DefaultTaggedImageSink(
      BlockingQueue<TaggedImage> queue,
      Pipeline pipeline,
      Datastore store,
  ...
  public void start(final Runnable sinkFullCallback) {
    Thread savingThread =
        new Thread("TaggedImage sink thread") {
          ...
          public void run() {
            ...
            try {
                ...
                if (tagged != null) {
                  ...
                  try {
                    ...
                    try {
                      ...
                      // here.
                      ReportingUtils.showError(e, "There was an error in processing images.");
              ...
              pipeline_.halt();
              studioEvents_.post(new DefaultAcquisitionEndedEvent(store_, engine_));
            }
