 ...
 * Various methods of the AcquisitionManager will consume or generate SequenceSettings, and you can
 * create your own to configure your custom acquisitions.
 *
 * <p>Maintainer note: This should be an interface, but kept as a class for backward compatibility.
 */
public final class SequenceSettings {
  ...
  @Deprecated public int numFrames = 1;
  /** @deprecated use Builder and intervalMs() instead */
  @Deprecated public double intervalMs = 0.0;

  private int displayTimeUnit = 0;
  /**
   * Whether or not to use custom time intervals. Do not set this to true if customIntervalsMs is
   * null!
   *
   ...
   */
  @Deprecated public String channelGroup = "";
  /**
   * true if we want to have multiple positions
   *
   * @deprecated use Builder and usePositionList() instead
   */
  @Deprecated public boolean usePositionList = false;
  /**
   * Minimum camera timeout, in ms, for sequence acquisitions (actual timeout depends on exposure
   * time and other factors)
   *
   * @deprecated use Builder and cameraTimeout() instead
   */
  @Deprecated public int cameraTimeout = 20000;
  /**
   * Whether or not to display images generated by the acquisition.
   *
   * @deprecated use Builder and shouldDisplayImages() instead
   */
  @Deprecated public boolean shouldDisplayImages = true;
  ...
  /**
   * Order of the various axes during acquisition as defined in {@link
   * org.micromanager.internal.utils.AcqOrderMode}
   */
  private int acqOrderMode;

  /**
   * Create a copy of this SequenceSettings. All parameters will be copied, with new objects being
   * created as necessary (i.e. this is a deep copy).
   *
   * @return Copy of this SequenceSettings.
   * @deprecated When used correctly, SequenceSettings are immutable. If you really need a copy, use
   *     copyBuilder().build();
   */
  @Deprecated
  ...
  /**
   * Number of time points to be acquired. Defines a sequence of time points together with {@link
   * #intervalMs()}. Will be overriden when {@link #useCustomIntervals()} and {@link
   * #customIntervalsMs()} are set.
   */
  public int numFrames() {
    return numFrames;
  }
  /**
   * Desired interval between the start of two consecutive time points in milliseconds. Defines a
   * sequence of time points together r with {@link #numFrames()}. Will be overriden when {@link
   * #useCustomIntervals()} and {@link #customIntervalsMs()} are set.
   */
  ...
  /**
   * Whether to base the z Top and Bottom for a z stack relative to a referent position or treat
  ...
  /**
   * File format to be used to save this acquisition when {@link #save()} is set. Formats are
   * defined in {@link org.micromanager.data.Datastore.SaveMode}. Currently: 0 - {@link
   * org.micromanager.data.Datastore.SaveMode#SINGLEPLANE_TIFF_SERIES} 1 - {@link
   * org.micromanager.data.Datastore.SaveMode#MULTIPAGE_TIFF}
   *
   ...
   *
   * @return True if positions in the list should all be visited, when false, only acquired at
   *     current position
   */
  ...
  /**
   * Order of the various axes during acquisition as defined in {@link
   * org.micromanager.internal.utils.AcqOrderMode} Currently available orders: 0 - {@link
   * org.micromanager.internal.utils.AcqOrderMode#TIME_POS_SLICE_CHANNEL} 1 - {@link
   * org.micromanager.internal.utils.AcqOrderMode#TIME_POS_CHANNEL_SLICE} 2 - {@link
   * org.micromanager.internal.utils.AcqOrderMode#POS_TIME_SLICE_CHANNEL} 3 - {@link
   * org.micromanager.internal.utils.AcqOrderMode#POS_TIME_CHANNEL_SLICE}
   *
