...
/**
 * This class tests that we properly transfer data between versions (1.4 to 2.0). It relies on an
 * acquisition that was run immediately after startup using the 1.4.23 MDA interface with the
 * following parameters: - 64x64 DemoCamera - 2 timepoints 5000ms apart - 4 stage positions arranged
 * in a 2x2 grid, 64 pix. apart and centered on the origin (so at -32, -32; 32, -32 -- just use
 * Create Grid and make a centered 2x2 grid) - Z start of 0, end of 5, step of 5 - Channel 1 is Cy5,
 * exposure time 25ms - Channel 2 is DAPI, exposure time 50ms - Acquisition comments of "acqcom" Due
 * to the size of this acquisition (80MB) it can't be stored in the repository.
 */
public class VersionCompatTest {
  /**
   * Path to the test data we use, which was generated by Micro-Manager 1.4.23. NOTE: to run this
   * test, the files in this directory must be uncompressed using gunzip!
   */
  private static final String FILE_PATH =
      System.getProperty("user.dir")
          + "/src/test/resources/org/micromanager/data/internal/VersionCompatTest";

  /**
   * Maps image coordinates to hashes of the pixel data expected to be found there. Hashes generated
   * via the imagePixelHash.bsh script in our test/resources directory.
   */
  ...
  static {
    ...
    double[] times =
        new double[] {
          441, 1258, 2053, 2558, 810, 1779, 2309, 2808, 608, 1535, 2166, 2667, 1007, 1933, 2439,
          2932, 5315, 5757, 6212, 6670, 5544, 5989, 6448, 6866, 5403, 5843, 6296, 6742, 5651, 6096,
          6572, 6968
        };
  ...
  /**
   * Load the 1.4 dataset and verify that it matches expectations. Then save it to disk using the
   * 2.0 format.
   */
  @Test
  ...
  private void testStore(Datastore store) throws JSONException {
    ...
    Assert.assertArrayEquals(
        "Channel names", summary.getChannelNames(), new String[] {"Cy5", "DAPI"});
    Assert.assertEquals("Z step", summary.getZStepUm(), 5.0, .00001);
    ...
    Assert.assertEquals("MetadataVersion", "10", summary.getMetadataVersion());
    Assert.assertEquals("ComputerName", "Chriss-MacBook-Pro-2.local", summary.getComputerName());
    Assert.assertEquals(
        "Directory", "/Users/chriswei/proj/vale/data/testData", summary.getDirectory());
    Assert.assertEquals("WaitInterval", summary.getWaitInterval(), 5000.0, .00001);
    Assert.assertArrayEquals("customIntervalsMs", summary.getCustomIntervalsMs(), new Double[] {});
    Assert.assertEquals(
        "intendedDimensions",
    ...
    JSONArray json =
        new JSONArray(
            "[{\"DeviceCoordinatesUm\":{\"XY\":[-32,-32],\"Z\":[-0]},\"GridColumnIndex\":0,\"Label\":\"1-Pos_000_000\",\"GridRowIndex\":0},{\"DeviceCoordinatesUm\":{\"XY\":[32,-32],\"Z\":[0]},\"GridColumnIndex\":1,\"Label\":\"1-Pos_001_000\",\"GridRowIndex\":0},{\"DeviceCoordinatesUm\":{\"XY\":[32,32],\"Z\":[0]},\"GridColumnIndex\":1,\"Label\":\"1-Pos_001_001\",\"GridRowIndex\":1},{\"DeviceCoordinatesUm\":{\"XY\":[-32,32],\"Z\":[0]},\"GridColumnIndex\":0,\"Label\":\"1-Pos_000_001\",\"GridRowIndex\":1}]");
    /*
    ...
    for (Coords coords : store.getUnorderedImageCoords()) {
      try {
        ...
        Assert.assertEquals(
            "X position for " + coords, column * 64 - 32.0, metadata.getXPositionUm(), .00001);
        Assert.assertEquals(
            "Y position for " + coords, row * 64 - 32.0, metadata.getYPositionUm(), .00001);
        Assert.assertEquals(
            "Z position for " + coords,
            metadata.getZPositionUm(),
            coords.getZ() == 0 ? 0.0 : 5.0,
            .00001);
        ...
        Assert.assertEquals(
            "elapsedTimeMs for " + coords,
            (Double) metadata.getElapsedTimeMs(0.0),
            IMAGE_ELAPSED_TIMES.get(coords));
        Assert.assertEquals(
            "Exposure time for " + coords,
            coords.getChannel() == 0 ? 25.0 : 50.0,
            metadata.getExposureMs(),
            .00001);
        Assert.assertEquals("pixelSizeUm for " + coords, metadata.getPixelSizeUm(), 1.0, .00001);
        Assert.assertEquals(
            "positionName for " + coords,
            metadata.getPositionName(""),
            String.format("1-Pos_%03d_%03d", column, row));
        Assert.assertEquals(
            "receivedTime for " + coords,
            metadata.getReceivedTime(),
            IMAGE_RECEIVED_TIMES.get(coords));
        Assert.assertEquals("ROI for " + coords, metadata.getROI(), new Rectangle(0, 0, 64, 64));
        Assert.assertEquals("uuid for " + coords, metadata.getUUID(), IMAGE_UUIDS.get(coords));
        ...
        for (String key : userData.getKeys()) {
          Assert.assertEquals(
              key + " for " + coords, userData.getString(key), DEVICE_PROPERTIES.get(key));
        }
